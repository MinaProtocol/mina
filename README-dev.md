...
`docker pull codaprotocol/coda:toolchain-7e6a9241c93510db64332a485c967538f0e2760c`
...
All the constants can be set at compile-time. A subset of the compile-time constants can be overriden when generating the genesis state using `runtime_genesis_ledger.exe`, and a subset of those can again be overriden at runtime by passing the new values to the daemon.

The constants at compile-time is set for different configurations using optional compilation. This is how integration tests/builds with multiple configurations are run.
...
<b> 1. Constants that can be overriden when generating the genesis state are:</b>
