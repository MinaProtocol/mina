...
```bash
let s:ocamlmerlin="/Users/USERNAME/.opam/5.3.0/share/merlin"
...
To add a new dependency, you most likely will need to create a new fresh switch to avoid pushing in any local dependency (like `ocaml-lsp`). The following commands assume that the version of the OCaml compiler used in the codebase is 5.3.0:

```shell
opam switch create mina_fresh 5.3.0
opam switch import opam.export
```
