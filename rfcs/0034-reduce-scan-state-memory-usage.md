...
This RFC analyzes current scan state memory usage and proposes a series of optimizations which can be performed in order to allow us to meet our target TPS goals without consuming an astronomical amount of RAM.
...
What these graphs show is that the dominate factor in reducing scan state memory usage is actually length of a slot time. However, the length of a slot time is, in turn, dependent on blockchain proving time. Furthermore, if slot time is reduced, but transaction proving time increases (due to, i.e., transaction snark bundling), then delay will need to be increased. Still, lowering the slot time (to reduce the scan state depth) and increasing delay is preferable to keeping the slot time higher to have a lower delay.
