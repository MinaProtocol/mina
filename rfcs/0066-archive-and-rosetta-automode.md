...
The pre-fork daemon, on generating its hard fork configuration, MUST send it to
its archive over a **new** RPC, distinct from `Send_archive_diff`:

```ocaml
let announce_hardfork :
    (Hardfork_announcement.t, Hardfork_ack.t) Rpc.Rpc.t =
  Rpc.Rpc.create ~name:"Announce_hardfork" ~version:1
    ~bin_query:Hardfork_announcement.Stable.V1.bin_t
    ~bin_response:Hardfork_ack.Stable.V1.bin_t
```

The query MUST name the era it was produced in, and MUST be built from
explicitly versioned types rather than `Stable.Latest`, whose shape changes at
the fork:

```ocaml
module Hardfork_announcement = struct
  type t =
    { fork_state_hash        : State_hash.Stable.V1.t
    ; fork_blockchain_length : Length.Stable.V1.t
    ; fork_global_slot       : Global_slot_since_genesis.Stable.V1.t
    ; protocol_version       : Protocol_version.Stable.V1.t
    ; config_json            : string
    }
end

module Hardfork_ack = struct
  module Refusal = struct
    type t =
      | Era_mismatch of
          { announced : Protocol_version.Stable.V1.t
          ; archive   : Protocol_version.Stable.V1.t }
      | Different_fork of
          { announced : State_hash.Stable.V1.t
          ; recorded  : State_hash.Stable.V1.t }
      | Not_recorded of string  (** the write failed; text is for the log *)
  end

  type t = (unit, Refusal.Stable.V1.t) Result.Stable.V1.t
end
```
...
The archive MUST refuse an announcement whose `protocol_version` is not the one
it was built for, and MUST answer `Era_mismatch` naming both versions. This
makes a check out of what would otherwise be an assumption about ordering: a
second daemon that has not yet crossed, a daemon rolled back to the pre-fork
package, and an archive restored from a pre-fork backup all produce this
pairing, and none of them is guaranteed to fail on decode.

The archive MUST NOT answer `Ok` until the record is durable. In particular an
implementation MUST NOT answer from a queue: a reply that means "received"
rather than "recorded" is worthless here, because the daemon exits moments later
and a failed write would leave nobody aware of it. A write that fails MUST be
answered `Not_recorded`. The archive MUST NOT raise in place of answering, which
at the daemon is indistinguishable from a dropped connection.

A daemon whose announcement is refused MUST log the refusal verbatim at error
level and MUST NOT retry the same payload against the same archive. It MUST
still complete its own fork transition: that transition does not depend on the
archive, and a daemon which stalled waiting for one would turn an archive
problem into a block production problem. A refusal is therefore an operator
facing event, and §9's packaging MUST NOT suppress it.
...
The heartbeat is also how a daemon learns that the archive it is paired with is
of another era, because the refusal names it; no separate query is needed and
none is specified.
...
#### 2.1 More than one daemon

The archive has no membership list and cannot know how many daemons feed it.
The rules below are therefore stated in terms of what arrives, not who sent it.

**Pre-fork blocks after an announcement.** Recording an announcement MUST NOT by
itself change which blocks the archive accepts. What the archive can decode is
decided by which binary is running, and that is §4's question, not §2's. A
pre-fork archive that has recorded an announcement and is still running under
`keep-running` MUST keep accepting pre-fork blocks, including from daemons that
have not yet crossed: those blocks are below the fork point, and the archive is
still of the era that can read them. Once post-fork code is running, pre-fork
blocks still missing MUST be backfilled through the block file path rather than
the RPC.

**Duplicates around the boundary.** Two daemons announcing the same fork is the
expected case, and §2's idempotence covers it. Two daemons delivering the same
block is covered by the `blocks` table's existing uniqueness on `state_hash`,
and this proposal adds no further rule. Blocks MAY arrive out of height order
and from more than one peer; nothing here requires otherwise, because §6 settles
on the seven conditions it lists rather than on arrival order.

**What starts the settle and the stand-down.** The first *accepted* announcement
starts both. The archive MUST NOT wait for a second daemon to agree, and cannot:
it does not know how many to expect, and an announcement naming a different fork
block is refused rather than counted, so a later one can only confirm the first.
Waiting would turn a daemon that never reports into an indefinite stall.

**What a refused daemon sees.** It receives the `Refusal` of §1, naming the
reason and both values compared. The archive MUST log every refusal at error
level with the same detail, so that the event outlives the daemon that exits. A
refusal MUST NOT alter the recorded row.
...
The upgrade is written so that a writer need not stop for it: it adds columns
with defaults, relaxes constraints, and sets a short `lock_timeout` so that it
aborts rather than blocking production traffic. One caveat bounds that today. A
pre-fork writer against an already-upgraded schema fails on zkApp field arrays,
because it depends on a unique constraint the upgrade drops (#19395). Until that
is fixed, `keep-running` across an early schema upgrade is safe only for an
archive receiving no zkApp commands, and the window §3 exists to shorten is in
practice bounded by it.
...
**Automatic hand-over requires a feeding daemon.** Every trigger in this
proposal begins with §1's announcement, and only a daemon sends one. An archive
filled solely from precomputed or extensional block files — a backfill, a
restored dump, an explorer's own ingestion — receives no announcement, and so
does not stand down, does not migrate, and does not settle its boundary.
Nothing breaks: such an archive behaves exactly as it does today. But it is not
covered by this proposal, and its operator performs the hand-over by hand, as
now. Implementations SHOULD provide a command that records the announcement
from a configuration file, so that "by hand" does not mean writing SQL.
