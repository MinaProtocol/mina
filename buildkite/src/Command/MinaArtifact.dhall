...
let pipeline : DebianVersion.DebVersion -> Pipeline.CompoundType = \(debVersion : DebianVersions.DebVersion) ->
  Pipeline.build
    Pipeline.Config::{
      spec =
        JobSpec::{
      ...
      steps = [
        ...
        Command.build
          Command.Config::{
            ...
            label = "Build Mina daemon package for Debian ${DebianVersions.capitalName(debVersion)}",
            key = "build-deb-pkg",
            target = Size.XLarge,
