...
let runInDockerWithPostgresConn
    ...
    = \(environment : List Text) ->
      ...
      \(innerScript : Text) ->
        ...
        -- The version of the archive data to use
        -- This is expected to be the latest version of the archive data
        -- which is stored in the `src/test/archive/sample_db/latest_version` file
        -- This file is expected to be generated by the `scripts/upload-archive-replayer-data.sh
        ...
        -- The URL to the SQL file that initializes the Postgres database
        -- with the archive data for the replayer
        -- This file is expected to be stored in a GCP bucket
        -- Requires the version which links to the current archive data bucket
        let initUrl
            : Text
