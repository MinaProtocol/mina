...
let module = \(environment : List Text) ->
  ...
  let inDocker : Docker.Type -> Cmd -> Cmd =
    ...
    in
    { line = "docker run -it --rm --init --volume ${sharedDir}:/shared --tmpfs ${outerDir}:/workdir --workdir /workdir${envVars}${if docker.privileged then " --privileged" else ""}${docker.entrypoint} ${docker.image} /bin/sh -c '${inner.line}'"
...
let tests =
  ...
  { line =
"docker run -it --rm --init --volume /var/buildkite/shared:/shared --tmpfs \\\$BUILDKITE_BUILD_CHECKOUT_PATH:/workdir --workdir /workdir --env ENV1 --env ENV2 --env TEST foo/bar:tag /bin/sh -c 'echo hello'"
  , readable =
...
''
  ./buildkite/scripts/cache-through.sh data.tar "docker run -it --rm --init --volume /var/buildkite/shared:/shared --tmpfs \$BUILDKITE_BUILD_CHECKOUT_PATH:/workdir --workdir /workdir --env ENV1 --env ENV2 --env TEST foo/bar:tag /bin/sh -c 'echo hello > /tmp/data/foo.txt && tar cvf data.tar /tmp/data'"''
  ===
  M.format (
    M.cacheThrough
      M.Docker::{
