...
let module = \(environment : List Text) ->
  ...
  let inDocker : Docker.Type -> Cmd -> Cmd =
    ...
    in
    { line = "source ./buildkite/scripts/export-git-env-vars.sh && docker run -it --rm --init --volume ${sharedDir}:/shared --volume ${outerDir}:/workdir --workdir /workdir${envVars}${if docker.privileged then " --privileged" else ""} ${docker.image} /bin/sh -c '${inner.line}'"
...
let tests =
  ...
  { line =
"source ./buildkite/scripts/export-git-env-vars.sh && docker run -it --rm --init --volume /var/buildkite/shared:/shared --volume /var/buildkite/builds/\\\$BUILDKITE_AGENT_NAME/\\\$BUILDKITE_ORGANIZATION_SLUG/\\\$BUILDKITE_PIPELINE_SLUG:/workdir --workdir /workdir --env ENV1 --env ENV2 --env TEST foo/bar:tag /bin/sh -c 'echo hello'"
  , readable =
