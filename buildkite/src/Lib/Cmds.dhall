...
let module = \(environment : List Text) ->
  ...
  let inDocker : Docker.Type -> Cmd -> Cmd =
    ...
    in
    { line = "docker run -it --rm --init --volume ${outerDir}:/workdir --workdir /workdir${envVars} ${docker.image} /bin/bash -c '${inner.line}'"
...
let tests =
  ...
  { line =
"docker run -it --rm --init --volume /var/buildkite/builds/$BUILDKITE_AGENT_NAME/$BUILDKITE_ORGANIZATION_SLUG/$BUILDKITE_PIPELINE_SLUG:/workdir --workdir /workdir --env ENV1 --env ENV2 --env TEST foo/bar:tag /bin/bash -c 'echo hello'"
  , readable =
