...
let buildToolCmd : Text -> Size -> Command.Type = \(tool : Text) -> \(cmd_target : Size) ->
  Command.build
    Command.Config::{
      commands = OpamInit.andThenRunInDocker ([ "PATH=/home/opam/.cargo/bin:/usr/lib/go/bin:\\\$PATH", "DUNE_PROFILE=testnet_postake_medium_curves"]) "eval \\\$(opam config env) && ocaml --version && make ${tool}",
