...
let buildTestCmd : Text -> Text -> Natural -> Size -> Command.Type = \(profile : Text) -> \(path : Text) -> \(timeout : Natural) -> \(cmd_target : Size) ->
  let timeout = Natural/show timeout in
  Command.build
    Command.Config::{
      commands = RunInToolchain.runInToolchain ([] : List Text)
        "buildkite/scripts/fuzzy-zkapp-test.sh ${profile} ${path} ${timeout}",
...
Pipeline.build
  Pipeline.Config::{
    ...
    steps = [
      buildTestCmd "dev" "src/lib/transaction_snark/test/zkapp_fuzzy/zkapp_fuzzy.exe" 3600 Size.Small
    ]
  }
