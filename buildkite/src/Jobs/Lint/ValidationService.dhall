...
let commands =
      ...
      let archivePath =
            "\"mix-cache-\\\$(sha256sum ${sigPath} | cut -d\" \" -f1).tar.gz\""

      let unpackageScript =
            "tar xzf ${archivePath} -C ${ValidationService.rootPath}"

      in  [ Cmd.runInDocker
              ValidationService.docker
              "elixir --version | tail -n1 > ${sigPath}"
          , Cmd.run "cat ${sigPath}"
          , Cmd.cacheThrough
              ValidationService.docker
              archivePath
              ...
              , package =
                  Cmd.run
                    "tar czf ${archivePath} -C ${ValidationService.rootPath} priv/plts"
              , create =
                  let scripts =
                        List/map
                          Cmd.Type
