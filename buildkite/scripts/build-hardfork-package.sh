...
existing_ro_files=$(aws s3 ls s3://snark-keys-ro.o1test.net/ | awk '{print $4}')
existing_w_files=$(aws s3 ls s3://snark-keys.o1test.net/ | awk '{print $4}')
for file in hardfork_ledgers/*; do
  filename=$(basename "$file")
  
  if echo "$existing_ro_files" | grep -q "$filename"; then
    echo "Info: $filename already exists in the read only bucket, packaging it instead."
  ...
  elif echo "$existing_w_files" | grep -q "$filename"; then
    echo "WARNING: $filename already exists in the writeable bucket, packaging it instead."
    oldhash=$(openssl dgst -r -sha3-256 "$file" | awk '{print $1}')
