...
if [[ -n $MINA_BUILD_ROSETTA ]]; then
  echo "--- Build/Release mina-rosetta to docker hub"
  docker pull gcr.io/o1labs-192920/mina-rosetta-opam-deps:develop
  ...
  scripts/release-docker.sh --service "mina-rosetta" --version "dev-${MINA_VERSION}" --commit "${MINA_GIT_HASH}" \
  ...
  scripts/release-docker.sh --service "mina-rosetta" --version "${MINA_VERSION}"  --commit "${MINA_GIT_HASH}" \
