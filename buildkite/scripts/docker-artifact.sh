...
scripts/release-docker.sh --service "${CODA_SERVICE}-puppeteered" --version "${CODA_VERSION}" --commit "${CODA_GIT_HASH}" \
 --extra-args "--build-arg coda_deb_version=${MINA_DEB_VERSION} --build-arg CODA_VERSION=${CODA_VERSION} --build-arg MINA_BRANCH=${CODA_GIT_BRANCH} --build-arg deb_repo=${CODA_DEB_REPO}"
