...
# Split cores between rayon and dune, to minimize thread swapping
export NUM_CORES=$(nproc --all)
export RAYON_NUM_THREADS=4
export NUM_DUNE_JOBS=$((${NUM_CORES} / ${RAYON_NUM_THREADS}))
...
echo "--- Run unit tests"
time dune runtest "${path}" --profile="${profile}" -j${NUM_DUNE_JOBS} || \
