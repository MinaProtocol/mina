...
2. Place it somewhere safe and set `HETZNER_KEY` env var pointing to that location
...
| Variable | Default | Description |
|----------|---------|-------------|
| `HETZNER_KEY` | `~/secrets/storagebox.key` | SSH key for Hetzner |
...
```
ERROR: Hetzner key not found at: <path>
```
