...
spawn-node $nodesfolder/seed 3000 -seed -discovery-keypair $SEED_PEER_KEY -archive-address 3086
seed_pid=$!
...
for i in $(seq 1 $whales); do
  ...
  mkdir $folder
  spawn-node $folder $(($WHALE_START_PORT+($i-1)*5)) -peer $SEED_PEER_ID -block-producer-key $keyfile $snark_worker_flags -archive-address 3086
  whale_pids[${i}]=$!
done
...
for i in $(seq 1 $fish); do
  ...
  mkdir $folder
  spawn-node $folder $(($FISH_START_PORT+($i-1)*5)) -peer $SEED_PEER_ID -block-producer-key $keyfile $snark_worker_flags -archive-address 3086
  fish_pids[${i}]=$!
done
...
for i in $(seq 1 $nodes); do
  ...
  mkdir $folder
  spawn-node $folder $(($NODE_START_PORT+($i-1)*5)) -peer $SEED_PEER_ID -archive-address 3086
  node_pids[${i}]=$!
done
