...
build_daemon_postfork_deb() {
  ...
  # packages own outside automode.  Replaces/Breaks on mina-generic covers the
  # develop-nightly layout that predates the runtime split.  The exclusion
  # against mina-${network} deliberately lives on the daemon L2 package, NOT
  # here: an unversioned Breaks also applies to virtual packages, and the
  # automode metapackage Provides mina-${network}, so declaring it here would
  # make automode and postfork mutually uninstallable.
  create_control_file "$package_name" "${SHARED_DEPS}${DAEMON_DEPS}" \
...
build_daemon_deb() {
  ...
  # nightlies' split layout), the config files from the regular
  # mina-${network}-config package (whose name the hardfork config package
  # still uses at newer versions), and the postfork automode runtime, which
  # owns the /usr/local/bin dispatcher symlinks and hardfork helper scripts —
  # the unversioned Breaks forces apt to remove postfork when the normal
  # daemon is reinstalled after a hardfork.  (The reverse exclusion cannot
  # live on the postfork package: automode Provides mina-${network}, and an
  # unversioned Breaks would hit that virtual too.)  The pre-split monolithic
  # mina-${network} upgrades by name.
  create_control_file "${package_name}" "${depends}" \
    "Mina Protocol daemon for ${network} (symlinks and configuration over the Mina runtime)" \
    "${SUGGESTED_DEPS}" \
    "mina-generic (<< ${MINA_DEB_VERSION}), mina-${network}-generic (<< ${MINA_DEB_VERSION}), mina-${network}-config (<< ${MINA_DEB_VERSION}), mina-${network}-postfork-${POSTFORK_CODENAME}" \
    "" \
