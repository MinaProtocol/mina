...
def _tag_stream(stream: TextIO, tag: str, target: TextIO) -> None:
    """Read lines from *stream*, prefix each with ``[{tag}] ``, write to *target*.

    Used as a daemon-thread target so that child stdout/stderr is forwarded
    line-by-line to the parent process without buffering until exit.
    """
    try:
        for line in iter(stream.readline, ""):
            target.write(f"[{tag}] {line}")
            target.flush()
    finally:
        stream.close()


def spawn_tagged_process(
    argv: List[str], env: Dict[str, str], name: str
) -> "subprocess.Popen[str]":
    """Launch *argv* as its own session leader, tagging its output with *name*.

    The shared launch primitive behind both real external processes
    (``launch_process``) and workloads (``SubprocessWorkload``).  Two
    properties here are load-bearing and must not drift between callers:

    * ``start_new_session=True`` makes the child a session leader, so its PGID
      equals its PID and process-group teardown reaps any grandchildren (e.g.
      ``mina client send-payment``) it shells out to.
    * stdout/stderr are drained by two daemon threads that prefix each line
      with ``[{name}] ``, so output is forwarded live rather than buffered
      until exit.
    """
    ...
    threading.Thread(
        target=_tag_stream,
        args=(proc.stdout, name, sys.stdout),
        daemon=True,
    ).start()
    threading.Thread(
        target=_tag_stream,
        args=(proc.stderr, name, sys.stderr),
        daemon=True,
    ).start()
    return proc
