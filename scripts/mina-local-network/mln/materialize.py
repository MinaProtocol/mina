...
def required_key_file_paths(plan: ResolvedPlan) -> List[str]:
    """Enumerate every key file (privkey + matching .pub) *plan* requires.

    Covers per-tier offline/online account keys, the snark coordinator key,
    and per-workload zkApp keys — the same key material ``do_materialize``
    generates. Used both for materialize's existing-artifact preflight check
    and by ``patch topology`` to verify a new plan doesn't need any key that
    ...
    for tier_name, tier_def in tiers.items():
        ...
        for idx in range(count):
            for prefix in ("offline", "online"):
                ...
                if key_dir_key not in key_dirs:
                    continue
                kd = key_dirs[key_dir_key]
                key_base = f"{prefix}_{tier_name}_account_{idx}"
                privkey_path = str(Path(kd) / key_base)
                paths.append(privkey_path)
                paths.append(privkey_path + ".pub")

    sc_key_dir = key_dirs.get("snark_coordinator")
    if sc_key_dir:
        sc_key_base = "snark_coordinator_account"
        sc_privkey_path = str(Path(sc_key_dir) / sc_key_base)
        paths.append(sc_privkey_path)
        paths.append(sc_privkey_path + ".pub")

    for wl in plan.workloads:
        match wl:
            case ZkappWorkload(name=wl_name):
                safe_name = re.sub(r"[^a-zA-Z0-9_]", "_", wl_name)
                zkapp_key_dir = f"{state_root}/zkapp_keys"
                zkapp_key_base = f"zkapp_account_{safe_name}"
                zkapp_privkey = str(Path(zkapp_key_dir) / zkapp_key_base)
                paths.append(zkapp_privkey)
                paths.append(zkapp_privkey + ".pub")
            case _:
                pass
...
def do_materialize(plan: ResolvedPlan, force: bool, dry_run: bool) -> MaterializeResult:
    ...
    if not dry_run:
        ...
        _preflight_artifacts.extend(required_key_file_paths(plan))
