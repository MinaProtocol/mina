...
usage() {
    ...
    echo "  -s <skip_verify>         Skip verification step (optional)"
    exit 1
}

while getopts "d:c:l:n:s" opt; do
    case $opt in
        d) DEB_FILE="$OPTARG" ;;
...
# Check for required ledgers: genesis_ledger (1), epoch (1 or 2)
# There might a be a corner case that a network is too young that 2 epoch ledgers will be the same.
#
# Example: epoch_ledger_02d5328ebf05c79ef392d9b7193404d45c8cb6d13ca504dbaa651229dbbc9d19.tar.gz
# The naming scheme would be:
#
# For genesis ledger:
# "genesis_ledger_" + 64 chars hash consists of a-z / 0-9
#
# For epoch ledger:
# "epoch_ledger_" + 64 chars hash consists of a-z / 0-9


# Check genesis_ledger
GENESIS_MATCHES=("$LEDGER_TARBALLS_FULL/genesis_ledger_"*.tar.gz)
...
# Check epoch (must have 1 or 2)
EPOCH_MATCHES=("$LEDGER_TARBALLS_FULL/epoch_ledger_"*.tar.gz)
if [[ ${#EPOCH_MATCHES[@]} -ne 1 && ${#EPOCH_MATCHES[@]} -ne 2 ]]; then
    echo "ERROR: Expected 1 or 2 ledger tarballs with prefix: epoch in $LEDGER_TARBALLS_FULL, found ${#EPOCH_MATCHES[@]}" >&2
    exit 1
fi

# Use the actual matched filenames for genesis_ledger and epoch tarballs
TARBALL_FILES=("${GENESIS_MATCHES[0]}" "${EPOCH_MATCHES[@]}")
