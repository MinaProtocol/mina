...
while kill -0 $sw_pid 2>/dev/null; do
  ...
  <localnet/exported_staged_ledger.json jq -r '.[].pk' | { shuf || true; } | while read acc; do
    ...
    if [[ "$MODE" == "docker" ]]; then
      docker exec -v "$PWD/localnet:/localnet" --env MINA_LIBP2P_PASS --env MINA_PRIVKEY_PASS "$sw_container_id" client send-payment --sender "$(cat /$CONF_DIR/bp.pub)" --receiver "$acc" \
