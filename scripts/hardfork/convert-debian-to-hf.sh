...
if [[ ! -f "$PATCHED_DEB2" ]]; then
	echo "ERROR: Final package '$PATCHED_DEB2' not found" >&2
	exit 1
fi

echo "Package size: $(ls -lh "$PATCHED_DEB2" | awk '{print $5}')"
# Quick file count check
FILE_COUNT=$(dpkg-deb -c "$PATCHED_DEB2" | grep -E "(config_.*\.json|.*ledger.*\.tar\.gz)" | wc -l)
echo "Files found (config + ledgers): $FILE_COUNT"

# Count source ledger files
if [[ -d "$LEDGER_TARBALLS_FULL" ]]; then
	SOURCE_LEDGER_COUNT=$(ls -1 "$LEDGER_TARBALLS_FULL"/*.tar.gz 2>/dev/null | wc -l)
else
	SOURCE_LEDGER_COUNT=$(ls -1 "$LEDGER_TARBALLS_FULL" 2>/dev/null | wc -l)
fi
...
if [[ $FILE_COUNT -lt $EXPECTED_MIN ]]; then
	...
	dpkg-deb -c "$PATCHED_DEB2" | grep -E "(config_|ledger)" >&2
	exit 1
fi
...
cd "$VERIFY_DIR"
ar x "$(readlink -f "$PATCHED_DEB2")"
mkdir data
tar -xzf data.tar.gz -C data

# Verify config file
RUNTIME_CONFIG_FULL=$(readlink -f "$RUNTIME_CONFIG_JSON")
RUNTIME_CONFIG_BASENAME=$(basename "$RUNTIME_CONFIG_FULL")
RUNTIME_CONFIG_HASH=$(sha256sum "$RUNTIME_CONFIG_FULL" | awk '{print $1}')
...
LEDGER_ERROR=0
if [[ -d "$LEDGER_TARBALLS_FULL" ]]; then
	for SOURCE_LEDGER in "$LEDGER_TARBALLS_FULL"/*.tar.gz; do
	...
	# Single file passed
	LEDGER_NAME=$(basename "$LEDGER_TARBALLS_FULL")
	PKG_LEDGER="data/var/lib/coda/$LEDGER_NAME"
	...
	if [[ ! -f "$PKG_LEDGER" ]]; then
		...
		SOURCE_HASH=$(sha256sum "$LEDGER_TARBALLS_FULL" | awk '{print $1}')
		PKG_HASH=$(sha256sum "$PKG_LEDGER" | awk '{print $1}')
...
echo "Output: $PATCHED_DEB2"

mv "$PATCHED_DEB2" "$DEB_FILE"
