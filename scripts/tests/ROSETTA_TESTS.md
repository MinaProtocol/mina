...
This document provides comprehensive documentation for all Rosetta API testing
scripts located in `./scripts/tests/`. These scripts are designed to test the
Mina Protocol Rosetta API implementation for functionality, performance, and
reliability.

## Overview

The Rosetta API test suite consists of four main scripts that work together to
...
**Purpose**: Core helper functions and utilities shared across all Rosetta test
scripts.
...
- **Validation**: Confirms transaction hash matches requested payment
  transaction
...
**Purpose**: Basic functionality testing of Rosetta API endpoints with
...
**Purpose**: Comprehensive performance and load testing of Rosetta API with
configurable test intervals and database-driven test data.

**Location**: `scripts/tests/rosetta-load.sh`

**Key Features**:

- **Database Integration**: Loads real test data from PostgreSQL archive
  database
...
- **Multiple Stop Conditions**: Duration-based or request-count-based
  termination
...
- `--address <address>`: Rosetta API endpoint (default:
  http://rosetta-mainnet.gcp.o1test.net)
...
- **Concurrent Testing**: All test types run simultaneously at their configured
  intervals
...
**Purpose**: End-to-end connectivity testing using Docker containers, combining
sanity and load testing in a controlled environment.
...
- `-n, --network <network>`: Network configuration (devnet or mainnet, default:
  devnet)
...
1. **Container Startup**: Launches Docker container with specified tag and
   network
...
- **Docker Issues**: Container startup or management problems (connectivity
  testing)
...
- **Start Conservative**: Begin with default intervals and increase frequency
  gradually
...
This documentation provides comprehensive coverage of all Rosetta testing
scripts and their capabilities for validating Mina Protocol's Rosetta API
implementation.
