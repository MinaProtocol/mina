...
# add account creation fee to blocks_internal_commands for internal commands, where
# the balance is the command amount minus the creation fee
...
WHERE balances.balance = ic.fee - $CREATION_FEE
EOF

# if a coinbase has an associated fee transfer, add account creation fee if
# the balance is coinbase amount minus the creation fee, minus the amount of the fee transfer
...
AND balances.balance = ic_coinbase.fee - $CREATION_FEE - ic_fee_transfer.fee
EOF

while read -r block_id internal_command_id sequence_no secondary_sequence_no; do
    psql $ARCHIVE <<EOF
    UPDATE blocks_internal_commands SET receiver_account_creation_fee_paid = $CREATION_FEE
