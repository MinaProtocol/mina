...
let is_input_association_list ~type_name json =
  ...
  compute_validity
    ( assoc_list json
    >>= ( key_and_value ~key:"name" ~value:(String type_name)
  ...
  association_list json
    ~f:
      (value_with_name "inputFields"
         ~f:(list ~f:(fun json_list -> Result.return (f json_list))))

let remove_key_from_input ~type_name fields =
  let open Combinators in
  let predicate = is_input_association_list ~type_name in
  let action json =
        ...
        List.filter json_list ~f:(fun json_elem ->
            ...
            @@ compute_validity
                 ...
                 Some
                   (List.exists
                      ~f:(fun field_name ->
                        Scalar.is_json_equal (String field_name) json )
                      fields) ) ) )
  in
...
let read_from_standard_input () =
  ...
  let json =
    Yojson.Basic.from_string json_string |> change_constraint
