...
daemon_port = args.daemon_port if args.daemon_port else str(default_port)
prog = args.executable if args.executable else default_prog
...
def add_node_statuses (output):
    ...
    for line in lines:
        ...
        try:
            ...
            print(f"[{e}] Error in node status response: {line}")
    ...
    for (peer_id, peer) in peers_to_query :
        ...
        if peer_id in seen_peer_ids:
            ...
            if peer_id_count == 0:
                ...
                formatted_peers = f"{peer},{formatted_peers}"
