{
  ...
  inputs.opam-nix.url = "github:georgeee/opam-nix";
  ...
  outputs = inputs@{ self, nixpkgs, utils, mix-to-nix, nix-npm-buildPackage
    ...
    in {
      ...
      in {
        ...
        packages = rec {
          inherit (ocamlPackages)
            mina experiment devnet mainnet mina_tests mina-ocaml-format mina_client_sdk test_executive with-instrumentation;
          inherit (pkgs)
