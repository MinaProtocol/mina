...
# this block producer upload script has additional logic to upload unique block producer passkeys
resource "null_resource" "block_producer_uploads" {
  provisioner "local-exec" {
    working_dir = "${path.module}/../../.."
    command     = "./scripts/upload-keys-k8s-unique.sh ${var.testnet_name}"
    environment = {
