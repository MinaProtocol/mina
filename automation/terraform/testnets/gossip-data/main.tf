terraform {
  ...
  backend "s3" {
    key     = "terraform-gossip-data.tfstate"
    encrypt = true
    region  = "us-west-2"
...
module "gossip-data" {
  ...
  mina_archive_schema = "https://raw.githubusercontent.com/MinaProtocol/mina/feature/gossip-qa-ledger/src/app/archive/create_schema.sql"
...
# if we want to do this automatically, we would need to modify generate-keys-and-ledger.sh to create a separate batch of files with pulbic keys of all block producer nodes and read those in
  ...
  seed_peers_url                  = "https://storage.googleapis.com/seed-lists/gossip-data_seeds.txt"
}
