...
function generate_key_files {
  ...
  for k in $(seq 1 $COUNT); do
    docker run \
      ...
      -c "CODA_PRIVKEY_PASS='${privkey_pass}' coda advanced generate-keypair -privkey-path /keys/${name_prefix}_account_${k} && chmod -R +rw /keys"
    docker run \
      --mount type=bind,source=${output_dir},target=/keys \
      --entrypoint /bin/bash $CODA_DAEMON_IMAGE \
      -c "CODA_LIBP2P_PASS='${privkey_pass}' coda advanced generate-libp2p-keypair -privkey-path /keys/${name_prefix}_libp2p_${k} && chmod -R +rw /keys"
  done
}
