...
do
  mina advanced unsafe-import -config-dir ~/coda-config -privkey-path $file
done
...
if [ -z "$NODAEMON" ] || [ "$NODAEMON" -eq 0 ]; then
  ...
  mina daemon -config-directory ~/coda-config $ROLE_COMMAND -client-port $DAEMON_CLIENT_PORT -rest-port $DAEMON_REST_PORT -external-port $DAEMON_EXTERNAL_PORT  -discovery-port $DAEMON_DISCOVERY_PORT -metrics-port $DAEMON_METRICS_PORT \
