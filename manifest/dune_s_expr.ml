...
let tokenize input =
  ...
  while !i < len do
    ...
    | ';' ->
        ...
        in
        if comment <> "" then
          tokens := COMMENT_TOK comment :: !tokens
    | '"' ->
        (* quoted string *)
        incr i;
        let buf = Buffer.create 64 in
        while !i < len && input.[!i] <> '"' do
          if input.[!i] = '\\' && !i + 1 < len then begin
            incr i;
            (match input.[!i] with
...
let rec strip_comments = function
  ...
  | List items ->
      let items' =
        List.filter_map strip_comments items
      in
      Some (List items')

let strip_comments_list l =
  List.filter_map strip_comments l

(** Get the "key" of a dune field for set-based comparison.
    E.g. [(name foo)] has key ["name"]. *)
let field_key = function
  | List (Atom key :: _) -> Some key
  | _ -> None
...
let is_keyed_stanza = function
  | List (Atom key :: _) ->
      List.mem key
        [ "library"; "executable"; "test"; "rule"
        ; "install"; "env"; "inline_tests"
        ; "foreign_stubs"; "preprocess"
        ; "instrumentation"
        ]
  | _ -> false
...
let sort_atoms items =
  List.sort
    (fun a b -> String.compare (atom_name a) (atom_name b))
    items
...
let rec equal a b =
  ...
  | List l1, List l2 ->
      ...
      if is_keyed_stanza a || is_keyed_stanza b then
        equal_fields l1 l2
...
let rec diff a b =
  ...
  | Atom s1, Atom s2 ->
      if s1 = s2 then None
      else Some (Printf.sprintf "atom %S vs %S" s1 s2)
  | Comment _, _ | _, Comment _ -> None
  | List l1, List l2 ->
      let l1 = strip_comments_list l1 in
      let l2 = strip_comments_list l2 in
      if is_keyed_stanza a || is_keyed_stanza b then
        ...
        Some
          (Printf.sprintf "list length %d vs %d"
             (List.length l1) (List.length l2))
      else
        List.fold_left2
          (fun acc a b ->
            ...
            | None -> diff a b)
          None l1 l2
  | Atom _, List _ -> Some "atom vs list"
  | List _, Atom _ -> Some "list vs atom"

and diff_fields l1 l2 =
  match l1, l2 with
  | Atom k1 :: rest1, Atom k2 :: rest2 when k1 = k2 ->
      let keyed1 =
        List.filter (fun f -> field_key f <> None) rest1
      in
      let keyed2 =
        List.filter (fun f -> field_key f <> None) rest2
      in
      ...
      (match missing with
       | Some f ->
           Some
             (Printf.sprintf "field %s: present in generated \
                              but missing in original"
                (match field_key f with
                 | Some k -> k
                 | None -> "?"))
       | None ->
  ...
  | _ ->
      if List.length l1 <> List.length l2 then
        Some
          (Printf.sprintf "stanza list length %d vs %d"
             (List.length l1) (List.length l2))
      else None
...
and diff_set l1 l2 =
  ...
  | Atom k1 :: rest1, Atom k2 :: rest2 when k1 = k2 ->
      ...
      if List.length s1 <> List.length s2 then
        Some
          (Printf.sprintf "%s: %d vs %d items"
             k1 (List.length s1) (List.length s2))
      else
        ...
        (match missing with
         | Some m ->
             Some
               (Printf.sprintf "%s: %s in generated \
                                but not in original"
                  k1 (atom_name m))
         | None ->
             let extra =
