...
let test_parse_quoted_string () =
  ...
  Alcotest.(check bool) "quoted string"
    true
    (match result with
...
let test_parse_comment () =
  ...
  Alcotest.(check int) "atom + comment"
    2 (List.length result);
  Alcotest.(check bool) "second is atom"
    true
    (match result with
     | [ Comment _; Atom "foo" ] -> true
     | _ -> false)
...
let test_roundtrip () =
  ...
  Alcotest.(check bool) "roundtrip"
    true (equal_stanzas [ sexpr ] parsed)

(* -- Structural comparison tests ---------------------------------- *)

let test_equal_atoms () =
  Alcotest.(check bool) "same atoms"
    true (equal (Atom "foo") (Atom "foo"));
  Alcotest.(check bool) "diff atoms"
    false (equal (Atom "foo") (Atom "bar"))
...
let test_equal_ignores_comments () =
  ...
  in
  Alcotest.(check bool) "ignores comments"
    true (equal a b)
...
let test_equal_field_order_insensitive () =
  ...
  in
  Alcotest.(check bool) "field order insensitive"
    true (equal a b)
...
let test_equal_dep_order_insensitive () =
  ...
  in
  Alcotest.(check bool) "dep order insensitive"
    true (equal a b)
...
let test_equal_detects_missing_dep () =
  ...
  in
  Alcotest.(check bool) "detects missing dep"
    false (equal a b)
...
let test_equal_detects_missing_field () =
  ...
  in
  Alcotest.(check bool) "detects missing field"
    false (equal a b)
...
let test_equal_stanzas_multiple () =
  ...
  in
  Alcotest.(check bool) "multiple stanzas equal"
    true (equal_stanzas a b)
...
let test_diff_missing_field () =
  ...
  match diff a b with
  | Some msg ->
      Alcotest.(check bool) "mentions synopsis"
...
let test_diff_dep_set () =
  ...
  let b =
    "library"
    @: [ "name" @: [ atom "foo" ]
       ; "libraries" @: [ atom "a"; atom "d" ]
       ]
  in
  match diff a b with
  | Some msg ->
      Alcotest.(check bool) "reports dep diff"
        true (String.length msg > 0)
  | None ->
      Alcotest.fail "expected diff"
...
let test_library_basic () =
  Manifest.library "test_lib"
    ~path:"/tmp/test_manifest_out/basic"
...
let test_library_with_extras () =
  Manifest.library "test_with_extras"
    ~path:"/tmp/test_manifest_out/extras"
    ~deps:
      [ Manifest.opam "base"
      ; Manifest.local "foo_lib"
      ]
    ~ppx:Manifest.Ppx.minimal
    ~inline_tests:true
...
let test_executable_basic () =
  Manifest.executable "test_exe"
    ~path:"/tmp/test_manifest_out/exe"
    ~package:"test_pkg"
...
let test_at_colon () =
  ...
  Alcotest.(check bool) "at_colon"
    true
    (match s with
     | List [ Atom "library"; Atom "foo" ] -> true
     | _ -> false)

let test_nested_construction () =
  let s =
    "preprocess"
    @: [ "pps" @: [ atom "ppx_version"; atom "ppx_jane" ] ]
  in
  Alcotest.(check bool) "nested"
    true
    (match s with
     | List
         [ Atom "preprocess"
  ...
  Alcotest.run "manifest"
    [ ( "s-expr parsing"
      , [ Alcotest.test_case "atom" `Quick test_parse_atom
        ...
        ; Alcotest.test_case "nested" `Quick
            test_parse_nested
        ; Alcotest.test_case "quoted" `Quick
            test_parse_quoted_string
        ; Alcotest.test_case "comment" `Quick
            test_parse_comment
        ; Alcotest.test_case "dune library" `Quick
            test_parse_dune_library
        ; Alcotest.test_case "roundtrip" `Quick
            test_roundtrip
        ] )
    ; ( "s-expr comparison"
      , [ Alcotest.test_case "equal atoms" `Quick
            test_equal_atoms
        ...
        ; Alcotest.test_case "dep order" `Quick
            test_equal_dep_order_insensitive
        ; Alcotest.test_case "missing dep" `Quick
            test_equal_detects_missing_dep
    ...
    ; ( "s-expr diff"
      , [ Alcotest.test_case "no diff" `Quick test_diff_equal
        ; Alcotest.test_case "missing field" `Quick
            test_diff_missing_field
        ; Alcotest.test_case "dep set diff" `Quick
            test_diff_dep_set
        ] )
    ; ( "manifest DSL"
      , [ Alcotest.test_case "library basic" `Quick
            test_library_basic
        ; Alcotest.test_case "library extras" `Quick
            test_library_with_extras
        ; Alcotest.test_case "executable" `Quick
