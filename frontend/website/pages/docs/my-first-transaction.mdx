...
Since the public key is quite long and difficult to remember, let's save it as an environment variable:
    ...
    export CODA_PUBLIC_KEY=<public-key>

Now we can access this everywhere as `$CODA_PUBLIC_KEY` -- see if it saved properly by trying `echo $CODA_PUBLIC_KEY`. Note that this will only be saved for the current shell session, so if you want to save it for future use, you can add it to your `~/.profile` or `~/.bash_profile`.
