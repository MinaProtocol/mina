...
<Alert kind="welcome">
  ...
  The current status of the testnet is **ONLINE** - The Phase 3 testnet is launched! We send further announcements out on the [Coda Discord server](https://bit.ly/CodaDiscord) in the #announcements channel, as well as through email, when the network is live.

  **Sign Up to Receive Stake** - [sign up here](http://bit.ly/StakingSignup) to receive a delegation to stake on the network for the next testnet. Block producers (nodes that stake coda and participate in consensus) are integral to the Coda network, and producing blocks will help you win testnet challenges and accumulate testnet points*. Reach out in the Discord server if you have any questions about signing up to stake on the testnet.
...
<Alert>
  ...
  This documentation is for the **beta** release. The commands and APIs may change before the initial release. Last updated for version `0.0.12-beta`.
