...
As always we will continue to post updates in the #announcements channel in [Discord](https://bit.ly/CodaDiscord).
...
This option will **NOT** connect you to the live network, instead it will create a single node network that allows you to play with some of the protocol features without the instability of a live network.

</Alert>

To get started quickly and play with some of the core featues of the protocol you can use this docker demo container. This node will support multiple accounts, sending transactions between them, and will also support delegating and staking. In fact since it's a single node network, you'll get all the block rewards! This process is supported on MacOS and Linux.  
...
```
docker run --publish 3085:3085 -it codaprotocol/coda-demo:0.0.12-beta-demo-mode-sync-status-a6de071
```

Now you have a node running on your local machine, head over to https://localhost:3085/graphql to play with the GraphQL API of your node! Check out [this document](https://github.com/CodaProtocol/coda/blob/develop/docs/demo.md) for more information on the demo container.
