...
Did you know that Coda supports 32bytes of memos in its transactions? You can fit a SHA256 hash. Think of the possibilities! For this challenge, we'd like you to send a single transaction with a memo inside of it. `coda client send-payment` now supports a `-memo` flag. In that memo please stick the string "Hello Memo". You can send this transaction to anyone, for example a friend. You'll earn 500 pts[\*](#disclaimer) for doing so. As always, please hit the faucet with your discord account so that we can associate a public key with your discord username in order to add your score to the [leaderboard](#leaderboard).
