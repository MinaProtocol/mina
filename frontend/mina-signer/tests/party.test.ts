...
import type { AccountUpdate, Signed } from "../src/TSTypes";

/**
 * This is an example of a Zkapp_command transaction. This can be generated by
...
describe("AccountUpdate", () => {
  ...
  it("generates a signed accountUpdate", () => {
    const keypair = client.genKeys();
    const zkapp_command = client.signAccountUpdate(
      {
        zkapp_command: mockedZkapp_command,
        feePayer: {
    ...
    expect(zkapp_command.data).toBeDefined();
    expect(zkapp_command.signature).toBeDefined();
  });

  it("generates a signed accountUpdate by using signTransaction", () => {
    const keypair = client.genKeys();
    const zkapp_command = client.signTransaction(
      {
        zkapp_command: mockedZkapp_command,
        feePayer: {
          feePayer: keypair.publicKey,
  ...
  it("should calculate a correct minimum fee", () => {
    expect(client.getAccountUpdateMinimumFee(mockedZkapp_command.accountUpdates, 1)).toBe(1);
  });
});
