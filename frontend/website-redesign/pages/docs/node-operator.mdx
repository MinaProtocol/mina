...
Normally we'd want to have more funds before staking, but since we have some funds in our wallet from [the previous step](/docs/my-first-transaction), we can try out staking its mina by issuing the following command:
    ...
    mina client set-staking -public-key $CODA_PUBLIC_KEY

Alternatively, you can restart the daemon with the `-block-producer-pubkey` flag:

<DaemonCommandExample args={["-block-producer-pubkey $CODA_PUBLIC_KEY"]} />
...
First make sure you've unlocked your account:
    ...
    mina account unlock -public-key $CODA_PUBLIC_KEY
...
You can start a snark-worker using these commands:
    ...
    mina client set-snark-worker -address $CODA_PUBLIC_KEY

Alternatively, you can restart the daemon with these flags:

<DaemonCommandExample
  args={["-run-snark-worker $CODA_PUBLIC_KEY", "-snark-worker-fee <fee>"]}
/>
