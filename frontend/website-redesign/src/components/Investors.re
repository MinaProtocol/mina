module Styles = {
  ...
  let rule = style([marginTop(`rem(2.)),media(Theme.MediaQuery.desktop, [marginTop(`rem(7.93))])]);
  let advisorGrid = style([display(`grid), gridColumnGap(`rem(1.)), gridTemplateColumns([`repeat(`num(2),`rem(10.))]), gridAutoRows(`rem(17.3)), media(Theme.MediaQuery.tablet, [gridTemplateColumns([`repeat(`num(4), `rem(10.))])]),
   media(Theme.MediaQuery.desktop, [gridTemplateColumns([`repeat(`num(6), `rem(11.))])])]);
  let advisors = merge([Theme.Type.h2, style([marginTop(`rem(2.)), marginBottom(`rem(0.5))])]);
  let advisorsSubhead = merge([Theme.Type.sectionSubhead, style([marginBottom(`rem(2.))])]);
};
...
let make = () => {
  <div className=Styles.backgroundImage>
    <div className=Styles.innerContainer>
      ...
      <p className=Styles.advisorsSubhead> {React.string("Supporting O(1) Labs")} </p>
