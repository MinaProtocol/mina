...
module Item = {
  ...
  module Styles = {
    ...
    let headerSpacing = style([
      marginBottom(`rem(2.3125))
    ]);

    let contentSpacing = style([
  ...
  let make = (~item) => {
    <div className=Styles.container>
      ...
      <img className=Styles.contentSpacing src={item.img} width="464" height="258" />
      <p className=Css.merge([Theme.Type.sectionSubhead, Styles.contentSpacing])>
