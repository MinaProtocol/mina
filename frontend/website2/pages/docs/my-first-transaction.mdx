...
Run the following command to start up a Coda node instance and connect to the network:
    ...
    coda daemon \
        -discovery-port 8303 \
        -peer /ip4/34.82.159.8/tcp/10002/ipfs/12D3KooWDjSiDwnatZyNw7Mf9G1iWL29YdAJWQF35vkRwcbu7fyL
...
If you are running the node on a cloud virtual machine, make sure to copy your generated private key file to your local machine. You can do this with the command `ssh copy <source> <destination>`. In the case that your machine shuts down, you will need to import the private key file using the following command and flag: 
