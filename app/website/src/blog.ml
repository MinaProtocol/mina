...
let call_to_action_footer =
  ...
  div [ class_ "call-to-action-footer"; id id_str ]
    [ div []
      ...
      ; node "a"
          [ href "https://twitter.com/o1_labs"
          ; class_ "twitter-link"
          ]
      ...
      ; node "div"
          ...
          ; Attribute.create "onclick"
              (sprintf
                 {js|document.getElementById('%s').remove()|js} (String.escaped id_str))
          ]
...
let wrap_post ~standalone (post : Post.t) =
  ...
  div [ class_ "post-container" ]
    ( [ h1 [ class_ "post-title" ]
      ...
      @
      if standalone
      then [ comments_section post; blog_footer ]
...
let load_posts () =
  let assert_no_duplicate_titles (posts : Post.t list) =
    let has_dup =
      List.contains_dup ~compare:(fun p1 p2 ->
          String.compare p1.Post.basename p2.basename)
        posts
    in
    if has_dup then begin failwith "Duplicate title in posts" end
  in
  let%bind post_paths =
    Sys.ls_dir posts_dir
    >>| List.filter ~f:(fun p -> let (_, e) = Filename.split_extension p in e = Some "markdown")
  in
  let%map posts =
  ...
  let posts =
    List.sort
      ~cmp:(fun p1 p2 -> Date.compare p2.Post.date p1.date)
      posts
  in
...
let headers =
  ...
  let mathjax_url = "https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.2/MathJax.js?config=TeX-MML-AM_CHTML" in
  [ link ~href:"/static/css/code.css"
  ; link ~href:"/static/css/blog-common.css"
  ; literal {html|<link media='only screen and (min-device-width: 700px)' rel='stylesheet' href='/static/css/blog.css'>|html}
  ; literal {html|<link media='only screen and (max-device-width: 700px)' rel='stylesheet' href='/static/css/blog-mobile.css'>|html}
...
let load () =
  ...
  in
  directory "blog"
    [ file (File.of_html ~name:"index.html" (index posts))
    ; posts_dir
    ]
