  ...
  let create ~image ~image_positioning ~icon_image ~important_text =
    ...
    let info =
      div [class_ "example-info"]
        [ div [ class_ "mb3" ] [icon_image]
        ; div [ class_ "mb2" ] [important_text]
        ]
    in
    let (left, right) =
...
let html title =
  h3 [Style.render (style title)] [text @@ to_string title]

let content_style title =
    Style.(Styles.copytext + (position title))
end
...
module Important_text = struct
  let create ~title ~content =
    ...
    let (content_style, title_html) = 
      Important_title.(content_style title, html title) in
    let content_length =
      List.length content
    in
...
module Investor = struct
  let create name ext =
    let icon =
      let basename =
        String.split ~on:' ' (String.lowercase name)
          |> List.hd_exn
      in
      Icon.investor
        basename
    ...
    [ div [class_ "ph2 ph3-ns"]
      ...
      ; h4 [Style.(render (Styles.heading_style + of_class "tc w-100 silver fw5 force-word-breaks f8 f7-ns"))] [text name]
      ]
    ]
end
...
module Link_list = struct
  let create ~orientation ~named xs =
    ...
    in
    let title_style =
      Style.(Styles.heading_style + of_class "black f4 tl")
    in
    let heading =
    ...
    let (maybe_vert_centering_style, orientation_style) =
      match orientation with
      | `Horizontal -> Style.(of_class "flex items-center", of_class "flex justify-around w-100")
    ...
    div [class_ "w-100"]
      ...
      ; ul [Style.(render (orientation_style + of_class "list ph0"))]
        (List.map xs ~f:(fun (name, link, label) ->
          let icon n =
            ksprintf Html.text {literal|<i class="ml-1 ml-2-ns fab f1 f2-m f1-l fa-%s mr3 mr2-m mr3-l"></i>|literal} n
          in
          let inline_block_maybe =
            let open Style in
            inline_block_maybe +
          ...
          let (link, maybe_second_link) =
            ...
            | `Two (next_name, s1, s2) -> (s1, fun style -> a Style.(style + of_class "ml-5px") s2 [Html.text next_name] label)
          in
          let name =
            match name with
            | `Icon n ->
                  [ icon (String.lowercase n)
                  ; span [] [text (" " ^ n)]
                  ]
            | `Read n -> [Html.text ("📖 " ^ n)]
            | `Watch n -> [Html.text ("🎥 " ^ n)]
          ...
          in
          let link_style =
            Style.(maybe_vert_centering_style + of_class "dib no-underline fw3 f4 silver lh-copy")
          in
          li [Style.(render (inline_block_maybe + of_class "mb3 mr3"))]
...
module Job_listing = struct
  let create ~title ~jobs ~scheme =
    ...
    let (content_style, title_html) =  
      Important_title.(content_style title, html title) in
    let of_jobs (name, link) =
      (`Event name, `One (link ^".html"), name)
    in
    let link_list = [(Link_list.create 
          ~named:""
          ~orientation:`Vertical
          (List.map jobs ~f:of_jobs))] in
    Section.section'
    (div [class_ "important-text"]
      [ Mobile_switch.create
        ~not_small:(Important_title.wrap title title_html)
...
let home () =
  let top scheme =
    Section.major_compound
      ...
      ~image:(
        Mobile_switch.create
          ~not_small: (
        Html_concise.(node "img" [ Attribute.src "/static/img/compare-outlined.svg"; Attribute.create "width" "600px" ] []))
          ~small:(
        Html_concise.(node "img" [ Attribute.src "/static/img/compare-outlined.svg"; Attribute.create "width" "600px"; Attribute.create "height" "200px" ] []))
      )
  ...
  let mission scheme =
    Section.major_text
      ?heading:(Some "Our Mission")
      ~important_text:(
        Important_text.create
          ~title:(`Center "We're making the world legible at a glance")
          ~content:
            [ "Coda is a cryptocurrency that can
scale to millions of users and thousands of transactions per second while
remaining decentralized. Any client, including smartphones, will be
able to instantly validate the state of the ledger. This is in service of our
goal to make software transparent, legible, and respectful."
  ...
  let examples =
    [ Example.create
        ...
        ~image_positioning:Image_positioning.Left
        ~icon_image:Icon.empty
        ~important_text:(
          Important_text.create
            ~title:(`Left "Compact blockchain")
            ~content:
              [ "Coda uses recursive composition of zk-SNARKs to compress the whole
                blockchain down to the size of a few tweets."
              ; "No one needs to store
                or download transaction history in order to verify the blockchain."
              ]
        )
    ; Example.create
        ...
        ~important_text:(
          Important_text.create
            ~title:(`Right "Instant sync")
            ~content:
              [ "Coda syncs instantly, allowing you to verify the blockchain even on a mobile phone. 
Interact with the blockchain anywhere."
  ...
  let job_examples =
    [ Example.create
        ...
        ~important_text:(
          Important_text.create
            ~title:(`Left "Functional programming")
            ~content:
              [ "A cornerstone of our approach is a focus on building
                reliable software through the use of statically-typed functional programming languages."
              ; "This is reflected in our OCaml codebase and style of
                structuring code around DSLs, as well as in the design of languages we're developing for Coda."
              ])
    ; Example.create
        ...
        ~important_text:(
          Important_text.create
            ~title:(`Left "Cryptography and mathematics")
            ~content:
    ...
    ; Example.create
        ...
        ~important_text:(
          Important_text.create
            ~title:(`Left "Distributed systems")
            ~content:
              [ "We implement state-of-the-art
                consensus protocols and have developed
                frameworks for describing distributed systems, enabling us to quickly
                iterate."
              ]
        )
    ]
  ...
  let protocol_design scheme =
    Section.with_examples
      ?heading:(Some "Protocol Design")
      ~content:None
      ~examples
      ~scheme
      ~cta_hint:"Whitepaper Draft"
  ...
  let community scheme =
    let social_list orientation =
      ...
      Link_list.create
        ...
        ~orientation
    in
    Section.three_subgroups
      ?heading:(Some "Community")
      ~groups:
        ( Mobile_switch.create
        ...
        , Link_list.create
            ...
            ~orientation:`Vertical
            [ 
              (`Read "Keeping Cryptocurrency Decentralized", `One (let (_, u, _) = Links.blog in u), "blogpost-main")
            ; (`Read "Coindesk: This Blockchain Tosses Blocks", `One "https://www.coindesk.com/blockchain-tosses-blocks-naval-metastable-back-twist-crypto-cash/", "coindesk-blogpost")
            ; (`Read "TokenDaily: Deep Dive with O(1) on Coda Protocol", `One "https://www.tokendaily.co/p/q-as-with-o-1-on-coda-protocol", "tokendaily-ama")
            ]
        , Link_list.create
            ~named:"Media"
            ~orientation:`Vertical
            [ (`Watch "Hack Summit 2018: Coda Talk"), `One "https://www.youtube.com/watch?v=eWVGATxEB6M", "hack-summit"
            ; (`Listen "Token Talks - Interview with Coda"), `One "https://simplecast.com/s/17ed0e8d", "token-talk"
            ; (`Watch "A High-Level Language for Verifiable Computation"), `One "https://www.youtube.com/watch?v=gYn6mTwJriw", "snarky1"
            ; (`Watch "Snarky, a DSL for Writing SNARKs", `One "https://www.youtube.com/watch?v=h0PUVR0s6Vg", "snarky2")
            ]
        )
      ~scheme
      ()
  ...
  let jobs scheme =
    Section.with_examples
      ?heading:(Some "Jobs")
      ~content:(Some "We're hiring engineers to work on exciting problems in cryptography, programming languages, and distributed systems, though by no means do applicants already need to be experts in these fields. We are committed to building a diverse, inclusive company. People of color, LGBTQIA individuals, women, and people with disabilities are strongly encouraged to apply.")
      ~examples:job_examples
      ~scheme
      ~cta_hint:"Apply Now"
      ~cta_label:"apply-cta"
      ~cta_url:(let (_, u, _) = Links.jobs in u)
      ()
  in
  let investors scheme =
    Section.investors
      ?heading:(Some "Investors")
      ~investors: Investor.(
        let c = create in
        [ c "Metastable" `Png
...
let load_job_posts jobs =
  Deferred.List.map jobs ~f:(fun (name, file) ->
    ...
    let%map description = Stationary.Html.to_string (Stationary.Html.load markdown) in
  ...
  Command.async ~summary:"build site"
    Command.Param.(
      ...
      let%map dst = anon ("BUILD-DIR" %: file)
          and working_directory = flag "working-directory" ~doc:"Working directory relative to executing filesystem commands" (required string) in
