...
module Member = struct
  ...
  let to_html { name; affiliation; bio } =
    ...
    let top =
      ...
      and name =
        h3 [ Style.(render (Styles.heading_style + ["black f4 fw5"])) ] [ text name ]
      and affiliation =
        match affiliation with
        | None -> span [] []
        | Some affiliation ->
            h4 [ Style.(render (Styles.heading_style + ["silver f5"])) ] [ text affiliation ]
      in
      div [class_ "flex items-center"]
        [ div [class_ "w-25"] [icon]
        ; div [class_ "ph4 w-100"]
          [ name
          ; affiliation
          ]
        ]
    in
    div [ class_ "ph3 ph4-ns pv3 relative team-card-shadow bg-white" ]
      [ top
      ; p [Style.(render Styles.copytext)] [text bio]
      ]
end
...
let core () =
  let open Member in
  let plain name affiliation bio = { name; bio; affiliation = Some affiliation } in
  ...
  and rebekah =
    plain "Rebekah Mercer"
      "Protocol Researcher"
      "Rebekah is a PhD student at Aarhus University, where she is advised by Claudio Orlandi and Ivan Damgård. Her research revolves around cryptography and privacy, particularly privacy in cryptocurrencies. Rebekah holds an MSc in Information Security from UCL and a BSc in Mathematics from the University of Manchester."
  and nacera =
    plain "Nacera Rodstein"
...
|literal}
  ...
  in
  return [ evan; izaak; brad; brandon; corey; deepthi; nathan; john; joel; rebekah; nacera; joe; akis; benedikt; jill; paul ]
