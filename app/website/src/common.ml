...
let analytics_handler label =
  let handler = sprintf {js|_gaq.push(['_trackEvent', 'coda', 'click', '%s', '0']);|js} label in
...
module Image = struct
  let draw ?(style=Style.empty) src xy =
    let inline_style =
      ...
      | `Free -> ""
    in
    ksprintf Html.literal{literal|<img style='%s' class="%s" src='%s'>|literal} inline_style (String.concat ~sep:" " style) src

  let placeholder ?(style=Style.empty) x y =
    let xStr = Int.to_string x in
    let yStr = Int.to_string y in
    draw ~style
      (Printf.sprintf "http://via.placeholder.com/%sx%s/8492A6/1F2D3D" xStr yStr)
      (`Fixed (x, y))
end
...
module Icon = struct
  ...
  let placeholder x y =
    ...
    Image.placeholder ~style:round x y
  ...
  let investor name ext =
    ...
    Image.draw ~style:round_shadowed
      (Printf.sprintf "/static/img/investors/%s" (String.concat ~sep:"." [name; ext]))
      (`Fixed (50, 50))
...
module Links = struct
  ...
  let blog = ("Blog", "https://medium.com/codaprotocol/coda-keeping-cryptocurrency-decentralized-e4b180721f42", "blog")
...
module Input_button = struct
  let create ~label ~button_hint ~extra_style ~url =
    ...
    node "a"
      [ href url
      ; Style.(render (of_class "user-select-none hover-bg-black white no-underline ttu tracked bg-silver icon-shadow ph3 pv3 br4 tc lh-copy" + extra_style))
  ...
  let fixed ~button_hint =
    let open Html in
    let open Html_concise in
    create ~button_hint ~extra_style:(Style.of_class "f5 bottomrightfixed br--top") ~label:"fixed" ~url:"https://goo.gl/forms/PTusW11oYpLKJrZH3"
end
  ...
  let navbar =
    ...
    let a style url children label open_new_tab =
      ...
      node "a"
        ...
        ] @ extra_attrrs) children
    in
    let coda style =
      div [ Style.render style ]
        [ node "img" [ Attribute.create "width" "170px"; Attribute.src "/static/img/logo.svg" ] [] ]
    in
    let a' ?(open_new_tab=false) ?(style=Style.empty) url children label =
      a (Style.(of_class "no-underline fw3 silver tracked ttu" + style)) url children label open_new_tab
    in
    div [ Style.(render (of_class "flex items-center mw9 center mt3 mt4-m mt5-l mb4 mb5-m" + Spacing.side_padding)) ]
      [ div [ class_ "w-50" ] [ coda Style.empty ]
      ; div [ class_ "flex justify-around w-75" ]
          [ (let (name, url, label) = Links.blog in a' ~open_new_tab:true url [ text name ] label)
          ; a' ~style:Visibility.only_large "index.html#community" [ text "Community" ] "community"
          ; (let (name, url, label) = Links.jobs in a' url [ text name ] label )
          ; a' ~style:Visibility.no_mobile ~open_new_tab:true "https://goo.gl/forms/PTusW11oYpLKJrZH3" [ text "Sign up" ] "sign-up"
          ]
      ]
end
...
module Footer = struct
  let title_style =
    Style.(
      Styles.heading_style + of_class "black f6 tl"
    )
...
module Section = struct
  let section' ?heading ?(footer=false) content scheme =
    ...
    let maybe_id =
      ...
      | Some heading -> 
          let section_id =
            String.split ~on:' ' (String.lowercase heading)
              |> List.hd_exn
          in
          [id section_id]
      | None -> []
    ...
    div [ Style.(render (["bxs-cb" ; bg_color] + maybe_footer))] [
      section ([Style.(render (["section-wrapper" ; "pv4"; "mw9"; "center" ; "bxs-bb"] + Spacing.side_padding)) ] @ maybe_id)
        (match heading with
  ...
  let major_compound ?heading ~important_text ~image ~image_positioning ~scheme () =
    ...
    let html =
      Mobile_switch.create
        ~not_small:(
          div [class_ "flex items-center mb5"]
            [ div [class_ "flex w-50"]
              [ div [class_ "mw6"] [left]
              ]
            ; div [class_ "flex w-50"] 
              [ div [class_ "center"] [right]
              ]
            ]
        )
        ~small:(
  ...
  let major_text ?heading ~important_text ~scheme () =
    let open Html_concise in
    let content =
      div [class_ "center mw7"] [ important_text ]
    in
  ...
  let with_examples ?heading ~content ~examples ~scheme ~cta_hint ~cta_label ~cta_url () =
    section' ?heading begin
      let open Html_concise in
      let top_style =
        let open Style in
        Styles.copytext + (of_class "mw6 tc center mb5")
      in
      let top =
        match content with
        | Some content ->
        ...
        | None ->
          div [class_ "dn"] []
      in
      let examples = List.map examples ~f:(fun e ->
        li [class_ "mb5 mw8 center"] [ e ])
      in
      div [class_ "with-examples"]
        [ top
        ; ul [class_ "list ph0"] examples
        ; div [class_ "flex justify-center"]
        [ Input_button.cta ~button_hint:cta_hint ~label:cta_label ~url:cta_url ]
        ]
    end scheme
  ...
  let footer' ?(fixed=false) ~links ~newsletter ~scheme =
    let content =
      ...
      in
      let cdots =
        (span [class_ "dn"] []) :: (List.init ((List.length links) - 1) ~f:(fun _ ->
          span [class_ "f6 silver"] [text "·"]
        )) |> List.rev
      in
      div [class_ "flex justify-center tc mb4"]
      [ ul [Style.(render (of_class "list ph0"))]
          (List.map (List.zip_exn links cdots) ~f:(fun ((name, link, label), cdot) ->
            li [class_ "mb2 dib"]
              [ a Style.(of_class "no-underline fw3 f6 silver") link [text name] label
              ; cdot
              ]
          ))
      ; newsletter
    ...
    let content =
      ...
      Mobile_switch.create
        ~not_small:(
          ...
          ; div [class_ "flex justify-around"]
            [ wrap b
            ; wrap c
            ]
          ]
...
let wrap ?(headers=[]) ?(fixed_footer=false) ?title sections =
  let title = Option.value_map ~default:"Coda Cryptocurrency Protocol" title ~f:title_string in
  let open Html in
  ...
  let head =
    node "head" []
      ...
      ; literal "<meta property='og:image' content='https://codaprotocol.com/static/img/compare-outlined-png.png' />"
      ; literal "<meta property='og:type' content='website' />"
      ; literal "<meta property='og:url' content='https://codaprotocol.com' />"
      ; ksprintf literal "<meta property='og:title' content='%s' />" title
      ; literal "<meta property='og:description' content='That means that no matter how many transactions are performed, verifying the blockchain remains inexpensive and accessible to everyone.' />"
      ; literal "<meta name='description' content='That means that no matter how many transactions are performed, verifying the blockchain remains inexpensive and accessible to everyone.' />"
      ; literal "<meta property='og:updated_time' content='1526001445' />"
      ; node "title" [] [ text title ]
      ; analytics
      ; link ~href:"https://fonts.googleapis.com/css?family=Rubik:500"
      ; link ~href:"https://fonts.googleapis.com/css?family=Alegreya+Sans:300,300i,400,400i,500,500i,700,700i,800,800i,900,900i"
      ; link ~href:"/static/css/common.css"
      ; literal {html|<link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.0.12/css/all.css" integrity="sha384-G0fIWCsCzJIMAVNQPfjH08cyYaUtMwjJwqiRKxxE/rx96Uroj1BtIQ6MLJuheaO9" crossorigin="anonymous">|html}
      ; literal {html|<link media='only screen and (min-device-width: 700px)' rel='stylesheet' href='/static/css/main.css'>|html}
      ; literal {html|<link media='only screen and (max-device-width: 700px)' rel='stylesheet' href='/static/css/mobile.css'>|html}
      ; literal {html|<link rel="icon" type="image/png" href="/static/favicon-32x32.png" sizes="32x32" />
  ...
  let body =
    ...
    let reified_sections =
      List.mapi (sections @ [footer]) ~f:(fun i s ->
        s (if (i % 2 = 0) then `Light else `Dark)
      )
    in
    node "body"
