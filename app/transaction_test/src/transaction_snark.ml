...
module Base = struct
  ...
  let main top_hash =
    ...
    in
    apply_transactions l1 ts >>= Ledger.assert_equal l2

  let keypair = generate_keypair main ~exposing:(tick_input ())
  let pk = Keypair.pk keypair
  let vk = Keypair.vk keypair

  let handler (public_key_to_index : Public_key.Compressed.t -> Account.Index.t) ledger =
    fun (With { request; respond }) ->
      let open Ledger in
      match request with
      | Get_element idx ->
        let path = Merkle_tree.get_path !ledger idx in
        let elt = Merkle_tree.get_exn !ledger idx in
        respond (Provide (elt, path))
      | Get_path idx ->
        respond (Provide (Merkle_tree.get_path !ledger idx))
      | Set (idx, account) ->
        ledger := Merkle_tree.update !ledger idx account;
        respond (Provide ())
      | Empty_entry -> unhandled
      | Find_index pk ->
        respond (Provide (public_key_to_index pk))
      | _ -> unhandled
  ;;
  ...
  let bundle public_key_to_index ledger1 transaction =
    ...
    in
    let ledger2 = apply_transaction ledger1 transaction in
    let state1 = Ledger.of_hash (Merkle_tree.root ledger1) in
    let state2 = Ledger.of_hash (Merkle_tree.root ledger2) in
    let prover_state : Prover_state.t =
      { state1; state2; transactions = [ transaction ] }
    in
    let top_hash =
      Pedersen.hash_fold Pedersen.params
        (List.fold (Ledger.to_bits state1 @ Ledger.to_bits state2))
    in
    let main top_hash =
