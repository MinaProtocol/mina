...
let string_to_bits s =
  List.init (8 * String.length s) ~f:(fun i ->
    ((Char.to_int s.[i / 8]) lsr (7 - (i mod 8))) land 1 = 1)
...
let gadget_string_to_bits s =
  ...
  let block =
    Block.of_list_exn
      (List.map ~f:Boolean.var_of_value (string_to_bits s))
  in
  let t =
  ...
  in
  let ((), x) =
    Or_error.ok_exn (run_and_check t ())
  in
  x
;;
...
let gadget_string_to_words s =
  ...
  in
  chunks_of 32 (gadget_string_to_bits s)
  |> List.map ~f:word_of_big_endian_bits

let () = 
  Quickcheck.test ~trials:30 (String.gen_with_length 64 Char.gen) ~f:(fun input ->
  [%test_eq: int32 list]
    (Array.to_list
