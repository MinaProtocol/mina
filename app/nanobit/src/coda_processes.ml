...
module Coda_processes = struct
  ...
  let net_configs n =
    ...
    let all_peers = List.map ports ~f:(fun p -> Host_and_port.create "127.0.0.1" p) in
    let peers = List.init n ~f:(fun i -> 
  ...
  let spawn_local_processes_exn n ~program_dir ~f =
    ...
    let scoped = 
      List.fold 
        ...
        ~f:(fun last fn -> fun ws -> fn (fun w -> last (w::ws)))
    in
    first (fun w -> 
