...
struct
  ...
  let spawn_local_processes_exn ?proposal_interval ?(should_propose= Fn.const true)
            ...
            Option.bind public_key ~f:(fun public_key ->
                Option.bind public_key ~f:(fun public_key ->
                    Some
                      ...
                      ; port= 20000 + i } ) )
          in
          Coda_process.spawn_local_exn ?proposal_interval ~peers ~discovery_port ~external_port
