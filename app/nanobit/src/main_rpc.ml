...
let main () =
  ...
  let init_pipes () : Main.pipes * Blockchain.t Linear_pipe.Reader.t =
    let ( gossip_net_strongest_block_reader
      ...
      Linear_pipe.fork6 strongest_block_reader
    in
  ...
  let run_main _ {Rpcs.Main.storage_location; initial_peers; should_mine; me} =
    ...
    don't_wait_for
      (Linear_pipe.iter rpc_strongest_block_reader ~f:(fun blockchain ->
    ...
    let%map membership =
      ...
      Main.main_nowait ~log ~prover ~storage_location ~genesis_blockchain
        ~initial_peers ~should_mine ~me
        ~remap_addr_port:(fun addr ->
          Host_and_port.create
            ~port:(Host_and_port.port addr + 1)
  ...
  let get_peers _ () =
    return
      (Option.map !membership_ref ~f:(fun membership ->
           Membership.Haskell.peers membership ))
  in
...
let command : Command.t =
  Command.async ~summary:"nanobit rpc server" (Command.Param.return main)
