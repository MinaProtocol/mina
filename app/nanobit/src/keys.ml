...
let create () =
  ...
  | None ->
    ...
    in
    let module T = Transaction_snark.Make(struct let keys = tx_keys end) in
    let module B = Blockchain_snark.Blockchain_transition.Make(T) in
    let module Step = B.Step(struct let keys = bc_keys.step end) in
    let module Wrap = B.Wrap(struct let verification_key = Tick.Keypair.vk bc_keys.step end)(struct let keys = bc_keys.wrap end) in
    let module M = struct
      let transaction_snark_keys = tx_keys

      module Step = struct
        include (Step : module type of Step with module Prover_state := Step.Prover_state)
      ...
      module Wrap = struct
        include (Wrap : module type of Wrap with module Prover_state := Wrap.Prover_state)
