...
module Transaction_snark_keys = struct
  let keys =
    if Insecure.key_generation then Transaction_snark.Keys.dummy ()
    else Transaction_snark.Keys.create ()
end

let transaction_snark_keys =
  let keys = Transaction_snark_keys.keys in
...
module Transition =
Blockchain_snark.Blockchain_transition.Make (Transaction_snark.Make (Transaction_snark_keys))
...
let real_tick_vk, tick_vk, tick_pk =
  if Insecure.key_generation then
    ...
    let tick_kp = Tick.generate_keypair (Step_base.input ()) Step_base.main in
...
let tock_vk, tock_pk =
  ...
  | `Secure tick_vk ->
      ...
      let tock_kp =
        Tock.generate_keypair
          (Wrap_base_applied.input ())
          Wrap_base_applied.main
      in
...
let _ =
  let do_write name key =
    let%bind writer =
      Writer.open_file (Printf.sprintf "/tmp/nanobit_%s" name)
    in
  ...
  and () = do_write "transaction_snark_keys" transaction_snark_keys in
  (* TODO Sha *)
  let open Md5 in
  let ( tick_pk_digest
      , tick_vk_digest
  ...
  let code =
    Printf.sprintf
      "\n    \
       ...
       open Snark_params\n\n      \
       let transaction_snark_keys =\n        \
       let b = Bigstring.of_string (In_channel.read_all \
       \"/tmp/nanobit_transaction_snark_keys\") in\n        \
       assert Md5.(Md5.digest_bytes (Bigstring.to_bytes b) = of_hex_exn \
       \"%s\");\n        \
       Transaction_snark.Keys.bin_read_t b ~pos_ref:(ref 0)\n\n      \
       module Transition = \
       Blockchain_transition.Make(Transaction_snark.Make(struct let keys = \
       transaction_snark_keys end))\n\n      \
      ...
      (to_hex tick_vk_digest) (to_hex tick_pk_digest) (to_hex tock_vk_digest)
      (to_hex tock_pk_digest)
  in
  let%map _ =
    Process.run_exn ~prog:"/bin/bash"
      ~args:["-c"; Printf.sprintf "echo \'%s\' > %s" code Sys.argv.(1)]
      ()
  in
  exit 0
