 ...
 fun () ->
  ...
  let balance_change_or_timeout
    ...
    let rec go () =
      ...
      | Some b
        when not (Currency.Balance.equal b initial_receiver_balance) ->
          return ()
      | _ ->
  ...
  let build_txn amount =
    ...
    let nonce = Run.get_nonce minibit Genesis_ledger.rich_pk |> Option.value_exn in
  ...
  let test_sending_transaction () =
    ...
    let prev_sender_balance = Run.get_balance minibit Genesis_ledger.rich_pk |> Option.value_exn in
    let prev_receiver_balance = Run.get_balance minibit Genesis_ledger.poor_pk |> Option.value_exn  in
    ...
    * will be rejected because the nonce is wrong *)
    ...
    let%map () = balance_change_or_timeout ~initial_receiver_balance:prev_receiver_balance in
