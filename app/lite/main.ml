...
let float_to_string value =
  Js.to_string ((Js.number_of_float value)##toString)
...
module Svg = struct
  ...
  let main ?class_ ~width ~height cs =
    Node.svg "svg"
      ...
      @ Option.to_list
          (Option.map class_ ~f:Attr.class_))
      cs
  ...
  let path points =
    let points =
      String.concat ~sep:" "
        (List.map points ~f:(fun {Pos.x;y} -> sprintf "%s,%s" (float_to_string x) (float_to_string y)))
    in
    Node.svg "polyline"
      [ Attr.create "points" points
      ; Attr.create "style" (sprintf "fill:none;stroke:%s;stroke-width:3" line_color)
      ]
      []
  ...
  let rect ?radius ~color ~width ~height ~(center : Pos.t) =
    ...
    in
    let rad_attrs =
      Option.value_map radius ~default:[]
        ~f:(fun r -> 
          [ Attr.create_float "rx" r
          ; Attr.create_float "ry" r ])
    in
    Node.svg "rect"
...
module State = struct
  type t =
    { verification
      : [ `Pending of int | `Complete of unit Or_error.t ]
    ; chain : Lite_chain.t
    }

  let init = { verification = `Complete (Ok ()); chain = Lite_params.genesis_chain }

  let chain_length chain = chain.Lite_chain.protocol_state.consensus_state.length 

  let should_update state (new_chain : Lite_chain.t) =
    Length.compare (chain_length new_chain)
      (chain_length state.chain) > 0
end
...
let drop_top_of_tree =
  ...
  let rec drop n t =
    ...
    then t
      ...
      | Node (_, Hash _ , t')
      | Node (_, t', Hash _) -> drop (n - 1) t'
      | Node (_, Account _, Node _)
      | Node (_, Node _, Account _) ->
  ...
  fun t ->
    ...
    if n <= desired_layers
    then t
    else drop (n - desired_layers) t

let style kvs =
  List.map ~f:(fun (k, v) -> sprintf "%s:%s" k v) kvs
  |> String.concat ~sep:";"
...
module Html = struct
  ...
  module Record = struct
    module Entry = struct
      type t =
        { label : string; value: string; class_: string option; width:string option }
      ...
      let render {label; value; class_; width} =
        div
          ...
          [ div
              [ Attr.class_ "record-entry-label" ]
              [ Node.text label ]
          ; div [ Attr.class_ "record-entry-value" ]
              [ Node.text value ]
          ]

      let create ?class_ ?width label value =
        { label; value; class_; width }
    end
    ...
    module Row = struct
      ...
      let render (t:t) =
        div
          [ Attr.class_ "record-row"
          ]
          (List.map ~f:Entry.render t)
    end
    ...
    let render { class_; rows; attrs } = 
      div (Attr.class_ (extend_class "record" class_) :: attrs)
...
*)
  ...
  fun (tree0 : ('hash, 'account) Sparse_ledger_lib.Sparse_ledger.tree) ->
    ...
    let specs =
      let rec go acc layer index (t : ('h, 'a) Sparse_ledger_lib.Sparse_ledger.tree)  =
        let pos : Pos.t = { x = x_pos ~layer ~index; y = y_pos ~layer } in
        let hash_spec h =
          Spec.Node { pos; color = color_of_hash h }
        in
    ...
    let rendered =
      ...
      let nodes =
        ...
        List.filter_mapi specs ~f:(fun i spec ->
          ...
          | Spec.Node { pos; color } ->
            Some (
              Svg.rect ~radius:3. ~color ~center:pos
                ~height ~width)
          | Spec.Account _ -> None
        )
      in
      ...
      let account =
        ...
        | Spec.Node _ -> None
        | Spec.Account { pos={x; y}; account={public_key;balance;nonce;receipt_chain_hash} } ->
      ...
      Node.div [Attr.class_ "merkle-tree"]
        ( Svg.main ~width:(image_width +. left_offset) ~height:(image_height +. top_offset)
            (edges :: nodes)
...
let state_html
      { State.verification
      ; chain =
          {protocol_state={previous_state_hash;blockchain_state;consensus_state}; ledger; proof}
      }
  =
  let { Blockchain_state.ledger_builder_hash; ledger_hash; timestamp } = blockchain_state in
  ...
  let state_record =
    ...
    create ~class_:"state"
      ...
      ; [ Entry.create "locked_ledger_hash"
            (field_to_base64 ledger_hash) ] 
      ]
  in
  div [ class_ "main" ]
    ...
    ; merkle_tree
        (Sparse_ledger_lib.Sparse_ledger.tree ledger)
    ]
;;
...
let main ~render ~get_data =
  ...
  Verifier.set_on_message verifier ~f:(fun resp ->
    ...
    | `Pending id when id = Verifier.Response.id resp ->
      update_state_and_vdom
        { !state with verification = `Complete (Verifier.Response.result resp) }
    | _ -> ()
  );
  ...
  let loop () =
    ...
    get_data ~on_result:(fun res ->
      ...
      then begin
        latest_completed := id;
        if State.should_update !state res
        then begin
          update_state_and_vdom
            { verification = `Pending id; chain = res };
          Verifier.send_verify_message verifier
            (res, id)
        end
      end
    )
