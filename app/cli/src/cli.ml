...
let () =
  ...
  let rec ensure_testnet_id_still_good () =
    ...
    let try_later hrs =
      Async.Clock.run_after (Time.Span.of_hr hrs) (fun () -> don't_wait_for @@ ensure_testnet_id_still_good ()) ()
    in
    if force_updates then (
      let%bind resp, body =
        Client.get (Uri.of_string "http://updates.o1test.net/testnet_id")
      in
      if resp.status <> `OK then (try_later 0.10 ; eprintf "Error %s while getting testnet id, checking again in 6 minutes." (Cohttp.Code.string_of_status resp.status); Deferred.unit) else (
