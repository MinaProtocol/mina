...
let constant x =
  ...
  let n = bigint_of_backend_bigint tick_n in
  { upper_bound = n
  ; lower_bound = n
  ; var = Cvar.constant x
  ; bits =
      Some 
        (List.init (bigint_num_bits n) ~f:(fun i ->
          Boolean.var_of_value (Impl.Bigint.test_bit tick_n i)))
  }

let field_size = bigint_of_backend_bigint Impl.Backend.field_size
