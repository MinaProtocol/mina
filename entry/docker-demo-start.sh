...
function cleanup
{
  ...
  doas -u postgres pg_ctl stop -D ${POSTGRES_DATA_DIR}
  exit
}
...
doas -u postgres pg_ctl start -D ${POSTGRES_DATA_DIR}
...
doas -u postgres psql --command "CREATE USER ${POSTGRES_USERNAME} WITH SUPERUSER PASSWORD '${POSTGRES_USERNAME}';"
