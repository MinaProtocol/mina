...
module State = struct
  ...
  type t = (Time.t, Target.t, Digest.t, Block.Body.t, Strength.t) t_
  ...
  let meets_target (target : Target.Packed.value) (hash : Digest.Packed.value) =
    let t = Bigint.of_field (target :> Field.t) in
    let h = Bigint.of_field hash in
    Bigint.compare h t < 0
  ;;
  ...
  let update_exn (state : value) (block : Block.t) =
    ...
    assert (meets_target target block_hash);
