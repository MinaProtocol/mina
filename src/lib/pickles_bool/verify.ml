...
let verify_heterogenous (ts : Instance.t list) =
  ...
  let tick_field : _ Plonk_checks_bool.field = (module Tick_field) in
  let check, result =
    let r = ref [] in
    let result () =
      match !r with
      ...
      ~f:(fun
        ...
        let non_chunking, expected_num_chunks =
          let expected_num_chunks =
            Option.value_map ~default:Plonk_checks_bool.num_chunks_by_default
        ...
        let deferred_values =
          let zk_rows =
            Option.value_map ~default:Plonk_checks_bool.zk_rows_by_default
