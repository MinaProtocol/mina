...
let shift_s_poly s_poly n =
  ...
  let rec gen_cancel_poly i =
    if i > n then Bivariate_fr_laurent.zero
    else
      Bivariate_fr_laurent.(
        create Int.(i + n) [Fr_laurent.create i [Fr.of_int 1]]
        + create Int.(i + n) [Fr_laurent.create Int.(-i) [Fr.of_int 1]]
  ...
  let shifted_poly =
    Bivariate_fr_laurent.(create Int.(n + 1 + degree) (coeffs y_shifted_poly))
  in
  shifted_poly
...
let convert_to_psi_polys poly n =
  ...
  let sigma_1, sigma_2, sigma_3, psi_1, psi_2, psi_3 =
    convert_to_sigmas_psis poly n
  in
...
let sc_p (srs : Srs.t) y z psi_polys =
  let proofs =
    List.map psi_polys ~f:(fun psi_poly -> perm_p srs y z psi_poly)
  in
  let psi_evals =
    List.map proofs ~f:(fun (proof : Perm_proof.t) -> proof.perm_psi_eval)
  in
  List.iter
    ~f:(fun x -> Printf.printf "psi_eval: %s\n" (Fr.to_string x))
    psi_evals ;
...
let sc_v (srs : Srs.t) y z psi_polys (s, proofs) =
  ...
  && List.fold_left ~init:true ~f:( && )
       (List.map2_exn psi_polys proofs ~f:(fun psi_poly proof ->
            perm_v srs y z psi_poly proof))
