...
let commitPoly (srs: Srs.t) maxm x poly =
  ...
  let gxi = if diff >= 0
    then List.nth_exn srs.gPositiveAlphaX diff
    else List.nth_exn srs.gNegativeAlphaX (abs diff - 1) in
  G1.scale gxi (Fq.to_bigint (evalLaurent poly x))

let openPoly _srs _commitment x z fPoly =
  let fz = evalLaurent fPoly z in
  let wPoly = quotLaurent (subtractLaurent fPoly (makeLaurent 0 [fz])) (makeLaurent 0 [Fq.negate z; Fq.one]) in
...
let pcV (srs: Srs.t) maxm commitment z (v, w) = 
  ...
  let hxi = if diff >= 0
    then List.nth_exn srs.hPositiveX diff
    else List.nth_exn srs.hNegativeX (abs diff - 1) in
  (Fq_target.( * )
    (Pairing.reduced_pairing w (List.nth_exn srs.hPositiveAlphaX 1))
    (Pairing.reduced_pairing (G1.(+) (G1.scale G1.one (Fq.to_bigint v)) (G1.scale w (Fq.to_bigint (Fq.negate z)))) (List.hd_exn srs.hPositiveAlphaX))
  )
