...
let%test_unit "test2" =
  ...
  (* -x^{-3} -2x^{-1} + 4 - x - 6x^2 *)
  let poly_product = make_laurent_for_test (-4) [-2; 0; -1; 4; 0; 0; 16; -2; -8] in
...
let%test_unit "evaluation test" =
  ...
  assert (
    Fq.equal (Fq_laurent.eval poly_a (Fq.of_int 3)) (Fq.of_int 11) ) ;
  assert (
    Fq.equal (Fq_laurent.eval poly_a (Fq.of_int (-3))) (Fq.of_int 11) )

let generic_laurent_test =
  QCheck.Test.make ~count:10 ~name:"generic laurent test"
  QCheck.((pair int (quad small_nat small_nat (small_list int) (small_list int))))
  (fun (l, (a_deg, b_deg, a_nums, b_nums)) -> Random.init l ;
...
let () =
  QCheck.Test.check_exn generic_laurent_test