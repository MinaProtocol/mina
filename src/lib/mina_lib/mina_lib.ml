...
let compile_config t = t.config.runtime_config.daemon.compile_config
  ...
  if uptime_mins <= wait then
    ...
    | Some timing -> (
        ...
        | Evaluating_vrf _last_checked_slot ->
            let vrf_poll_interval = (compile_config t).vrf_poll_interval in
...
let context ~commit_id (config : Config.t) : (module CONTEXT) =
  ( module struct
    ...
    let vrf_poll_interval =
      config.runtime_config.daemon.compile_config.vrf_poll_interval
...
let start t =
  ...
  perform_compaction
    t.config.runtime_config.daemon.compile_config.compaction_interval t ;
  let () =
...
let create ~commit_id ?wallets (config : Config.t) =
  ...
  Async.Scheduler.within' ~monitor (fun () ->
      let set_itn_data (type t) (module M : Itn_settable with type t = t) (t : t)
          =
        if config.runtime_config.daemon.compile_config.itn_features then
