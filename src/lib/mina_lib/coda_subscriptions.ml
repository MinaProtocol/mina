...
let add_new_subscription (t : t) ~pk =
  ...
  ignore (Optional_public_key.Table.find_or_add t.subscribed_block_users (Some pk)
      ~default:(fun () -> [Pipe.create ()]) : (Filtered_external_transition.t, State_hash.t)
  With_hash.t reader_and_writer list);
  (* add a new payment user if we're not already tracking it *)
  ignore (Public_key.Compressed.Table.find_or_add t.subscribed_payment_users pk
      ~default:Pipe.create : Signed_command.t reader_and_writer)
