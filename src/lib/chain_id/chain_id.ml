...
let of_inputs (inputs : Inputs.t) =
  ...
  let all_snark_keys =
    List.map constraint_system_digests ~f:(fun (_, digest) ->
        Md5.to_hex digest )
    |> String.concat ~sep:""
  in
...
let make ~(genesis_constants : Genesis_constants.t) ~constraint_constants
    ~genesis_ledger
    ~(genesis_epoch_data :
  ...
  let protocol_state_with_hashes =
    Mina_state.Genesis_protocol_state.t
      ~genesis_ledger:(Consensus.Genesis_data.Hashed.zero_totals genesis_ledger)
      ~genesis_epoch_data:
...
let of_precomputed_values (pv : Precomputed_values.t) =
  let genesis_ledger =
    Consensus.Genesis_data.Ledger.to_hashed pv.genesis_ledger
  in
  let genesis_epoch_data =
    Consensus.Genesis_data.Epoch.to_hashed pv.genesis_epoch_data
  in
