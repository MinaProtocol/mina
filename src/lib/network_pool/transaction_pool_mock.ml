...
module Mock_transition_frontier = struct
  ...
  let create ~vk ?permissions :
   ...
   fun () ->
    ...
    in
    let initial_balance = Currency.Balance.of_mina_string_exn "900000000.0" in
        ...
        | `Both ((committed, _), (uncommitted, _)) ->
            failwithf
              !"Command \
                ...
                in both locally generated committed and uncommitted with times \
                %s and %s"
...
let mk_basic_zkapp ?(fee = 10_000_000_000) ?(empty_update = false)
  ...
  let account_updates =
    if empty_update then []
      ...
      mk_forest
        [ mk_node
            (mk_account_update_body Signature No fee_payer_kp Token_id.default 0
...
let mk_transfer_zkapp_command ~constraint_constants ?valid_period ?fee_payer_idx
  ...
  let receiver = receiver_kp.public_key |> Signature_lib.Public_key.compress in
  let fee_payer =
...
let canonicalize t = Result.map t ~f:(List.sort ~compare:User_command.compare)
...
let add_commands ?(local = true) test cs =
  ...
  in
  let result = Test.Resource_pool.Diff.unsafe_apply test.txn_pool verified in
  let tm1 = Time.now () in
...
let mk_linear_case_test t cmds =
  ...
  let%bind () = advance_chain t (List.take (List.drop independent_cmds 1) 2) in
