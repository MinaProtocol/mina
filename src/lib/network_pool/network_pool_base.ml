  ...
  let apply_and_broadcast ({ logger; _ } as t)
    ...
    O1trace.sync_thread apply_and_broadcast_thread_label (fun () ->
        ...
        | Ok (`Reject, accepted, rejected) ->
            Resource_pool.Diff.log_internal ~logger "rejected_apply" env ;
            Broadcast_callback.reject accepted rejected cb
        | Error (`Locally_generated res) ->
            Resource_pool.Diff.log_internal ~logger "rejected_locally_generated"
              env ;
            rebroadcast res
        | Error (`Other e) ->
            [%log' debug t.logger]
            ...
            Resource_pool.Diff.log_internal ~logger "rejected_other" env ;
