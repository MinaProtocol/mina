...
let replace_fee : Currency.Fee.t = Currency.Fee.nanomina_exn 1
  ...
  ( module struct
      ...
      Quickcheck.test (gen_cmd ()) ~f:(fun cmd ->
          ...
          let add_res =
            add_from_gossip_exn pool cmd Account_nonce.zero
              (Currency.Amount.nanomina_exn 500)
          in
          if
            Option.value_exn (currency_consumed ~constraint_constants cmd)
    ...
    let%test_unit "age-based expiry" =
      Quickcheck.test ~trials:1
        ...
        ~f:(fun (cmd, _, _, _) ->
          ...
          in
          add_from_gossip_exn pool cmd Account_nonce.zero
            (Currency.Amount.mina_exn 3_000_000)
          |> function
          | Ok (_, pool', dropped) ->
              assert (Sequence.is_empty dropped) ;
              Unix.sleep 15 ;
    ...
    let%test_unit "replacement" =
      ...
      let gen :
        ...
        let%bind init_nonce =
          ...
          @@ Int.gen_incl 0 1000
        in
        let init_balance = Currency.Amount.mina_exn 100_000 in
        ...
        let rec go current_nonce current_balance n =
          if n > 0 then
            ...
            let%bind fee =
              Currency.Amount.(
                gen_incl zero (min (nanomina_exn 10) cmd_currency))
            in
            let amount =
        ...
        let replace_cmd =
          modify_payment replace_cmd_skeleton ~sender ~body:Fn.id
            ~common:(fun c ->
              { c with fee = Currency.Fee.mina_exn (10 + (5 * (size + 1))) } )
        in
    ...
    let init_permissionless_ledger ledger account_info =
      ...
      List.iter account_info ~f:(fun (public_key, amount) ->
          ...
          in
          let balance = Balance.nanomina_exn @@ Amount.int_of_nanomina amount in
    ...
    let%test_unit "support for zkapp_command commands" =
      ...
      let amount = Amount.nanomina_exn @@ Fee.int_of_nanomina fee in
