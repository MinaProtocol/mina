open Core
open Mina_base
open Mina_transaction

type key = Transaction_hash.User_command_with_valid_signature.t

type value = Time_float.t * [ `Batch of int ]

type t =
  (Time_float.t * [ `Batch of int ] * User_command.Valid.t)
  Transaction_hash.Map.t
  ref

let find_and_remove (t : t) cmd =
  let hash =
    Transaction_hash.User_command_with_valid_signature.transaction_hash cmd
  in
  let%map.Option time, batch, _ = Map.find !t hash in
  t := Map.remove !t hash ;
  (time, batch)

let add_exn (t : t) ~key:cmd ~data:(time, batch) =
  let hash =
    Transaction_hash.User_command_with_valid_signature.transaction_hash cmd
  in
  let cmd_ = Transaction_hash.User_command_with_valid_signature.data cmd in
  t := Map.add_exn !t ~key:hash ~data:(time, batch, cmd_)

let mem (t : t) cmd =
  let hash =
    Transaction_hash.User_command_with_valid_signature.transaction_hash cmd
  in
  Map.mem !t hash

let create () = ref Transaction_hash.Map.empty

let update (t : t) cmd
    ~(f :
          (Time_float.t * [ `Batch of int ]) option
       -> Time_float.t * [ `Batch of int ] ) =
  let hash =
    Transaction_hash.User_command_with_valid_signature.transaction_hash cmd
  in
  let cmd_ = Transaction_hash.User_command_with_valid_signature.data cmd in
  t :=
    Map.update !t hash ~f:(fun found ->
        let found' = Option.map ~f:(fun (t, b, _) -> (t, b)) found in
        let t', b' = f found' in
        (t', b', cmd_) )

let filteri_inplace (t : t) ~f =
  t :=
    Map.filteri !t ~f:(fun ~key ~data:(time, batch, cmd) ->
        f
          ~key:(Transaction_hash.User_command_with_valid_signature.make cmd key)
          ~data:(time, batch) )

let to_alist (t : t) =
  Map.to_alist !t
  |> List.map ~f:(fun (hash, (time, batch, cmd)) ->
      ( Transaction_hash.User_command_with_valid_signature.make cmd hash
      , (time, batch) ) )

let iter_intersection (a : t) (b : t) ~f =
  Map.iteri !a ~f:(fun ~key ~data:(time1, batch1, cmd) ->
      Map.find !b key
      |> Option.iter ~f:(fun (time2, batch2, _) ->
          f
            ~key:
              (Transaction_hash.User_command_with_valid_signature.make cmd key)
            (time1, batch1) (time2, batch2) ) )

let iteri (t : t) ~f =
  Map.iteri !t ~f:(fun ~key ~data:(time, batch, cmd) ->
      f
        ~key:(Transaction_hash.User_command_with_valid_signature.make cmd key)
        ~data:(time, batch) )
