...
let initialize t ~root_data =
  ...
  in
  let root_transition = Mina_block.unwrap root_transition in
  [%log' trace t.logger]
...
let add ~arcs_cache ~transition =
  ...
  let transition_unwrapped = With_hash.data transition |> Mina_block.unwrap in
  fun batch ->
...
let get_root ~proof_cache_db t =
  get_root_impl t |> Result.map ~f:(Root_data.Minimal.generate ~proof_cache_db)
