...
module Make (Inputs : Intf.Inputs_with_worker_intf) = struct
  ...
  type work = {diffs: Frontier.Diff.Lite.E.t list; target_hash: Frontier.Hash.t}
  ...
  let create ~base_hash ~worker =
    {diff_array= DynArray.create (); worker; target_hash= base_hash}
  ...
  let write t ~diffs ~hash_transition =
    ...
    if should_flush t && not (Worker.is_working t.worker) then flush t
