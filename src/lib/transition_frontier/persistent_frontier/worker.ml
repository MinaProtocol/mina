...
module Make (Inputs : Intf.Inputs_with_db_intf) :
  ...
  module Worker = struct
    ...
    let apply_diff (type mutant) (t : t) (diff : mutant Frontier.Diff.Lite.t) :
    ...
    let perform t (diffs, target_hash) =
      ...
      match%map
        ...
        let%bind base_hash =
          Deferred.return
            ( Db.get_frontier_hash t.db
              :> (Frontier.Hash.t, perform_error) Result.t )
        in
        ...
        Deferred.return
          ( Result.ok_if_true
              (Frontier.Hash.equal target_hash result_hash)
