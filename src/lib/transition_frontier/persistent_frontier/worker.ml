...
module Make (Inputs : Intf.Inputs_with_db_intf) :
  ...
  module Worker = struct
    ...
    let apply_diff_error_internal_to_string =
      function
      | `Not_found `New_root_transition -> "new root transition not found"
      | `Not_found `Old_root_transition -> "old root transition not found"
      | `Not_found `Parent_transition -> "parent transition not found"
      | `Not_found `Best_tip -> "best tip not found"

    let apply_diff (type mutant) (t : t) (diff : mutant Frontier.Diff.Lite.t) : (mutant, apply_diff_error) Result.t =
      let map_error result ~diff_type ~diff_type_name =
        Result.map_error result ~f:(fun err ->
          Logger.error t.logger ~module_:__MODULE__ ~location:__LOC__
            "error applying %s diff: %s"
            diff_type_name
      ...
      | New_node (Lite transition) ->
          map_error ~diff_type:`New_node ~diff_type_name:"New_node"
            (Db.add t.db ~transition :> (mutant, apply_diff_error_internal) Result.t)
      | Root_transitioned {new_root; garbage} ->
          map_error ~diff_type:`Root_transitioned ~diff_type_name:"Root_transitioned"
            (Db.move_root t.db ~new_root ~garbage :> (mutant, apply_diff_error_internal) Result.t)
      | Best_tip_changed best_tip_hash ->
          map_error ~diff_type:`Best_tip_changed ~diff_type_name:"Best_tip_changed"
            (Db.set_best_tip t.db best_tip_hash :> (mutant, apply_diff_error_internal) Result.t)
    ...
    let rec deferred_result_list_fold ls ~init ~f =
      ...
      match ls with
      | []     -> return init
      | h :: t -> let%bind init = f init h in deferred_result_list_fold t ~init ~f

    type perform_error =
      [ apply_diff_error
      | [`Not_found of [`Frontier_hash]]
      | [`Invalid_resulting_frontier_hash of Frontier.Hash.t] ]

    let perform t (diffs, target_hash) =
      let open Deferred.Let_syntax in
      match%map
        let open Deferred.Result.Let_syntax in
        let%bind base_hash = Deferred.return (Db.get_frontier_hash t.db :> (Frontier.Hash.t, perform_error) Result.t) in
        let%bind result_hash =
           ...
           *)
          deferred_result_list_fold diffs
            ~init:base_hash
            ~f:(fun acc_hash diff ->
              Deferred.return (handle_diff t acc_hash diff :> (Frontier.Hash.t, perform_error) Result.t))
        in
        Deferred.return (
          Result.ok_if_true (Frontier.Hash.equal target_hash result_hash)
      ...
      | Error (`Not_found `Frontier_hash) ->
          failwith "Failed to find frontier hash in persistent transition frontier"
      | Error (`Invalid_resulting_frontier_hash result_hash) ->
