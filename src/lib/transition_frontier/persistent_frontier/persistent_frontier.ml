...
module Make (Inputs : Inputs.With_extensions_intf) = struct
  ...
  module rec Instance_type : sig
    type t = {db: Db.t; mutable sync: Sync.t option; factory: Factory_type.t}
  end =
    Instance_type
  ...
  module Instance = struct
    ...
    let check_database t = Db.check t.db
    ...
    let fast_forward t target_root :
      ...
      let%bind root =
        lift_error (Db.get_root t.db) "failed to get root hash"
      in
      if State_hash.equal root.hash target_root.state_hash then
        ...
        let%bind frontier_hash =
          lift_error (Db.get_frontier_hash t.db) "failed to get frontier hash"
        in
        (* TODO: gracefully recover from this state *)
        Result.ok_if_true
          (Frontier.Hash.equal frontier_hash target_root.frontier_hash)
          ~error:
            (`Failure
    ...
    let load_full_frontier t ~root_ledger ~consensus_local_state =
      ...
      let wrap_transition protocol_state =
        With_hash.of_data
      ...
      let%bind root, root_transition, best_tip, base_hash =
        ...
        |> Result.map_error ~f:(fun err ->
               `Failure (Db.Error.not_found_message err) )
        |> Deferred.return
      in
      Printf.printf
        !"genesis: %s\nroot transition: %s\nsnarked ledger db: %s\n%!"
      ...
      let frontier =
        Frontier.create ~logger:t.factory.logger ~base_hash
          ~root_data:
            { transition=
                wrap_transition External_transition.Validated.protocol_state
                  root_transition
            ; staged_ledger= root_staged_ledger }
          ~root_ledger ~consensus_local_state
      in
      let root_breadcrumb = Frontier.root frontier in
      let%bind extensions =
        Deferred.map (Extensions.create root_breadcrumb) ~f:Result.return
      in
      let apply_diff diff =
        let (`New_root _) = Frontier.apply_diffs frontier [diff] in
        Extensions.notify extensions ~frontier
          ~diffs:[Frontier.Diff.Full.E.to_lite diff]
        |> Deferred.map ~f:Result.return
      in
      (* crawl through persistent frontier and load transitions into in memory frontier *)
      let%bind () =
        Deferred.map
          (Db.crawl_successors t.db root.hash ~init:root_breadcrumb
             ~f:(fun parent transition ->
               let%bind breadcrumb =
                 Frontier.Breadcrumb.build ~logger:t.factory.logger
                   ~verifier:t.factory.verifier
               ...
               in
               let%map () =
                 apply_diff Frontier.Diff.(E (New_node (Full breadcrumb)))
               in
               breadcrumb ))
          ~f:
