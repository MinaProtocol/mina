...
module type Db_intf = sig
  ...
  val initialize : t -> root_data:root_data -> base_hash:frontier_hash -> unit
  ...
  val get_root : t -> (minimal_root_data, [> `Not_found of [> `Root]]) Result.t
  ...
  val get_frontier_hash :
    t -> (frontier_hash, [> `Not_found of [> `Frontier_hash]]) Result.t

  val set_frontier_hash : t -> frontier_hash -> unit
