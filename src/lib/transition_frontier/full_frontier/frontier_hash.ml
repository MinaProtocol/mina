...
module Make (Inputs : Inputs.With_diff_intf) :
  ...
  let merge_pending_coinbase acc pending_coinbase =
    ...
    merge_string acc (Pending_coinbase.Hash.raw_hash_bytes root_hash)

  let merge_scan_state acc scan_state =
    let hash = Staged_ledger.Scan_state.hash scan_state in
    merge_string acc (Staged_ledger_hash.Aux_hash.raw_hash_bytes hash)

  let merge_root_data acc
      {Diff.Minimal_root_data.Stable.V1.hash; scan_state; pending_coinbase} =
  ...
  let merge_diff : type mutant. t -> (Diff.lite, mutant) Diff.t -> t =
   fun acc diff ->
    match diff with
    | New_node (Lite node) ->
        merge_with_hash ~merge_data:merge_transition acc node
