...
module Make (Inputs : Inputs.S) : sig
  ...
  let create ~logger ~root_data ~root_ledger ~base_hash ~consensus_local_state
    ...
    Coda_metrics.(Guage.set Transition_frontier.active_breadcrumbs 1.0) ;
    { logger
    ; root_ledger
    ; root= root_hash
    ; best_tip= root_hash
  ...
  let close t =
    Coda_metrics.(Guage.set Transition_frontier.active_breadcrumbs 0.0) ;
    failwith
      "TODO: detach root breadcrumb staged ledger ledger mask from root \
       snarked ledger database"
  ...
  let calculate_root_transition_diff t heir =
    ...
    in
    let garbage_hashes =
      List.map garbage_breadcrumbs ~f:Breadcrumb.state_hash
    in
    let new_root_data =
      { hash= heir_hash
  ...
  let apply_diff (type mutant) t (diff : (Diff.full, mutant) Diff.t) :
    ...
    | New_node (Full breadcrumb) ->
        ...
        Coda_metrics.(Gauge.inc_one Transition_frontier.total_breadcrumbs) ;
        ((), None)
    ...
    | Root_transitioned {new_root= {hash= new_root_hash; _}; garbage} ->
        ...
        let new_root_mask =
          ...
          List.iter garbage ~f:(fun garbage_hash ->
              let breadcrumb = find t garbage_hash in
              let mask = Breadcrumb.mask garbage_breadcrumb in
              Ledger.Maskable.unregister_mask_exn
                (Ledger.Mask.Attached.parent mask)
                mask ;
              Hashtbl.remove t.table garbage_hash ) ;
          Hashtbl.remove t.root ;
          Ledger.commit m1 ;
          ...
          [%test_result: Ledger_hash.t]
            ~message:
              "Merkle root of old root's staged ledger mask is the same as \
               the new root's staged ledger mask after committing"
            ~expect:m1_hash_pre_commit (Ledger.merkle_root m0) ;
          (* reparent all the successors of m1 onto m0 *)
          Ledger.Maskable.unregister_mask_exn m0 m1 ;
          if new_root_node.breadcrumb.just_emitted_a_proof then (
            let s = Ledger.of_database t.root_snarked_ledger in
            let mt = Ledger.Maskable.register_mask s (Ledger.Mask.create ()) in
            Non_empty_list.iter
              (Staged_ledger.proof_txns
                 (Breadcrumb.staged_ledger new_root_node.breadcrumb))
              ~f:(fun txn ->
                ignore (Or_error.ok_exn (Leger.apply_transaction mt txn)) ) ;
            Ledger.commit mt ;
        ...
        Coda_metrics.(
          let num_breadcrumbs_removed =
            Int.to_float (1 + List.length garbage)
          in
          ...
          in
          let num_root_snarked_ledger_accounts = Int.to_float List.length in
          ...
          in
          Guage.dec Transition_frontier.active_breadcrumbs
            (Int.to_float (1 + List.length garbage)) ;
          Guage.set Transition_frontier.recently_finalized_staged_txns
            num_finalized_staged_txns ;
          Guage.inc Transition_frontier.finalized_staged_txns
            num_finalized_staged_txns ;
          Guage.set Transition_frontier.root_snarked_ledger_accounts
            num_root_snarked_ledger_accounts ;
          Guage.set Transition_frontier.root_snarked_ledger_total_currency
