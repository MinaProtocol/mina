...
struct
  ...
  module Breadcrumb = struct
    ...
    let to_yojson {transition_with_hash; staged_ledger= _; just_emitted_a_proof}
      ...
      `Assoc
        [ ( "transition_with_hash"
          , With_hash.to_yojson Consensus.External_transition.Verified.to_yojson
          ...
          let%bind ( `Hash_after_applying staged_ledger_hash
            ...
            match%map
              Inputs.Staged_ledger.apply ~logger staged_ledger
                (Consensus.External_transition.Verified.staged_ledger_diff
                   transition)
            with
            | Ok x -> Ok x
    ...
    type display =
      { state_hash: string
      ; blockchain_state:
          Consensus.External_transition.Protocol_state.Blockchain_state.display
  ...
  let add_breadcrumb_exn t breadcrumb =
    O1trace.measure "add_breadcrumb" (fun () ->
        let consensus_state_of_breadcrumb b =
          ...
          |> With_hash.data
          |> Consensus.External_transition.Verified.protocol_state
          |> Consensus.External_transition.Protocol_state.consensus_state
        in
        let hash =
