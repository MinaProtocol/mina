...
module Instance = struct
  ...
  let destroy t =
    List.iter (Queue.to_list t.potential_snarked_ledgers) ~f:Mina_stdlib.File_system.rmrf ;
    Mina_stdlib.File_system.rmrf (Locations.potential_snarked_ledgers t.factory.directory) ;
        ...
        ~f:(fun _ location ->
          ...
          then (
            ...
            | Ok _ ->
                ...
                Mina_stdlib.File_system.rmrf @@ Locations.snarked_ledger factory.directory ;
                Sys.rename
                  (Locations.tmp_snarked_ledger factory.directory)
                  (Locations.snarked_ledger factory.directory) ;
                List.iter potential_snarked_ledgers ~f:Mina_stdlib.File_system.rmrf ;
                Mina_stdlib.File_system.rmrf
...
let reset_to_genesis_exn t ~precomputed_values =
  assert (Option.is_none t.instance) ;
  Mina_stdlib.File_system.rmrf t.directory ;
