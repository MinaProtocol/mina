...
module Make (Inputs : Inputs.With_base_frontier_intf) = struct
  ...
  let with_file ?size filename access_level ~f =
    ...
    let shared, mode =
      ...
      | `Read -> (false, [O_RDONLY])
      | `Write -> (true, [O_RDWR; O_TRUNC; O_CREAT])
    in
    let fd = Unix.openfile filename ~mode in
    let buf_size =
      match size with
      | None -> Int64.to_int_exn Unix.(fstat fd).st_size
      | Some sz -> sz
    in
  ...
  module Instance = struct
    ...
    let create factory =
      let snarked_ledger = Ledger.Db.create ~directory_name:(Locations.snarked_ledger factory.directory) () in
    ...
    let set_root_identifier t new_root_identifier =
      Logger.trace t.factory.logger ~module_:__MODULE__ ~location:__LOC__
        ~metadata:[("root_identifier", Frontier.Root_identifier.Stable.Latest.to_yojson new_root_identifier)]
        "Setting persistent root identifier";
      let size = Frontier.Root_identifier.Stable.Latest.bin_size_t new_root_identifier in
      with_file (Locations.root_identifier t.factory.directory) `Write ~size ~f:(fun buf ->
        ignore (Frontier.Root_identifier.Stable.Latest.bin_write_t buf ~pos:0 new_root_identifier))
    ...
    let load_root_identifier t =
      ...
      | Ok () ->
        with_file file `Read ~f:(fun buf ->
          let root_identifier = Frontier.Root_identifier.Stable.Latest.bin_read_t buf ~pos_ref:(ref 0) in
          Logger.trace t.factory.logger ~module_:__MODULE__ ~location:__LOC__
            ~metadata:[("root_identifier", Frontier.Root_identifier.Stable.Latest.to_yojson root_identifier)]
  ...
  let create ~logger ~directory =
    {directory; logger; instance= None}
  ...
  let reset_to_genesis_exn t =
    ...
    with_instance_exn t ~f:(fun instance ->
      ignore (Ledger_transfer.transfer_accounts ~src:Genesis_ledger.t ~dest:(Instance.snarked_ledger instance));
