...
type t =
  ...
  | Bit of Bit_catchup_state.extended_t
...
let create t ~logger ~root ~is_header_relevant ~is_in_frontier ~iter_frontier
  ...
  | `Bit (transition_states, known_body_refs, block_storage, header_storage) ->
      Bit
        (Bit_catchup_state.create ~logger ~root ~is_header_relevant
           ~is_in_frontier transition_states ~block_storage ~header_storage
           ~known_body_refs ~iter_frontier ~block_storage_actions )
