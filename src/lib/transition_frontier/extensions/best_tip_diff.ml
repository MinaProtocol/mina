    ...
    let path_from_to t1 t2 =
      let rec go cursor acc =
        if Breadcrumb.equal cursor t1 then acc
          ...
          go
            ( Full_frontier.find frontier (Breadcrumb.parent_hash cursor)
            |> Option.value_exn
                 ~message:"get_path_diff: parent not found in frontier" )
            (cursor :: acc)
      in
      go t2 []
    in
    let ancestor =
      Full_frontier.find frontier ancestor_hash
      |> Option.value_exn ~message:"ancestor not found in frontier"
    in
    [%log' debug t.logger]
      !"Common ancestor: %{sexp: State_hash.t}"
      ancestor_hash ;
    (path_from_to ancestor bc1, path_from_to ancestor bc2)
        ...
        ~f:
          (fun ( ({ new_commands; removed_commands; reorg_best_tip = _ } as acc)
            ...
            | E (Best_tip_changed new_best_tip_hash, old_best_tip_hash) ->
                let new_best_tip =
                  Full_frontier.find frontier new_best_tip_hash
                  |> Option.value_exn
                       ~message:"new best tip not found in frontier"
                in
                let old_best_tip =
                  (*FIXME #4404*)
                  Full_frontier.find frontier old_best_tip_hash
                  |> Option.value_exn
                       ~message:"old best tip not found in frontier"
                in
