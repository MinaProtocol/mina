...
module type Base_ext_intf = sig
  ...
  val handle_diffs :
    t -> Full_frontier.t -> Diff.Lite.E.t list -> view option
end
...
module type Broadcast_extension_intf = sig
  ...
  val update_and_create_broadcast_job :
    t -> Full_frontier.t -> Diff.Lite.E.t list -> unit -> unit Deferred.t
end
...
module Make_broadcastable (Ext : Base_ext_intf) :
  ...
  let create ~logger frontier =
    let t, initial_view = Ext.create ~logger frontier in
...
module Root_history = struct
  ...
  let create ~logger:_ frontier =
    let root_history = T.create (2 * Full_frontier.max_length frontier) in
    ...
    let should_produce_view =
      List.exists diffs ~f:(function
        | Lite.E.E (Root_transitioned _) ->
            T.enqueue root_history (Full_frontier.root transition_frontier) ;
            true
        | Lite.E.E _ ->
            false )
    in
    ...
    let should_produce_view =
      List.exists diffs ~f:(function
        | Lite.E.E (New_node (Lite transition)) ->
            T.notify transition_registry (External_transition.Validated.state_hash transition) ;
            true
        | Lite.E.E _ ->
            false )
    in
...
module Snark_pool_refcount = struct
  ...
  let create ~logger:_ frontier =
    ...
    let (_ : bool) = add_breadcrumb_to_ref_table t (Full_frontier.root frontier) in
    (t, (0, t))
...
module Best_tip_diff = struct
  ...
  let create ~logger frontier =
    ( {logger}
    , { new_user_commands= Breadcrumb.user_commands (Full_frontier.root frontier)
        ...
        ~f:
          ...
          | Lite.E.E (Best_tip_changed new_best_tip) ->
              ...
              let added_to_best_tip_path, removed_from_best_tip_path =
                get_path_diff t frontier new_best_tip_breadcrumb
                  old_best_tip
              in
              Logger.debug
                t.logger
...
let create ~logger frontier : t Deferred.t =
  ...
  let%bind root_history = Root_history.create ~logger frontier in
  let%bind snark_pool_refcount = Snark_pool_refcount.create ~logger frontier in
  let%bind best_tip_diff = Best_tip_diff.create ~logger frontier in
  let%bind transition_registry = Transition_registry.create ~logger frontier in
  let%map identity = Identity.create ~logger frontier in
  { root_history
  ; snark_pool_refcount
  ; best_tip_diff
  ; transition_registry
