...
module Wrap_singleton_table
     ...
     * reported metrics *)
  : Hashtbl_limited_intf
      with type key = Tbl.key
       and type 'a _real = 'a Tbl.t
= struct
  include Tbl
  ...
  let gauge = 
    Gauge.v Metric_info.name ~help:Metric_info.help
  ...
  let add_exn t ~key ~data =
    try Tbl.add_exn t ~key ~data; Gauge.inc_one gauge
    with exn -> raise exn

  let set t ~key ~data =
    (* assuming Tbl.set cannot throw an exn *)
    (if not @@ Tbl.mem t key then
      Gauge.inc_one gauge);
    Tbl.set t ~key ~data
  ...
  let change t key ~f =
    Tbl.change t key ~f:(fun x ->
      ...
      | None, None -> None
      | None, Some y -> Gauge.inc_one gauge; Some y
      | Some _, None -> Gauge.dec_one gauge; None)
