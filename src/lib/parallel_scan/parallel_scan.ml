...
let%test_module "state operations" =
  ( module struct
    ...
    let%test_unit "empty state has correct max_base_jobs" =
      let s = empty ~max_base_jobs:16 ~delay:2 in
      [%test_eq: int] s.max_base_jobs 16
    ...
    let%test_unit "empty state next_on_new_tree is true" =
      (* on empty state, current tree is fully free, so next data goes to new tree *)
      let s = empty ~max_base_jobs:8 ~delay:1 in
    ...
    let%test_unit "free_space equals max_base_jobs" =
      ...
      (* free_space stays constant after update *)
      let data = [ 1; 2; 3 ] in
      let jobs =
        List.concat (work_for_next_update s ~data_count:(List.length data))
      in
    ...
    let%test_unit "State.map preserves structure" =
      ...
      [%test_eq: int] s_mapped.max_base_jobs s'.max_base_jobs ;
    ...
    let%test_unit "jobs_for_next_update provides enough work" =
      let s = empty ~max_base_jobs:4 ~delay:0 in
      let data = [ 1; 2; 3; 4 ] in
      let jobs = work_for_next_update s ~data_count:(List.length data) in
