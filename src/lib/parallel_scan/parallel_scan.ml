...
module Weight = struct
  ...
  (* code that would have been generated by `deriving lens`
     workaround for interaction between %%versioned and lens.ppx_deriving
     see: https://discuss.ocaml.org/t/odd-deriving-error/4606
 
     TODO: use lens deriver again when 

      https://github.com/ocaml-ppx/ppx_deriving/issues/203
  ...
  let base =
    let open Lens in
    {get= (fun r -> r.base); set= (fun v r -> {r with base= v})}

  and merge =
    let open Lens in
    {get= (fun r -> r.merge); set= (fun v r -> {r with merge= v})}
end
...
module T = struct
  module Stable = struct
    module V1 = struct
       ...
       *)
      let with_leaner_trees ({T.trees; _} as t) =
        let trees =
      ...
      include Binable.Of_binable2
                (T.Stable.Latest)
                (struct
                  type nonrec ('merge, 'base) t = ('merge, 'base) T.t
