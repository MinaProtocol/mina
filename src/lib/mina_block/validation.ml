...
let validate_time_received ~(precomputed_values : Precomputed_values.t) ~time_received
    (t, validation) =
  let consensus_state =
    t |> With_hash.data |> Block.header |> Header.protocol_state |> Protocol_state.consensus_state
  in
...
let skip_genesis_protocol_state_validation
    `This_block_was_generated_internally (t, validation) =
...
let validate_proofs ~verifier tvs =
...
let validate_single_proof ~verifier t =
  let open Deferred.Result.Let_syntax in
  let%map res = validate_proofs ~verifier [t] in
  List.hd_exn res

let skip_proof_validation `This_block_was_generated_internally
    (t, validation) =
  (t, Unsafe.set_valid_proof validation)
...
let extract_delta_block_chain_witness = function
  | ( _
    , _
    , _
    , (`Delta_block_chain, Truth.True delta_block_chain_witness)
    , _
    , _
    , _ ) ->
...
let validate_delta_block_chain (t, validation) =
  let header = t |> With_hash.data |> Block.header in
  match
    Transition_chain_verifier.verify ~target_hash:(header |> Header.protocol_state |> Protocol_state.previous_state_hash)
      ~transition_chain_proof:(Header.delta_block_chain_proof header)
  with
  | Some hashes ->
      Ok
        (t, Unsafe.set_valid_delta_block_chain validation hashes)
  | None ->
      Error `Invalid_delta_block_chain_proof

let skip_delta_block_chain_validation
    `This_block_was_not_received_via_gossip (t, validation) =
  let previous_protocol_state_hash = t |> With_hash.data |> Block.header |> Header.protocol_state |> Protocol_state.previous_state_hash in
  ( t
  , Unsafe.set_valid_delta_block_chain validation
      (Non_empty_list.singleton previous_protocol_state_hash) )

let validate_frontier_dependencies ~logger ~consensus_constants ~root_block ~get_block_by_hash (t, validation) =
  let open Result.Let_syntax in
  let protocol_state = Fn.compose Header.protocol_state Block.header  in
  let consensus_state = Fn.compose Protocol_state.consensus_state protocol_state in
  let hash = With_hash.hash t in
  let parent_hash = t |> With_hash.data |> protocol_state |> Protocol_state.previous_state_hash in
  let%bind () =
    Result.ok_if_true
...
let validate_staged_ledger_diff
  ?skip_staged_ledger_verification
  ~logger ~precomputed_values ~verifier
  ~parent_staged_ledger ~parent_protocol_state
  (t, validation)
= let open Deferred.Result.Let_syntax in
  let target_hash_of_ledger_proof = Fn.compose Ledger_proof.statement_target Ledger_proof.statement in
  ...
  let%bind ( `Hash_after_applying staged_ledger_hash
    ...
    Staged_ledger.apply ?skip_verification:skip_staged_ledger_verification
      ~constraint_constants:precomputed_values.Precomputed_values.constraint_constants ~logger
      ...
      ~supercharge_coinbase:(Consensus_state.supercharge_coinbase consensus_state)
...
let validate_staged_ledger_hash
  ...
  let state = t |> With_hash.data |> Block.header |> Header.protocol_state |> Protocol_state.blockchain_state in
  if
    Staged_ledger_hash.equal staged_ledger_hash (Blockchain_state.staged_ledger_hash state)
...
let skip_staged_ledger_diff_validation
    `This_block_has_a_trusted_staged_ledger (t, validation) =
  (t, Unsafe.set_valid_staged_ledger_diff validation)
...
let skip_protocol_versions_validation
    `This_block_has_valid_protocol_versions (t, validation) =
