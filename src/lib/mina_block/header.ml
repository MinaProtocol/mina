  ...
  module V1 = struct
    ...
    module Creatable = struct
      ...
      let map_creator c ~f ~protocol_state ~protocol_state_proof
        ...
        f
          (c ~protocol_state ~protocol_state_proof
             ~delta_block_chain_proof ~body_reference
             ?proposed_protocol_version_opt ())
      ...
      let create ~protocol_state ~protocol_state_proof
        ...
        let current_protocol_version =
          ...
          with _ ->
            failwith
              "Cannot create block header before setting current \
               protocol version"
        in
        { protocol_state
...
let protocol_version_status body =
  let valid_current = Protocol_version.is_valid (current_protocol_version body) in
  let valid_next =
    Option.for_all
