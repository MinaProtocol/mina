...
module Make_impl (Cache : Disk_cache_intf.S_with_count with module Data := Mock) :
  ...
  let simple_write ?additional_checks () =
    ...
    @@ fun () ->
    Mina_stdlib.File_system.with_temp_dir "disk_cache"
  ...
  let remove_data_on_gc () =
    Async.Thread_safe.block_on_async_exn
    @@ fun () ->
    Mina_stdlib.File_system.with_temp_dir "disk_cache-remove_data_on_gc"
  ...
  let initialization_special_cases () =
    Async.Thread_safe.block_on_async_exn
    @@ fun () ->
    Mina_stdlib.File_system.with_temp_dir "disk_cache-invalid-initialization"
