...
module Chain = struct
  ...
  let find_chain ?(max_size=5) desired_value =
    ...
    |> List.min_elt ~compare:(fun c1 c2 -> compare (List.length c1) (List.length c2))
    |> value_exn
end
...
*)
  ...
  let add_block ~state block =
    Array.iteri block ~f:(fun i bi ->
      state.(i) <- Field.(+) state.(i) bi)
 ...
 *)
  ...
  let apply matrix v =
    let dotv row =
      Array.reduce_exn (Array.map2_exn v row ~f:Field.scale)
        ~f:Field.(+)
    in
  ...
  let block_cipher state ~rounds ~round_constants ~mds =
    ...
    for_ rounds ~init:state ~f:(fun r state ->
