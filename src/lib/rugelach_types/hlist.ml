...
module type T3 = sig type (_, _, _) t end

module type T4 = sig type (_, _, _, _) t end

module E13 (T : T1) = struct type ('a, _, _) t = 'a T.t end
module E23 (T : T2) = struct type ('a, 'b, _) t = ('a, 'b) T.t end

module E01(T : T0) = struct type _ t = T.t end
module E02(T : T0) = struct type (_, _) t = T.t end
module E03(T : T0) = struct type (_, _, _) t = T.t end
module E04(T : T0) = struct type (_, _, _, _) t = T.t end
...
module Tuple4(F: T3)(G: T3)(H : T3)(I: T3) = struct
  type ('a, 'b, 'c) t = ('a, 'b, 'c) F.t * ('a, 'b, 'c) G.t* ('a, 'b, 'c) H.t * ('a, 'b, 'c) I.t
end

module Tuple5(F: T3)(G: T3)(H : T3)(I: T3)(J : T3) = struct
  type ('a, 'b, 'c) t = ('a, 'b, 'c) F.t * ('a, 'b, 'c) G.t* ('a, 'b, 'c) H.t * ('a, 'b, 'c) I.t * ('a, 'b, 'c) J.t 
end
...
module H1 = struct
  ...
  module Iter(F : T1)
      (C : sig val f: 'a F.t -> unit end) = struct
  ...
  module Map(F : T1)(G : T1)
      (C : sig val f: ('a) F.t -> ('a) G.t end) = struct
  ...
  module Fold(F : T1)(X : T0)
      (C : sig val f: X.t -> ('a) F.t -> X.t end) = struct
    ...
    let rec f
      ...
      | [] -> init
      | x :: xs ->
        f ~init:(C.f init x) xs
  end

  module Map_reduce(F : T1)(X : T0)(C : sig
  ...
  module Binable(F: T1)(G : T1)(C : sig
      val f : 'a F.t -> 'a G.t Binable.m
    end)
  = struct
    let rec f : type xs. xs T(F).t
      ->  xs T(G).t Binable.m
      =
      fun ts ->
        match ts with
  ...
  module Typ(Impl: sig type field end)  (F: T1)(Var : T1)(Val : T1)(C : sig
      val f : 'a F.t -> ('a Var.t, 'a Val.t, Impl.field) Snarky.Typ.t
    end)
  = struct
    let rec f : type xs. xs T(F).t
      ->  ( xs T(Var).t , xs T(Val).t, Impl.field ) Snarky.Typ.t
      =
      let transport, transport_var, tuple2,unit = Snarky.Typ.(transport, transport_var, tuple2, unit) in
      fun ts ->
        match ts with
        | t :: ts ->
          let tail = f ts in
          transport (tuple2 (C.f t) tail)
...
module H2 = struct
  ...
  module Map(F : T2)(G : T2)
      (C : sig val f: ('a, 'b) F.t -> ('a, 'b) G.t end) = struct
...
module H3_2 = struct
  ...
  struct
    ...
    let rec length
      : type t1 t2 t3 e1 e2. (t1, t2, t3, e1, e2) t -> t1 Length.n
      =
      function
      | [] -> T (Z, Z)
      | _ :: xs ->
...
module H3_4 = struct
  ...
  struct
    type (_, _, _, 's1, 's2, 's3, 's4) t =
      ...
      | ( :: ) :
          ('a1, 'a2, 'a3, 's1, 's2, 's3, 's4) F.t * ('b1, 'b2, 'b3, 's1, 's2, 's3, 's4) t
...
module H2_1 = struct
  ...
  struct
    ...
    let rec length
      : type tail1 tail2 e. (tail1, tail2, e) t -> tail1 Length.n =
      function
      | [] -> T (Z, Z)
      | _ :: xs ->
        let T (n, p) = length xs in
  ...
  module Iter(F : T3)
      (C : sig val f: ('a, 'b, 'c) F.t -> unit end) = struct
  ...
  module Map_(F : T3)(G : T3)(Env : sig type t end)
      (C : sig val f: ('a, 'b, Env.t) F.t -> ('a, 'b, Env.t) G.t end) = struct
  ...
  module Map(F : T3)(G : T3)
      (C : sig val f: ('a, 'b, 'c) F.t -> ('a, 'b, 'c) G.t end) = struct
    ...
    let f : type a b c.
      ...
      =
      fun xs ->
        let module M = Map_(F)(G)(struct type t = c end)(struct let f = C.f end) in
        M.f xs
  end
  ...
  module Zip3(F1 : T3)(F2: T3)(F3 : T3) = struct
    let rec f
      ...
      fun xs ys zs ->
        match xs, ys, zs with
        | [], [], [] -> []
        | x :: xs , y::ys, z::zs -> (x, y, z) :: f xs ys zs
  end
  ...
  module Zip4(F1 : T3)(F2: T3)(F3 : T3)(F4 : T3) = struct
    let rec f
      ...
      fun xs ys zs ws ->
        ...
        | [], [], [], [] -> []
        | x :: xs , y::ys, z::zs, w::ws -> (x, y, z, w) :: f xs ys zs ws
  end
...
module H3 = struct
  ...
  struct
    ...
    let rec length
      : type tail1 tail2 tail3 e. (tail1, tail2, tail3) t -> tail1 Length.n =
      function
      | [] -> T (Z, Z)
      | _ :: xs ->
        let T (n, p) = length xs in
  ...
  module Map1_to_H1
      (F : T3)(G : T1)
      (C : sig val f: ('a, 'b, 'c) F.t -> 'a G.t end) = struct
  ...
  module Map2_to_H1
      (F : T3)(G : T1)
      (C : sig val f: ('a, 'b, 'c) F.t -> 'b G.t end) = struct
...
module H4 = struct
  ...
  struct
    ...
    let rec length
      : type tail1 tail2 tail3 tail4. (tail1, tail2, tail3, tail4) t -> tail1 Length.n =
      function
      | [] -> T (Z, Z)
      | _ :: xs ->
        let T (n, p) = length xs in
  ...
  module Fold(F : T4)(X : T0)
      (C : sig val f: X.t -> _ F.t -> X.t end) = struct
  ...
  module Map(F : T4)(G : T4)
      (C : sig val f: ('a, 'b, 'c, 'd) F.t -> ('a, 'b, 'c, 'd) G.t end) = struct
...
module Map_1_specific(F : T2)(G : T2)
    (C : sig type b1 type b2 val f: ('a, b1) F.t -> ('a, b2) G.t end) = struct
...
module type Max_s = sig type ns type n val n : n t val p : (ns, n) Hlist_1(Lte).t end
...
let rec max : type n ns. (n * ns) H1.T(Nat).t -> (n * ns) max =
  ...
  | x :: ((_ :: _) as ys) ->
    ...
    | `Gt gt ->
      ...
      let module M = Map_1_specific(Lte)(Lte)(struct
          type b1 = Max.n
          type b2 = n

          let f : type a. (a, Max.n) Lte.t -> (a, n) Lte.t =
            fun a_lt_max ->
            Lte.trans a_lt_max max_lt_x
        end)
      in
...
let max_exn : type ns. ns H1.T(Nat).t -> ns max =
  function
...
module Maxes = struct
  ...
  type 'length t =
    | T : 'ns H1.T(Nat).t * ('ns, 'length) Length.t -> 'length t

  let rec f
    : type branches n.
      ((int, branches) Vector.t, n) Vector.t
      -> n t
    =
    function
    | [] -> T ( [], Length.Z )
  ...
  let m (type length) (vs : (_, length) Vector.t)
    ...
    let g : type length ns.
      ns H1.T(Nat).t -> (ns, length) Length.t
      -> (module S with type length = length)
      =
      fun maxes length ->
        (module (struct
