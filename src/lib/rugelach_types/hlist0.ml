module Length = struct
  ...
  let rec to_nat : type xs n. (xs, n) t -> n Nat.t =
    function
    | Z -> Z
    | S n -> S (to_nat n)

  type 'xs n =
    | T : 'n Nat.t * ('xs, 'n) t -> 'xs n

  let rec contr : type xs n m. (xs, n) t -> (xs, m) t -> (n, m) Core_kernel.Type_equal.t =
    fun t1 t2 ->
    match t1, t2 with
    | Z, Z -> T
    | S n, S m ->
...
struct
  ...
  let rec length
    : type tail1. tail1 t -> tail1 Length.n =
    function
    | [] -> T (Z, Z)
    | _ :: xs ->
