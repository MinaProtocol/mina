...
val registers :
     (_, 'snarked_ledger_hash, _, _) Poly.t
  -> ('snarked_ledger_hash, unit) Registers.Blockchain.t

val timestamp : (_, _, 'time, _) Poly.t -> 'time

val body_reference : (_, _, _, 'ref) Poly.t -> 'ref

val create_value :
     staged_ledger_hash:Staged_ledger_hash.t
  -> genesis_ledger_hash:Frozen_ledger_hash.t
  -> registers:(Frozen_ledger_hash.t, unit) Registers.Blockchain.t
...
val set_timestamp :
     ('staged_ledger_hash, 'lh, 'time, 'body_ref) Poly.t
  -> 'time
  -> ('staged_ledger_hash, 'lh, 'time, 'body_ref) Poly.t
