...
module Locations = struct
  ...
  let internal_tracing ?(after_hf = false) config_dir =
    let folder = if after_hf then "internal_tracing" else "internal-tracing" in
    config_dir ^/ folder
...
module AutoPolyfilled = struct
  ...
  let create_internal_tracing ~fork_config_dir =
    Unix.mkdir ~p:() (Locations.internal_tracing ~after_hf:true fork_config_dir)
