...
a three-column unique key, and `insert_multi_into_col` inserts a list whose
length varies per call.

`upsert_into_cols_returning` is covered on branches that have it; this branch's
`Mina_caqti` does not define that helper.
