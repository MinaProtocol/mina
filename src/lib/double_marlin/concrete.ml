      ...
      let new_bulletproof_challenges, b =
        let pre_chals =
            bulletproof_challenges
              ...
              proof.openings.proof.lr
        in
        let b =
          let open Fq in
          let b_poly = b_poly (compute_challenges pre_chals) in
...
let%test_unit "concrete" =
  ...
  let wrap_kp =
    Impls.Dlog_based.generate_keypair
      ...
      (fun x () -> Pairing_based_proof.M.main ~bulletproof_log2 x)
  in
  let step_kp =
    Impls.Pairing_based.generate_keypair
      ~exposing:[Impls.Pairing_based.input ~bulletproof_log2]
      (fun x () -> Dlog_based_proof.M.main (module Pairing_main_inputs.App_state) x)
  in
