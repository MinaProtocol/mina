...
let compute_sg chals =
  Snarky_bn382.Fq_urs.b_poly_commitment
    ...
    (Fq.Vector.of_array (compute_challenges chals))
  |> G.Affine.of_backend
...
module Dlog_based_proof = struct
  ...
  let public_input_of_statement prev_statement =
    let input =
      Impls.Dlog_based.generate_public_input [Impls.Dlog_based.input]
  ...
  let step (type state)
      (module State : App_state_intf with type Constant.t = state)
      ~dlog_marlin_index ~pairing_marlin_index pk dlog_vk
      (next_app_state : state)
      ({ proof= prev_proof
       ; statement= prev_statement
       ; prev_evals
       ; prev_x_hat_beta_1 } :
        state dlog_based_proof) : state pairing_based_proof =
    let prev_challenges =
          ...
          { prev_statement.proof_state with
            me_only=
              Common.hash_dlog_me_only
                ...
                ; pairing_marlin_acc=
                    prev_statement.proof_state.me_only.pairing_marlin_acc } }
      }
    in
    ...
    let new_bulletproof_challenges, b =
      let prechals =
        Array.map (O.opening_prechallenges o) ~f:(fun x -> (x, Fq.is_square x))
      in
      let chals =
          ...
          { prev_proof.openings with
            proof=
              { prev_proof.openings.proof with
                sg=
                  ( if was_base_case then compute_sg new_bulletproof_challenges
      ...
      let combined_inner_product =
        ...
        let combine x_hat pt e =
          ...
          Pcs_batch.combine_evaluations
            ...
            ~crs_max_degree:dlog_crs_max_degree ~xi ~mul ~add ~one
            ~evaluation_point:pt (b_poly pt :: x_hat :: a) b
        in
    ...
    let handler (Snarky.Request.With {request; respond}) =
      ...
      | Prev_app_state State.Tag ->
          k prev_statement.pass_through.app_state
      | Prev_sg ->
          k prev_statement.pass_through.sg
      | Prev_x_hat_beta_1 ->
          k prev_x_hat_beta_1
      | Me_only State.Tag ->
...
module Make (State : App_state_intf) : sig
  ...
  let wrap_kp =
    Impls.Dlog_based.generate_keypair ~exposing:[Impls.Dlog_based.input]
      (fun x () ->
  ...
  let step_kp =
    Impls.Pairing_based.generate_keypair
      ~exposing:[Impls.Pairing_based.input ~bulletproof_log2] (fun x () ->
        let () = Dlog_based_proof.M.main (module State) x in
        () )
  ...
  let negative_one : State.Constant.t dlog_based_proof =
    ...
    let opening_proof_lr = Array.init bulletproof_log2 ~f:(fun _ -> (g, g)) in
  ...
  let wrap proof =
    Pairing_based_proof.wrap
      (module State)
      ~dlog_marlin_index ~pairing_marlin_index step_vk wrap_pk proof
end
...
let%test_unit "concrete" =
  ...
  let open Common in
  let proof = time "first step" (fun () -> M.step M.negative_one Fp.zero) in
