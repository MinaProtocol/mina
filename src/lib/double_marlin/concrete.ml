(*
module SqrtExample (Impl : Snarky.Snark_intf.Run with type prover_state = unit) = struct
    let sqrt x () =
        let y =
            Impl.exists Impl.Field.typ
                ~compute:Impl.As_prover.( fun () ->
                    Impl.Field.Constant.sqrt (read_var x)
                )
        in
        Impl.assert_r1cs y y x;
        ()

    let input () = Impl.Data_spec.[ Impl.Field.typ ]
    let keys = Impl.generate_keypair ~exposing:(input ()) sqrt
    let statement = Impl.Field.Constant.of_int 4
    let proof = Impl.prove (Impl.Keypair.pk keys) (input ()) sqrt () statement
    let () = assert (Impl.verify proof (Impl.Keypair.vk keys) (input ()) statement)
end
include SqrtExample(Snarky.Snark.Run.Make(Snarky.Snark.Backends.Bn128.Default) (Core.Unit))
*)
module RootExample (Impl : Snarky.Snark_intf.Run with type prover_state = unit) = struct
    let root x16 () =
        let x4 =
            Impl.exists Impl.Field.typ
                ~compute:Impl.As_prover.
                (
                    fun () -> Impl.Field.Constant.sqrt (read_var x16)
                )
        in
        let x2 =
            Impl.exists Impl.Field.typ
                ~compute:Impl.As_prover.
                (
                    fun () -> Impl.Field.Constant.sqrt (read_var x4)
                )
        in
        let x =
            Impl.exists Impl.Field.typ
                ~compute:Impl.As_prover.
                (
                    fun () -> Impl.Field.Constant.sqrt (read_var x2)
                )
        in
        Impl.assert_r1cs x x x2;
        Impl.assert_r1cs x2 x2 x4;
        Impl.assert_r1cs x4 x4 x16;
        ()

    let input () = Impl.Data_spec.[ Impl.Field.typ ]
    let keys = Impl.generate_keypair ~exposing:(input ()) root
    let statement = Impl.Field.Constant.of_int 256
    let proof = Impl.prove (Impl.Keypair.pk keys) (input ()) root () statement
    let () = assert (Impl.verify proof (Impl.Keypair.vk keys) (input ()) statement)
end
...
module MerkleTree (Snark : Snarky.Snark_intf.S) = struct
    ...
    let typ : (var, value) Snark.Typ.t = Snark.Typ.list ~length Snark.Boolean.typ
    ...
    module Merkle_tree_checked = Snarky.Merkle_tree.Checked (Snark) (Hash) (Value)
    ...
    *)
    ...
    let rec go curr_root = function
        ...
        | (x, addr) :: updates' ->
            ...
            let%bind prev_value =
                ...
                ~compute:
                    Snark.As_prover.(
                    map2 ~f:Snarky.Merkle_tree.get_exn get_state (read address_typ addr))
            in
    ...
    let verified =
    Snark.verify proof (Snark.Keypair.vk keypair) (input ()) root_start updates root_end
