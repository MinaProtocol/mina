...
struct
  ...
  module Dlog_based_reduced_me_only = struct
    ...
    type t =
      { pairing_marlin_acc:
          (g1, g1 Int.Map.t) Pairing_marlin_types.Accumulator.t
      ; old_bulletproof_challenges:
          (Challenge.Constant.t, bool) Bulletproof_challenge.t Bp_vector.t
          Branching_vector.t }
    [@@deriving bin_io, sexp_of]

    let prepare {pairing_marlin_acc; old_bulletproof_challenges} =
  ...
  module Dlog_based_proof = struct
    ...
    let step (type state)
            ...
            let combined_inner_product =
              ...
              let combine x_hat pt e =
                ...
                Pcs_batch.combine_evaluations
                  (Common.dlog_pcs_batch (Branching.add Nat.N19.n)
      ...
      let next_statement : _ Statement.Pairing_based.t =
        ...
        { proof_state=
            { unfinalized_proofs=
                Vector.map unfinalized_proofs ~f:(fun x ->
        ...
        Impls.Pairing_based.prove pk [input]
          (fun x () ->
            ( Impls.Pairing_based.handle
                (fun () ->
                  ( M.main ~wrap_domains:dlog_domains
  ...
  module Pairing_based_proof = struct
      ...
      Impls.Pairing_based.verify proof vk [typ]
        ...
        ; pass_through=
            Vector.map statement.pass_through ~f:(fun x ->
                Common.hash_dlog_me_only (Dlog_based_reduced_me_only.prepare x)
            ) }
    ...
    let accumulate_pairing_checks (proof : Pairing_based.Proof.t)
        (prev_acc : _ Pairing_marlin_types.Accumulator.t) ~domain_h ~domain_k
        ~r ~r_k ~r_xi_sum ~beta_1 ~beta_2 ~beta_3 (f_1, f_2, f_3) =
      let open G1 in
      let prev_acc =
      ...
      Pairing_marlin_types.Accumulator.map ~f:to_affine_exn
        ...
        ; opening_check=
            ( Core.printf
                !"Obetas: %{sexp:Impls.Pairing_based.Field.Constant.t} \
                  %{sexp:Impls.Pairing_based.Field.Constant.t} \
                  %{sexp:Impls.Pairing_based.Field.Constant.t}\n\
                  %!"
                beta_1 beta_2 beta_3 ;
              Core.printf
                !"Ofs: %{sexp:Impls.Dlog_based.Field.Constant.t * \
                  Impls.Dlog_based.Field.Constant.t}  \
                  %{sexp:Impls.Dlog_based.Field.Constant.t * \
                  Impls.Dlog_based.Field.Constant.t}  \
                  %{sexp:Impls.Dlog_based.Field.Constant.t * \
                  Impls.Dlog_based.Field.Constant.t}\n\
                  %!"
                (G1.to_affine_exn f_1) (G1.to_affine_exn f_2)
    ...
    let wrap (type s) (module App : App_state_intf with type Constant.t = s)
        ~wrap_domains ~step_domains ~dlog_marlin_index ~pairing_marlin_indices
        which_index pairing_vk pk
      ...
      in
      let prev_me_only =
        Vector.map ~f:Dlog_based_reduced_me_only.prepare
        ...
        let me_only : Dlog_based_reduced_me_only.t =
          ...
          let prev_pairing_acc =
            ...
            Vector.map prev_statement.pass_through ~f:(fun t ->
                printf
                  !"vv %{sexp:(Impls.Dlog_based.Field.Constant.t * \
                    Impls.Dlog_based.Field.Constant.t, \
                    (Impls.Dlog_based.Field.Constant.t * \
          ...
          in
          printf
            !"Ocombined_acc  %{sexp:(Impls.Dlog_based.Field.Constant.t * \
              Impls.Dlog_based.Field.Constant.t, \
              (Impls.Dlog_based.Field.Constant.t * \
              Impls.Dlog_based.Field.Constant.t) Int.Map.t) \
              Pairing_marlin_types.Accumulator.t}\n\
          ...
          { pairing_marlin_acc=
              (let domain_h, domain_k = step_domains in
               accumulate_pairing_checks ~domain_h ~domain_k proof
      ...
      let me_only_prepared =
        Dlog_based_reduced_me_only.prepare next_statement.proof_state.me_only
      in
      let next_proof =
        ...
        Impls.Dlog_based.prove pk
          ...
          (fun x () ->
            ( Impls.Dlog_based.handle
                (fun () ->
                  ( M.main ~wrap_domains ~step_domains ~pairing_marlin_indices
                      (conv x)
                    : unit ) )
                handler
              : unit ) )
  ...
  module Make (State : App_state_intf) : sig
        ...
        let t =
          ...
          let main x () : unit =
            Pairing_based_proof.M.main (conv x)
              ...
              ~wrap_domains:rough_domains ~step_domains:rough_domains
          in
    ...
    let wrap proof =
      Pairing_based_proof.wrap
        ...
        ~wrap_domains:Domains.Dlog.(h, k)
        ~step_domains:Domains.Pairing.(h, k)
        ~dlog_marlin_index ~pairing_marlin_indices:[pairing_marlin_index] 0
        step_vk wrap_pk proof
  end
end
