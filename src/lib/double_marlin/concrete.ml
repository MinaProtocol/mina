module RootExample (Impl : Snarky.Snark_intf.Run with type prover_state = unit) =
struct
  let root x16 () =
    let x4 =
      Impl.exists Impl.Field.typ
        ~compute:
          Impl.As_prover.(fun () -> Impl.Field.Constant.sqrt (read_var x16))
    in
    let x2 =
      Impl.exists Impl.Field.typ
        ~compute:
...
module MerkleTree (Snark : Snarky.Snark_intf.S) = struct
  ...
  let verified =
    Snark.verify proof (Snark.Keypair.vk keypair) (input ()) root_start updates
      root_end
