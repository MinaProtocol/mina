let%test_module "backend test" = (module (struct
    ...
    module ComputationExample (Impl : Snarky.Snark_intf.Run with type prover_state = unit) = struct
        let computation x16 () =
            ...
            Impl.assert_r1cs y1 y2 one;
            Assert.equal y3 zero;
    ...
    include ComputationExample(Snarky.Snark.Run.Make(Zexe_backend.Tweedle.Dum_based) (Core.Unit))
