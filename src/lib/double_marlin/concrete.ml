let%test_module "bn382 backend test" = (module (struct
    ...
    module RootExample (Impl : Snarky.Snark_intf.Run with type prover_state = unit) = struct
        ...
        let%test_unit "check bn382 backend RootExample proof" =
    ...
    module MerkleTree (Snark : Snarky.Snark_intf.S) = struct
        ...
        *)
        ...
        let%test_unit "check bn382 backend MerkleTree proof" =
            assert (Snark.verify proof (Snark.Keypair.vk keypair) (input ()) root_start updates root_end)
    end
