let%test_module "bn382 backend test" =
  ( module struct
    module RootExample
    ...
    struct
      let root x16 () =
        ...
        let x2 =
          Impl.exists Impl.Field.typ
            ~compute:
              Impl.As_prover.(fun () -> Impl.Field.Constant.sqrt (read_var x4))
        in
        let x =
          Impl.exists Impl.Field.typ
            ~compute:
              Impl.As_prover.(fun () -> Impl.Field.Constant.sqrt (read_var x2))
        in
      ...
      let proof =
        Impl.prove (Impl.Keypair.pk keys) (input ()) root () statement
    ...
    include RootExample
              (Snarky.Snark.Run.Make
                 (Snarky_bn382_backend.Pairing_based)
                 (Core.Unit))
    include RootExample
              (Snarky.Snark.Run.Make
                 (Snarky_bn382_backend.Dlog_based)
                 (Core.Unit))
