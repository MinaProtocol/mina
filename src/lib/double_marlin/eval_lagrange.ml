...
struct
  ...
  module Fp_repr = struct
    ...
    *)
    type t = {base: B.t; term_bound: B.t; terms: Fq.t array}

    let fq_sum ts = Option.value (Array.reduce ts ~f:Fq.( + )) ~default:Fq.zero
    ...
    let ( * ) t1 t2 =
      ...
      {term_bound; base= t1.base; terms= res}
...
*)
    ...
    let create ~domain_size =
      ...
      let cs =
        Array.mapi zetas ~f:(fun i zeta_i ->
            Array.foldi zetas ~init:Fp.one ~f:(fun j acc zeta_j ->
                if i = j then acc else Fp.(acc * (zeta_i - zeta_j)) )
            |> Fp.inv )
      in
  ...
  let div_term ~chunk_size ~(precomp : Precomputation.t) (x : Fp_repr.t) i =
    let d =
      exists (Typ.list ~length:Fp.size_in_bits Boolean.typ) ~compute:(fun () ->
          let x = Fp_repr.read x in
          Fp.to_bits Fp.(precomp.cs.(i) * inv (x + precomp.neg_zetas.(i))) )
    in
    ...
    ( d
    , `Check_equal
        (Fp_repr.of_fp ~chunk_size precomp.cs.(i), Fp_repr.(bottom * d)) )

  let batch_eq_checks ~chunk_size ~sponge eq_checks =
    let n = List.length eq_checks in
    let randomness =
      Array.init n ~f:(fun _ ->
          Fp_repr.of_bits ~chunk_size
            (Sponge.squeeze sponge ~length:challenge_length) )
    in
    let scale_sum xs =
