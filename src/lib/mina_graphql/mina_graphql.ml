  ...
  module Itn = struct
    ...
    let metadatum =
      obj "logMetadatum" ~fields:(fun _ ->
          ...
          ; field "value"
              ~args:Arg.[]
              ~doc:"metadatum value" ~typ:(non_null string)
...
module Mutations = struct
  ...
  module Itn = struct
    ...
    let commands =
      [ schedule_payments; stop_payments; update_gating; flush_internal_logs ]
  end
end
  ...
  module Itn = struct
    ...
    let internal_logs =
      field "internalLogs"
        ...
        ~resolve:(fun _ _ start_log_id -> Itn_logger.get_logs start_log_id)
