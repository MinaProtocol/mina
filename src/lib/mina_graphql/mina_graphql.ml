...
module Subscriptions = struct
  ...
  let new_sync_update =
    subscription_field "newSyncUpdate"
      ...
      ~typ:(non_null @@ Sync_status_unix.Graphql.sync_status ())
      ~args:Arg.[]
...
module Mutations = struct
  ...
  let add_wallet =
    io_field "addWallet"
      ...
      ~typ:(non_null Payload_types.create_account)
      ~args:
        Arg.[ arg "input" ~typ:(non_null Input.AddAccountInput.arg_typ) ]
  ...
  let create_hd_account =
    io_field "createHDAccount"
      ...
      ~args:
        Arg.
          [ arg "input" ~typ:(non_null Input.CreateHDAccountInput.arg_typ)
          ]
  ...
  let unlock_wallet =
    io_field "unlockWallet"
      ...
      ~typ:(non_null Payload_types.unlock_account)
      ~args:Arg.[ arg "input" ~typ:(non_null Input.UnlockInput.arg_typ) ]
  ...
  let lock_wallet =
    field "lockWallet"
      ...
      ~typ:(non_null Payload_types.lock_account)
      ~args:Arg.[ arg "input" ~typ:(non_null Input.LockInput.arg_typ) ]
  ...
  let delete_wallet =
    io_field "deleteWallet"
      ...
      ~typ:(non_null Payload_types.delete_account)
      ~args:
        Arg.
          [ arg "input" ~typ:(non_null Input.DeleteAccountInput.arg_typ) ]
  ...
  let reload_wallets =
    io_field "reloadWallets" ~doc:"Reload tracked account information from disk"
      ...
      ~typ:(non_null Payload_types.reload_accounts)
      ~args:Arg.[]
  ...
  let import_account =
    io_field "importAccount" ~doc:"Reload tracked account information from disk"
      ~typ:(non_null Payload_types.import_account)
      ~args:
        Arg.
          [ arg "path"
              ~doc:
  ...
  let reset_trust_status =
    io_field "resetTrustStatus"
      ...
      ~typ:(list (non_null Payload_types.trust_status))
      ~args:
        Arg.
          [ arg "input"
              ~typ:(non_null Input.ResetTrustStatusInput.arg_typ)
          ]
  ...
  let send_user_command coda user_command_input =
    ...
    | `Active f -> (
        ...
        | Ok user_command ->
            Ok
              { UserCommand.With_status.data = user_command
              ; status = Unknown
              }
        | Error e ->
  ...
  let send_signed_user_command ~signature ~coda ~nonce_opt ~signer ~memo ~fee
    ...
    UserCommand.With_status.map cmd ~f:(fun cmd ->
        { With_hash.data = cmd
  ...
  let send_delegation =
    io_field "sendDelegation"
      ...
      ~typ:(non_null Payload_types.send_delegation)
      ~args:
        Arg.
          [ arg "input" ~typ:(non_null Input.SendDelegationInput.arg_typ)
          ; Input.Fields.signature
          ]
      ~resolve:(fun { ctx = coda; _ } ()
        ...
        | None ->
            ...
            |> Deferred.Result.map ~f:UserCommand.mk_user_command
        | Some signature ->
            let%bind signature = signature |> Deferred.return in
  ...
  let send_payment =
    io_field "sendPayment" ~doc:"Send a payment"
      ~typ:(non_null Payload_types.send_payment)
      ~args:
        Arg.
          [ arg "input" ~typ:(non_null Input.SendPaymentInput.arg_typ)
          ; Input.Fields.signature
          ]
      ~resolve:(fun { ctx = coda; _ } ()
                    ( from
  ...
  let create_token =
    io_field "createToken" ~doc:"Create a new token"
      ~typ:(non_null Payload_types.create_token)
      ~args:
        Arg.
          [ arg "input" ~typ:(non_null Input.SendCreateTokenInput.arg_typ)
          ; Input.Fields.signature
          ]
      ~resolve:(fun { ctx = coda; _ } ()
                    ( fee_payer_pk
  ...
  let create_token_account =
    io_field "createTokenAccount" ~doc:"Create a new account for a token"
      ~typ:(non_null Payload_types.create_token_account)
      ~args:
        Arg.
          [ arg "input"
              ~typ:(non_null Input.SendCreateTokenAccountInput.arg_typ)
          ; Input.Fields.signature
          ]
      ~resolve:(fun { ctx = coda; _ } ()
                    ( token_owner
  ...
  let mint_tokens =
    io_field "mintTokens" ~doc:"Mint more of a token"
      ~typ:(non_null Payload_types.mint_tokens)
      ~args:
        Arg.
          [ arg "input" ~typ:(non_null Input.SendMintTokensInput.arg_typ)
          ; Input.Fields.signature
          ]
      ~resolve:(fun { ctx = coda; _ } ()
                    ( token_owner
  ...
  let send_rosetta_transaction =
    io_field "sendRosettaTransaction"
      ...
      ~typ:(non_null Payload_types.send_rosetta_transaction)
      ~args:
        Arg.
          [ arg "input" ~typ:(non_null Input.RosettaTransaction.arg_typ) ]
  ...
  let export_logs =
    io_field "exportLogs" ~doc:"Export daemon logs to tar archive"
      ...
      ~typ:(non_null Payload_types.export_logs)
  ...
  let set_coinbase_receiver =
    field "setCoinbaseReceiver" ~doc:"Set the key to receive coinbases"
      ~args:
        Arg.
          [ arg "input"
              ~typ:(non_null Input.SetCoinbaseReceiverInput.arg_typ)
          ]
      ~typ:(non_null Payload_types.set_coinbase_receiver)
  ...
  let set_snark_worker =
    io_field "setSnarkWorker"
      ...
      ~args:
        Arg.
          [ arg "input" ~typ:(non_null Input.SetSnarkWorkerInput.arg_typ)
          ]
      ~typ:(non_null Payload_types.set_snark_worker)
  ...
  let set_snark_work_fee =
    result_field "setSnarkWorkFee"
      ...
      ~args:
        Arg.[ arg "input" ~typ:(non_null Input.SetSnarkWorkFee.arg_typ) ]
      ~typ:(non_null Payload_types.set_snark_work_fee)
  ...
  let set_connection_gating_config =
    io_field "setConnectionGatingConfig"
      ~args:
        Arg.
          [ arg "input"
              ~typ:(non_null Input.SetConnectionGatingConfigInput.arg_typ)
          ]
      ~doc:
        "Set the connection gating config, returning the current config after \
         the application (which may have failed)"
      ~typ:(non_null Payload_types.set_connection_gating_config)
    ...
    io_field "addPeers"
      ~args:
        Arg.
          [ arg "peers"
              ~typ:
                (non_null @@ list @@ non_null @@ Input.NetworkPeer.arg_typ)
          ; arg "seed" ~typ:bool
          ]
      ...
      ~args:
        Arg.
          [ arg "block" ~doc:"Block encoded in precomputed block format"
              ~typ:(non_null Input.PrecomputedBlock.arg_typ)
          ]
      ~typ:
        (non_null
      ...
      ~args:
        Arg.
          [ arg "block" ~doc:"Block encoded in extensional block format"
              ~typ:(non_null Input.ExtensionalBlock.arg_typ)
          ]
      ~typ:
        (non_null
           (obj "Applied" ~fields:(fun _ ->
...
module Queries = struct
  ...
  let sync_status =
    io_field "syncStatus" ~doc:"Network sync status" ~args:[]
      ~typ:(non_null @@ Sync_status_unix.Graphql.sync_status ()) ~resolve:(fun { ctx = coda; _ } () ->
        let open Deferred.Let_syntax in
  ...
  let trust_status_all =
    field "trustStatusAll"
      ~typ:(non_null @@ list @@ non_null Payload_types.trust_status)
      ~args:Arg.[]
  ...
  let tracked_accounts_resolver { ctx = coda; _ } () =
    ...
    |> List.map ~f:(fun pk ->
           { AccountObj.account =
               AccountObj.Partial_account.of_pk coda pk
      ...
      ~args:
        Arg.
          ...
          ; arg' "token"
              ...
              ~typ:Input.TokenId.arg_typ ~default:Token_id.default
          ]
      ~resolve:(fun { ctx = coda; _ } () pk token ->
        Option.bind (get_ledger_and_breadcrumb coda)
          ~f:(fun (ledger, breadcrumb) ->
            ...
            AccountObj.Partial_account.of_full_account ~breadcrumb account
            |> AccountObj.lift coda pk ) )
      ...
      ~resolve:(fun { ctx = coda; _ } () pk ->
        ...
        | Some (ledger, breadcrumb) ->
            ...
            List.filter_map tokens ~f:(fun token ->
                ...
                in
                let%map account = Ledger.get ledger location in
                AccountObj.Partial_account.of_full_account ~breadcrumb
                  account
                |> AccountObj.lift coda pk )
        | None ->
            [] )
      ...
      ~args:
        Arg.
          [ arg "token" ~doc:"Token to find the owner for"
              ~typ:(non_null Input.TokenId.arg_typ)
          ]
  ...
  let transaction_status =
    result_field "transactionStatus" ~doc:"Get the status of a transaction"
      ~typ:(non_null @@ Transaction_inclusion_status_unix.Graphql.transaction_status ())
      ~args:
  ...
  let next_available_token =
    field "nextAvailableToken"
      ...
      ~args:Arg.[]
      ~typ:(non_null @@ Graphql_lib.Scalars.TokenId.typ ())
      ~resolve:(fun { ctx = coda; _ } () ->
        coda |> Mina_lib.best_tip |> Participating_state.active
        |> Option.map ~f:(fun tip ->
               Transition_frontier.Breadcrumb.staged_ledger tip
  ...
  let evaluate_vrf =
    io_field "evaluateVrf"
      ...
      ~args:
        Arg.
          [ arg "message" ~typ:(non_null Input.VrfMessageInput.arg_typ)
          ; arg "publicKey" ~typ:(non_null Input.PublicKey.arg_typ)
          ; arg "vrfThreshold" ~typ:Input.VrfThresholdInput.arg_typ
          ]
  ...
  let check_vrf =
    field "checkVrf"
      ...
      ~args:
        Arg.
          [ arg "input" ~typ:(non_null Input.VrfEvaluationInput.arg_typ) ]
