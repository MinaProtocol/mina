...
let check  : Command_transaction.Verifiable.t ->
  [ `Valid of Command_transaction.Valid.t
  | `Invalid
  | `Valid_assuming of (Command_transaction.Valid.t * _ list) ]
  = function
      ...
      with_return (fun { return } ->
        let payload = lazy 
                      Snapp_command.(Payload.(
                        Digested.digest (digested (to_payload cmd))))
        in 
        ...
        let statements_to_check : _ list =
          List.filter_map  ~f:statement_to_check
            ...
            | Proved_signed r, `One vk1 ->
                check_signature r.two.authorization
                  r.two.data.body.pk ;
                [(vk1, r.one, r.two.data.body)]
            ...
            | Signed_signed r, `Zero ->
                check_signature r.one.authorization
                  r.one.data.body.pk ;
                check_signature r.two.authorization
                  r.two.data.body.pk ;
                []
        ...
        | [] -> `Valid v
        | _::_ -> `Valid_assuming (v, statements_to_check)
      )
