...
let z3_balance_change_staked_target () =
  Quickcheck.test ~trials
    ...
    ~f:(fun
        (fee_payer, staked_target, unstaked_counterparty, (validator, fee)) ->
  ...
  Quickcheck.test ~trials
    Quickcheck.Generator.(
      tuple4 (gen_account ()) (gen_account ()) (gen_account ())
        (tuple2 Public_key.Compressed.gen fee_gen) )
    ~f:(fun
        (fee_payer, opt_in_target, new_delegatee, (fee_payer_validator, fee)) ->
  ...
  Quickcheck.test ~trials
    Quickcheck.Generator.(
      tuple4 (gen_account ()) (gen_account ()) (gen_account ())
        (tuple2 Public_key.Compressed.gen fee_gen) )
    ~f:(fun
        (fee_payer, target, intermediate_validator, (fee_payer_validator, fee))
      ->
...
let z7_distinct_targets () =
  Quickcheck.test ~trials
    ...
    ~f:(fun
        ( (fee_payer, opt_in_target, opt_out_target)
        , new_delegatee
...
let z9_non_default_token_balance_change () =
  Quickcheck.test ~trials
    (let open Quickcheck.Generator.Let_syntax in
     ...
     in
     let%map fee_payer_validator = Public_key.Compressed.gen
     and fee = fee_gen in
