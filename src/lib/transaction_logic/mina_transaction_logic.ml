...
module type S = sig
  ...
  module Global_state : sig
    type t =
      ...
      ; protocol_state : Zkapp_predicate.Protocol_state.View.t
      }
  end
...
module Make (L : Ledger_intf.S) : S with type ledger := L.t = struct
  ...
  let apply_body ~is_start
    ...
    let%map () =
      let predicate_is_accept =
        Zkapp_predicate.Account.is_accept @@ Party.Predicate.to_full predicate
      in
      List.exists ~f:Fn.id
        [ predicate_is_accept
        ; increment_nonce
  ...
  module Inputs = struct
    ...
    module Protocol_state_predicate = struct
      include Zkapp_predicate.Protocol_state
    end
  ...
  module Env = struct
    ...
    type t =
      ...
      ; local_state :
          ...
          , Transaction_status.Failure.t option )
          Parties_logic.Local_state.t
      ; protocol_state_predicate : Zkapp_predicate.Protocol_state.t
    ...
    let perform ~constraint_constants:_ (type r)
      ...
      | Check_protocol_state_predicate (pred, global_state) -> (
          Zkapp_predicate.Protocol_state.check pred global_state.protocol_state
      ...
      | Check_predicate (_is_start, party, account, _global_state) -> (
          ...
          | Full p ->
              Or_error.is_ok (Zkapp_predicate.Account.check p account) )
  ...
  let apply_parties_unchecked_aux (type user_acc)
      ~(constraint_constants : Genesis_constants.Constraint_constants.t)
      ~(state_view : Zkapp_predicate.Protocol_state.View.t) ~(init : user_acc)
  ...
  let apply_transaction ~constraint_constants
      ~(txn_state_view : Zkapp_predicate.Protocol_state.View.t) ledger
      (t : Transaction.t) =
...
module For_tests = struct
  ...
  let view : Zkapp_predicate.Protocol_state.View.t =
