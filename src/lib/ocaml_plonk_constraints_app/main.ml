...
let%test_module "backend test" =
  ...
  ( module struct
    let () =
      Zexe_backend.Tweedle.Dee_plonk_based.Keypair.set_urs_info
        [On_disk {directory= "/tmp/"; should_write= true}]

    module ComputationExample
        (Impl : Snarky.Snark_intf.Run with type prover_state = unit and type field = Dee_plonk.Field.t)
    ...
    module Impl = Snarky.Snark.Run.Make(Zexe_backend.Tweedle.Dee_plonk_based) (Core.Unit)
