...
module Constraints (Intf : Snark_intf.Run with type prover_state = unit) = struct
  ...
  let unpack (scalar : Field.t) : Field.t array =
            ...
            (
              ...
              for i = 1 to rounds do
                ...
                [|
                  ...
                  s2;
                  s3;
                  s*(of_int 16) + s0*(of_int 8) + s1*(of_int 4) + s2*(of_int 2) + s3
                |];
    ...
    in
    state.(rounds).(4) <- scalar;
    Intf.assert_
      [{
        basic= Plonk_constraint.T (Pack { state }) ;
        annotation= None
      }];
    let ret = ref [] in
    for i = 0 to len - 1 do
      let elm = state.(i / 4 + 1).(i % 4) in
      if elm <> Field.zero || List.length !ret > 0 then ret := List.append !ret [elm];
    done;
    Array.of_list !ret
  ...
  let pack (bits : Field.t array) : Field.t =
    ...
    let bits = Array.init len ~f:(fun i -> (if i < padlen then Field.zero else bits.(i - padlen))) in
    let rounds = len / 4 in
