...
(** the type for a replacable strict pipe *)
type ('data_in_pipe, 'pipe_kind, 'write_return) t

(** [create ?warn_on_drop ~name type] creates a replacable strict pipe
...
(** [write t data] writes to a replacable strict pipe, it would return value
...
(** [request_reader ~reader_name t] requests replacable to prepare a new pipe,
...
(** [kill t] kills a replacable strict pipe, it will close all related writer in
    itself. *)
