...
let convert_zkapp_command_from_selector ~partitioner ~job ~pairing
  ...
  let unscheduled_segments =
    Snark_worker_shared.Zkapp_command_inputs.read_all_proofs_from_disk
      unscheduled_segments
    |> Mina_stdlib.Nonempty_list.mapi ~f:(fun idx (witness, spec, statement) ->
           ( Work.Spec.Sub_zkapp.Stable.Latest.Segment
               { statement; witness; spec }
           , Work.Id.Range.{ first = idx; last = idx } ) )
  in
  let pending_zkapp_command, (first_segment, range) =
  ...
  partitioner.pending_zkapp_commands <-
    Single_id_map.add_exn ~key:pairing ~data:pending_zkapp_command
      partitioner.pending_zkapp_commands ;
  register_pending_zkapp_command_job ~id:pairing ~partitioner ~range
...
(** Submits a result of a single job.
    ...
    check reschedule because it's never in sent single job pool in the first
    place. 
    *)
let submit_single ?(is_from_zkapp = false) ~partitioner
  ...
  match
    ...
    , is_from_zkapp )
  with
  | (None as removed_job), Some combining_result, true
