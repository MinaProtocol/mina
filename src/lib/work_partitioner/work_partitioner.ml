...
module Zkapp_command_job_with_status = With_job_status.Make (struct
  type t = Partitioned_work.Zkapp_command_job.Stable.Latest.t
end)
...
module Pending_Zkapp_command = struct
  type t =
    ...
    ; unscheduled_segments :
        Partitioned_work.Zkapp_command_job.Spec.Stable.Latest.t Queue.t
  ...
  let generate_merge ~(t : t) () =
    ...
    Partitioned_work.Zkapp_command_job.Spec.Stable.Latest.Merge
      { proof1; proof2 }
  ...
  let generate_job_spec (t : t) :
      Partitioned_work.Zkapp_command_job.Spec.Stable.Latest.t option =
    List.find_map ~f:(fun f -> f ()) [ generate_merge ~t; generate_segment ~t ]

  let submit_proof (t : t) ~(p : Ledger_proof.t)
