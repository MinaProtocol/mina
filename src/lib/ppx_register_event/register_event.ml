...
let check_interpolations ~loc msg label_names =
  match
    Ppxlib.Ast_pattern.(parse (some (estring __)))
      ~on_error:(fun () -> None)
      loc (Some msg) Option.some
  with
  | Some s -> (
...
let generate_loggers_and_parsers ~loc:_ ~path ty_ext msg_opt =
  ...
  let deriver_loc =
    ...
    List.find_map_exn ty_ext.ptyext_attributes ~f:(fun ({ txt; _ }, payload) ->
        if String.equal txt "deriving" then
          match payload with
          | PStr stris -> Some (List.find_map_exn stris ~f:find_deriver)
          | _ -> failwith (sprintf "Expected structure payload for %s" deriver)
        else None)
  in
  ...
  let to_yojson ty =
    To_selected.copy_expression
      (Ppx_deriving_yojson.ser_expr_of_typ (Of_selected.copy_core_type ty))
  in
  let of_yojson ~path ty =
    To_selected.copy_expression
      (Ppx_deriving_yojson.desu_expr_of_typ ~path
         (Of_selected.copy_core_type ty))
  in
