...
module Spec = struct
  ...
  module Stable = struct
    ...
    module V1 = struct
      type t = Single_spec.Stable.V1.t Work.Spec.Stable.V1.t
      [@@deriving sexp, yojson]
  ...
  type t = Single_spec.t Work.Spec.t

  let materialize : t -> Stable.Latest.t =
    Work.Spec.map ~f:Single_spec.materialize

  let cache ~(proof_cache_db : Proof_cache_tag.cache_db) : Stable.Latest.t -> t
      =
    Work.Spec.map ~f:(Single_spec.cache ~proof_cache_db)
end
