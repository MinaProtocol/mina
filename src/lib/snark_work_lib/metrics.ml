...
let collect_single ~(single_spec : Selector.Single.Spec.t)
  ...
  | Work.Single.Spec.Transition (_, { transaction; _ }) ->
...
let emit_proof_metrics ~data =
  ...
  | Partitioned.Spec.Poly.Sub_zkapp_command
      { spec = { spec = Partitioned.Zkapp_command_job.Spec.Segment _; _ }
      ; metric
      } ->
      (* WARN:
  ...
  | Partitioned.Spec.Poly.Sub_zkapp_command
      { spec = { spec = Partitioned.Zkapp_command_job.Spec.Merge _; _ }
      ; metric
      } ->
      (* WARN:
         I don't know if this is the desired behavior, we need CI engineers.
