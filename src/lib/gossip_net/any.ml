...
module Make (Rpc_intf : Network_peer.Rpc_intf.Rpc_interface_intf) :
  ...
  let broadcast_transaction_pool_diff ?origin_topics (Any ((module M), t)) =
    M.broadcast_transaction_pool_diff ?origin_topics t

  let broadcast_snark_pool_diff ?origin_topics (Any ((module M), t)) =
    M.broadcast_snark_pool_diff ?origin_topics t
