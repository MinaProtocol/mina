...
use plonk_plookup_circuits::{
    constraints::{zk_polynomial, zk_w1, zk_w3, ConstraintSystem as PlonkConstraintSystem},
    domains::EvaluationDomains as PlonkEvaluationDomains,
    wires::COLUMNS,
};
use plonk_plookup_protocol_dlog::index::{
    ...
    G::ScalarField: CommitmentField + ToBytes,
{
    ...
    write_poly_comm(&vk.lkp_comm, &mut w)?;
    write_poly_comm(&vk.table_comm, &mut w)?;
    ...
    G::ScalarField: CommitmentField + FromBytes,
{
    ...
    let lkp_comm = read_poly_comm(&mut r)?;
    let table_comm = read_poly_comm(&mut r)?;
    ...
    G::ScalarField: CommitmentField + FromBytes,
{
    ...
    write_dense_polynomial(&c.lkpm, &mut w)?;
    write_dense_polynomial(&c.tablem, &mut w)?;
    ...
    write_plonk_evaluations(&c.lkpl4, &mut w)?;
    write_plonk_evaluations(&c.lkpl8, &mut w)?;
    write_plonk_evaluations(&c.table8w, &mut w)?;
    write_plonk_evaluations(&c.table8, &mut w)?;
    write_plonk_evaluations(&c.table1, &mut w)?;

    write_plonk_evaluations(&c.l14, &mut w)?;
    write_plonk_evaluations(&c.l18, &mut w)?;
    ...
    G::ScalarField: CommitmentField + FromBytes,
{
    ...
    let lkpm = read_dense_polynomial(&mut r)?;
    let tablem = read_dense_polynomial(&mut r)?;
    ...
    let lkpl4 = read_plonk_evaluations(&mut r)?;
    let lkpl8 = read_plonk_evaluations(&mut r)?;
    let table8w = read_plonk_evaluations(&mut r)?;
    let table8 = read_plonk_evaluations(&mut r)?;
    let table1 = read_plonk_evaluations(&mut r)?;

    let l14 = read_plonk_evaluations(&mut r)?;
    let l18 = read_plonk_evaluations(&mut r)?;
