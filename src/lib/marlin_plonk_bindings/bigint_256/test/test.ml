...
let () =
  ...
  comparisons x y ;
  comparisons y x ;
  Format.printf "compare(%s, %s)=%i@." (to_string x) (to_string x)
    (compare x x) ;
  Format.printf "compare(%s, %s)=%i@." (to_string x) (to_string y)
    (compare x y) ;
  Format.printf "compare(%s, %s)=%i@." (to_string y) (to_string x)
    (compare y x) ;
  let z = div x y in
