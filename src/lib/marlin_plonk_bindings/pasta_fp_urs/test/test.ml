open Marlin_plonk_bindings_pasta_fp_urs
module Fp = Marlin_plonk_bindings_pasta_fp
...
let () =
  ...
  | Finite (x, y) ->
      Format.printf "h= (%s, %s)@."
        Marlin_plonk_bindings_pasta_fq.(to_string x)
        Marlin_plonk_bindings_pasta_fq.(to_string y) ) ;
