open Marlin_plonk_bindings_tweedle_fq_urs
module Fq = Marlin_plonk_bindings_tweedle_fq
...
let () =
  ...
  | Finite (x, y) ->
      Format.printf "h= (%s, %s)@."
        Marlin_plonk_bindings_tweedle_fp.(to_string x)
        Marlin_plonk_bindings_tweedle_fp.(to_string y) ) ;
