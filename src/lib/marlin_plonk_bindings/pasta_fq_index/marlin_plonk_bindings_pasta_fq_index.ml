...
module Gate_vector = struct
  ...
  external add :
    t -> Marlin_plonk_bindings_tweedle_fq.t Plonk_gate.t -> unit
    = "caml_pasta_fq_plonk_gate_vector_add"

  external get :
    t -> int -> Marlin_plonk_bindings_tweedle_fq.t Plonk_gate.t
...
external create :
  Gate_vector.t -> int -> Marlin_plonk_bindings_tweedle_fq_urs.t -> t
