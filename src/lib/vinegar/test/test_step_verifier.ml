...
module Step_main_inputs = Vinegar__Step_main_inputs
...
let test_side_loaded_domains () =
  ...
  let open Vinegar__Side_loaded_verification_key in
  ...
  List.iter domains ~f:(fun ds ->
      ...
      in
      let checked_domain () =
        Vinegar__Step_verifier.For_tests_only.side_loaded_domain
