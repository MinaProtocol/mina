...
module Verification_key = struct
  type t = Marlin_plonk_bindings.Pasta_5_wires_fq_verifier_index.t

  let to_string _ = failwith __LOC__

  let of_string _ = failwith __LOC__

  let shifts = Marlin_plonk_bindings.Pasta_5_wires_fq_verifier_index.shifts
end

module R1CS_constraint_system =
  Plonk_5_wires_constraint_system.Make
    (Field)
    (Marlin_plonk_bindings.Pasta_5_wires_fq_index.Gate_vector)
    (struct
      let params =
        Sponge.Params.(
...
module Keypair = Dlog_plonk_5_wires_based_keypair.Make (struct
  ...
  module Index = Pasta_5_wires_fq_index
  module Curve = Curve
  module Poly_comm = Fq_poly_comm
  module Scalar_field = Field
  module Verifier_index = Pasta_5_wires_fq_verifier_index
  module Gate_vector = Pasta_5_wires_fq_index.Gate_vector
...
module Proof = Plonk_5_wires_dlog_proof.Make (struct
  ...
  module Backend = struct
    include Pasta_5_wires_fq_proof
    ...
    let create_aux ~f:create (pk : Keypair.t) primary auxiliary prev_chals
      ...
      in
      let n = Pasta_5_wires_fq_index.domain_d1_size pk.index in
...
module Oracles = Plonk_dlog_oracles.Make (struct
  ...
  module Backend = struct
    include Marlin_plonk_bindings.Pasta_5_wires_fq_oracles
