...
module type Inputs_intf = sig
  ...
  val store :
    (string, Marlin_plonk_bindings.Pasta_fq_urs.t) Store.Disk_storable.t
    ...
    module Backend = struct
      ...
      let batch_verify =
        with_lagranges (fun lgrs vks ts ->
            Inputs.run_in_thread (fun () -> batch_verify lgrs vks ts))
