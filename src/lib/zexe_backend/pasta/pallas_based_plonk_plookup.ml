...
module Verification_key = struct
  type t = Marlin_plonk_bindings.Pasta_plookup_fq_verifier_index.t

  let to_string _ = failwith __LOC__

  let of_string _ = failwith __LOC__

  let shifts = Marlin_plonk_bindings.Pasta_plookup_fq_verifier_index.shifts
end

module R1CS_constraint_system =
  Plonk_plookup_constraint_system.Make
    (Field)
    (Marlin_plonk_bindings.Pasta_plookup_fq_index.Gate_vector)
    (struct
      let params =
        Sponge.Params.(
...
module Keypair = Dlog_plonk_plookup_based_keypair.Make (struct
  ...
  module Index = Pasta_plookup_fq_index
  module Curve = Curve
  module Poly_comm = Fq_poly_comm
  module Scalar_field = Field
  module Verifier_index = Pasta_plookup_fq_verifier_index
  module Gate_vector = Pasta_plookup_fq_index.Gate_vector
...
module Proof = Plonk_plookup_dlog_proof.Make (struct
  ...
  module Backend = struct
    include Pasta_plookup_fq_proof
...
module Oracles = Plookup_dlog_oracles.Make (struct
  ...
  module Backend = struct
    include Marlin_plonk_bindings.Pasta_plookup_fq_oracles
