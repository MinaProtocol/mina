  ...
  let f = 
    Memo.general
      ...
      (fun domain_log2 ->
        ...
        Array.iter Precomputed.Lagrange_precomputations.(dee.(index_of_domain_log2 domain_log2)) ~f:(fun g ->
...
module Keypair = Plonk_dlog_keypair.Make (struct
  ...
  module Index = struct
    ...
    let create (sys : R1CS_constraint_system.t) =
      create (R1CS_constraint_system.underlying sys)
  end 
  module Curve = Curve
  module Poly_comm = Fp_poly_comm
  module Scalar_field = Field
