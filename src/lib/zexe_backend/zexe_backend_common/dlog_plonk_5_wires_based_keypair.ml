...
module type Inputs_intf = sig
  ...
  module Constraint_system : sig
    type t = (Gate_vector.t, Scalar_field.t) Plonk_5_wires_constraint_system.t
...
module Make (Inputs : Inputs_intf) = struct
  ...
  type t =
    { index: Index.t
    ; cs: (Gate_vector.t, Scalar_field.t) Plonk_5_wires_constraint_system.t }
  ...
  let create cs =
    ...
    let conv =
      Plonk_5_wires_constraint_system.Row.to_absolute
        ~public_input_size:(Set_once.get_exn cs.public_input_size [%here])
    in
    Plonk_5_wires_constraint_system.V.Table.iter cs.equivalence_classes
      ~f:(fun x ->
        if List.length x > 1 then
          let h = List.hd_exn x in
          let t = List.last_exn x in
          let col_to_index = Plonk_5_wires_constraint_system.col_to_index in
  ...
  let vk_commitments (t : Verifier_index.t) :
      ...
      Plonk_5_wires_verification_key_evals.t =
    let f (t : Poly_comm.Backend.t) =
      ...
      | _ ->
          assert false
    in
    Plonk_5_wires_verification_key_evals.map ~f t.evals
end
