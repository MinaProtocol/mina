module type Inputs_intf = sig
  ...
  module Index : sig
    ...
    val create :
         Gate_vector.t Plonk_constraint_system.t
      -> Unsigned.Size_t.t
      -> Urs.t
      -> t
  end
...
module Make (Inputs : Inputs_intf) = struct
  ...
  let create cs =
    let {Plonk_constraint_system.auxiliary_input_size;} = cs in 
    let t = Index.create cs (Unsigned.Size_t.of_int auxiliary_input_size) (load_urs ())
