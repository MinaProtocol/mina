...
module type Inputs_intf = sig
  ...
  module Backend : sig
    type t =
      ...
      , Curve.Affine.Backend.t
      , Poly_comm.Backend.t )
      Marlin_plonk_bindings_types.Plonk_5_wires_proof.t
...
module Make (Inputs : Inputs_intf) = struct
    ...
    module Stable = struct
      module V1 = struct
        type t =
          ...
          Dlog_plonk_5_wires_types.Proof.Stable.V1.t
        ...
        type 'a creator =
             messages:( G.Affine.t
                      ...
                      Dlog_plonk_5_wires_types.Messages.Stable.V1.t
          -> openings:( G.Affine.t
                      , Fq.t
                      , Fq.t Dlog_plonk_types.Pc_array.t )
                      Dlog_plonk_5_wires_types.Openings.Stable.V1.t
          -> 'a

        let map_creator c ~f ~messages ~openings = f (c ~messages ~openings)

        let create ~messages ~openings =
          let open Dlog_plonk_5_wires_types.Proof in
          {messages; openings}
      end
    end]
  end)
  ...
  let opening_proof_of_backend (t : Opening_proof_backend.t) =
    ...
    let gpair (type a) (t : G.Affine.Backend.t * G.Affine.Backend.t) :
      ...
      (g (fst t), g (snd t))
    in
    { Dlog_plonk_5_wires_types.Openings.Bulletproof.lr= Array.map ~f:gpair t.lr
    ; z_1= t.z1
    ; z_2= t.z2
    ; delta= g t.delta
    ; sg= g t.sg }
  ...
  let of_backend (t : Backend.t) : t =
    ...
    let evals =
      ...
      |> Tuple_lib.Double.map ~f:(fun e ->
             ...
             { Dlog_plonk_5_wires_types.Evals.l
             ; r
             ; o
             ; q
             ; p
