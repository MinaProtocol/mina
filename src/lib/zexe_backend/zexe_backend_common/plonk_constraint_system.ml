...
*)
  ...
  type t =
    ...
    ; internal_vars : ((Fp.t * V.t) list * Fp.t option) Internal_var.Table.t
    ; mutable rows_rev : V.t option array list
    ; mutable backend: Constraints.t
  ...
  *)
  ...
  let add_constraint ?label:_ sys (constr : (Fp.t Snarky_backendless.Cvar.t, Fp.t) Snarky_backendless.Constraint.basic) =
    ...
    | Plonk_constraint.T (Poseidon { start; state }) ->
      ...
      let add_round_state a = 
        Array.iteri a ~f:(fun i x ->
            V.Table.add_multi sys.equivalence_classes ~key:x 
              ~data:{ row= sys.next_row; col= i } );
        add_row sys
          (Array.map a ~f:(fun x -> Some x))
      in
    ...
    | Plonk_constraint.T (EC_add { p1; p2; p3 }) ->
      let ps = Array.map [| p1; p2; p3 |] ~f:(fun (x, y) -> (reduce_to_v x, reduce_to_v y)) in
      Array.iteri ps ~f:(fun i (x, y) ->
      ...
      (* add rows
         ...
         p1.x p2.x p3.x
      *)
      add_row sys (Array.map ps ~f:(fun (_, y) -> Some y)) ;
      add_row sys (Array.map ps ~f:(fun (x, _) -> Some x)) ;
    ...
    | Plonk_constraint.T (EC_scale { state }) ->
      ...
      *)
      ...
      let add_ecscale_round (round: V.t Scale_round.t) =
        V.Table.add_multi sys.equivalence_classes ~key:(round.xt) ~data:{ row= sys.next_row; col= 0 } ;
        V.Table.add_multi sys.equivalence_classes ~key:(round.b) ~data:{ row= sys.next_row; col= 1 } ;
        V.Table.add_multi sys.equivalence_classes ~key:(round.yt) ~data:{ row= sys.next_row; col= 2 } ;
        V.Table.add_multi sys.equivalence_classes ~key:(round.xp) ~data:{ row= sys.next_row+1; col= 0 } ;
        V.Table.add_multi sys.equivalence_classes ~key:(round.l1) ~data:{ row= sys.next_row+1; col= 1 } ;
        V.Table.add_multi sys.equivalence_classes ~key:(round.yp) ~data:{ row= sys.next_row+1; col= 2 } ;
        V.Table.add_multi sys.equivalence_classes ~key:(round.xs) ~data:{ row= sys.next_row+2; col= 0 } ;
        V.Table.add_multi sys.equivalence_classes ~key:(round.ys) ~data:{ row= sys.next_row+2; col= 2 } ;
        List.iter ~f:(fun r -> add_row sys (Array.map r ~f:Option.return)) 
          [ [| round.xt; round.b; round.yt |]; 
            [| round.xp; round.l1; round.yp |]; 
            [| round.xs; round.xt; round.ys |] ]
      in
      Array.iter state ~f:(fun round ->
