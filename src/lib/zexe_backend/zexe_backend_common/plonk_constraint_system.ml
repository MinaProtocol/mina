...
struct
  ...
  let gate_vector ({gates; _} : t) : Gates.t = gates
...
*)
  ...
  let wire sys key row col =
    let prev =
      ...
      | Some x -> (
        match List.last x with Some x -> x | None -> {row; col} )
      | None ->
          {row; col}
    in
  ...
  let add_generic_constraint sys ?m ?c ?o ?r ((ls : Fp.t), (lx : V.t)) : unit =
    ...
    add_row sys
      [|Some lx; Option.map r ~f:snd; Option.map o ~f:snd|]
      1 lp rp op
  ...
  let completely_reduce sys (terms : (Fp.t * int) list) =
    ...
    let rec go = function
      ...
      | (ls, lx) :: t ->
          ...
          add_generic_constraint sys (ls, lx) ~r:(rs, rx)
            ~o:(neg_one, s1x1_plus_s2x2) ;
  ...
  let reduce_lincom sys (x : Fp.t Snarky_backendless.Cvar.t) =
    ...
    | _, (c0, i0) :: terms -> (
        ...
        | (ls, lx) :: tl ->
            ...
            (* res = ls * lx + rs * rx *)
            add_generic_constraint sys ?c:constant (ls, External lx)
              ~r:(rs, rx) ~o:(neg_one, res) ;
            (Fp.one, `Var res) )
  ...
  let add_constraint ?label:_ sys
    ...
    | Snarky_backendless.Constraint.Equal (v1, v2) -> (
        ...
        | `Var x1, `Var x2 ->
            if s1 <> s2 then
              add_generic_constraint sys (Fp.negate s1, x1) ~r:(s2, x2)
        ...
        | `Var x1, `Constant ->
            add_generic_constraint sys (Fp.negate s1, x1) ~c:s2
        | `Constant, `Var x2 ->
            add_generic_constraint sys (Fp.negate s2, x2) ~c:s1
        | `Constant, `Constant ->
    ...
    | Snarky_backendless.Constraint.Square (v1, v2) -> (
        ...
        | `Var x1, `Constant ->
            add_generic_constraint sys
              ...
              ~c:s2 (s1, x1) ~r:(s1, x1)
        | `Constant, `Var x2 ->
            add_generic_constraint sys (Fp.negate s2, x2) ~c:(Fp.square s1)
        | `Constant, `Constant ->
            assert (Fp.(equal (square s1) s2)) )
    | Snarky_backendless.Constraint.R1CS (v1, v2, v3) -> (
        ...
        | `Var x1, `Var x2, `Constant ->
            add_generic_constraint sys
              ~m:Fp.(s1 * s2)
              ~c:(Fp.negate s3) (s1, x1) ~r:(s2, x2)
        | `Var x1, `Constant, `Var x3 ->
            add_generic_constraint sys ~o:(Fp.negate s3, x3) (Fp.(s1 * s2), x1)
        | `Constant, `Var x2, `Var x3 ->
            add_generic_constraint sys ~o:(Fp.negate s3, x3) (Fp.(s1 * s2), x2)
        | `Var x1, `Constant, `Constant ->
            add_generic_constraint sys ~c:(Fp.negate s3) (Fp.(s1 * s2), x1)
        | `Constant, `Var x2, `Constant ->
            add_generic_constraint sys ~c:(Fp.negate s3) (Fp.(s1 * s2), x2)
        | `Constant, `Constant, `Var x3 ->
            add_generic_constraint sys ~c:Fp.(negate s1 * s2) (s3, x3)
