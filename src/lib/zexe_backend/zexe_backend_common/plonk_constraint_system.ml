...
module type Constraint_system_intf = sig
  ...
  val create : unit -> t
  val init : t -> size_t -> unit
  val add_gate : t -> int -> size_t -> size_t -> size_t -> size_t -> size_t -> size_t -> field_vector -> unit
end
...
module Make (Fp : sig
    ...
    (Constraints : Constraint_system_intf with type field_vector := Fp.Vector.t) =
struct
  open Core
  ...
  type nonrec t = Constraints.t t

  let create () =
    { public_input_size= 0
    ; backend= Constraints.create()
    ; next_row= 0
