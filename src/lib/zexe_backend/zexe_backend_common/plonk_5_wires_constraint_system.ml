...
module type Gate_vector_intf = sig
  ...
  val add : t -> field Plonk_5_wires_gate.t -> unit

  val get : t -> int -> field Plonk_5_wires_gate.t
end
...
module Gate_spec = struct
  type ('row, 'f) t =
    { kind: Plonk_5_wires_gate.Kind.t
    ; row: 'row
    ; lrow: 'row
    ; lcol: Plonk_5_wires_gate.Col.t
    ; rrow: 'row
    ; rcol: Plonk_5_wires_gate.Col.t
    ; orow: 'row
    ; ocol: Plonk_5_wires_gate.Col.t
    ; qrow: 'row
    ; qcol: Plonk_5_wires_gate.Col.t
    ; prow: 'row
    ; pcol: Plonk_5_wires_gate.Col.t
    ; coeffs: 'f array }
...
module Position = struct
  type t = {row: Row.t; col: Plonk_5_wires_gate.Col.t}
end
