...
module Helper = struct
  ...
  module Rpcs = struct
    ...
    module Set_node_status_data = struct
      type input = {data: string} [@@deriving yojson]

      type output = string [@@deriving yojson]

      let name = "setNodeStatusData"
    end

    module Get_peer_node_status_data = struct
      type input = {peer_multiaddr: string} [@@deriving yojson]

      type output = string [@@deriving yojson]

      let name = "getPeerNodeStatusData"
    end
...
let set_node_status_data net data =
  match%map
    Helper.do_rpc net (module Helper.Rpcs.Set_node_status_data) {data}
  with
  | Ok "setNodeStatusData success" ->
      Ok ()
  | Ok v ->
      failwithf "helper broke RPC protocol: setNodeStatusData got %s" v ()
  | Error e ->
      Error e

let get_peer_node_status_data net peer =
  Helper.do_rpc net
    (module Helper.Rpcs.Get_peer_node_status_data)
