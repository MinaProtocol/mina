...
let alice a ad (pc, _) msgs =
      ...
      ~handle_and_validate_incoming_message:(fun env cb ->
        Deferred.return
          ( if String.equal env.data msgs.topic_a_msg_1 then
            ...
            if !topic_a_received_1 && !topic_a_received_2 && !topic_a_received_3
  ...
  let%bind status_b = get_peer_node_status a ad.b_addr >>| Or_error.ok_exn in
...
let bob b ad (pc, _) msgs =
  ...
  let%bind subC =
    Pubsub.subscribe_encode b topic_c
      ~on_decode_failure:(`Call (fun _ _ -> raise UnexpectedState))
      ~bin_prot:bin_typed_msg
      ~handle_and_validate_incoming_message:(fun _ _ -> raise UnexpectedState)
    >>| Or_error.ok_exn
  in
...
let carol c ad (pc, _) msgs =
  ...
  let%bind () = Pubsub.publish_raw c ~topic:"topic/b" "not to be received" in
...
let setup_node ?keypair ?(seed_peers = []) ?(gating_config = def_gating_config)
    ?(ignore_advertise_error = false) local_name ~on_peer_connected
  ...
  let shutdown () =
    [%log info] "Shutting down $name" ~metadata:[ ("name", `String local_name) ] ;
...
let test_def () =
  ...
  let msgs =
    ...
    ; topic_c_msg_2 = { a = Random.int 1073741824; b = random_msg () |> Some }
...
let () =
  ...
  run "IPC" [ ("Test", [ test_case "basic" `Quick test_ipc ]) ]
