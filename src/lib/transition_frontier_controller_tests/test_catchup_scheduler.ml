  ...
  ( module struct
    ...
    let%test_unit "after the timeout expires, the missing node still doesn't \
      ...
      Thread_safe.block_on_async_exn (fun () ->
          ...
          |> don't_wait_for ;
          let%map catchup_hash =
            Ivar.read result_ivar >>| Cache_lib.Cached.peek >>| With_hash.hash
          in
      ...
      Thread_safe.block_on_async_exn (fun () ->
          ...
          List.(
            iter (rev dangling_transitions) ~f:(fun transition ->
                Catchup_scheduler.watch scheduler ~timeout_duration
                  ~cached_transition:(Cache_lib.Cached.pure transition) ;
          ...
          let%map received_rose_tree =
            extract_children_from ~reader:catchup_breadcrumbs_reader
              ~root:(Cache_lib.Cached.pure missing_breadcrumb)
          in
          assert (
            List.equal
              ( Rose_tree.flatten received_rose_tree
              |> List.map ~f:Cache_lib.Cached.peek )
      ...
      Thread_safe.block_on_async_exn (fun () ->
          ...
          List.iter (List.permute dangling_transitions)
            ~f:(fun dangling_transition ->
              Catchup_scheduler.watch scheduler ~timeout_duration
                ~cached_transition:(Cache_lib.Cached.pure dangling_transition)
          ) ;
          ...
          assert (
            Rose_tree.equiv
              (Rose_tree.map received_rose_tree ~f:Cache_lib.Cached.peek)
              upcoming_rose_tree ~f:Transition_frontier.Breadcrumb.equal ) ;
          Strict_pipe.Writer.close catchup_breadcrumbs_writer ;
          Strict_pipe.Writer.close catchup_job_writer )
  end )
