...
let existential_to_string = function
  | Event_type_logger Log_error ->
      "Log_error"
  | Event_type_logger Node_initialization ->
      "Node_initialization"
  | Event_type_logger Transition_frontier_diff_application ->
      "Transition_frontier_diff_application"
  | Event_type_logger Block_produced ->
      "Block_produced"
  | Event_type_logger Breadcrumb_added ->
      "Breadcrumb_added"
  | Event_type_logger Block_gossip ->
      "Block_gossip"
  | Event_type_logger Snark_work_gossip ->
      "Snark_work_gossip"
  | Event_type_logger Transactions_gossip ->
      "Transactions_gossip"

let to_string e = existential_to_string (Event_type_logger e)
...
let type_of_event (Event (t, _)) = Event_type_logger t
...
let event_type_module :
    type a. a t -> (module Event_type_logger_intf with type t = a) = function
  | Log_error ->
      (module Log_error)
  | Node_initialization ->
...
let to_structured_event_id ev_type =
  let (Event_type_logger t) = ev_type in
  let (module Type) = event_type_module t in
...
let parse_event (message : Logger.Message.t) =
  ...
  | Some event_id ->
      let (Event_type_logger ev_type) =
        of_structured_event_id event_id
        |> Option.value ~default:(Event_type_logger Log_error)
      in
