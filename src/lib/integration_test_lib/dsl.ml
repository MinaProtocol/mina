  ...
  let wait_for t condition =
    ...
    [%log' info t.logger]
      ...
      ~metadata:
        [ ( "soft_timeout"
          , `String
              (Network_time_span.to_string ~genesis_constants
                 ~constraint_constants condition.soft_timeout ) )
        ; ( "hard_timeout"
          , `String
              (Network_time_span.to_string ~genesis_constants
                 ~constraint_constants condition.hard_timeout ) )
        ] ;
    let%bind result =
