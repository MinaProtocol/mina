...
type t = {
 ...
 let create ~logger_metadata ~uri ~enabled ~logger =
  ...
  let exec t ?(num_tries = 10) ?(retry_delay_sec = 30.0) ?(initial_delay_sec = 0.) ~logger ~query_name query_obj =
  ...
  let send_payment_with_raw_sig t ~sender_pub_key ~receiver_pub_key
  ...
  let send_payment_graphql () =
    ...
    in
    exec ~logger:t.logger t ~query_name:"send_payment_graphql"
      send_payment_obj
  in
  ...
  let set_snark_worker t ~new_snark_pub_key =
    ...
    in
     [%log' info t.logger] "snark worker changed, lastSnarkWorker: %s" last_snark_worker
      ~metadata:[ ("lastSnarkWorker", `String last_snark_worker) ] ;
    ()
  
    let send_zkapp_batch (t : t)
      ~(zkapp_commands : Mina_base.Zkapp_command.t list) =
     [%log' info t.logger] "Sending zkapp transactions"
      ~metadata:t.logger_metadata;
    let open Deferred.Or_error.Let_syntax in
    ...
    let send_zkapp_graphql () =
      ...
      in
      exec ~logger:t.logger t ~query_name:"send_zkapp_graphql"
        send_zkapp_obj
    in
  ...
  let get_pooled_zkapp_commands (t : t)
     ...
     [%log' info t.logger] "Retrieving zkapp_commands from transaction pool"
      ~metadata:(t.logger_metadata @ [("pub_key", Signature_lib.Public_key.Compressed.to_yojson pk)]);
    let open Deferred.Or_error.Let_syntax in
  ...
  let send_payment t ~password ~sender_pub_key ~receiver_pub_key ~amount ~fee =
    ...
    let unlock_sender_account_graphql () =
      let unlock_account_obj =
        Requests.Unlock_account.(
          make
          @@ makeVariables ~password ~public_key:sender_pub_key ())
      in
    ...
    let send_payment_graphql () =
      ...
      in
      let send_payment_obj =
        Requests.Send_payment.(make @@ makeVariables ~input ())
      in
      exec ~logger:t.logger t ~query_name:"send_payment_graphql"
        send_payment_obj
    in
    ...
    let get_account_obj =
      Requests.Account.(
        ...
        @@ makeVariables
             ...
             ())
    in
    exec ~logger:t.logger t ~query_name:"get_account_graphql"
      get_account_obj
  ...
  *)
  ...
  let get_best_chain ?max_length t =
    ...
    let%bind result =
      exec ~logger:t.logger t ~query_name:"best_chain" query
    in
  ...
  let get_peer_id t =
    let open Deferred.Or_error.Let_syntax in
     [%log' info t.logger] "Getting node's peer_id, and the peer_ids of node's peers"
      ~metadata:t.logger_metadata ;
    ...
    let peer_ids = Core.List.map peers ~f:(fun peer -> peer.peerId) in
     [%log' info t.logger] "get_peer_id, result of graphql query (self_id,[peers]) (%s,%s)"
      self_id
      (Core.String.concat ~sep:" " peer_ids) ;
    return (self_id, peer_ids)

  
    let must_get_peer_id t =
      get_peer_id t |> Deferred.bind ~f:Malleable_error.or_hard_error
  
    let must_get_best_chain ?max_length t =
      get_best_chain ?max_length  t
      |> Deferred.bind ~f:Malleable_error.or_hard_error
  
    let must_get_account_data  t ~account_id =
      get_account_data  t ~account_id
      |> Deferred.bind ~f:Malleable_error.or_hard_error
  
    let must_send_payment t ~password ~sender_pub_key ~receiver_pub_key ~amount ~fee
        =
      send_payment t ~sender_pub_key ~receiver_pub_key ~amount ~fee ~password
      |> Deferred.bind ~f:Malleable_error.or_hard_error
  
    let must_send_delegation  t ~sender_pub_key ~receiver_pub_key ~fee =
      send_delegation  t ~sender_pub_key ~receiver_pub_key ~fee
      |> Deferred.bind ~f:Malleable_error.or_hard_error
  
    let must_send_payment_with_raw_sig t ~sender_pub_key ~receiver_pub_key
        ~amount ~fee ~nonce ~memo ~valid_until ~raw_signature =
      send_payment_with_raw_sig  t ~sender_pub_key ~receiver_pub_key
        ~amount ~fee ~nonce ~memo ~valid_until ~raw_signature
      |> Deferred.bind ~f:Malleable_error.or_hard_error
  
    let must_send_test_payments ~repeat_count ~repeat_delay_ms t ~senders
        ~receiver_pub_key ~amount ~fee =
      send_test_payments ~repeat_count ~repeat_delay_ms t ~senders
        ~receiver_pub_key ~amount ~fee 
      |> Deferred.bind ~f:Malleable_error.or_hard_error
  
  
    let must_set_snark_worker t ~new_snark_pub_key =
      set_snark_worker t ~new_snark_pub_key
      |> Deferred.bind ~f:Malleable_error.or_hard_error
