...
let run_cmd_or_error dir prog args ?env () =
  let%bind output = run_cmd dir prog args ?env () in
  check_cmd_output ~prog ~args output 

let run_cmd_exn dir prog args ?env () =
  match%map run_cmd_or_error dir prog args ?env () with
  | Ok output ->
      output
  | Error error ->
      Error.raise error

let run_cmd_or_hard_error ?exit_code dir prog args ?env () =
  let%bind output = run_cmd dir prog args ?env () in
  Deferred.bind
