...
let get_peer_id ~logger node_uri =
  ...
  let query_obj = Queries.Query_peer_id.(make @@ makeVariables ()) in
...
let send_online_payment ?(node_password = default_node_password) ~logger
  ...
  let send_payment_graphql () =
    ...
    let send_payment_obj =
      Queries.Send_payment.(make @@ makeVariables ~input ())
    in
...
let send_zkapp_batch ~logger node_uri
  ...
  let%bind zkapp_ids =
    Deferred.Array.fold ~init:(Ok []) sent_zkapp_obj.internalSendZkapp
      ~f:(fun acc (zkapp_obj : Queries.Send_test_zkapp.t_internalSendZkapp) ->
        let%bind res =
...
let send_delegation ?(node_password = default_node_password) ~logger node_uri
    ~sender_pub_key ~receiver_pub_key ~fee =
  ...
  let send_delegation_graphql () =
    ...
    let send_delegation_obj =
      Queries.Send_delegation.(make @@ makeVariables ~input ())
    in
...
let send_payment_with_raw_sig ~logger node_uri ~sender_pub_key ~receiver_pub_key
  ...
  let open Deferred.Or_error.Let_syntax in
  let send_payment_graphql () =
    let open Queries.Send_payment_with_raw_sig in
    let input =
      Mina_graphql.Types.Input.SendPaymentInput.make_input ~from:sender_pub_key
...
let send_test_payments ~(repeat_count : Unsigned.UInt32.t)
    ...
    ~(receiver_pub_key : Account.key) ~amount ~fee =
  [%log info] "Sending a series of test payments"
    ~metadata:[ ("node_uri", `String (Uri.to_string node_uri)) ] ;
  let open Deferred.Or_error.Let_syntax in
  let send_payment_graphql () =
    let send_payment_obj =
      Queries.Send_test_payments.(
        make
...
let set_snark_worker ~logger node_uri ~new_snark_pub_key =
  ...
  let set_snark_worker_graphql () =
    ...
    let set_snark_worker_obj =
      Queries.Set_snark_worker.(make @@ makeVariables ~input ())
    in
    exec_graphql_request ~logger ~node_uri
...
let get_metrics ~logger node_uri =
  ...
  [%log info] "Getting node's metrics"
    ~metadata:[ ("node_uri", `String (Uri.to_string node_uri)) ] ;
  let query_obj = Queries.Query_metrics.make () in
  let%bind query_result_obj =
...
let start_filtered_log node_uri ~logger ~log_filter ~retry_delay_sec =
  let open Deferred.Let_syntax in
  let query_obj =
    Queries.StartFilteredLog.(make @@ makeVariables ~filter:log_filter ())
  in
  let%bind res =
...
let get_detailed_best_chain ?max_length ~logger node_uri =
  let open Deferred.Or_error.Let_syntax in
  let query_obj =
    Queries.Best_chain_for_slot_end_test.(make @@ makeVariables ?max_length ())
  in
  let%bind result =
