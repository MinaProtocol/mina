...
module Make (Inputs : Inputs_intf) :
  ...
  let find_in_root_history frontier state_hash =
    ...
    Frontier_base.Root_data.Historical.validated_block root_data
  ...
  module Merkle_list = Merkle_list_prover.Make_ident (struct
    ...
    let get_previous ~context block =
      let parent_hash =
        block
      ...
      Option.merge
        Transition_frontier.(
          find context parent_hash >>| Breadcrumb.validated_block)
  ...
  let prove ?length ~frontier state_hash =
    ...
    let%map requested_transition =
      Option.merge
        Transition_frontier.(
          find frontier state_hash >>| Breadcrumb.validated_block)
        (find_in_root_history frontier state_hash)
        ~f:Fn.const
    in
    let first_block, merkle_list =
      Merkle_list.prove ?length ~context:frontier requested_transition
    in
    (first_block |> Mina_block.Validated.state_hash, merkle_list)
end
