...
let cat = Schema.(obj "Cat"
  ~fields:(fun _ -> [
    field "name"
      ~typ:(non_null string)
      (* ~args:Arg.[] *)
      ~args:Arg.[arg "name_cat" ~typ:(non_null int)]
      ~resolve:(fun _ (cat : cat) i -> cat.name ^ (string_of_int i))
    ;
    field "kittens"
      ~typ:(non_null int)
      ~args:Arg.[]
...
let dog = Schema.(obj "Dog"
  ~fields:(fun _ -> [
    field "name"
      ~typ:(non_null string)
      (* ~args:Arg.[] *)
      ~args:Arg.[arg "name_dog" ~typ:(non_null int)]
      ~resolve:(fun _ (dog : dog) i -> dog.name ^ (string_of_int i))
    ;
    field "puppies"
      ~typ:(non_null int)
      ~args:Arg.[]
...
let named : (unit, [`named]) Schema.abstract_typ = Schema.(interface "Named"
  ~fields:(fun _ -> [
    abstract_field "name"
      ~typ:(non_null string)
      ~args:Arg.[arg "name_adsarg" ~typ:(non_null int)]
      (* ~args:Arg.[] *)
  ])
)
...
let schema = Schema.(schema @@ Fields.to_ocaml_grapql_server_fields
[
    field "pet"
      ~typ:(non_null pet)
      ~args:Arg.[
...
let suite = [
  ("dog as pet", `Quick, fun () ->
    let query = "{ pet(type: \"DOG\") { ... on Dog { name(name_dog: 3) puppies } } }" in
  ...
  ("cat as pet", `Quick, fun () ->
    let query = "{ pet(type: \"CAT\") { ... on Cat { name(name_cat: 3) kittens } } }" in
    test_query query (`Assoc [
      "data", `Assoc [
        "pet", `Assoc [
          "name", `String "Meow3";
  ...
  ("pets", `Quick, fun () ->
    let query = "{ pets { ... on Dog { name(name_dog: 3) puppies } ... on Cat { name(name_cat: 4) kittens } } }" in
    test_query query (`Assoc [
      "data", `Assoc [
        "pets", `List [
          `Assoc [
  ...
  ("named_objects", `Quick, fun () ->
    let query = "{ named_objects { name(name_dog: 4, name_cat:3) ... on Dog { puppies } ... on Cat { kittens } } }" in
    test_query query (`Assoc [
      "data", `Assoc [
        "named_objects", `List [
          `Assoc [
  ...
  ("nested fragments with union and interface", `Quick, fun () ->
      let query = "fragment NamedFragment on Named { ... on Dog { name(name_dog: 3) } ... on Cat { name(name_cat: 4) } } fragment PetFragment on Pet { ... NamedFragment ... on Dog { puppies } ... on Cat { kittens } } { pets { ... PetFragment } }" in
    test_query query (`Assoc [
      "data", `Assoc [
        "pets", `List [
          `Assoc [
