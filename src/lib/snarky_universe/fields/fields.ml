...
module Make_test (F : Intf.Basic) = struct
  let test arg_typ gen_arg sexp_of_arg label unchecked checked =
    ...
    let converted x =
      let (), r =
        run_and_check
          ...
          let x = exists arg_typ ~compute:(As_prover.return x) in
    ...
    test ~trials:50 gen_arg ~f:(fun x ->
        ...
        if not (F.Constant.equal r1 r2) then
          failwithf
            !"%s test failure: %{sexp:arg} -> %{sexp:F.Constant.t} vs \
              %{sexp:F.Constant.t}"
            label x r1 r2 ()
        else () )

  let test1 l f g = test F.typ F.Constant.gen F.Constant.sexp_of_t l f g

  let test2 l f g =
    let open F in
    test (Impl.Typ.( * ) typ typ)
      (Quickcheck.Generator.tuple2 Constant.gen Constant.gen)
      [%sexp_of: Constant.t * Constant.t] l (Tuple2.uncurry f)
      (Tuple2.uncurry g)
end
...
module Make (F : Intf.Basic) = struct
  ...
  let equal x y =
     Boolean.all
      (List.map2_exn (F.to_list x) (F.to_list y) ~f:Field.equal)
  ...
  let%test_unit "add" =
    ...
    M.test2 "add" Constant.( + ) (fun x y -> (x + y))
  ...
  let div_unsafe x y =
    ...
    | Some x, Some y ->
        (constant Constant.(x / y))
    | _, _ ->
      let x_over_y =
        exists typ
          ~compute:
            (fun () -> Constant.( / )
              (read typ x) (read typ y))
      in
      assert_r1cs y x_over_y x;
      x_over_y
          ...
          | _, _ ->
            let res =
              exists typ
                ~compute:
                  (fun () ->
                    Constant.( * ) (read typ x) (read typ y) )
            in
            assert_r1cs x y res ;
            res )
  ...
  let square =
    ...
    | `Define -> (
        fun x ->
          ...
          | Some x ->
              (constant (Constant.square x))
          | None ->
              let res =
                exists typ
                  ~compute:(fun () -> Constant.square(read typ x) )
              in
              assert_square x res ;
              res )
  ...
  let%test_unit "square" =
    let module M = Make_test (F) in
    M.test1 "square" Constant.square square
  ...
  let inv_exn =
    ...
    | `Define -> (
        fun t ->
          ...
          | Some x ->
              (constant (Constant.inv x))
          | None ->
              let res =
                exists typ
                  ~compute:(fun () -> Constant.inv(read typ t) )
              in
              assert_r1cs t res one ;
              res )
end
...
struct
  ...
  let if_ b ~then_ ~else_ =
    (A.map2 then_ else_ ~f:(fun t e -> F.if_ b ~then_:t ~else_:e))
...
module F (Impl : Snarky.Snark_intf.Run) :
...
struct
  module T = struct
    ...
    let negate t = Field.scale t Constant.(negate one)

    let assert_square = `Custom (fun a c -> assert_square a c)

    let ( * ) = `Custom Field.mul

    let square = `Custom Field.square

    let inv_exn = `Custom Field.inv
...
module E2
  ...
  module T = struct
    ...
    module Impl = F.Impl
    open Impl
    module Constant = Snarkette.Fields.Make_fp2 (F.Constant) (Params)
   ...
   a + b sqrt(s). Then all operations are just what follow algebraically. *)
    ...
    let mul_field (a, b) x =
      let a = Base.mul_field a x and b = Base.mul_field b x in
      (a, b)
    ...
    *)
    let square (a, b) =
      let open F in
      let ab = a * b and t = (a + b) * (a + mul_by_non_residue b) in
      (t - ab - mul_by_non_residue ab, ab + ab)

    let assert_square (a, b) (a2, b2) =
      let open F in
      let ab = scale b2  Field.Constant.(one / (of_int 2)) in
      assert_r1cs a b ab ;
      assert_r1cs (a + b)
    ...
    *)
    let ( * ) (a1, b1) (a2, b2) =
      let open F in
      let a = a1 * a2 and b = b1 * b2 and t = (a1 + b1) * (a2 + b2) in
    ...
    let assert_r1cs (a1, b1) (a2, b2) (a3, b3) =
      ...
      assert_r1cs a1 a2 a ;
      assert_r1cs (a1 + b1) (a2 + b2) (b3 + a + b) 
...
module E3
    (F : Intf.S) (Params : sig
        ...
        val frobenius_coeffs_c1 : F.Constant.t array

        val frobenius_coeffs_c2 : F.Constant.t array
  ...
  module T = struct
    module Base = F
    module Constant = Snarkette.Fields.Make_fp3 (F.Constant) (Params)
    module Impl = F.Impl
    open Impl
...
module F3
  ...
  module T = struct
    ...
    let assert_r1cs (a0, a1, a2) (b0, b1, b2) (c0, c1, c2) =
      let open F in
      let v0 = a0 * b0 and v4 = a2 * b2 in
      let beta = Params.non_residue in
      let beta_inv = F.Constant.inv beta in
      assert_r1cs
        (a0 + a1 + a2)
        (b0 + b1 + b2)
        ( c1 + c2 + F.scale c0 beta_inv
        + F.(scale v0 Constant.(one - beta_inv))
        + F.(scale v4 Constant.(one - beta)) ) ;
      assert_r1cs
        (a0 - a1 + a2)
        (b0 - b1 + b2)
        ( c2 - c1
        ...
        let four = Field.Constant.of_int 4 in
        let sixteen = Field.Constant.of_int 16 in
        let eight_beta_inv = Impl.Field.Constant.(mul (of_int 8) beta_inv) in
      assert_r1cs
        ...
        + F.(scale v0 Constant.(one - eight_beta_inv))
        + F.(scale v4 Constant.(sixteen - (beta + beta))) )
...
module Cyclotomic_square = struct
  module Make_F4 (F2 : Intf.S_with_primitive_element) = struct
    let cyclotomic_square (c0, c1) =
      let open F2 in
      let open Impl in
      let b_squared = square (c0 + c1) and a = square c1 in
      let c = b_squared - a in
      let d = mul_by_primitive_element a in
      let e = c - d in
      let f = scale d (Field.Constant.of_int 2) + one in
      let g = e - one in
      (f, g)
  end
...
module F6
    ...
    (Fq2 : Intf.S_with_primitive_element
        ...
        module Params : sig
          val non_residue : Fq.Constant.t

          val frobenius_coeffs_c1 : Fq.Constant.t array

          val frobenius_coeffs_c2 : Fq.Constant.t array
        end
    end) (Params : sig
      val frobenius_coeffs_c1 : Fq.Constant.t array
    end) =
struct
  include E2
            (Fq3)
            (struct
              let non_residue : Fq3.Constant.t =
                Fq.Constant.(zero, one, zero)

              let mul_by_non_residue = Fq3.mul_by_primitive_element
            end)

  let read = Impl.As_prover.read
  ...
  let assert_special_mul ((((a00, a01, a02) as a0), a1) : t)
    ...
    let open Impl in
    let ((v10, v11, v12) as v1) = Fq3.(a1 * b1) in
    let v0 =
      exists Fq3.typ
        ~compute:(Fq3.Constant.(fun () ->  ( * ) (read Fq3.typ a0) (read Fq3.typ b0) ) )
      (* v0
  ...
  let special_div_unsafe a b =
    ...
    let result =
      exists typ
        ~compute:As_prover.(map2 ~f:Constant.( / ) (read typ a) (read typ b))
    in
    (* result * b = a *)
