...
let generate_test_ledger =
  Command.async ~summary:"Generate a ledger for testing"
    (let open Command.Let_syntax in
     ...
     @@ fun () ->
     let min_balance =
       Option.value ~default:Currency.Balance.zero min_balance
     in
     let max_balance =
     ...
     let ledger =
       ...
       |> Sequence.map ~f:(fun balance ->
           ...
           let pk =
             ...
             |> Public_key.Compressed.to_base58_check
           in
           (* Delegate each account to itself. An account with no delegate is
              unstaked, so without this a generated ledger has no stake at all
              and a network started from it never produces a block. *)
