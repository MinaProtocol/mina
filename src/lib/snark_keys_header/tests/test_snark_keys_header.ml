...
module Test_helpers = struct
  ...
  let valid_header_string = Yojson.Safe.to_string (Header.to_yojson valid_header)
...
let test_parse_without_prefix () =
  ...
  Alcotest.(check bool)
    "Should fail to parse without prefix"
    true
...
let test_parse_with_incorrect_prefix () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should fail to parse with incorrect prefix"
    true
...
let test_parse_with_matching_length_prefix () =
  ...
  Alcotest.(check bool)
    "Should fail to parse with matching-length but different prefix"
    true
    (Header.parse_lexbuf (lexbuf_from_string (fake_prefix ^ valid_header_string))
    |> Or_error.is_error)
...
let test_parse_with_partial_matching_prefix () =
  ...
  Alcotest.(check bool)
    "Should fail to parse with partial matching prefix"
    true
    (Header.parse_lexbuf (lexbuf_from_string (partial_prefix ^ valid_header_string))
    |> Or_error.is_error)

let test_parse_with_short_file () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should fail to parse with short file"
    true
...
let test_parse_with_prefix_only () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should fail to parse with prefix only"
    true
...
let test_parse_valid_header_with_prefix () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should successfully parse valid header with prefix"
    true
...
let test_offset_parse_without_prefix () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should fail to parse without prefix (offset)"
    true
...
let test_offset_parse_with_incorrect_prefix () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should fail to parse with incorrect prefix (offset)"
    true
    (Header.parse_lexbuf (lexbuf_from_string_offset ("BLAH" ^ valid_header_string))
    |> Or_error.is_error)
...
let test_offset_parse_with_matching_length_prefix () =
  ...
  Alcotest.(check bool)
    ...
    true
    (Header.parse_lexbuf (lexbuf_from_string_offset (fake_prefix ^ valid_header_string))
    |> Or_error.is_error)
...
let test_offset_parse_with_partial_matching_prefix () =
  ...
  let partial_prefix = String.sub prefix ~pos:0 ~len:(prefix_len - 1) ^ " " in
  Alcotest.(check bool)
    "Should fail to parse with partial matching prefix (offset)"
    true
    (Header.parse_lexbuf (lexbuf_from_string_offset (partial_prefix ^ valid_header_string))
    |> Or_error.is_error)

let test_offset_parse_with_short_file () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should fail to parse with short file (offset)"
    true
...
let test_offset_parse_with_prefix_only () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should fail to parse with prefix only (offset)"
    true
...
let test_offset_parse_valid_header_with_prefix () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should successfully parse valid header with prefix (offset)"
    true
...
let test_offset_parse_valid_header_with_prefix_and_data () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should successfully parse valid header with prefix and additional data (offset)"
    true
    (Header.parse_lexbuf
...
let test_refill_parse_without_prefix () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should fail to parse without prefix (refill)"
    true
...
let test_refill_parse_with_incorrect_prefix () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should fail to parse with incorrect prefix (refill)"
    true
...
let test_refill_parse_with_matching_length_prefix () =
  ...
  Alcotest.(check bool)
    ...
    true
    (Header.parse_lexbuf (lexbuf_with_refill (fake_prefix ^ valid_header_string))
    |> Or_error.is_error)
...
let test_refill_parse_with_partial_matching_prefix () =
  ...
  let partial_prefix = String.sub prefix ~pos:0 ~len:(prefix_len - 1) ^ " " in
  Alcotest.(check bool)
    "Should fail to parse with partial matching prefix (refill)"
    true
    (Header.parse_lexbuf (lexbuf_with_refill (partial_prefix ^ valid_header_string))
    |> Or_error.is_error)

let test_refill_parse_with_short_file () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should fail to parse with short file (refill)"
    true
...
let test_refill_parse_with_prefix_only () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should fail to parse with prefix only (refill)"
    true
...
let test_refill_parse_valid_header_with_prefix () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should successfully parse valid header with prefix (refill)"
    true
...
let test_refill_parse_valid_header_with_prefix_and_data () =
  let open Test_helpers in
  Alcotest.(check bool)
    "Should successfully parse valid header with prefix and additional data (refill)"
    true
    (Header.parse_lexbuf
...
let () =
  Alcotest.run "Snark_keys_header"
    [ ( "Standard parsing tests"
      , [ Alcotest.test_case "Parse without prefix" `Quick test_parse_without_prefix
        ; Alcotest.test_case "Parse with incorrect prefix" `Quick test_parse_with_incorrect_prefix
        ; Alcotest.test_case "Parse with matching-length prefix" `Quick test_parse_with_matching_length_prefix
        ; Alcotest.test_case "Parse with partial matching prefix" `Quick test_parse_with_partial_matching_prefix
        ; Alcotest.test_case "Parse with short file" `Quick test_parse_with_short_file
        ; Alcotest.test_case "Parse with prefix only" `Quick test_parse_with_prefix_only
        ; Alcotest.test_case "Parse valid header with prefix" `Quick test_parse_valid_header_with_prefix
        ; Alcotest.test_case "Parse valid header with prefix and data" `Quick test_parse_valid_header_with_prefix_and_data
        ] )
    ; ( "Offset parsing tests"
      , [ Alcotest.test_case "Offset: Parse without prefix" `Quick test_offset_parse_without_prefix
        ; Alcotest.test_case "Offset: Parse with incorrect prefix" `Quick test_offset_parse_with_incorrect_prefix
        ; Alcotest.test_case "Offset: Parse with matching-length prefix" `Quick test_offset_parse_with_matching_length_prefix
        ; Alcotest.test_case "Offset: Parse with partial matching prefix" `Quick test_offset_parse_with_partial_matching_prefix
        ; Alcotest.test_case "Offset: Parse with short file" `Quick test_offset_parse_with_short_file
        ; Alcotest.test_case "Offset: Parse with prefix only" `Quick test_offset_parse_with_prefix_only
        ; Alcotest.test_case "Offset: Parse valid header with prefix" `Quick test_offset_parse_valid_header_with_prefix
        ; Alcotest.test_case "Offset: Parse valid header with prefix and data" `Quick test_offset_parse_valid_header_with_prefix_and_data
        ] )
    ; ( "Refill parsing tests"
      , [ Alcotest.test_case "Refill: Parse without prefix" `Quick test_refill_parse_without_prefix
        ; Alcotest.test_case "Refill: Parse with incorrect prefix" `Quick test_refill_parse_with_incorrect_prefix
        ; Alcotest.test_case "Refill: Parse with matching-length prefix" `Quick test_refill_parse_with_matching_length_prefix
        ; Alcotest.test_case "Refill: Parse with partial matching prefix" `Quick test_refill_parse_with_partial_matching_prefix
        ; Alcotest.test_case "Refill: Parse with short file" `Quick test_refill_parse_with_short_file
        ; Alcotest.test_case "Refill: Parse with prefix only" `Quick test_refill_parse_with_prefix_only
        ; Alcotest.test_case "Refill: Parse valid header with prefix" `Quick test_refill_parse_valid_header_with_prefix
        ; Alcotest.test_case "Refill: Parse valid header with prefix and data" `Quick test_refill_parse_valid_header_with_prefix_and_data
        ] )
    ]