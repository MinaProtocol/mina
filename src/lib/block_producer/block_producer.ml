...
let run ~context:(module Context : CONTEXT) ~vrf_evaluator ~prover ~verifier
  ...
  let open Context in
  O1trace.sync_thread "produce_blocks"
  @@ fun () ->
  ...
  let produce intr_module (scheduled_time, block_data, winner_pubkey) =
    ...
    | Some frontier -> (
        ...
        let%bind.Intr next_state_opt =
          generate_next_state ~intr_module ~constraint_constants ~scheduled_time
               ...
               | Completed ->
                   let epoch_end_time =
                     Consensus.Hooks.epoch_end_time
                       ~constants:consensus_constants epoch_data_for_vrf.epoch
                   in
           ...
           | Some slot_won -> (
               ...
               then (
                 ...
                 Singleton_supervisor.dispatch production_supervisor
                   ...
                   ~finalizer:(check_next_block_timing new_global_slot i') )
               else
                 match
                 ...
                 | None ->
                     [%log warn]
                       "Skipping block production for global slot $slot_won \
                        because it has passed. Current global slot is \
                        $curr_slot"
                       ~metadata:
                         [ ( "slot_won"
                           , Mina_numbers.Global_slot_since_hard_fork.to_yojson
                 ...
                 | Some slot_diff ->
                     [%log info] "Producing a block in $slots slots"
                       ~metadata:
                         [ ( "slots"
                           , Mina_numbers.Global_slot_span.to_yojson slot_diff
                           )
                         ] ;
                     let time =
                       Consensus.Data.Consensus_time.(
  ...
  if Block_time.( >= ) now genesis_state_timestamp then start ()
    ...
    [%log warn]
      ~metadata:
        [ ( "time_till_genesis"
          , `Int (Int64.to_int_exn (Block_time.Span.to_ms time_till_genesis)) )
        ]
      "Node started before genesis: waiting $time_till_genesis milliseconds \
       before starting block producer" ;
    ignore
      ( Block_time.Timeout.create time_controller time_till_genesis ~f:start
