  ...
  O1trace.sync_thread "produce_blocks" (fun () ->
      let%bind () =
        if Mina_numbers.Length.(i' > i) then
          Vrf_evaluation_state.update_epoch_data ~vrf_evaluator
            ~epoch_data_for_vrf ~logger vrf_evaluation_state ~vrf_poll_interval
        else Deferred.unit
      in
      let%bind () =
      ...
      | Some slot_won -> (
          ...
          then (
            ...
            | None ->
                [%log warn]
                  "Skipping block production for global slot $slot_won because \
                   it has passed. Current global slot is $curr_slot"
                  ~metadata:
                    [ ( "slot_won"
                      , Mina_numbers.Global_slot_since_hard_fork.to_yojson
                          winning_global_slot )
                   ...
                   *)
                   let scheduled_genesis_time =
                     time_of_ms
                       Int64.(
                         time
                         - of_int constraint_constants.block_window_duration_ms)
                   in
                   let span_till_time =
