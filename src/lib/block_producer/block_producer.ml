            ...
            log_if_slot_diff_is_less_than ~diff_limit:480 ~every:60
              ~message:
                "Block producer will begin producing only empty blocks after \
                 $slot_diff slots"
              slot_tx_end ;
            let next_check_now = check_next_block_timing new_global_slot i' in
              ...
              ( iteration
                  ~schedule_next_check:
                    (Fn.compose Deferred.return
                       (Singleton_scheduler.schedule scheduler ~f:next_check_now) )
                  ...
                  ~schedule_dispatch:(fun (time, data, winner) ->
                    ...
                    Deferred.unit )
                  ~next_check_now:(Fn.compose Deferred.return next_check_now)
                  ~genesis_breadcrumb
                  ~context:(module Context)
