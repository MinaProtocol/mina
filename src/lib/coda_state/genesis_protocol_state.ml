...
let create_with_custom_ledger ~genesis_consensus_state ~genesis_ledger
  ...
  let negative_one_protocol_state_hash =
    Protocol_state.(hash (negative_one ~genesis_ledger ~genesis_constants))
  in
  ...
  in
  let coda_constants =
    Coda_constants_checked.of_coda_constants
      (Coda_constants.create_t genesis_constants)
  in
  let state =
    Protocol_state.create_value
      ...
      ~consensus_state:genesis_consensus_state ~coda_constants
  in
  With_hash.of_data ~hash_data:Protocol_state.hash state

let t ~genesis_ledger ~genesis_constants =
  ...
  let genesis_consensus_state =
    Consensus.Data.Consensus_state.create_genesis
      ~negative_one_protocol_state_hash ~genesis_ledger ~genesis_constants
  in
  let coda_constants =
    Coda_constants_checked.of_coda_constants
      (Coda_constants.create_t genesis_constants)
