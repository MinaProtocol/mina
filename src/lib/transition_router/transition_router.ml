...
let set_bootstrap_phase ~controller_type root_state bootstrap_controller_writer
    =
...
let run ~logger ~trust_system ~network ~time_controller
  ...
  let clean_transition_frontier_controller_and_start_bootstrap ~controller_type
    ...
    set_bootstrap_phase ~controller_type root_state bootstrap_controller_writer ;
    Strict_pipe.Writer.write bootstrap_controller_writer
      (`Transition _incoming_transition, `Time_received (to_unix_timestamp _tm)) ;
