...
(* Parts of this implementation is still untouched from the Kubernetes implementation. This is done
  just to make the compiler happy as work is done on the local engine.
*)
...
module Node = struct
  ...
  let run_in_postgresql_container node ~cmd =
    ...
    let kubectl_cmd =
      Printf.sprintf "%s -c %s exec -i %s-0 -- %s" base_kube_cmd
        node.service_id node.service_id cmd
    in
  ...
  let get_logs_in_container node =
    ...
    let base_kube_cmd = "kubectl " ^ String.concat ~sep:" " base_args in
    let pod_cmd =
      sprintf "%s get pod -l \"app=%s\" -o name" base_kube_cmd node.service_id
    in
    let%bind cwd = Unix.getcwd () in
    let%bind pod = Util.run_cmd_exn cwd "sh" ["-c"; pod_cmd] in
    let kubectl_cmd =
      Printf.sprintf "%s logs -c %s -n %s %s" base_kube_cmd node.service_id
        node.swarm_name pod
    in
    Util.run_cmd_exn cwd "sh" ["-c"; kubectl_cmd]
  ...
  module Graphql = struct
    let ingress_uri node =
      let host = Printf.sprintf "%s.graphql.test.o1test.net" node.swarm_name in
      let path = Printf.sprintf "/%s/graphql" node.service_id in
  ...
  let get_peer_id ~logger t =
    ...
    [%log info] "Getting node's peer_id, and the peer_ids of node's peers"
      ~metadata:
        [("namespace", `String t.swarm_name); ("pod_id", `String t.service_id)] ;
  ...
  let get_balance ~logger t ~account_id =
    ...
    [%log info] "Getting account balance"
      ~metadata:
        [ ("namespace", `String t.swarm_name)
        ; ("pod_id", `String t.service_id)
  ...
  let rec poll n =
    ...
    let bad_service_statuses =
      List.filter pod_statuses ~f:(fun (_, status) ->
          ...
          assert (List.length parts = 2) ;
          let num, denom = (List.nth_exn parts 0, List.nth_exn parts 1) in
          String.strip num <> String.strip denom )
    in
