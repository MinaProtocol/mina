...
module Network_manager = struct
  ...
  let generate_docker_stack_file ~logger ~docker_dir ~docker_compose_file_path
    ...
    let%bind () =
      if%bind Mina_stdlib.File_system.dir_exists docker_dir then (
        [%log info] "Old docker stack directory found; removing to start clean" ;
        Mina_stdlib.File_system.remove_dir docker_dir )
      else return ()
    in
  ...
  let cleanup t =
    ...
    let%bind () = Mina_stdlib.File_system.remove_dir t.docker_dir in
    Deferred.unit
