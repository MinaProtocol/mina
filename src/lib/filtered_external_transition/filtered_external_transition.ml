...
let validate_transactions block =
  let consensus_state = block |> Mina_block.header |> Mina_block.Header.protocol_state |> Mina_state.Protocol_state.consensus_state in
  let coinbase_receiver = Consensus_state.coinbase_receiver consensus_state in
  let supercharge_coinbase = Consensus_state.supercharge_coinbase consensus_state in
  let staged_ledger_diff = block |> Mina_block.body |> Mina_block.Body.staged_ledger_diff in
...
let filter_protocol_state protocol_state : Protocol_state.t =
  { previous_state_hash= Mina_state.Protocol_state.previous_state_hash protocol_state
  ; blockchain_state= Mina_state.Protocol_state.blockchain_state protocol_state
  ; consensus_state= Mina_state.Protocol_state.consensus_state protocol_state }

let filter_block block tracked_participants (calculated_transactions : Transaction.t With_status.t list) =
  let header = Mina_block.header block in
  let staged_ledger_diff = block |> Mina_block.body |> Mina_block.Body.staged_ledger_diff in
  let consensus_state = header |> Mina_block.Header.protocol_state |> Mina_state.Protocol_state.consensus_state in
  let protocol_state = header |> Mina_block.Header.protocol_state |> filter_protocol_state in
  ...
  let next_available_token = protocol_state.blockchain_state.snarked_next_available_token in
