...
module Kvdb = struct
      type t =
          { mutable lmdb : (Bigstring.t,Bigstring.t,[ `Uni ]) Lmdb.Map.t
          ; path : string
          }

      let t_of_sexp _sexp = failwith "Can't deserialize a database handle?"
      let sexp_of_t _t = failwith "TODO serialize?"
