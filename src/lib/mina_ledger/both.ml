...
struct
        ...
        fun cursor ->
          ...
          | k_first, _ ->
              ...
              let rec check_dups k m =
                ...
                | ((exception Not_found) [@alert "-deprecated"]) ->
                    [%log info] "No dups in lmdb"
                | k_next ->
                    if k = k_next then (
                      [%log fatal] "Found a duplicate $label"
                        ~metadata:[ ("label", `String label) ] ;
                      exit 1 ) ;
                    if k_next = k_first then (
                      [%log fatal] "Key looped to first" ;
                      exit 1 ) ;
                    check_dups k_next m'
              in
              check_dups k_first Maps.empty)

  let validate_order ((l, r) : t) (label : string) =
    let label () =
        ...
        R.fold_until r
          ...
          ~finish:(fun lmdb_state ->
            ...
            | Some (key, data) ->
                ...
                [%log fatal]
                  "Validation error LMDB had extra data $key $data , $rd"
                  ~metadata:
                    [ ("key", log_bs key)
                    ; ("data", log_bs data)
                    ; ("rd", log_opt_bs rd)
  ...
  let validate ((l, r) : t) (label : string) =
    ...
    if l_len <> r_len then (
      [%log fatal] "database sizes differ $l $r"
        ~metadata:[ ("l", `Int l_len); ("r", `Int r_len) ] ;
      exit 1 ) ;
    validate_order (l, r) label
