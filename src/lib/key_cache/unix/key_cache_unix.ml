...
module Async : S with module M := Async_kernel.Deferred.Or_error = struct
  open Async_kernel
  ...
  let on_disk to_string read write prefix =
    ...
    let read k =
      let p = path k in
      match%bind Async.Sys.file_exists p with
      | `No | `Unknown ->
    ...
    in
    let write key v =
      match%bind Async.Sys.is_directory prefix with
      | `No | `Unknown ->
          return
  ...
  module Disk_storable = struct
    ...
    let of_binable (type t) to_string (module B : Binable.S with type t = t) =
      let read _ ~path = Async.Reader.load_bin_prot path B.bin_reader_t in
      let write _ t path =
        Deferred.map
          (Async.Writer.save_bin_prot path B.bin_writer_t t)
          ~f:Or_error.return
      in
      { to_string; read; write }
  ...
  let read spec { Disk_storable.to_string; read = r; write = w } k =
    Deferred.Or_error.find_map_ok spec ~f:(fun s ->
        ...
        | S3 { bucket_prefix; install_path } ->
            let%bind.Deferred () = Async.Unix.mkdir ~p:() install_path in
            let%map res =
    ...
    let%map errs =
      Deferred.List.filter_map spec ~f:(fun s ->
          let res =
            ...
            | Spec.On_disk { directory; should_write } ->
                if should_write then
                  let%bind () = Async.Unix.mkdir ~p:() directory in
                  (on_disk to_string r w directory).write k v
