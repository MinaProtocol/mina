...
let run ~logger ~trust_system ~verifier ~network ~time_controller
  ...
  let valid_transition_reader, valid_transition_writer =
    Strict_pipe.create ~logging_enabled:true ~name:"valid transitions"
  ...
  let primary_transition_reader, primary_transition_writer =
    Strict_pipe.create ~logging_enabled:true ~name:"primary transitions"
  ...
  let processed_transition_reader, processed_transition_writer =
    Strict_pipe.create ~logging_enabled:true ~name:"processed transitions"
      (Buffered (`Capacity 30, `Overflow Crash))
  in
  let catchup_job_reader, catchup_job_writer =
    Strict_pipe.create ~logging_enabled:true ~name:"catchup jobs"
      (Buffered (`Capacity 30, `Overflow Crash))
  in
  let catchup_breadcrumbs_reader, catchup_breadcrumbs_writer =
    Strict_pipe.create ~logging_enabled:true ~name:"catchup breadcrumbs"
      (Buffered (`Capacity 30, `Overflow Crash))
  in
