...
*)
  let rec upon_f ~context ~transition_states ~state_hashes (actions, res) =
    ...
    let top_state_hash () =
      List.find
        (List.rev @@ Non_empty_list.to_list state_hashes)
        ~f:(fun state_hash ->
          Option.is_some
          @@ let%bind.Option st =
               Transition_states.find transition_states state_hash
    ...
    | Result.Ok (Result.Ok lst) -> (
        match
          List.fold2 ~init:([], false)
            (Non_empty_list.to_list state_hashes)
            lst ~f
        with
        ...
        Option.iter (top_state_hash ())
          ~f:
            (actions.Misc.mark_invalid
               ~error:(Error.of_string @@ "invalid " ^ F.data_name) )
  ...
  and start_impl ~context ~actions ~transition_states =
    ...
    Fn.compose (Non_empty_list.iter ~f) (F.split_to_batches ~context)
