...
let test_query schema ctx ?variables ?operation_name query expected =
  ...
  | Ok doc ->
      let result = match Schema.execute schema ctx ?variables ?operation_name doc with
      ...
      if result <> expected then
        failwith @@ Format.asprintf "@[result:@;%a@;expected:@;%a@]" Yojson.Basic.pp result Yojson.Basic.pp expected
