...
use mina_curves::pasta::fields::fp::Fp;
...
pub fn caml_pasta_fp_is_square(x: ocaml::Pointer<CamlFp>) -> bool {
    let s = x.as_ref().0.pow(Fp::MODULUS_MINUS_ONE_DIV_TWO);
    s.is_zero() || s.is_one()
}
...
pub fn caml_pasta_fp_of_bigint(x: CamlBigInteger256) -> Result<CamlFp, ocaml::Error> {
    Fp::from_bigint(x.0).map(CamlFp).ok_or_else(|| {
        let err = format!(
...
pub fn caml_pasta_fp_two_adic_root_of_unity() -> CamlFp {
    let res: Fp = Fp::TWO_ADIC_ROOT_OF_UNITY;
    CamlFp(res)
}
