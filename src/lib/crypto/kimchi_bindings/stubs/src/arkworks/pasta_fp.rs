...
pub fn caml_pasta_fp_of_string(s: CamlBytesString) -> Result<CamlFp, ocaml::Error> {
    ...
    CamlFp::try_from(bigint).map_err(|_| ocaml::Error::Message("caml_pasta_fp_of_string"))
}
...
#[ocaml_gen::func]
#[ocaml::func]
pub fn caml_pasta_fp_to_bigint(x: ocaml::Pointer<CamlFp>) -> BigInteger256 {
    x.as_ref().into_repr().into()
}

#[ocaml_gen::func]
#[ocaml::func]
pub fn caml_pasta_fp_of_bigint(x: BigInteger256) -> Result<CamlFp, ocaml::Error> {
    Fp::from_repr(x.0).map(CamlFp::from).ok_or_else(|| {
        let err = format!(
