...
use crate::bn254_fp_plonk_index::CamlBn254FpPlonkIndexPtr;
use crate::pairing_srs::bn254_fp::CamlBn254FpSrs;
...
pub type CamlBn254FpPlonkVerifierIndex =
    CamlPlonkVerifierIndex<CamlBn254Fp, CamlBn254FpSrs, CamlPolyComm<CamlGBn254>>;

impl From<VerifierIndex<Bn254, PairingProof<Bn<Parameters>>>> for CamlBn254FpPlonkVerifierIndex {
    fn from(vi: VerifierIndex<Bn254, PairingProof<Bn<Parameters>>>) -> Self {
        Self {
            domain: CamlPlonkDomain {
                log_size_of_group: vi.domain.log_size_of_group as isize,
                group_gen: CamlBn254Fp(vi.domain.group_gen),
            },
            max_poly_size: vi.max_poly_size as isize,
            public: vi.public as isize,
            prev_challenges: vi.prev_challenges as isize,
            srs: CamlBn254FpSrs(vi.srs.clone()),
...
impl From<CamlBn254FpPlonkVerifierIndex> for VerifierIndex<Bn254, PairingProof<Bn<Parameters>>> {
    fn from(index: CamlBn254FpPlonkVerifierIndex) -> Self {
        ...
        let coefficients_comm: Vec<PolyComm<Bn254>> =
        ...
        let sigma_comm: Vec<PolyComm<Bn254>> = evals.sigma_comm.iter().map(Into::into).collect();
...
pub fn read_raw(
    ...
    srs: CamlBn254FpSrs,
    path: String,
) -> Result<VerifierIndex<Bn254, PairingProof<Bn<Parameters>>>, ocaml::Error> {
    let path = Path::new(&path);
    let &endo_q = Bn254::other_curve_endo();
    VerifierIndex::<Bn254, PairingProof<Bn<Parameters>>>::from_file(
        srs.0,
        path,
...
pub fn caml_bn254_fp_plonk_verifier_index_read(
    ...
    srs: CamlBn254FpSrs,
    path: String,
) -> Result<CamlBn254FpPlonkVerifierIndex, ocaml::Error> {
...
pub fn caml_bn254_fp_plonk_verifier_index_write(
    append: Option<bool>,
    index: CamlBn254FpPlonkVerifierIndex,
    path: String,
) -> Result<(), ocaml::Error> {
    let index: VerifierIndex<Bn254, PairingProof<Bn<Parameters>>> = index.into();
    let path = Path::new(&path);
    index.to_file(path, append).map_err(|_e| {
        ocaml::Error::invalid_argument("caml_bn254_fp_plonk_verifier_index_raw_read")
            .err()
            .unwrap()
    })
}
...
pub fn caml_bn254_fp_plonk_verifier_index_create(
    index: CamlBn254FpPlonkIndexPtr,
) -> CamlBn254FpPlonkVerifierIndex {
    {
...
#[ocaml_gen::func]
#[ocaml::func]
pub fn caml_bn254_fp_plonk_verifier_index_shifts(log2_size: ocaml::Int) -> Vec<CamlBn254Fp> {
...
#[ocaml_gen::func]
#[ocaml::func]
pub fn caml_bn254_fp_plonk_verifier_index_dummy() -> CamlBn254FpPlonkVerifierIndex {
    fn comm() -> CamlPolyComm<CamlGBn254> {
        let g: CamlGBn254 = Bn254::prime_subgroup_generator().into();
        CamlPolyComm {
            shifted: Some(g),
            unshifted: vec![g, g, g],
        }
    }
    fn vec_comm(num: usize) -> Vec<CamlPolyComm<CamlGBn254>> {
    ...
    CamlPlonkVerifierIndex {
        ...
        public: 0,
        prev_challenges: 0,
        srs: CamlBn254FpSrs::new(PairingSRS::create(x, 0)),
        evals: CamlPlonkVerificationEvals {
...
pub fn caml_bn254_fp_plonk_verifier_index_deep_copy(
    x: CamlBn254FpPlonkVerifierIndex,
) -> CamlBn254FpPlonkVerifierIndex {
    x
}
