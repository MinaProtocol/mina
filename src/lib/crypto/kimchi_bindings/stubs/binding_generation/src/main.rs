...
fn generate_bindings(mut w: impl std::io::Write) {
    ...
    decl_module!(w, env, "Foundations", {
        ...
        decl_module!(w, env, "Affine", {
            decl_type!(w, env, CamlGroupAffine<T1> => "t");
        });

        decl_module!(w, env, "Projective", {
            decl_type!(w, env, CamlGroupProjectivePallas => "pallas");
            decl_type!(w, env, CamlGroupProjectiveVesta => "vesta");
        });
    });
