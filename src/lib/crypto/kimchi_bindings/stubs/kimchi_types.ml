(* This file is generated automatically with ocaml_gen. *)
  ...
  type nonrec ('caml_g) prover_commitments = { w_comm: (('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm * ('caml_g) poly_comm); z_comm: ('caml_g) poly_comm; t_comm: ('caml_g) poly_comm; lookup: (('caml_g) lookup_commitments) option }
  type nonrec ('caml_g, 'caml_f) prover_proof = { commitments: ('caml_g) prover_commitments; proof: ('caml_g, 'caml_f) opening_proof; evals: ('caml_f) proof_evaluations; ft_eval1: 'caml_f; public: ('caml_f) array; prev_challenges: (('caml_g, 'caml_f) recursion_challenge) array }
  type nonrec ('caml_g, 'caml_f) proof_with_public = { public_evals: ((('caml_f) array) point_evaluations) option; proof: ('caml_g, 'caml_f) prover_proof }
  type nonrec wire = { row: int; col: int }
  type nonrec gate_type = Zero | Generic | Poseidon | CompleteAdd | VarBaseMul | EndoMul | EndoMulScalar | Lookup | CairoClaim | CairoInstruction | CairoFlags | CairoTransition | RangeCheck0 | RangeCheck1 | ForeignFieldAdd | ForeignFieldMul | Xor16 | Rot64
  type nonrec lookup_pattern = Xor | Lookup | RangeCheck | ForeignFieldMul
  type nonrec lookup_patterns = { xor: bool; lookup: bool; range_check: bool; foreign_field_mul: bool }
  type nonrec lookup_features = { patterns: lookup_patterns; joint_lookup_used: bool; uses_runtime_tables: bool }
  type nonrec feature_flag = RangeCheck0 | RangeCheck1 | ForeignFieldAdd | ForeignFieldMul | Xor | Rot | LookupTables | RuntimeLookupTables | LookupPattern of lookup_pattern | TableWidth of int | LookupsPerRow of int
  type nonrec ('f) circuit_gate = { typ: gate_type; wires: wire * wire * wire * wire * wire * wire * wire; coeffs: ('f) array }
  type nonrec curr_or_next = Curr | Next
  type nonrec ('f) oracles = { o: ('f) random_oracles; p_eval: ('f * 'f); opening_prechallenges: ('f) array; digest_before_evaluations: 'f }

  module VerifierIndex = struct 

    module Lookup = struct 
      type nonrec lookups_used = Single | Joint
      type nonrec lookup_info = { max_per_row: int; max_joint_size: int; features: lookup_features }
      type nonrec ('t) lookup_selectors = { lookup: ('t) option; xor: ('t) option; range_check: ('t) option; ffmul: ('t) option }
      type nonrec ('poly_comm) t = { joint_lookup_used: bool; lookup_table: ('poly_comm) array; lookup_selectors: ('poly_comm) lookup_selectors; table_ids: ('poly_comm) option; lookup_info: lookup_info; runtime_tables_selector: ('poly_comm) option }
    end

    type nonrec ('fr) domain = { log_size_of_group: int; group_gen: 'fr }
    type nonrec ('poly_comm) verification_evals = { sigma_comm: ('poly_comm) array; coefficients_comm: ('poly_comm) array; generic_comm: 'poly_comm; psm_comm: 'poly_comm; complete_add_comm: 'poly_comm; mul_comm: 'poly_comm; emul_comm: 'poly_comm; endomul_scalar_comm: 'poly_comm; xor_comm: ('poly_comm) option; range_check0_comm: ('poly_comm) option; range_check1_comm: ('poly_comm) option; foreign_field_add_comm: ('poly_comm) option; foreign_field_mul_comm: ('poly_comm) option; rot_comm: ('poly_comm) option }
    type nonrec ('fr, 'srs, 'poly_comm) verifier_index = { domain: ('fr) domain; max_poly_size: int; public: int; prev_challenges: int; srs: 'srs; evals: ('poly_comm) verification_evals; shifts: ('fr) array; lookup_index: (('poly_comm) Lookup.t) option; zk_rows: int }
  end
