...
impl<T> std::iter::IntoIterator for FlatVector<T> {
...
impl<'a, T> std::iter::IntoIterator for &'a FlatVector<T> {
...
impl<T> std::iter::FromIterator<T> for FlatVector<T> {
    fn from_iter<I>(iter: I) -> FlatVector<T>
    where
        I: IntoIterator<Item = T>,
    {
        FlatVector(std::iter::FromIterator::from_iter(iter))
    }
}

impl<T> std::default::Default for FlatVector<T> {
    fn default() -> Self {
        FlatVector(std::default::Default::default())
    }
}

impl<T> std::iter::Extend<T> for FlatVector<T> {
