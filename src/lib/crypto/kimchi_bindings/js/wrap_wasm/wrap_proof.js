...
function wrap_wasm_proof(plonk_wasm,plonk_intf){
  ...
  plonk_intf.caml_pasta_fp_plonk_proof_verify = function (index, proof) {
  ...
  plonk_intf.caml_pasta_fp_plonk_proof_batch_verify = function (indexes, proofs) {
  ...
  plonk_intf.caml_pasta_fp_plonk_proof_dummy = function () {
    return tsRustConversion.fp.proofFromRust(
      plonk_wasm.caml_pasta_fp_plonk_proof_dummy()
    );
  };

  plonk_intf. caml_pasta_fp_plonk_proof_deep_copy = function (proof) {
    return tsRustConversion.fp.proofFromRust(
  ...
  plonk_intf.caml_pasta_fq_plonk_proof_create = function (
    index,
    witness_cols,
    caml_runtime_tables,
    prev_challenges,
  ...
  plonk_intf.caml_pasta_fq_plonk_proof_verify = function (index, proof) {
  ...
  plonk_intf.caml_pasta_fq_plonk_proof_batch_verify = function (indexes, proofs) {
    indexes = tsRustConversion.mapMlArrayToRustVector(
      indexes,
  ...
  plonk_intf.caml_pasta_fq_plonk_proof_dummy = function () {
