...
let verify_heterogenous (ts : Instance.t list) =
  ...
  in
  let lr_lengths_ok =
    let rounds = Nat.to_int Backend.Tock.Rounds.n in
    List.for_all ts ~f:(fun (T (_, _, _, _, _, T { proof; _ })) ->
        Array.length proof.bulletproof.lr = rounds )
  in
  if not lr_lengths_ok then (
    (* A wrap proof's bulletproof lr must match the wrap round count. *)
    check (lazy "bulletproof lr length", false) ;
    ...
    let _computed_bp_chals, deferred_values =
      List.map ts
        ~f:(fun
          ...
          Timer.clock __LOC__ ;
          let deferred_values =
            { deferred_values with bulletproof_challenges }
          in
          let () =
