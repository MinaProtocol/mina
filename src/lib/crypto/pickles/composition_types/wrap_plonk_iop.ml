...
module Minimal = struct
  ...
  let map_challenges (t : _ Poly.t) ~f ~scalar : _ Poly.t =
    { t with
  ...
  module Constant = struct
    ...
    let to_stable
        ({ alpha; beta; gamma; zeta; joint_combiner; feature_flags } : t) :
  ...
  (** Step-circuit (Tick) instantiation of {!In_circuit.t}: the
      shape used when the Step verifier constructs Plonk-IOP
      challenges in-circuit. {!to_in_circuit} bridges to the
      polymorphic {!In_circuit.t}. *)
  module Step = struct
    type t =
  ...
  (** Wrap-circuit (Tock) instantiation of {!In_circuit.t}.
      ...
      Plonk-IOP challenges in-circuit. Fresh record; see {!Step}
      for the rationale. *)
  module Wrap = struct
    type t =
  ...
  (** Endo'd Tick-field instantiation of {!Stable.V1.t}.
      ...
      {!of_stable} bridge to the wire-format polymorphic
      skeleton. *)
  module Tick = struct
    type t =
    ...
    (** [Poly.t] specialised to [Tick] field elements — the
        result type of {!to_stable}, also the canonical shape
        consumed by [Wrap.combined_inner_product]. *)
    ...
    let of_stable
        ({ alpha; beta; gamma; zeta; joint_combiner; feature_flags } : poly_t) :
        t =
      { alpha; beta; gamma; zeta; joint_combiner; feature_flags }
  end

  (** Endo'd Tock-field counterpart of {!Tick}: the shape used
      when the prover prepares a step proof's deferred values
      for verification (e.g. dummy unfinalized values).
      {!to_stable} / {!of_stable} bridge to the wire-format
  ...
  module Tock = struct
    ...
    let of_stable
        ({ alpha; beta; gamma; zeta; joint_combiner; feature_flags } :
          (Backend.Tock.Field.t, Backend.Tock.Field.t, bool) Poly.t ) : t =
      { alpha; beta; gamma; zeta; joint_combiner; feature_flags }
  end
end
...
module In_circuit = struct
  ...
  *)
  ...
  let typ (type fp) ~dummy_scalar_challenge ~challenge ~scalar_challenge ~bool
      ~feature_flags:
  ...
  type ('a, 'b, 'c, 'd, 'e, 'f) in_circuit_t = ('a, 'b, 'c, 'd, 'e, 'f) t

  (** Wire-form (out-of-circuit) instantiation of {!t}.

      ['fp] left polymorphic because the constant-side scalars
      are passed around in either [_ Shifted_value.Type1.t] or
      [_ Shifted_value.Type2.t] depending on which curve's
      deferred values they describe. The ['fp_opt] phantom
      from the polymorphic {!t} is dropped (no field uses it);
...
let to_minimal (type challenge scalar_challenge fp fp_opt lookup_opt)
    (t :
      (challenge, scalar_challenge, fp, fp_opt, lookup_opt, 'bool) In_circuit.t
