...
(* Mainnet `Hash_prefix_states.signature ~signature_kind:Mainnet` prefix
   state. Pickles can't import `hash_prefix_states` directly (would cycle
   through `random_oracle`), so the three Tick.Field values are baked in
   here in LE-byte hex form — same constants the PS-side
   `Data.Schnorr.ChainId.signaturePrefix Mainnet` uses. Regenerated by
   `dump_schnorr_prefix_states.exe`. *)
let signature_prefix_mainnet_hex_le : string array =
  [| "494a6a0c1b0d8ca7bc1cef37a3d69c4133b8a994e760c2ff5ba1a723a67e393f"
   ; "4c2561efcd3dd41998ac7f4b20d12d2947bb0c4177d7f4e6a7753ccc78a4ce1c"
   ; "fbabea66d176d79be0265717a2ee07e019a7f9fe0638b128472fe9ed93e12305"
  |]

let tick_field_of_hex_le (s : string) : Impl.Field.Constant.t =
  ...
  for i = 0 to n - 1 do
    let nibble c =
      ...
      | 'A' .. 'F' -> Char.to_int c - Char.to_int 'A' + 10
      | _ -> failwith "tick_field_of_hex_le: not a hex digit"
    in
    let hi = nibble s.[2 * i] in
    let lo = nibble s.[(2 * i) + 1] in
    Bytes.set bytes i (Char.of_int_exn ((hi lsl 4) lor lo))
  done ;
  Kimchi_pasta_basic.Bigint256.of_bytes bytes
  |> Kimchi_pasta_snarky_backend.Vesta_based_plonk.Field.of_bigint

let signature_prefix_mainnet_state : Impl.Field.t Sponge.State.t =
  Array.map signature_prefix_mainnet_hex_le ~f:(fun hex ->
      Impl.Field.constant (tick_field_of_hex_le hex) )
...
let schnorr_verify_circuit
    ( ((((pk_x, pk_y), r), s), (message : Impl.Field.t array))
      : ((((Impl.Field.t * Impl.Field.t) * Impl.Field.t) * Impl.Field.t)
        * Impl.Field.t array) )
    () =
  (* 1. Hash: seed sponge with `Hash_prefix_states.signature
     ~signature_kind:Mainnet`, then absorb message field-elements first,
     then pk_x, pk_y, r (matches Mina's
     `Random_oracle.Input.Chunked.append message {pk; r}` order). *)
  let params =
    Sponge.Params.map Tick_field_sponge.params ~f:Impl.Field.constant
  in
  let sponge =
    Step_main_inputs.Sponge.create ~init:signature_prefix_mainnet_state params
  in
