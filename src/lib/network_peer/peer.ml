...
module Inet_addr = struct
  ...
  module Stable = struct
    module V1 = struct
      type t = Core_unix.Inet_addr.t [@@deriving sexp, compare, hash]

      let to_latest = Fn.id

      let of_yojson = function
        | `String s ->
            Ok (Core_unix.Inet_addr.of_string s)
        | _ ->
            Error "expected string"

      let to_yojson ip_addr = `String (Core_unix.Inet_addr.to_string ip_addr)

      include Mina_stdlib.Bounded_types.String.Of_stringable (struct
        type nonrec t = t

        [%%define_locally Core_unix.Inet_addr.(to_string, of_string)]
      end)
    end
  end]
...
module Stable = struct
  ...
  module V1 = struct
    ...
    let to_yojson { host; peer_id; libp2p_port } =
      `Assoc
        [ ("host", `String (Core_unix.Inet_addr.to_string host))
...
type t = Stable.Latest.t =
  { host : Core_unix.Inet_addr.Blocking_sexp.t
  ; libp2p_port : int
  ; peer_id : string
  }
[@@deriving compare, sexp]
...
let to_display { host; libp2p_port; peer_id } =
  Display.
    { host = Core_unix.Inet_addr.to_string host
    ; libp2p_port
    ; peer_id = Id.to_string peer_id
    }

let of_display { Display.host; libp2p_port; peer_id } =
  { host = Core_unix.Inet_addr.of_string host
  ; libp2p_port
