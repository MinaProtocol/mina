...
module Stable = struct
  module V1 = struct
    module T = struct
      ...
      let to_yojson {host; libp2p_port; communication_port} =
        `Assoc
        ...
        | `Assoc ls ->
            ...
            Result.of_option ~error:"missing keys"
              (let%bind host_str =
               ...
               {host; libp2p_port; communication_port})
        | _ ->
...
let create host ~libp2p_port ~communication_port =
  {host; libp2p_port; communication_port}

(** localhost with dummy ports *)
let local =
  create Unix.Inet_addr.localhost ~libp2p_port:0 ~communication_port:0

let to_libp2p_host_and_port t =
...
let to_string {host; libp2p_port; communication_port} =
  sprintf
    !"[host : %s, libp2p_port : %s, communication_port : %s]"
