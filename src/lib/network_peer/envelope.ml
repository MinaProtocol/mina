...
module Sender = struct
  type t =
    | Local
    | Remote of (Unix.Inet_addr.Stable.V1.t * Peer.Id.t)
      ...
      ->
        Ok
          (Remote
             ...
             , Peer.Id.unsafe_of_string peer_id ))
    | _ ->
        Error "Expected JSON representing envelope sender"

  let remote_exn = function | Local -> failwith "Sender.remote_exn of Local sender" | Remote x -> x
end
...
module Incoming = struct
  ...
  let remote_sender_exn t = match t.sender with | Local -> failwith "Incoming.sender_sender_exn of Local envelope" | Remote x -> x
end
