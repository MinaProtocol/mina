         ...
         ~f:(fun frontier_opt ->
           ...
           | Some frontier ->
               ...
               *)
               ...
               in
               Logger.debug t.logger ~module_:__MODULE__ ~location:__LOC__
                 !"Re-validated transaction pool after restart: dropped %i of \
                   %i previously in pool"
...
let%test_module _ =
  ( module struct
    ...
    let setup_test () =
      ...
      let pool =
        Test.create ~logger:(Logger.null ()) ~trust_system
    ...
    let%test_unit "transactions are removed in linear case" =
      Thread_safe.block_on_async_exn (fun () ->
          ...
          assert_pool_txs [] ;
          let%bind apply_res =
            Test.Diff.apply pool (Envelope.Incoming.local independent_cmds)
          in
    ...
    let%test_unit "Now-invalid transactions are removed from the pool on fork \
      ...
      Thread_safe.block_on_async_exn (fun () ->
          ...
          in
          let%bind apply_res =
            Test.Diff.apply pool @@ Envelope.Incoming.local [cmd1]
          in
    ...
    let%test_unit "transaction replacement works and drops later transactions"
      ...
      let%bind apply_res =
        Test.Diff.apply pool (Envelope.Incoming.local txs_all)
      in
      ...
      let%bind apply_res_2 =
        Test.Diff.apply pool (Envelope.Incoming.local replace_txs)
      in
      [%test_eq: User_command.t list Or_error.t]
    ...
    let%test_unit "it drops queued transactions if a committed one makes \
      ...
      let%bind apply_res =
        Test.Diff.apply pool @@ Envelope.Incoming.local txs
      in
