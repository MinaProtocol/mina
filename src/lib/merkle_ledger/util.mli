module type Inputs_intf = sig
  ...
  module Location_binable : sig
    val hash_fold_t :
      Ppx_hash_lib.Std.Hash.state -> Location.t -> Ppx_hash_lib.Std.Hash.state

    val hash : Location.t -> Ppx_hash_lib.Std.Hash.hash_value

    val hashable : Location.t Core_kernel__.Hashtbl.Hashable.t
  ...
  module Account_id : sig
    ...
    module Table : sig
      ...
      type ('a, 'b) hashtbl = ('a, 'b) Location_binable.Table.hashtbl
  ...
  module Base : sig
    ...
    val get : t -> Location_binable.Table.key -> Account.t option

    val last_filled : t -> Location_binable.Table.key option
  end

  val get_hash : Base.t -> Location_binable.Table.key -> Hash.t

  val location_of_account_addr : Location.Addr.t -> Location_binable.Table.key

  val location_of_hash_addr : Location.Addr.t -> Location_binable.Table.key

  val ledger_depth : Base.t -> int

  val set_raw_hash_batch :
    Base.t -> (Location_binable.Table.key * Hash.t) list -> unit

  val set_raw_account_batch :
    Base.t -> (Location_binable.Table.key * Account.t) list -> unit

  val set_location_batch :
       last_location:Location_binable.Table.key
    -> Base.t
    -> (Account_id.t * Location_binable.Table.key) Non_empty_list.t
    -> unit
end
...
module Make : functor
  (Inputs : sig
     ...
     val location_of_account_addr :
       Location.Addr.t -> Location_binable.Table.key
  ...
  val set_hash_batch :
       Inputs.Base.t
    -> (Inputs.Location_binable.Table.key * Inputs.Hash.t) list
    -> unit

  val set_batch :
       Inputs.Base.t
    -> (Inputs.Location_binable.Table.key * Inputs.Account.t) list
    -> unit
