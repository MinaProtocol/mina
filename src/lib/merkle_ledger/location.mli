module Bigstring : sig
  module Stable : sig
    module V1 : sig
      type t = Core_kernel.Bigstring.Stable.V1.t
      ...
      val equal : Core.Bigstring.t Base.Equal.equal

      val hash : Core.Bigstring.t -> Ppx_hash_lib.Std.Hash.hash_value
  ...
  val get : Core.Bigstring.t -> Base.int -> Base.char

  val length : Core.Bigstring.t -> Base.int

  val equal : Core.Bigstring.t Base.Equal.equal

  val create :
       ?max_mem_waiting_gc:Core_kernel__.Byte_units.t
    -> Core_kernel__.Import.int
    -> Core.Bigstring.t

  val to_string :
    ?pos:Base.int -> ?len:Base.int -> Core.Bigstring.t -> Base.string

  val set : Core.Bigstring.t -> Base.int -> Base.char -> Base.unit

  val blit : (Core.Bigstring.t, Core.Bigstring.t) Base__.Blit_intf.blit

  val sub : (Core.Bigstring.t, Core.Bigstring.t) Base__.Blit_intf.sub
...
module T : sig
  ...
  type t =
    ...
    | Account of Merkle_address.t
    | Hash of Merkle_address.t
  ...
  val last_direction : Merkle_address.t -> Direction.t

  val build_generic : Bigstring.t -> t

  val parse :
    ledger_depth:Core_kernel__Int.t -> Bigstring.t -> (t, unit) Core.Result.t

  val prefix_bigstring :
    Unsigned.UInt8.t -> Core.Bigstring.t -> Core.Bigstring.t

  val to_path_exn : t -> Merkle_address.t
  ...
  val location_of_sexp : Ppx_sexp_conv_lib.Sexp.t -> t

  val sexp_of_location : t -> Ppx_sexp_conv_lib.Sexp.t
  ...
  module Map : sig
    ...
    module Tree : sig
      type 'a t =
        (location, 'a, comparator_witness) Core_kernel__.Map_intf.Tree.t

      val empty : 'a t

      val singleton : location -> 'a -> 'a t

      val of_alist :
        (location * 'a) list -> [ `Duplicate_key of location | `Ok of 'a t ]

      val of_alist_or_error : (location * 'a) list -> 'a t Base__.Or_error.t

      val of_alist_exn : (location * 'a) list -> 'a t

      val of_alist_multi : (location * 'a) list -> 'a list t

      val of_alist_fold :
        (location * 'a) list -> init:'b -> f:('b -> 'a -> 'b) -> 'b t

      val of_alist_reduce : (location * 'a) list -> f:('a -> 'a -> 'a) -> 'a t

      val of_sorted_array : (location * 'a) array -> 'a t Base__.Or_error.t

      val of_sorted_array_unchecked : (location * 'a) array -> 'a t

      val of_increasing_iterator_unchecked :
        len:int -> f:(int -> location * 'a) -> 'a t

      val of_increasing_sequence :
        (location * 'a) Base__.Sequence.t -> 'a t Base__.Or_error.t

      val of_sequence :
           (location * 'a) Base__.Sequence.t
        -> [ `Duplicate_key of location | `Ok of 'a t ]

      val of_sequence_or_error :
        (location * 'a) Base__.Sequence.t -> 'a t Base__.Or_error.t

      val of_sequence_exn : (location * 'a) Base__.Sequence.t -> 'a t

      val of_sequence_multi : (location * 'a) Base__.Sequence.t -> 'a list t
      ...
      val of_sequence_reduce :
        (location * 'a) Base__.Sequence.t -> f:('a -> 'a -> 'a) -> 'a t

      val of_iteri :
           iteri:(f:(key:location -> data:'v -> unit) -> unit)
        -> [ `Duplicate_key of location | `Ok of 'v t ]

      val of_tree : 'a t -> 'a t

      val of_hashtbl_exn : (location, 'a) Bigstring.Table.hashtbl -> 'a t

      val of_key_set :
        (location, comparator_witness) Base.Set.t -> f:(location -> 'v) -> 'v t

      val quickcheck_generator :
           location Core_kernel__.Quickcheck.Generator.t
      ...
      val add :
        'a t -> key:location -> data:'a -> 'a t Base__.Map_intf.Or_duplicate.t

      val add_exn : 'a t -> key:location -> data:'a -> 'a t

      val set : 'a t -> key:location -> data:'a -> 'a t

      val add_multi : 'a list t -> key:location -> data:'a -> 'a list t

      val remove_multi : 'a list t -> location -> 'a list t

      val find_multi : 'a list t -> location -> 'a list

      val change : 'a t -> location -> f:('a option -> 'a option) -> 'a t

      val update : 'a t -> location -> f:('a option -> 'a) -> 'a t

      val find : 'a t -> location -> 'a option

      val find_exn : 'a t -> location -> 'a

      val remove : 'a t -> location -> 'a t

      val mem : 'a t -> location -> bool

      val iter_keys : 'a t -> f:(location -> unit) -> unit

      val iter : 'a t -> f:('a -> unit) -> unit

      val iteri : 'a t -> f:(key:location -> data:'a -> unit) -> unit

      val iteri_until :
           'a t
        -> f:(key:location -> data:'a -> Base__.Map_intf.Continue_or_stop.t)
      ...
      val mapi : 'a t -> f:(key:location -> data:'a -> 'b) -> 'b t

      val fold :
        'a t -> init:'b -> f:(key:location -> data:'a -> 'b -> 'b) -> 'b
      ...
      val filter_keys : 'a t -> f:(location -> bool) -> 'a t

      val filter : 'a t -> f:('a -> bool) -> 'a t

      val filteri : 'a t -> f:(key:location -> data:'a -> bool) -> 'a t

      val filter_map : 'a t -> f:('a -> 'b option) -> 'b t

      val filter_mapi : 'a t -> f:(key:location -> data:'a -> 'b option) -> 'b t

      val partition_mapi :
           'a t
        -> f:(key:location -> data:'a -> [ `Fst of 'b | `Snd of 'c ])
        -> 'b t * 'c t
      ...
      val partitioni_tf :
        'a t -> f:(key:location -> data:'a -> bool) -> 'a t * 'a t
      ...
      val keys : 'a t -> location list

      val data : 'a t -> 'a list

      val to_alist :
        ?key_order:[ `Decreasing | `Increasing ] -> 'a t -> (location * 'a) list
      ...
      val symmetric_diff :
        ...
        -> 'a t
        -> data_equal:('a -> 'a -> bool)
        -> (location, 'a) Base__.Map_intf.Symmetric_diff_element.t
           Base__.Sequence.t
      ...
      val min_elt : 'a t -> (location * 'a) option

      val min_elt_exn : 'a t -> location * 'a

      val max_elt : 'a t -> (location * 'a) option

      val max_elt_exn : 'a t -> location * 'a

      val for_all : 'a t -> f:('a -> bool) -> bool

      val for_alli : 'a t -> f:(key:location -> data:'a -> bool) -> bool

      val exists : 'a t -> f:('a -> bool) -> bool

      val existsi : 'a t -> f:(key:location -> data:'a -> bool) -> bool

      val count : 'a t -> f:('a -> bool) -> int

      val counti : 'a t -> f:(key:location -> data:'a -> bool) -> int

      val split : 'a t -> location -> 'a t * (location * 'a) option * 'a t
      ...
      val subrange :
           'a t
        -> lower_bound:location Base__.Maybe_bound.t
        -> upper_bound:location Base__.Maybe_bound.t
        -> 'a t
      ...
      val fold_range_inclusive :
        ...
        -> max:location
        -> init:'b
        -> f:(key:location -> data:'a -> 'b -> 'b)
        -> 'b

      val range_to_alist :
        'a t -> min:location -> max:location -> (location * 'a) list
      ...
      val nth : 'a t -> int -> (location * 'a) option

      val nth_exn : 'a t -> int -> location * 'a

      val rank : 'a t -> location -> int option

      val to_tree : 'a t -> 'a t

      val to_sequence :
           ?order:[ `Decreasing_key | `Increasing_key ]
        -> ?keys_greater_or_equal_to:location
        -> ?keys_less_or_equal_to:location
        -> 'a t
        -> (location * 'a) Base__.Sequence.t

      val binary_search :
           'a t
        -> compare:(key:location -> data:'a -> 'key -> int)
        -> [ `First_equal_to
      ...
      val binary_search_segmented :
           'a t
        -> segment_of:(key:location -> data:'a -> [ `Left | `Right ])
        -> [ `First_on_right | `Last_on_left ]
        -> (location * 'a) option

      val key_set : 'a t -> (location, comparator_witness) Base.Set.t

      val quickcheck_observer :
           location Core_kernel__.Quickcheck.Observer.t
        -> 'v Core_kernel__.Quickcheck.Observer.t
        -> 'v t Core_kernel__.Quickcheck.Observer.t

      val quickcheck_shrinker :
           location Core_kernel__.Quickcheck.Shrinker.t
        -> 'v Core_kernel__.Quickcheck.Shrinker.t
        -> 'v t Core_kernel__.Quickcheck.Shrinker.t

      module Provide_of_sexp : functor
        (K : sig
           val t_of_sexp : Ppx_sexp_conv_lib.Sexp.t -> location
         end)
        -> sig
        val t_of_sexp :
             (Ppx_sexp_conv_lib.Sexp.t -> 'v_x__001_)
    ...
    type 'a t = (location, 'a, comparator_witness) Core_kernel__.Map_intf.Map.t
    ...
    val of_sequence_fold :
      (location * 'a) Base__.Sequence.t -> init:'b -> f:('b -> 'a -> 'b) -> 'b t
    ...
    val fold : 'a t -> init:'b -> f:(key:location -> data:'a -> 'b -> 'b) -> 'b
    ...
    val fold_symmetric_diff :
      ...
      -> data_equal:('a -> 'a -> bool)
      -> init:'c
      -> f:('c -> (location, 'a) Base__.Map_intf.Symmetric_diff_element.t -> 'c)
      -> 'c
    ...
    module Provide_bin_io : functor
      (Key : sig
         val bin_size_t : location Bin_prot.Size.sizer

         val bin_write_t : location Bin_prot.Write.writer

         val bin_read_t : location Bin_prot.Read.reader

         val __bin_read_t__ : (int -> location) Bin_prot.Read.reader

         val bin_shape_t : Bin_prot.Shape.t

         val bin_writer_t : location Bin_prot.Type_class.writer

         val bin_reader_t : location Bin_prot.Type_class.reader

         val bin_t : location Bin_prot.Type_class.t
       end)
      -> sig
      val bin_shape_t : Bin_prot.Shape.t -> Bin_prot.Shape.t
    ...
    module Provide_hash : functor
      (Key : sig
         val hash_fold_t : Base__.Hash.state -> location -> Base__.Hash.state
       end)
      -> sig
      val hash_fold_t :
  ...
  module Set : sig
    ...
    module Tree : sig
      type t = (location, comparator_witness) Core_kernel__.Set_intf.Tree.t

      val compare : t -> t -> Core_kernel__.Import.int

      type named =
        (location, comparator_witness) Core_kernel__.Set_intf.Tree.Named.t

      val length : t -> int

      val is_empty : t -> bool

      val iter : t -> f:(location -> unit) -> unit

      val fold : t -> init:'accum -> f:('accum -> location -> 'accum) -> 'accum

      val fold_result :
           t
        -> init:'accum
        -> f:('accum -> location -> ('accum, 'e) Base__.Result.t)
        -> ('accum, 'e) Base__.Result.t

      val exists : t -> f:(location -> bool) -> bool

      val for_all : t -> f:(location -> bool) -> bool

      val count : t -> f:(location -> bool) -> int
      ...
      val find : t -> f:(location -> bool) -> location option

      val find_map : t -> f:(location -> 'a option) -> 'a option

      val to_list : t -> location list

      val to_array : t -> location array
      ...
      val symmetric_diff :
        t -> t -> (location, location) Base__.Either.t Base__.Sequence.t
      ...
      val fold_until :
           t
        -> init:'b
        -> f:('b -> location -> ('b, 'final) Base__.Set_intf.Continue_or_stop.t)
        -> finish:('b -> 'final)
        -> 'final

      val fold_right : t -> init:'b -> f:(location -> 'b -> 'b) -> 'b

      val iter2 :
           t
        -> t
        -> f:
             (   [ `Both of location * location
                 | `Left of location
                 | `Right of location ]
              -> unit)
        -> unit

      val filter : t -> f:(location -> bool) -> t

      val partition_tf : t -> f:(location -> bool) -> t * t

      val elements : t -> location list

      val min_elt : t -> location option

      val min_elt_exn : t -> location

      val max_elt : t -> location option

      val max_elt_exn : t -> location

      val choose : t -> location option

      val choose_exn : t -> location

      val split : t -> location -> t * location option * t

      val group_by : t -> equiv:(location -> location -> bool) -> t list

      val find_exn : t -> f:(location -> bool) -> location

      val nth : t -> int -> location option
      ...
      val to_sequence :
        ...
        -> ?greater_or_equal_to:location
        -> ?less_or_equal_to:location
        -> t
        -> location Base__.Sequence.t

      val binary_search :
           t
        -> compare:(location -> 'key -> int)
        -> [ `First_equal_to
           | `First_greater_than_or_equal_to
           | `First_strictly_greater_than
           | `Last_equal_to
      ...
      val binary_search_segmented :
           t
        -> segment_of:(location -> [ `Left | `Right ])
        -> [ `First_on_right | `Last_on_left ]
        -> location option
      ...
      val merge_to_sequence :
        ...
        -> t
        -> t
        -> (location, location) Base__.Set_intf.Merge_to_sequence_element.t
           Base__.Sequence.t

      val to_map :
           t
        -> f:(location -> 'data)
        -> (location, 'data, comparator_witness) Base.Map.t
      ...
      val singleton : location -> t

      val union_list : t list -> t

      val of_list : location list -> t

      val of_array : location array -> t

      val of_sorted_array : location array -> t Base__.Or_error.t

      val of_sorted_array_unchecked : location array -> t

      val of_increasing_iterator_unchecked : len:int -> f:(int -> location) -> t

      val stable_dedup_list : location list -> location list

      val map :
        ('a, 'b) Core_kernel__.Set_intf.Tree.t -> f:('a -> location) -> t

      val filter_map :
        ('a, 'b) Core_kernel__.Set_intf.Tree.t -> f:('a -> location option) -> t

      val of_tree : t -> t

      val of_hash_set : location Core_kernel__.Hash_set.t -> t

      val of_hashtbl_keys : (location, 'a) Bigstring.Table.hashtbl -> t

      val of_map_keys : (location, 'a, comparator_witness) Base.Map.t -> t
    ...
    type t = (location, comparator_witness) Base.Set.t

    val compare : t -> t -> Core_kernel__.Import.int

    type named = (location, comparator_witness) Core_kernel__.Set_intf.Named.t
    ...
    val map : ('a, 'b) Base.Set.t -> f:('a -> location) -> t

    val filter_map : ('a, 'b) Base.Set.t -> f:('a -> location option) -> t
