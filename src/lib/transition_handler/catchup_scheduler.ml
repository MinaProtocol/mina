(** * [Catchup_scheduler] defines a process which schedules catchup jobs and *
    ...
    tearing down catchup jobs constantly. The [Catchup_scheduler] must *
    receive notifications whenever a new transition is added to the *
    transition frontier so that it can determine if any pending catchup * jobs
    can be invalidated. When catchup jobs are invalidated, the * catchup
    scheduler extracts all of the invalidated catchup jobs and * spins up a
    process to materialize breadcrumbs from those transitions, * which will
    write the breadcrumbs back into the processor as if * catchup had
    ...
    ; catchup_job_writer:
        ...
        (* `collected_transitins` stores all seen transitions as its keys, and
           * values are a list of direct children of those transitions. The *
           ...
           in this table. Even if a transition doesn't has a child, its
           corresponding * value in the hash table would just be an empty list.*)
