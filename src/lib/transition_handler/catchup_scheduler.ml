...
let create ~logger ~precomputed_values ~verifier ~reputation ~frontier
      ...
      (fun (initial_hash, transition_branches) ->
        match%map
          Breadcrumb_builder.build_subtrees_of_breadcrumbs
            ...
            ~precomputed_values ~verifier ~reputation ~frontier ~initial_hash
            transition_branches
        with
...
let%test_module "Transition_handler.Catchup_scheduler tests" =
  ( module struct
    ...
    let create = create ~logger ~reputation ~time_controller
