...
let%test_module "Transition_handler.Processor tests" =
  ( module struct
    ...
    module Transaction_snark_work = struct
      ...
      type unchecked = t [@@deriving sexp, bin_io]
    ...
    module Staged_ledger_diff = struct
      ...
      type diff =
        ...
        ; user_commands: Coda_base.User_command.t list }
      [@@deriving sexp, bin_io]
    ...
    module Staged_ledger = struct
      ...
      let of_scan_state_and_ledger ~snarked_ledger_hash:_ ~ledger:_
          ~scan_state:_ =
        Or_error.return ()
      ...
      let create_diff () ~self:_ ~logger:_ ~transactions_by_fee:_
          ~get_completed_work:_ =
        failwith "stub"
    ...
    let gen_external_transition previous_protocol_state =
      ...
      let%map consensus_state =
        Consensus.Mechanism.For_tests.gen_consensus_state
          ~gen_slot_advancement:(Int.gen_incl 1 10) ~previous_protocol_state
          ~snarked_ledger_hash:Coda_base.Frozen_ledger_hash.empty_hash
      in
      let protocol_state =
        Consensus.Mechanism.Protocol_state.create_value ~blockchain_state
          ~consensus_state
          ~previous_state_hash:(With_hash.hash previous_protocol_state)
      in
      let transition =
      ...
      ; hash= Consensus.Mechanism.Protocol_state.hash protocol_state }
    ...
    let dummy_protocol_state =
      ...
      let consensus_state =
        Consensus.Mechanism.Protocol_state.consensus_state
          Consensus.Mechanism.genesis_protocol_state
      in
      let protocol_state =
        Consensus.Mechanism.Protocol_state.create_value ~blockchain_state
          ~consensus_state
      ...
      in
      With_hash.of_data ~hash_data:Consensus.Mechanism.Protocol_state.hash
        protocol_state
    ...
    let gen_transition ~seed ~choice frontier =
      if choice then
        ...
        in
        ( `Exists
        , Quickcheck.random_value ~seed (gen_external_transition parent) )
      else
        ( `Does_not_exist
        , Quickcheck.random_value ~seed
            (gen_external_transition
               (With_hash.map dummy_external_transition
                  ~f:External_transition.protocol_state)) )
      ...
      Thread_safe.block_on_async_exn (fun () ->
          ...
          let root_transition =
            { With_hash.data=
                External_transition.create
                  ...
                  ~staged_ledger_diff:dummy_staged_ledger_diff
            ; hash=
                Consensus.Mechanism.Protocol_state.hash
                  Consensus.Mechanism.genesis_protocol_state }
          in
          let frontier =
         ...
         *)
          Deferred.create (fun test_completed ->
              ignore
                (Time.Timeout.create ()
                   (Time.Span.of_ms (Int64.of_int 500))
                   ~f:(fun _ ->
                     assert (
                       List.for_all (Transition_frontier.all_hashes frontier)
                         ~f:(fun hash ->
                           Set.exists !expected_transitions
                             ~f:(Coda_base.State_hash.equal hash) ) ) ;
