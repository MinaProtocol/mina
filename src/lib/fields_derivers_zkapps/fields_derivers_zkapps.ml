...
let%test_unit "verification key, roundtrip json" =
  let open Pickles.Side_loaded.Verification_key in
  (* we do this because the dummy doesn't have a wrap_vk on it *)
  let vk = dummy |> to_base58_check |> of_base58_check_exn in
  let o = verification_key @@ o () in
  [%test_eq: t] vk (of_json o (to_json o vk))
