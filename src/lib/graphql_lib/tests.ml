...
module Make_test (S : Scalars.S_JSON) (G : GENERATOR with type t = S.t) = struct
  ...
  let query_server_and_compare value =
    ...
    let () = Format.printf "calling test with value %s\n" (G.sexp_of_t value |> Sexp.to_string) in
...
let%test_unit "block_time_to_int64" =
  ignore (Unsigned.UInt64.to_int64 (Unsigned.UInt64.of_string "3183349418695935435"): Int64.t)
