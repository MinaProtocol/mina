...
module Make_str (_ : Wire_types.Concrete) = struct
  ...
  *)
  ...
  let%test_module "test uncorrelated values in wrap proof" =
    ( module struct
      let () = Backtrace.elide := false
      ...
      (* Takes a modifier that tells us what/how to tweak proof creation in an invalid way.
         This will create tests checking that both in-circuit and out-of-circuit verifiers can't verify such a proof. *)
      module Make_test (Evil_modifier : sig
        val wrapper :
          ('max_proofs_verified, 'branches, 'prev_varss) wrap_main_generic
      end) =
      struct
        let tag, _, p, Provers.[ step ] =
          compile_with_wrap_main_override_promise ()
            ~override_wrap_main:Evil_modifier.wrapper
      ...
      (**************************************************************
       * We instantiate a number of test with the previous functor. *          **************************************************************)

      (** This creates an uncorrelated value `b` in the wrap proof. *)
      module Test_b_value = Make_test (struct
        let wrapper =
          { wrap_main =
              (fun _ _ _ _ _ _ _ ->
      ...
      (** This creates uncorrelated bulletproof challenges in the wrap proof. *)
      module Test_bulletproof_challenges = Make_test (struct
        let wrapper =
          { wrap_main =
              (fun _ _ _ _ _ _ _ ->
                ( Requests.Wrap.create ()
          ...
          ; tweak_statement =
              (fun stmt ->
                let some_number = 16 in
                let overwritten_prechals =
                  Array.init some_number ~f:(fun _ ->
                      Scalar_challenge.create
                        (Import.Challenge.Constant.of_tick_field
                ...
                let sg_new =
                  ...
                  Kimchi_bindings.Protocol.SRS.Fp.batch_accumulator_generate urs
                    1 chals
                in
                let sg_new =
                  match sg_new with
                ...
                in
                let overwritten_prechals =
                  Array.map overwritten_prechals ~f:Bulletproof_challenge.unpack
                in
                { stmt with
                  proof_state =
                    { stmt.proof_state with
                      deferred_values =
                        { stmt.proof_state.deferred_values with
                          bulletproof_challenges =
                            Vector.of_array_and_length_exn overwritten_prechals
                              Tick.Rounds.n
                        }
                    ; messages_for_next_wrap_proof =
                        { stmt.proof_state.messages_for_next_wrap_proof with
