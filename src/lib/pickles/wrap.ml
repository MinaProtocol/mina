...
let%test_module "pickles gate finalization" =
  ( module struct
    type example =
    ...
    (* Run a test for [feature_flags] based on a [Kimchi_bindings] example, that
       is an example actually written in
       file [src/lib/crypto/kimchi_bindings/stubs/src/pasta_fp_plonk_proof.rs]. *)
    let runtest ~(feature_flags : feature_flags) ~(example : example) =
    ...
    let listify_public_input1 f =
      let example srs b =
        let idx, public_input, proof = f srs b in
        (idx, [ public_input ], proof)
      in
      example

    let listify_public_input2 f =
      let example srs b =
        let idx, (input1, input2), proof = f srs b in
        (idx, [ input1; input2 ], proof)
      in
      example
    ...
    let%test "lookup finalization" =
      runtest
        ...
        ~example:
          (listify_public_input1
             Kimchi_bindings.Protocol.Proof.Fp.example_with_lookup )
