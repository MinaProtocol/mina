...
let%test_module "gate finalization" =
  ( module struct
    let set_flag ~flag_value feature_flags flag =
      let open Plonk_types.Features in
      match flag with
      | `Chacha ->
          { feature_flags with chacha = flag_value }
      | `Range_check ->
          { feature_flags with range_check = flag_value }
      | `Foreign_field_add ->
          { feature_flags with foreign_field_add = flag_value }
      | `Foreign_field_mul ->
          { feature_flags with foreign_field_mul = flag_value }
      | `Xor ->
          { feature_flags with xor = flag_value }
      | `Rot ->
          { feature_flags with rot = flag_value }
      | `Lookup ->
          { feature_flags with lookup = flag_value }
      | `Runtime_tables ->
          { feature_flags with runtime_tables = flag_value }

    let set_flags ?(init = Plonk_types.Features.none) flag_value flags =
      List.fold_left ~f:(set_flag ~flag_value) ~init flags

    let activate_flags = set_flags Plonk_types.Opt.Flag.Yes

    let maybe_flags = set_flags Plonk_types.Opt.Flag.Maybe

    let runtest ~feature_flags ~example =
      ...
      in
      let index, public_input, proof = example srs true in
    ...
    (*******************************
       Lookup gate finalization test
      *******************************)

    let%test "lookup finalization" =
      let feature_flags = maybe_flags [ `Lookup; `Runtime_tables ] in
      let example srs b =
      ...
      in
      runtest ~feature_flags ~example

    (*****************************************************
       Foreign field multiplication gate finalization test
      *****************************************************)

    let foreign_field_mul_feature_flags =
      [ `Range_check; `Foreign_field_mul; `Lookup ]

    let ffmul_test set_flags =
      let feature_flags = set_flags foreign_field_mul_feature_flags in
      let example srs _b =
        let index, proof =
          Kimchi_bindings.Protocol.Proof.Fp.example_with_foreign_field_mul srs
        in
        (* There is no actual public input for this test *)
        (index, [], proof)
      in
      runtest ~feature_flags ~example

    let%test "foreign field multiplication finalization (maybe)" =
      ffmul_test maybe_flags

    let%test "foreign field multiplication finalization (yes)" =
      ffmul_test activate_flags
  end )
