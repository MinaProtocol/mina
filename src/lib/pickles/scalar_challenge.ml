...
let to_field_checked (type f)
  ...
  for i = 128/2 - 1 downto 0 do
    let s = 
      F.if_ bits.(2*i) ~then_:one ~else_:neg_one
    in
    a := F.(scale !a two);
    b := F.(scale !b two);
...
let to_field_constant (type f)
  ...
  for i = 128/2 - 1 downto 0 do
    let s = 
      if bits.(2*i) then one else neg_one
    in
    a := F.(!a + !a);
    b := F.(!b + !b);
    let r_2i1 = bits.(2*i + 1) in
...
= struct
  ...
  module Constant = struct
    ...
    let to_field =
      to_field_constant ~endo:Endo.scalar
        (module Scalar)
  end

  let typ : (t, Constant.t) Typ.t =
    SC.typ Challenge.typ
      ...
      then acc
        ...
        let sx =
          exists Field.typ ~compute:As_prover.(fun () ->
            if read Boolean.typ b_2i1
            then Field.Constant.(Endo.base * read_var x)
            else read_var x )
        in
