...
type ('local_statement, 'local_max_branching, 'local_num_branches) t =
  ...
  * ( Challenge.Make(Impl).t
    ...
    , Pairing_main.Make(Pickles_base.Step_main_inputs).Other_field.t
    , unit
    , Digest.Make(Impl).t
  ...
  * (Impl.Field.t array Dlog_plonk_types.Evals.t * Impl.Field.t)
    Tuple_lib.Double.t
  * (Pickles_base.Step_main_inputs.Inner_curve.t, 'local_max_branching) Vector.t
...
let typ (type n avar aval m)
    (statement : (avar, aval) Pickles_base.Impls.Step.Typ.t)
    (local_max_branching : n Nat.t) (local_branches : m Nat.t) ~step_domains :
    ((avar, n, m) t, (aval, n, m) Constant.t) Pickles_base.Impls.Step.Typ.t =
