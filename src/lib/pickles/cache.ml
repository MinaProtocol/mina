...
module Step = struct
  ...
  let storable =
    Memo.unit (fun () ->
        ...
        T.Store.Disk_storable.simple Key.Proving.to_string
  ...
  let vk_storable =
    Memo.unit (fun () ->
        let (module T) = Zexe_backend_platform_specific.get () in
        T.Store.Disk_storable.simple Key.Verification.to_string
      ...
      lazy
        ( match
            Common.time "step keypair read" (fun () ->
                T.Store.read cache s_p (Lazy.force k_p))
          with
        | Ok (pk, dirty) ->
        ...
        | Error _e ->
            ...
            Timer.clock __LOC__ ;
            ignore
              ( T.Store.write cache s_p (Lazy.force k_p) (Keypair.pk r)
                : unit Or_error.t ) ;
    ...
    let vk =
      lazy
        (let k_v = Lazy.force k_v in
         match
           Common.time "step vk read" (fun () -> T.Store.read cache s_v k_v)
         with
         | Ok (vk, _) ->
             (vk, `Cache_hit)
         | Error _e ->
             ...
             ignore (T.Store.write cache s_v k_v vk : unit Or_error.t) ;
             (vk, c))
    in
    (pk, vk)
end
    ...
    let pk =
      lazy
        (let k = Lazy.force k_p in
         match
           Common.time "wrap key read" (fun () -> T.Store.read cache s_p k)
         with
         | Ok (pk, d) ->
         ...
         | Error _e ->
             let r =
               Common.time "wrapkeygen" (fun () ->
                   generate_keypair ~exposing:[ typ ] main)
             in
             ignore (T.Store.write cache s_p k (Keypair.pk r) : unit Or_error.t) ;
             (r, `Generated_something))
    in
    let vk =
      lazy
             ...
             (fun (_, header, _) t path ->
               Or_error.try_with (fun () ->
                   Snark_keys_header.write_with_header
                     ...
                     header path))
         in
         match T.Store.read cache s_v k_v with
         | Ok (vk, d) ->
             (vk, d)
         | Error e ->
             ...
             in
             ignore (T.Store.write cache s_v k_v vk : unit Or_error.t) ;
             let _vk = T.Store.read cache s_v k_v in
