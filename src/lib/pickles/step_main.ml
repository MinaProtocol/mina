...
module Make (IR : Inductive_rule.Intf) = struct
  ...
  let step_main :
   ...
   fun (module Req) max_proofs_verified ~self_branches ~local_signature
    ...
    let main () : _ Types.Step.Statement.t Promise.t =
      ...
      let%bind.Promise { IR.previous_proof_statements
        ...
        *)
                ...
                let previous_proof_statements =
                  let rec go :
                      type prev_vars prev_values ns1 ns2.
                         (prev_vars, ns1) H2.T(IR.Previous_proof_statement).t
          ...
          let proof_witnesses =
            ...
            let rec go :
                ...
                -> (vars, ns1) H2.T(IR.Previous_proof_statement).t
