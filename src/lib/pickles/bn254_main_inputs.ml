...
let sponge_params_constant = Kimchi_bn254_basic.poseidon_params_fp
...
module Inner_curve = struct
  module C = Kimchi_bn254.Bn254.Curve
  ...
  module Inputs = struct
    ...
    module Constant = struct
      ...
      module Scalar = struct
        include Kimchi_bn254.Basic.Bn254_fp
        ...
        let project =
          let rec go x acc = function
            | [] ->
                acc
            | b :: bs ->
                go (add x x) (if b then add acc x else acc) bs
          in
          fun bs -> go one zero bs
      end
      ...
      let zero = Kimchi_bn254.Basic.Bn254_fq.(zero, zero)

      let ( + ) t1 t2 =
        let is_zero (x, _) = Kimchi_bn254.Basic.Bn254_fq.(equal zero x) in
          ...
          As_prover.(
            fun () ->
              C.scale
                ...
                (Bn254.Field.inv
                   (Bn254.Field.of_bits (List.map ~f:(read Boolean.typ) bs)) )
              |> C.to_affine_exn)
    in
...
module Ops = Plonk_curve_ops.Make (Impls.Bn254) (Inner_curve)
