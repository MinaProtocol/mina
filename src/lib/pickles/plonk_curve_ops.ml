...
module Make 
    ...
    (G : Intf.Group(Impl).S with type t = Impl.Field.t * Impl.Field.t) = struct
  ...
  let scale_fast 
      ((xt, yt) as t : Field.t * Field.t) (`Times_two_plus_1_plus_2_to_len r) :
      Field.t * Field.t =
    ...
    let () =
      for i = 0 to n - 1 do
        let (xp, yp) = !acc in
        let xq = xt in
        let l1 = exists Field.typ ~compute:As_prover.(fun () ->
            (* (yq - yp) / (xq - xp) *)
        ...
        let xr =
          As_prover.Ref.create As_prover.(fun () ->
              (* B l1^2 - A - xp - xq *)
              let open Field.Constant in
              G.Params.b * square (read_var l1) - G.Params.a - read_var xp - read_var xq
            )
        in 
        let l2 =
          As_prover.Ref.create As_prover.(fun () ->
        ...
        in 
        let row = 
          { Zexe_backend_common.Scale_round.
            xt
          ; yt
          ; b= (r.(i) :> Field.t)
          ; l1; xp; yp; xs; ys
