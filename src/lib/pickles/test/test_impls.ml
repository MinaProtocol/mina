let test_step_circuit_behavior () =
  ...
  in
  let str_list =
    List.map Impls.Step.Other_field.forbidden_shifted_values ~f:(fun (a, b) ->
        (Backend.Tick.Field.to_string a, b) )
  in
...
let test_wrap_circuit_behavior () =
  ...
  in
  let str_list =
    List.map Impls.Wrap.Other_field.forbidden_shifted_values
