...
let run_recursive_proof_test (actual_feature_flags : Plonk_types.Features.flags)
  ...
  let { Wrap.For_tests_only.deferred_values
    ...
    Wrap.For_tests_only.deferred_values ~feature_flags ~actual_feature_flags
      ~sgs:[] ~prev_challenges:[] ~step_vk:vk ~public_input ~proof
      ...
      { deferred_values with
        plonk =
          { deferred_values.plonk with
            lookup =
              Plonk_types.Opt.to_option_unsafe deferred_values.plonk.lookup
          ; optional_column_scalars =
              Composition_types.Wrap.Proof_state.Deferred_values.Plonk
              .In_circuit
              .Optional_column_scalars
              .map ~f:Plonk_types.Opt.to_option
            ...
            (`Known
              [ { h =
                    Pow_2_roots_of_unity
                      vk.Kimchi_types.VerifierIndex.domain.log_size_of_group
                }
              ] )
...
module Range_check_64 = Make (struct
  ...
  let actual_feature_flags =
    { Plonk_types.Features.none_bool with range_check0 = true; lookup = true }
end)
...
let tests =
  [ ("Lookup gate", Lookup.tests)
  ; ("Foreign field multiplication", FFMul.tests)
  ; ("Foreign field addition", FFAdd.tests)
  ; ("Rot", Rot.tests)
  ; ("Xor", Xor.tests)
  ; ("Range check", Range_check.tests)
  ; ("Range_check 64 bits", Range_check_64.tests)
  ]
