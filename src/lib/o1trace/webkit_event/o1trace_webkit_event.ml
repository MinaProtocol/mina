...
let _log_thread_existence name tid =
  Option.iter !current_wr ~f:(fun wr ->
      emit_event wr { (new_event New_thread) with name; tid })

module Implementation = struct
  module Hooks = struct
    let _trace_thread_switch _ (new_ctx : Execution_context.t) =
      Option.iter !current_wr ~f:(fun wr ->
  ...
  let trace (_name : string) (_f : unit -> 'a) = failwith "TODO"
  ...
  let _measure (name : string) (f : unit -> 'a) : 'a =
...
(* O1trace.Thread.iter_threads ~f:log_thread_existence ;
   emit_event wr
