module type S = sig
  (** Emit an instantaneous named event.
      ...
      These show up as little vertical bars underneath the horizontal bars
      measuring tasks.
  *)
  ...
  *)
  ...
  (** Trace some deferred work that happens more than once.
      ...
      [trace_recurring name f] is the same as [trace ("R&" ^ name) f]. The
      prefix collapses all the traces into a single row.
  *)
  ...
  *)
  ...
  (** Trace the execution of some background loop that happens more than once.
      ...
      [trace_recurring_task name f] is the same as
      [trace_task ("R&" ^ name) f].
  *)
