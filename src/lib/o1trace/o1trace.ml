...
let emitk (k : event_kind) pos =
  ...
  | End -> 7
  in
  Bigstring.set_uint8 buf ~pos num ; pos+1

let emiti (i : int) pos =
  Bigstring.set_uint64_le buf ~pos i ; pos+8
...
let trace_task (name : string) (f : unit -> 'a) =
  let new_ctx = Execution_context.with_tid (Scheduler.current_execution_context (Scheduler.t ())) !tid in
  tid := !tid + 1 ;
  (!Async_kernel.Tracing.fns).trace_new_thread name new_ctx ;
  match Scheduler.within_context new_ctx f with
  | Error () -> failwith "traced task failed, exception reported to parent monitor"
  | Ok x -> x
...
let measure name (f : unit -> 'a) : 'a = (Obj.magic !measure_impl : string -> (unit -> 'a) -> 'a) name f

let start_tracing wr =
  emitk Pid_is 0 |> emiti (Unix.getpid() |> Pid.to_int) |> finish wr ;
  Async_kernel.Tracing.fns := { trace_thread_switch= trace_thread_switch' wr; trace_new_thread= trace_new_thread' wr } ;
  trace_event_impl := trace_event' wr ;
  measure_impl := measure' wr ;
  Scheduler.set_on_end_of_cycle (Scheduler.t ()) (fun () -> emitk Cycle_end 0 |> emitt |> finish wr)
