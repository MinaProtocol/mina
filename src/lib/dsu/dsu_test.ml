...
let test_reallocation () =
  ...
  (* we add 1 to account for the default 0 element used for dynamic deletions in the dsu *)
    "verifying occupancy is the size of the array additions" (!arr_size + 1) (IntKeyDsu.occupancy dsu);
  ...
  Array.iter arr ~f:(fun x -> IntKeyDsu.add_exn ~key:x ~value:x dsu) ;
  Alcotest.(check int)
    "verifying capacity is 512 meaning there have been 4 re-allocations" (IntKeyDsu.capacity dsu) 512
