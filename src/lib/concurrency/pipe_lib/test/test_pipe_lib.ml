...
let test_reader_hang () =
  let replacable =
    Strict_pipe.Swappable.create ~name:"replacable"
      (Buffered (`Capacity 50, `Overflow (Drop_head (const ()))))
  in
  let%bind _hanging_reader =
    Strict_pipe.Swappable.swap_reader ~reader_name:"hanging reader" replacable
  in
  let () = Strict_pipe.Swappable.write replacable () in
  let%bind good_reader =
    Strict_pipe.Swappable.swap_reader ~reader_name:"good reader" replacable
  in
