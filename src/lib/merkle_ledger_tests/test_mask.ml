...
module Make (Test : Test_intf) = struct
  ...
  let () =
    add_test "mask delegates to parent" (fun () ->
        Test.with_instances (fun maskable mask ->
            let attached_mask = Maskable.register_mask maskable mask in
            ...
            assert (Option.is_some mask_result) ;
            let mask_account = Option.value_exn mask_result in
            [%test_eq: Account.t] dummy_account mask_account ) )
  ...
  let () =
    add_test "mask and parent agree on Merkle root before set" (fun () ->
        Test.with_instances (fun maskable mask ->
            ...
            assert (Hash.equal mask_merkle_root maskable_merkle_root) ) )
          ...
          Test.with_instances (fun maskable mask ->
              ...
              let retrieved_accounts =
                ...
                @@ Mask.Attached.get_all_accounts_rooted_at_exn attached_mask
                     (Mask.Addr.root ())
              in
              (* TODO: Use List length comparison functions *)
  ...
  let () =
    add_test "removing accounts from mask restores Merkle root" (fun () ->
        Test.with_instances (fun maskable mask ->
            let attached_mask = Maskable.register_mask maskable mask in
  ...
  let () =
    add_test "removing accounts from parent restores Merkle root" (fun () ->
        Test.with_instances (fun maskable mask ->
            let attached_mask = Maskable.register_mask maskable mask in
            let num_accounts = 5 in
            let account_ids = Account_id.gen_accounts num_accounts in
  ...
  let () =
    add_test "removing accounts from parent and mask restores Merkle root"
      (fun () ->
        Test.with_instances (fun maskable mask ->
            let attached_mask = Maskable.register_mask maskable mask in
  ...
  let () =
    add_test "reuse of locations for removed accounts" (fun () ->
        Test.with_instances (fun maskable mask ->
            let attached_mask = Maskable.register_mask maskable mask in
            let num_accounts = 5 in
            let account_ids = Account_id.gen_accounts num_accounts in
  ...
  let () =
    add_test
      "setting an account in the parent doesn't remove the masked copy if the \
       mask is still dirty for that account" (fun () ->
        Test.with_instances (fun maskable mask ->
            let attached_mask = Maskable.register_mask maskable mask in
