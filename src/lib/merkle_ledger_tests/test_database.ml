...
module Make (Test : Test_intf) = struct
  ...
  let () =
    add_test "getting a non existing account returns None" (fun () ->
  ...
  let () =
    add_test
      "If the entire database is full, let addresses_and_accounts = \
    ...
    add_test "get_at_index_exn t (index_of_account_exn t public_key) = account"
      (fun () ->
        Test.with_instance (fun mdb ->
            ...
            List.iter accounts ~f:(fun account ->
                ignore @@ create_new_account_exn mdb account ) ;
            let accts = Sequence.of_list accounts in
            assert (
              Sequence.for_all accts ~f:(fun account ->
                  let indexed_account =
  ...
  let () =
    add_test "Add 2^d accounts (for testing, d is small)" (fun () ->
  ...
  let () =
    add_test "removing accounts restores Merkle root" (fun () ->
        Test.with_instance (fun mdb ->
