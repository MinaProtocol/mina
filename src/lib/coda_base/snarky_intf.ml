(* snarky_intf.ml -- enough of an interface to allow the field-parameterized functors to work *)
...
module type Tick_S = sig
  ...
  module rec Checked : sig
    ...
    module List : sig
      ...
      val iter : 'a t -> f:('a -> (unit, 's) Checked.t) -> (unit, 's) Checked.t
    end
  end
  ...
  module Pedersen : sig
    module Digest : sig
      ...
      module Stable : sig
        module V1 : sig
          type t = field [@@deriving sexp, compare, hash, yojson]
        end
      end]
...
module type Data_hash_type_S = sig
  type t [@@deriving sexp, compare, hash, yojson]

  (* N.B.: can't use %%versioned here, which generates:

      module Latest = V1

     resulting in a type error

     see: https://github.com/ocaml/ocaml/issues/9195
   *)
