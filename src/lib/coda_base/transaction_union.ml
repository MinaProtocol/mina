...
let of_transaction : Transaction.t -> t = function
  | User_command cmd ->
      ...
      ; sender
      ; signature }
  | Coinbase {producer; fee_transfer; amount; state_body_hash= _} ->
      let other_pk, other_amount =
        Option.value ~default:(producer, Fee.zero) fee_transfer
      in
      { payload=
          { common=
              { fee= other_amount
          ...
          ; body= {public_key= producer; amount; tag= Tag.Coinbase} }
