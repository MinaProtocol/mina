...
module Tag0 = struct
  type t = Curr_ledger | Epoch_ledger [@@deriving enum]
end

module Tag = struct
  include Tag0
  module Checked = Enumerable (Tag0)

  let curr_ledger = Checked.var Curr_ledger

  let epoch_ledger = Checked.var Epoch_ledger
end
...
*)
let%snarkydef fetch_and_update_account t pk
...
let%snarkydef modify_account t pk ~filter ~f =
  fetch_and_update_account t pk ~filter ~f ~tag:Tag.curr_ledger >>| fst
...
(*
   ...
   which is [t] but with the account [f account] at path [addr].
*)
let%snarkydef modify_account_recv t pk ~f ~tag =
  fetch_and_update_account t pk ~tag
    ~filter:(fun account ->
      let%bind account_already_there =
        Public_key.Compressed.Checked.equal account.public_key pk
      in
