...
module Account = struct
  ...
  module Poly = struct
    ...
    module Stable = struct
      module V1 = struct
        type ('before, 'account_state) t =
          { account_state: 'account_state Transition.Stable.V1.t
          ; before: 'before
          }
          [@@deriving hlist, sexp, eq, yojson, hash, compare]
      end
  ...
  module Stable = struct
    module V1 = struct
      type t = (Before.Stable.V1.t, Account_state.Stable.V1.t) Poly.Stable.V1.t
        [@@deriving  sexp, eq, yojson, hash, compare]
      let to_latest = Fn.id
    end
  end]
  ...
  module Checked = struct
    type t = (Before.Checked.t, Account_state.Checked.t) Poly.t
  end

  let check ({ before; account_state=a } : t) ~prev
      ~(account_state : [ `Empty | `Snapp | `User ] Transition.t)
    =
    let open Or_error.Let_syntax in
    let%bind () = Before.check before prev in
    let%map () = Account_state.check a.prev account_state.prev
    and () = Account_state.check a.next account_state.next
    in
    ()

  let typ () =
    let open Poly in
    Typ.of_hlistable  [ Transition.typ Account_state.typ;  Before.typ ();  ]
