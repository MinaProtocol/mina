...
module Per_account = struct
  (* Account fields are
      - public_key: cannot change
      - token_id: cannot change
      - token_permissions: cannot change
      - balance: can only change by decreasing the fee
      - nonce: cannot change manually
      - receipt_chain_hash: 'receipt_chain_hash
      - delegate: can change
      - voting_for: currently unused
      - timing: cannot change manually
      - snapp: can change state, should make it so that key can change too,
        may require a change to how keys are hashed for optimal efficiency
  *)

  (* Cannot update the balance, apart from the fee! *)
  module Poly = struct
    [%%versioned
    ...
    type t =
      | Snapp_snapp of
          ...
          ; snapp2: Snapp_body.Stable.V1.t
                (* TODO: This presentation is a bit asymmetric and ugly. It can be improved at the UI layer. *)
...
module Payload = struct
  (* There will be these snapp branches.
     - 1 snapp + 1 optional fee payer
     - 2 snapps + 1 optional fee payer
     - create snapp
       - fee payer = modify + signature verify
       - merkle modify
     - user to snapp
       - 1 snapp
       - 1 fee payer = modify + signature 
     -  snapp to user
       - 1 fee payer
       - merkle modify
       - snark verify
     snapp = modify + snark verify + optional signature verify
     fee payer = modify + signature verify
  *)
...
(* TODO: Must include fee payer nonce in the payload *)
  ...
  | User_to_snapp
      ...
      let fee_payer_nonce_opt =
        ...
        | `Other {nonce; _} ->
            Some nonce
      in
      (* We don't really need both the user nonce and the fee payer nonce
         ever simultaneously, but it would be annoying to implement this twice
         with that being the only difference. *)
      return
        (One_snapp
           { tag
           ; fee= fee_payment.fee
