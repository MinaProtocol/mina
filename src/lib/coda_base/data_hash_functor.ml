(* Functors for parameterizing Data_hash over field *)
...
module Make_sigs (Impl : Tick_S) = struct
  open Impl

  module type Basic = sig
    include Data_hash_type_S with type t = Pedersen.Digest.t
...
(* Impl contains the field definition and related code
   Data_hash_type wraps that type in a versioned module 
 *)

module Make
    (Impl : Tick_S)
    (Data_hash_type : Data_hash_type_S with type t = Impl.Pedersen.Digest.t) =
struct
  open Impl

  module Make_basic (M : sig
    val length_in_bits : int
  end) : Make_sigs(Impl).Basic = struct
    include Data_hash_type
    ...
    let gen : Data_hash_type.t Quickcheck.Generator.t =
      let m =
  ...
  module Make_full_size () : Make_sigs(Impl).Full_size = struct
  ...
  module Make_small (M : sig
    val length_in_bits : int
  end) : Make_sigs(Impl).Small = struct
    let () = assert (M.length_in_bits < Field.size_in_bits)

    include (Make_basic (M) : Make_sigs(Impl).Basic)
