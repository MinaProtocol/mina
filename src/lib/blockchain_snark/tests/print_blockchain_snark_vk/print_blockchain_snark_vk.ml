...
let () =
  let vk =
    Async.Thread_safe.block_on_async_exn (fun () ->
        ...
        in
        let module Transaction_snark_instance =
        Transaction_snark.Make (struct
  ...
  in
  let filename =
    Format.sprintf "%s_blockchain_snark_vk.json.computed" Node_config.profile
  in
  Out_channel.write_all filename ~data:vk ;
  Format.printf "Wrote %s@." filename
