...
let rec to_vector : type a n. (a, n) t -> a Vector.e =
  function
  | [] -> T []
  | x :: xs ->
...
module Yojson (N : Nat.Intf) : Vector.Yojson_intf1 with type 'a t := ('a, N.n) t =
struct
...
module Binable (N : Nat.Intf) : Binable.S1 with type 'a t := ('a, N.n) t = 
  Binable.Of_binable1(List)(struct
...
module Sexpable (N : Nat.Intf) : Sexpable.S1 with type 'a t := ('a, N.n) t = 
  Sexpable.Of_sexpable1(List)(struct
  type nonrec 'a t = ('a, N.n) t
