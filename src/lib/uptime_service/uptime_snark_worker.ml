...
module Worker_state = struct
  ...
  let create ~constraint_constants ~signature_kind () : t Deferred.t =
    Deferred.return
      (let module M = struct
...
module Worker = struct
  module T = struct
    ...
    struct
      ...
      let init_worker_state (logger, constraint_constants, signature_kind) =
        [%log info] "Uptime SNARK worker started" ;
        Worker_state.create ~constraint_constants ~signature_kind ()
...
let create ~logger ~constraint_constants ~signature_kind ~pids : t Deferred.t =
  let on_failure err =
  ...
  let%map connection, process =
    Worker.spawn_in_foreground_exn ~connection_timeout:(Time.Span.of_min 1.)
      ...
      (logger, constraint_constants, signature_kind)
  in
  [%log info]
