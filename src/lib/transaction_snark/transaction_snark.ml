      ...
      ( if not Insecure.transaction_replay then
        ...
        let%bind root =
          ...
          Frozen_ledger_hash.modify_account_send root
            ...
            sender_compressed ~f:(fun ~account_empty account ->
              with_label __LOC__
                (let%bind next_nonce =
                 ...
                 in
                 let%bind delegate =
                   Public_key.Compressed.Checked.if_ account_empty
        ...
        let%map root =
          Frozen_ledger_hash.modify_account_recv root receiver
            ~f:(fun ~account_empty account ->
              let%map balance = Balance.Checked.(account.balance + amount)
              and delegate =
                Public_key.Compressed.Checked.if_ account_empty ~then_:receiver
