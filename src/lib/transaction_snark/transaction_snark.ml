...
let construct_input ~proof_type ~sok_digest ~state1 ~state2 ~supply_increase
  ...
  let init =
    ...
    | `Base ->
        Hash_prefix.Random_oracle.base_snark
    | `Merge wrap_vk_state ->
        wrap_vk_state
  in
...
let merge_top_hash wrap_vk_state =
  construct_input ~proof_type:(`Merge wrap_vk_state)
        ...
        (let%bind sok_digest =
         ...
         make_checked (fun () ->
             Random_oracle.Checked.(
               hash ~init:Hash_prefix.Random_oracle.base_snark
                 (pack_input input)) )
  ...
  let verify_transition tock_vk tock_vk_precomp wrap_vk_hash_state
    ...
    let%bind top_hash_init =
      hash_state_if is_base
        ~then_:
          (Random_oracle.State.map ~f:Run.Field.constant
             Hash_prefix.Random_oracle.base_snark)
    ...
    let%bind wrap_vk_hash_state =
      make_checked (fun () ->
          Random_oracle.(
            Checked.update
              ~state:
                (State.map Hash_prefix_states.Random_oracle.merge_snark
...
module Verification = struct
  ...
  struct
    ...
    let wrap_vk_state =
      Random_oracle.update ~state:Hash_prefix.Random_oracle.merge_snark
