...
let mk_ledgers_and_fee_payers ?(is_timed = false) ~num_of_fee_payers () =
  ...
  let (initial_balance : Currency.Balance.t) =
    Currency.Balance.mina_unsafe 1_000_000
  in
  let (fee_payer_accounts : Account.t array) =
    if is_timed then
      let initial_minimum_balance = Currency.Balance.mina_unsafe 1_000_000 in
      ...
      let vesting_increment = Currency.Amount.mina_unsafe 100 in
