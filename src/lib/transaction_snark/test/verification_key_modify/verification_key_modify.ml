  ...
  ( module struct
    ...
    module Deploy_account_update = struct
        ...
        ; update =
            { Account_update.Update.dummy with
              verification_key =
                Set
                  { data = vk
                  ; hash = Zkapp_account.digest_vk vk
                  }
            ; permissions =
                Set
                  { edit_state = Proof
      ...
      let account_update (vk : Side_loaded_verification_key.t) :
          Account_update.t =
        Account_update.with_aux ~body:(account_update_body vk)
    ...
    let test_zkapp_command ?expected_failure zkapp_commands =
      let fix_command nonce zkapp_command : Zkapp_command.t =
          ...
          Zkapp_command.Transaction_commitment.create_complete
            ...
            ~fee_payer_hash:
              (Zkapp_command.Call_forest.Digest.Account_update.create
                 ~signature_kind (Account_update.of_fee_payer fee_payer) )
        in
              ...
              ~f:(fun (account_update : Account_update.t) ->
                ...
                | { body = { public_key; use_full_commitment; _ }
                  ...
                  when Public_key.Compressed.equal public_key pk_compressed ->
                    ...
                    in
                    Account_update.with_aux ~body:account_update.body
                      ~authorization:
      ...
      in
      let zkapp_commands =
        List.mapi zkapp_commands ~f:(fun i cmd ->
            fix_command (Mina_numbers.Account_nonce.of_int i) cmd )
      in
    ...
    let%test_unit "Verification key swapped, later account updates bad, \
      ...
      let _account =
        test_zkapp_command
          ~expected_failure:
            ( Transaction_status.Failure.Unexpected_verification_key_hash
            , Pass_2 )
          [ []
            |> Zkapp_command.Call_forest.cons_tree
                 Trivial_account_update1.account_update
    ...
    (* Test requested by deepthi: account already has a zkapp deployed,
       then a second transaction uses it, swaps vk, and uses the new one. *)
