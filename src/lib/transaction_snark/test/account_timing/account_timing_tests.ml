...
(* Basic timing check tests *)

let before_cliff_time () =
...
let positive_min_balance () =
  let account =
    account_with_default_vesting_schedule (Balance.of_mina_int_exn 100_000)
  in
  let txn_amount = Currency.Amount.of_mina_int_exn 100 in
  let txn_global_slot = Mina_numbers.Global_slot_since_genesis.of_int 1_900 in
  let timing_with_min_balance =
...
let curr_min_balance_of_zero () =
  ...
  in
  let txn_amount = Currency.Amount.of_mina_int_exn 100 in
  let txn_global_slot = Mina_numbers.Global_slot_since_genesis.of_int 2_000 in
  let timing_with_min_balance =
    validate_timing_with_min_balance ~txn_amount ~txn_global_slot ~account
  in
  match timing_with_min_balance with
...
let below_calculated_min_balance () =
  let account =
  ...
  let txn_global_slot = Mina_numbers.Global_slot_since_genesis.of_int 1_010 in
  ...
  | Error err ->
      ...
      assert (checked_timing_should_fail account txn_amount txn_global_slot)
  | _ ->
      assert false
...
let past_full_vesting () =
  let account =
    account_with_default_vesting_schedule (Balance.of_mina_int_exn 100_000)
  in
  let txn_amount = Currency.Amount.of_mina_int_exn 100_000 in
  let txn_global_slot = Mina_numbers.Global_slot_since_genesis.of_int 3000 in
  let timing_with_min_balance =
    validate_timing_with_min_balance ~txn_amount ~txn_global_slot ~account
  in
  match timing_with_min_balance with
...
let make_cliff_amount_test slot =
  ...
  in
  let txn_amount = Currency.Amount.of_mina_int_exn 100_000 in
  let txn_global_slot = Mina_numbers.Global_slot_since_genesis.of_int slot in
  (txn_amount, txn_global_slot, account)

let before_cliff_amount_doesnt_affect_min_balance () =
...
let at_cliff_time_cliff_amount_allows_spending () =
...
let check_transaction_snark
  ...
  let coinbase_stack_target =
    let stack_with_state =
      Pending_coinbase.Stack.(
        push_state state_body_hash txn_global_slot Pending_coinbase.Stack.empty)
    in
    match transaction with
    | Coinbase c ->
  ...
  Transaction_snark.check_transaction ~constraint_constants ~sok_message
    ...
    ~pending_coinbase_stack_state:
      { source = Pending_coinbase.Stack.empty; target = coinbase_stack_target }
    ~supply_increase
    ...
    ( List.map txns ~f:(fun txn ->
          ...
          | Ok txn_applied ->
              ...
              | Applied -> (
                  ...
                  | Some failure ->
                      failwithf
                        "Transaction applied without failures but expected to \
                         failwith with %s"
              ...
              | Failed failuress -> (
                  ...
                  | None ->
                      failwithf
                        "Transaction expected to be applied successfully but \
                         failed with %s"
                        failures_str () ) ) ;
...
let user_commands_before_cliff_time_sufficient_balance () =
  let gen =
    ...
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
          ...
          let (timing : Account_timing.t) =
            Timed
              { initial_minimum_balance = Currency.Balance.of_mina_int_exn 50
              ; cliff_time =
    ...
    ~trials:2 gen ~f:(fun (ledger_init_state, user_commands) ->
      Mina_ledger.Ledger.with_ephemeral_ledger
        ~depth:constraint_constants.ledger_depth ~f:(fun ledger ->
          Mina_ledger.Ledger.apply_initial_ledger_state ledger ledger_init_state ;
          apply_user_commands_at_slot ledger ~constraint_constants
            Mina_numbers.Global_slot_since_genesis.(succ zero)
            user_commands ) )

let user_command_before_cliff_time_min_balance_violation () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
          ...
          let (timing : Account_timing.t) =
            Timed
              { initial_minimum_balance = Currency.Balance.of_mina_int_exn 9_995
              ; cliff_time =
                  Mina_numbers.Global_slot_since_genesis.of_int 10_000
              ; cliff_amount = Currency.Amount.of_nanomina_int_exn 100
              ; vesting_period = Mina_numbers.Global_slot_span.of_int 1
  ...
  in
  Quickcheck.test ~seed:(`Deterministic "user command, before cliff") ~trials:1
...
let user_command_just_before_cliff_time_insufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
  ...
  Quickcheck.test ~seed:(`Deterministic "user command, just before cliff")
    ~trials:1
    ~sexp_of:[%sexp_of: Mina_ledger.Ledger.init_state * Signed_command.t] gen
      ...
      Mina_ledger.Ledger.with_ephemeral_ledger
        ~depth:constraint_constants.ledger_depth ~f:(fun ledger ->
          ...
          apply_user_commands_at_slot ledger ~constraint_constants
            ...
            ~expected_rejection:true
            [ Mina_transaction.Transaction.Command (Signed_command user_command)
            ] ) )

let user_command_at_cliff_time_sufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
          ...
          let (timing : Account_timing.t) =
            Timed
              { initial_minimum_balance = Currency.Balance.of_mina_int_exn 9_995
              ; cliff_time = Mina_numbers.Global_slot_since_genesis.of_int 10000
              ; cliff_amount = Currency.Amount.of_mina_int_exn 9_995
              ; vesting_period = Mina_numbers.Global_slot_span.of_int 1
              ; vesting_increment = Currency.Amount.of_nanomina_int_exn 10
              }
  ...
  Quickcheck.test ~seed:(`Deterministic "user command, at cliff") ~trials:1
    ~sexp_of:
      [%sexp_of: Mina_ledger.Ledger.init_state * Mina_transaction.Transaction.t]
    gen ~f:(fun (ledger_init_state, user_command) ->
      Mina_ledger.Ledger.with_ephemeral_ledger
        ~depth:constraint_constants.ledger_depth ~f:(fun ledger ->
          Mina_ledger.Ledger.apply_initial_ledger_state ledger ledger_init_state ;
...
let user_command_while_vesting_sufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    ...
    (ledger_init_state, user_command)
  in
  Quickcheck.test ~seed:(`Deterministic "user command, while vesting") ~trials:1
    ~sexp_of:
      [%sexp_of: Mina_ledger.Ledger.init_state * Mina_transaction.Transaction.t]
    gen ~f:(fun (ledger_init_state, user_command) ->
      Mina_ledger.Ledger.with_ephemeral_ledger
...
let user_command_after_vesting_sufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
    ...
    (ledger_init_state, user_command)
  in
  Quickcheck.test ~seed:(`Deterministic "user command, after vesting") ~trials:1
    ~sexp_of:
      [%sexp_of: Mina_ledger.Ledger.init_state * Mina_transaction.Transaction.t]
    gen ~f:(fun (ledger_init_state, user_command) ->
      Mina_ledger.Ledger.with_ephemeral_ledger
...
let user_command_after_vesting_insufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
...
let payment_fee_more_than_available_min_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
...
let payment_amount_more_than_available_min_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
...
let payment_fee_payer_timed_to_untimed_receiver_untimed () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
...
let payment_receiver_timed_to_untimed_fee_payer_untimed () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    ...
    ~seed:
      (`Deterministic
        "Payment- sufficient amount; receiver goes from timed to untimed; fee \
         payer remains untimed" ) ~trials:1 gen
    ~f:(fun (ledger_init_state, user_command) ->
      Mina_ledger.Ledger.with_ephemeral_ledger
        ~depth:constraint_constants.ledger_depth ~f:(fun ledger ->
          Mina_ledger.Ledger.apply_initial_ledger_state ledger ledger_init_state ;
...
(* Generic user transaction test module *)

module Generic_timed_account_test = struct
  type t =
  ...
  let gen ?balance ?init_min_bal ?cliff_time ?cliff_amt ?vest_period ?vest_incr
      ?slot ?amount () : t Quickcheck.Generator.t =
  ...
  let expected_failure { available_funds; balance; cmd; _ } =
    ...
    let amount = Option.value ~default:zero @@ Signed_command.amount cmd in
    let fee =
  ...
  let execute_test (input : t) =
    ...
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
          ...
          in
          (keypair, Currency.Balance.to_amount input.balance, nonce, timing) )
      |> Array.of_list
    in
    Mina_ledger.Ledger.with_ephemeral_ledger
      ~depth:constraint_constants.ledger_depth ~f:(fun ledger ->
        ...
        | expected, Ok txn_applied -> (
            ...
            | Applied ->
                failwith "Transaction succeeded where it was expected to fail."
            | Failed failuress ->
                let failure_str =
                  List.map (List.concat failuress) ~f:(fun failure ->
                      Transaction_status.Failure.to_string failure )
        ...
        | expected, Error err ->
            ...
            if not (String.equal err_str expected_err_str) then
              failwithf "Unexpected transaction error: %s.\nExpected error: %s"
                err_str expected_err_str () )
end

let generic_user_transaction () =
  Quickcheck.test ~seed:(`Deterministic "generic, timed account")
    ~sexp_of:Generic_timed_account_test.sexp_of_t
    ~examples:Generic_timed_account_test.examples ~trials:1000
    (Generic_timed_account_test.gen ())
    ~f:Generic_timed_account_test.execute_test
...
let check_zkapp_failure expected_failure = function
  | Ok
      ...
      , ( (local_state :
        ...
        , _amount ) ) -> (
      let failure_statuses = local_state.failure_status_tbl |> List.concat in
      ...
      | Failed failuress ->
          ...
          then
            failwithf
              "Got unxpected transaction failure(s): %s, expected failure: %s"
...
let zkapp_command_before_cliff_time_sufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
...
let zkapp_command_before_cliff_time_min_balance_violation () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
        ...
        ~depth:constraint_constants.ledger_depth ~f:(fun ledger ->
          ...
          let result =
            Mina_ledger.Ledger.apply_zkapp_command_unchecked ~signature_kind
              ...
              ~state_view:Transaction_snark_tests.Util.genesis_state_view ledger
              zkapp_command
          in
          check_zkapp_failure
            Transaction_status.Failure.Source_minimum_balance_violation result ) )

let zkapp_command_before_cliff_time_fee_payer_fails () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
...
let gen_untimed_account_and_create_timed_account ~balance ~min_balance =
  ...
  let (create_timed_account_spec
    ...
    ; memo =
        Signed_command_memo.create_from_string_exn "zkApp create timed account"
        ...
        (let timing =
           Zkapp_basic.Set_or_keep.Set
             ( { initial_minimum_balance =
               ...
               ; cliff_time = Mina_numbers.Global_slot_since_genesis.of_int 1000
               ; cliff_amount = Currency.Amount.of_nanomina_int_exn 100_000_000
               ; vesting_period = Mina_numbers.Global_slot_span.of_int 10
               ; vesting_increment =
...
let zkapp_command_timed_account_creation_min_balance_gt_balance () =
  Quickcheck.test
    ~seed:
      (`Deterministic
...
let zkapp_command_account_creation_min_balance_eq_balance () =
  Quickcheck.test
    ~seed:
      (`Deterministic "zkApp command, account creation, min_balance = balance")
    ~sexp_of:[%sexp_of: Mina_ledger.Ledger.init_state * Zkapp_command.t]
    ~trials:1
    (gen_untimed_account_and_create_timed_account ~balance:100_000_000_000_000
       ~min_balance:100_000_000_000_000 )
    ~f:(fun (ledger_init_state, zkapp_command) ->
      Mina_ledger.Ledger.with_ephemeral_ledger
        ~depth:constraint_constants.ledger_depth ~f:(fun ledger ->
...
let zkapp_command_account_creation_min_balance_lt_balance () =
  Quickcheck.test
    ~seed:
      (`Deterministic "zkapp command, account creation, min_balance < balace")
    ~sexp_of:[%sexp_of: Mina_ledger.Ledger.init_state * Zkapp_command.t]
    ~trials:1
    (gen_untimed_account_and_create_timed_account ~balance:150_000_000_000_000
       ~min_balance:100_000_000_000_000 )
    ~f:(fun (ledger_init_state, zkapp_command) ->
      Mina_ledger.Ledger.with_ephemeral_ledger
        ~depth:constraint_constants.ledger_depth ~f:(fun ledger ->
...
let zkapp_command_just_before_cliff_time_insufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
      ...
      Mina_ledger.Ledger.with_ephemeral_ledger
        ~depth:constraint_constants.ledger_depth ~f:(fun ledger ->
          ...
          match
            Mina_ledger.Ledger.apply_zkapp_command_unchecked ~signature_kind
              ~constraint_constants
              ~global_slot:Mina_numbers.Global_slot_since_genesis.(of_int 9999)
              ~state_view:Transaction_snark_tests.Util.genesis_state_view ledger
              zkapp_command
          with
          | Ok _txn_applied ->
...
let zkapp_command_at_cliff_time_sufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let ledger_init_state =
      List.map keypairs ~f:(fun keypair ->
...
let zkapp_command_while_vesting_sufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
      ...
      List.map keypairs ~f:(fun keypair ->
          ...
          let (timing : Account_timing.t) =
            Timed
              ...
              ; cliff_amount = Currency.Amount.zero
              ; vesting_period = Mina_numbers.Global_slot_span.of_int 1
              ; vesting_increment = Currency.Amount.of_nanomina_int_exn 100_000
              }
          in
          let balance_as_amount = Currency.Balance.to_amount balance in
          (keypair, balance_as_amount, nonce, timing) )
...
let zkapp_command_while_vesting_insufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let balance_int = 100_000_000_000_000 in
    let init_min_balance_int = 100_000_000_000_000 in
...
let zkapp_command_after_vesting_sufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let balance_int = 100_000_000_000_000 in
    let init_min_balance_int = 100_000_000_000_000 in
    ...
    ~trials:1 gen ~f:(fun (ledger_init_state, zkapp_command) ->
      Mina_ledger.Ledger.with_ephemeral_ledger
        ~depth:constraint_constants.ledger_depth ~f:(fun ledger ->
          Mina_ledger.Ledger.apply_initial_ledger_state ledger ledger_init_state ;
          apply_zkapp_commands_at_slot ledger
            Mina_numbers.Global_slot_since_genesis.(of_int (100_000 + 10_000))
            [ zkapp_command ] ) )

let zkapp_command_after_vesting_insufficient_balance () =
  let gen =
    let open Quickcheck.Generator.Let_syntax in
    let balance_int = 100_000_000_000_000 in
    let init_min_balance_int = 100_000_000_000_000 in
...
let zkapp_command_create_timed_account_wrong_authorization () =
  let ledger_init_state =
    List.map keypairs ~f:(fun keypair ->
  ...
  Async.Thread_safe.block_on_async_exn (fun () ->
      Async.Quickcheck.async_test
        ...
        ~f:(fun (ledger_init_state, create_timed_account_zkapp_command) ->
          ...
          Transaction_snark_tests.Util.check_zkapp_command_with_merges_exn
            ~expected_failure:
              (Transaction_status.Failure.Update_not_permitted_timing, Pass_2)
            ledger
            [ create_timed_account_zkapp_command ] ) )

let zkapp_command_change_untimed_to_timed () =
  Async.Thread_safe.block_on_async_exn (fun () ->
      ...
      let sender_keypair = List.hd_exn keypairs in
      let zkapp_keypair = List.nth_exn keypairs 1 in
      let (update_timing_spec : Transaction_snark.For_tests.Update_states_spec.t)
          =
        { sender = (sender_keypair, Account.Nonce.zero)
        ; fee = Currency.Fee.of_nanomina_int_exn 1_000_000
        ; fee_payer = None
...
let zkapp_command_invalid_update_for_timed_account () =
  Async.Thread_safe.block_on_async_exn (fun () ->
      let ledger_init_state =
        List.mapi keypairs ~f:(fun i keypair ->
            let balance = Currency.Amount.of_mina_int_exn 100_000 in
