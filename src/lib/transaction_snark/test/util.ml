    ...
    let random_wallet () : t =
      ...
      ; account =
          Account.create account_id
            (Balance.mina_of_int_exn (50 + Random.int 100))
      }
    in
  ...
  let user_command ~fee_payer ~receiver_pk amt fee nonce memo =
    ...
    let payload : Signed_command.Payload.t =
      Signed_command.Payload.create ~fee
        ...
        ~body:
          (Payment
             { source_pk; receiver_pk; amount = Amount.nanomina_of_int_exn amt }
          )
    in
...
let check_balance pk balance ledger =
  ...
  [%test_eq: Balance.t] acc.balance (Balance.nanomina_of_int_exn balance)
