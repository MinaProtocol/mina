...
let make_chain_id ~signature_kind ~(genesis_constants : Genesis_constants.t)
...
module Genesis_proof = struct
  ...
  let generate_inputs ~runtime_config ~proof_level ~ledger ~genesis_epoch_data
    ...
    let protocol_state_with_hashes, chain_id =
      ...
      in
      let chain_id =
        make_chain_id ~signature_kind:Mina_signature_kind.t_DEPRECATED
...
let init_from_config_file ~cli_proof_level ~genesis_constants
    ...
    (config : Runtime_config.t) :
    (Precomputed_values.t * string) Deferred.Or_error.t =
