...
let copy_file ~logger ~genesis_id ~source ~target ~filename =
  let source_file = source ^ "/" ^ filename ^ "." ^ genesis_id in
  let target_file = target ^ "/" ^ filename in
  match%map
...
let retrieve_genesis_state dir_opt ~logger ~conf_dir ~daemon_conf :
  ...
  let genesis_id = Cache_dir.genesis_dir_name Genesis_constants.compiled in
  let tar_filename = genesis_id ^ ".tar.gz" in
  let proof_filename = proof_filename_root ^ "." ^ genesis_id in
  let constants_filename = constants_filename_root ^ "." ^ genesis_id in
  ...
  let extract_tar_file ~tar_dir ~target =
    match%map
      Monitor.try_with_or_error ~extract_exn:true (fun () ->
          ...
          let tar_file = tar_dir ^/ genesis_id ^ ".tar.gz" in
          let%map _result =
    ...
    | Ok () ->
        Logger.info ~module_:__MODULE__ ~location:__LOC__ logger
          ...
          ~metadata:[("source", `String tar_dir); ("path", `String target)]
    | Error e ->
        Logger.debug ~module_:__MODULE__ ~location:__LOC__ logger
  ...
  let retrieve_genesis_data tar_dir =
    ...
    let target = conf_dir ^/ genesis_id in
    let%bind () = extract_tar_file ~tar_dir ~target in
    let%bind () =
      copy_file ~logger ~genesis_id ~source:tar_dir ~target
        ~filename:proof_filename_root
    in
    let%bind () =
      copy_file ~logger ~genesis_id ~source:tar_dir ~target
        ~filename:constants_filename_root
    in
    let ledger_dir = target ^/ ledger_subdir in
    let proof_file = target ^/ proof_filename_root in
    let constants_file = target ^/ constants_filename_root in
    if
      Core.Sys.(
...
let download_proof_from_s3_exn ~logger =
  (* genesis_id.ml is generated by building runtime_genesis_ledger, then
     running the script genesis_id_for_nonconsensus.py
  *)
  let proof_filename = proof_filename_root ^ "." ^ Genesis_id.genesis_id in
...
let retrieve_genesis_proof dir_opt ~logger ~conf_dir : Proof.t Deferred.t =
  let genesis_id = Genesis_id.genesis_id in
  let retrieve_genesis_data src_dir =
    Logger.debug ~module_:__MODULE__ ~location:__LOC__ logger
      "Retrieving genesis proof from $path"
      ~metadata:[("path", `String src_dir)] ;
    let extract_target = conf_dir ^/ genesis_id in
    let%bind () =
      copy_file ~logger ~filename:proof_filename_root ~genesis_id ~src_dir
        ~extract_target
    in
