...
let create_expected_statement ~constraint_constants
  ...
  let first_pass_source_merkle_root =
    Frozen_ledger_hash.of_ledger_hash
    @@ Sparse_ledger.merkle_root first_pass_ledger_witness
  in
  let second_pass_source_merkle_root =
    Frozen_ledger_hash.of_ledger_hash
  ...
  let%bind after, applied_transaction =
    ...
    |> Or_error.join
  in
  let first_pass_target_merkle_root =
    Sparse_ledger.merkle_root after |> Frozen_ledger_hash.of_ledger_hash
  in
  let second_pass_target_merkle_root = second_pass_source_merkle_root in
  ...
  { Transaction_snark.Statement.source =
      { first_pass_ledger = first_pass_source_merkle_root
      ; second_pass_ledger = second_pass_source_merkle_root
      ; pending_coinbase_stack = statement.source.pending_coinbase_stack
      ; local_state = empty_local_state
      }
  ; target =
      { first_pass_ledger = first_pass_target_merkle_root
      ; second_pass_ledger = second_pass_target_merkle_root
    ...
    let check_registers (reg1 : _ Mina_state.Registers.t)
      ...
      let%map () =
        clarify_error
          ...
          "did not connect with first pass snarked ledger hash"
      and () =
        clarify_error
          (Frozen_ledger_hash.equal reg1.second_pass_ledger
             reg2.second_pass_ledger )
          "did not connect with second pass snarked ledger hash"
      and () =
        clarify_error
