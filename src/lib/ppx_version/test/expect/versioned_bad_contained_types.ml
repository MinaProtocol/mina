...
module Foo = struct
  include struct
    module Stable = struct
      module V1 = struct
        type t = int List.t [@@deriving bin_io, version]
          ...
          let _group =
            Bin_prot.Shape.group
              (Bin_prot.Shape.Location.of_string
                 "test/versioned_bad_contained_types.ml:8:6" )
              [ ( Bin_prot.Shape.Tid.of_string "t"
                , []
                , List.bin_shape_t bin_shape_int )
              ]
          in
        ...
        let (bin_size_t : t Bin_prot.Size.sizer) =
         fun v -> List.bin_size_t bin_size_int v

        let _ = bin_size_t

        let (bin_write_t : t Bin_prot.Write.writer) =
         fun buf ~pos v -> (List.bin_write_t bin_write_int) buf ~pos v
        ...
        let (__bin_read_t__ : (int -> t) Bin_prot.Read.reader) =
         fun buf ~pos_ref vint ->
          (List.__bin_read_t__ bin_read_int) buf ~pos_ref vint

        let _ = __bin_read_t__

        let (bin_read_t : t Bin_prot.Read.reader) =
         fun buf ~pos_ref -> (List.bin_read_t bin_read_int) buf ~pos_ref
          ...
          let (__bin_read_t__ : (int -> t) Bin_prot.Read.reader) =
           fun _buf ~pos_ref _vint ->
            Bin_prot.Common.raise_variant_wrong_type
              "test/versioned_bad_contained_types.ml.Foo.Stable.V1.With_version.t"
              !pos_ref
