          ...
          let (__bin_read_t__ : (int -> t) Bin_prot.Read.reader) =
           fun _buf ~pos_ref _vint ->
            Bin_prot.Common.raise_variant_wrong_type
              "test/good_version_syntax.ml.M1.Stable.V1.With_version.t" !pos_ref
  ...
  module V1 = struct
    module T = struct
        ...
        let _group =
          Bin_prot.Shape.group
            ...
            [ ( Bin_prot.Shape.Tid.of_string "query"
              , []
              , Core_kernel.Int.Stable.V1.bin_shape_t )
            ]
        in
        (Bin_prot.Shape.top_app _group (Bin_prot.Shape.Tid.of_string "query"))
          []
      ...
      let bin_writer_query =
        { Bin_prot.Type_class.size = bin_size_query; write = bin_write_query }
