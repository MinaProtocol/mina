...
let make_type_decl_no_op name ?str_args ?str_rw ?sig_args ?sig_rw () =
  ...
  | None, None ->
      failwith
        "Expected args and generator for either a structure, signature, or both"
...
let make_type_ext_no_op name ?str_args ?str_rw ?sig_args ?sig_rw () =
  ...
  | None, None ->
      failwith
        "Expected args and rewriter for either a structure, signature, or both"
...
let register_dummies () =
  let register_dummy_type_decl_derivers () =
    let derivers =
      [ "dhall_type"; "hlist"; "to_enum"; "to_representatives"; "annot" ]
    in
    (* for structures only, not for signatures *)
    Core_kernel.List.iter derivers ~f:(fun name ->
        make_type_decl_no_op name ~str_args:Args.empty ~str_rw:type_decl_rw0 () )
  in
  let register_dummy_type_ext_derivers () =
    ...
    make_type_ext_no_op "register_event" ~str_args:register_event_arg
      ~str_rw:type_ext_rw1 ~sig_args:Args.empty ~sig_rw:type_ext_rw0 ()
  in
