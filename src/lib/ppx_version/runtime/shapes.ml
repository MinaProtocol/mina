...
open Core
module Shape_tbl = Hashtbl.Make (String)

let shape_tbl : (Bin_prot.Shape.t * string) Shape_tbl.t = Shape_tbl.create ()

let find path_to_type = Hashtbl.find shape_tbl path_to_type

let iteri ~f = Hashtbl.iteri shape_tbl ~f
...
let register path_to_type (shape : Bin_prot.Shape.t) (ty_decl : string) =
  match Hashtbl.add shape_tbl ~key:path_to_type ~data:(shape, ty_decl) with
  | `Ok ->
      ()
  | `Duplicate -> (
