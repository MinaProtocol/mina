module Job : sig
  type ('key, 'attempt, 'a) t =
    ...
    ; res :
        ( 'a Network_peer.Envelope.Incoming.t * 'attempt Network_peer.Peer.Map.t
        , [ `Finished ] )
        Core.Result.t
        Async.Ivar.t
    }
  ...
  val result :
       ('a, 'b, 'c) t
    -> ( 'c Network_peer.Envelope.Incoming.t * 'b Network_peer.Peer.Map.t
       , [ `Finished ] )
       Core.Result.t
       Async_kernel__.Deferred0.t
end

type 'a pred = 'a -> bool

val sexp_of_pred :
  ('a -> Ppx_sexp_conv_lib.Sexp.t) -> 'a pred -> Ppx_sexp_conv_lib.Sexp.t

val pred_to_yojson : 'a -> 'b -> [> `String of string ]

val sexp_opaque_to_yojson : 'a -> 'b -> [> `String of string ]

module Claimed_knowledge : sig
  type 'key t = [ `All | `Call of 'key pred | `Some of 'key list ]

  val sexp_of_t :
    ('key -> Ppx_sexp_conv_lib.Sexp.t) -> 'key t -> Ppx_sexp_conv_lib.Sexp.t

  val to_yojson : ('a -> Yojson.Safe.t) -> 'a t -> Yojson.Safe.t

  val check :
       equal:('a -> 'a -> bool)
    -> [< `All | `Call of 'a -> bool | `Some of 'a list ]
    -> 'a
    -> bool
end
...
module Make : functor
  ...
  module Job : sig
    ...
    val result :
         t
      -> ( Result.t Network_peer.Envelope.Incoming.t
           * Attempt.t Network_peer.Peer.Map.t
         , [ `Finished ] )
         Base.Result.t
         Async.Deferred.t
  end
  ...
  val download :
    t -> key:Key.t -> attempts:Attempt.t Network_peer.Peer.Map.t -> Job.t
  ...
  val update_knowledge :
    t -> Network_peer.Peer.t -> Key.t Claimed_knowledge.t -> unit
