...
let run_cycle ~context:(module Context : CONTEXT) ~trust_system ~verifier
  ...
  *)
  ...
  let%bind ( staged_ledger_data_download_time
    ...
    let target_peer_id =
      match Sys.getenv "MINA_DEBUG_STAGED_LEDGER_PEER_ID" with
      | Some override_peer_id ->
          [%log debug]
            "Overriding staged_ledger RPC peer_id from environment variable"
            ~metadata:
              [ ("override_peer_id", `String override_peer_id)
              ; ("original_peer_id", `String (Peer.Id.to_string sender.peer_id))
              ] ;
          Peer.Id.unsafe_of_string override_peer_id
      | None ->
          sender.peer_id
    in
    [%log debug]
      ...
      ~metadata:
        [ ("peer_id", `String (Peer.Id.to_string target_peer_id))
