...
let get_status ~flag t =
  ...
  let config = Coda_lib.config t in
  let protocol_config = config.runtime_config.protocol in
  let consensus_constants = Consensus.Constants.create ~protocol_config in
  let uptime_secs =
  ...
  in
  let consensus_configuration = Consensus.Configuration.t ~protocol_config in
