    ...
    let to_account (a : t) : Mina_base.Account.t =
      ...
      let permissions =
        ...
        Mina_base.Permissions.Poly.Stable.V2.
          ...
          ; set_token_symbol = Json_layout.Accounts.Single.Permissions.Auth_required.to_account_perm perms.set_token_symbol
          ; increment_nonce = Json_layout.Accounts.Single.Permissions.Auth_required.to_account_perm perms.increment_nonce
          ; set_voting_for = Json_layout.Accounts.Single.Permissions.Auth_required.to_account_perm perms.set_voting_for
          ; set_timing = Json_layout.Accounts.Single.Permissions.Auth_required.to_account_perm perms.set_timing
          }
      in
      let mk_zkapp (app : Zkapp_account.t) : (Mina_base__.Zkapp_state.Value.Stable.V1.t,
            Mina_base__.Verification_key_wire.Stable.V1.t option,
            Zkapp_account.Zkapp_version.t, Zkapp_account.Field.t,
        ...
        ; verification_key = Option.map ~f:With_hash.(of_data ~hash_data) app.verification_key
        ; zkapp_version = app.zkapp_version
        ; action_state = Pickles_types.Vector.Vector_5.of_list_exn app.action_state
        ; last_action_slot = 
      ...
      in
      { public_key =
          Public_key.Compressed.of_base58_check_exn a.pk
      ; token_id =
