...
module Make (Inputs : Inputs_intf) :
  ...
  module Merkle_list_prover = Merkle_list_prover.Make_ident (struct
    ...
    let get_previous ~context valid_block =
      valid_block
      ...
      |> Transition_frontier.find context
      |> Option.map ~f:Transition_frontier.Breadcrumb.validated_block
  end)
  ...
  let prove ~logger frontier =
    ...
    let validated_best_tip =
      Transition_frontier.Breadcrumb.validated_block best_tip_breadcrumb
    in
    let best_tip = Mina_block.Validated.forget validated_best_tip in
    let root = Transition_frontier.Breadcrumb.validated_block @@ Transition_frontier.root frontier in
    let _, merkle_list =
      Merkle_list_prover.prove ~context:frontier validated_best_tip
    in
    [%log debug]
      ~metadata:
        [ ( "merkle_list"
    ...
    Proof_carrying_data.
      { data = best_tip
      ; proof =
          (merkle_list, With_hash.data @@ Mina_block.Validated.forget root )
      }

  let validate_proof ~verifier ~genesis_state_hash (block : Mina_block.with_hash) : Mina_block.initial_valid_block Deferred.Or_error.t =
    let open Mina_block in
    let%map validation =
      let open Deferred.Result.Let_syntax in
      Validation.wrap block
      |> Validation.skip_time_received_validation `This_block_was_not_received_via_gossip
      |> Validation.skip_delta_block_chain_validation `This_block_was_not_received_via_gossip
      |> Fn.compose Deferred.return @@ Validation.validate_protocol_versions
      >>= Fn.compose Deferred.return @@ Validation.validate_genesis_protocol_state ~genesis_state_hash
      >>= Validation.validate_single_proof ~verifier
    in
    ...
    | Error err ->
        Or_error.error_string (
          match err with
          | `Invalid_genesis_protocol_state -> "invalid genesis state"
          | `Invalid_protocol_version | `Mismatched_protocol_version -> "invalid protocol version"
          | `Invalid_proof -> "invalid proof"
          | `Verifier_error e -> Printf.sprintf "verifier error: %s" (Error.to_string_hum e))
  ...
  let verify ~verifier ~genesis_constants ~precomputed_values
    ...
    let best_tip_with_hash = Mina_block.wrap_with_hash best_tip in
    let root_with_hash = Mina_block.wrap_with_hash root in
    let root_is_genesis = State_hash.(With_hash.hash root_with_hash = genesis_state_hash) in
    let%bind () =
      Deferred.return (
        Result.ok_if_true
           ~error:
    ...
    let%bind (_ : State_hash.t Non_empty_list.t) =
      Deferred.return (
        Result.of_option
           (Merkle_list_verifier.verify ~init:root_with_hash.hash
    ...
    let%map root, best_tip =
      Deferred.Or_error.both
        (validate_proof ~verifier ~genesis_state_hash root_with_hash)
