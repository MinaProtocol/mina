  ...
  | Some loc -> (
      ...
      | Some account ->
          ...
          if b then
            ...
            let%bind (predicate_account : Zkapp_predicate.Account.t) =
              let%bind balance =
                ...
                in
                Or_ignore.gen
                  (return { Zkapp_predicate.Closed_interval.lower; upper })
              in
              let%bind nonce =
              ...
              in
              return
                { Zkapp_predicate.Account.Poly.balance
                ; nonce
                ; receipt_chain_hash
                ; public_key
                ; delegate
            ...
            if succeed then return (Party.Predicate.Full predicate_account)
              ...
              let%bind faulty_predicate_account =
                ...
                | Balance ->
                    ...
                    let balance =
                      Or_ignore.Check
                        { Zkapp_predicate.Closed_interval.lower = new_balance
                        ; upper = new_balance
                        }
                    in
                ...
                | Nonce ->
                    ...
                    in
                    let%bind nonce =
                      Zkapp_predicate.Numeric.gen (return new_nonce)
...
let closed_interval_exact value =
  Zkapp_predicate.Closed_interval.{ lower = value; upper = value }
...
let gen_epoch_data_predicate
    (epoch_data :
      ...
      Zkapp_predicate.Protocol_state.Epoch_data.Poly.t) :
    Zkapp_predicate.Protocol_state.Epoch_data.t Base_quickcheck.Generator.t =
  let open Quickcheck.Let_syntax in
  let%bind ledger =
    let%bind hash =
...
let gen_protocol_state_predicate (psv : Zkapp_predicate.Protocol_state.View.t) :
    Zkapp_predicate.Protocol_state.t Base_quickcheck.Generator.t =
  let open Quickcheck.Let_syntax in
  let%bind snarked_ledger_hash =
    Zkapp_basic.Or_ignore.gen @@ return psv.snarked_ledger_hash
  in
  let%bind timestamp =
    Zkapp_predicate.Closed_interval.
  ...
  *)
  ...
  let%bind protocol_state =
    Option.value_map protocol_state_view ~f:gen_protocol_state_predicate
      ~default:(return Zkapp_predicate.Protocol_state.accept)
  in
...
let gen_parties_from ?(succeed = true)
  ...
  *)
  ...
  let protocol_state_predicate_hash =
    Zkapp_predicate.Protocol_state.digest
