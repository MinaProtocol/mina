  ...
  let%bind global_slot_since_genesis =
    ...
    let%bind epsilon1 =
      Global_slot_since_genesis.gen_incl
        (Global_slot_since_genesis.of_int 0)
...
let gen_invalid_protocol_state_precondition
  ...
  | Global_slot_since_genesis ->
      ...
      let%map global_slot_since_genesis =
        let%map epsilon =
          Global_slot_since_genesis.(gen_incl (of_int 1) (of_int 10))
        in
        if
          lower
          || Global_slot_since_genesis.(psv.global_slot_since_genesis > epsilon)
        then
