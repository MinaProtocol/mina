...
module Generator = struct
  ...
  let fresh_peer ~genesis_ledger ~base_proof ~genesis_constants
      ~max_frontier_length =
    ...
    let%map frontier =
      Transition_frontier.For_tests.gen ~genesis_ledger ~base_proof
        ~genesis_constants ~consensus_local_state
        ~max_length:max_frontier_length ~size:0 ()
    in
    {frontier; consensus_local_state}

  let peer_with_branch ~frontier_branch_size ~genesis_ledger ~base_proof
      ~genesis_constants ~max_frontier_length =
    let consensus_local_state =
      Consensus.Data.Local_state.create Public_key.Compressed.Set.empty
        ~genesis_ledger:(Genesis_ledger.Packed.t genesis_ledger)
    in
    let%map frontier, branch =
      Transition_frontier.For_tests.gen_with_branch ~genesis_ledger ~base_proof
        ~genesis_constants ~max_length:max_frontier_length ~frontier_size:0
  ...
  let gen ~genesis_ledger ~base_proof ~genesis_constants ~max_frontier_length
      configs =
    let open Quickcheck.Generator.Let_syntax in
    let%map states =
      Vect.Quickcheck_generator.map configs ~f:(fun config ->
          config ~genesis_ledger ~base_proof ~genesis_constants
            ~max_frontier_length )
    in
    setup states
end
