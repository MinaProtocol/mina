...
let zkapp_command (t : t) : _ Call_forest.t =
  ...
  let fee_payer : Account_update.t =
    let p = t.fee_payer in
    { authorization =
        Account_update.Fee_payer.Authorization.to_control p.authorization
    ; body
    }
  in
...
module Verifiable : sig
  ...
  let all_account_updates
      ({ fee_payer = fee_payer, vk; account_updates; memo = _ } : t) :
      'data Call_forest.With_hashes_and_data.t =
    let fee_payer = Account_update.Fee_payer.to_account_update fee_payer in
    let fee_payer_digest = Call_forest.Digest.Account_update.create fee_payer in
    let elt =
      { Call_forest.Tree.account_update = (fee_payer, vk)
      ; account_update_digest = fee_payer_digest
      ; calls = account_updates
      }
    in
  ...
  let create ({ fee_payer; account_updates; memo } : T.t) ~ledger ~get
    ...
    With_return.with_return (fun { return } ->
        ...
        in
        let fee_payer =
          let account_id = Account_update.Fee_payer.account_id fee_payer in
          let vk =
            if
              Control.(
                Tag.equal Tag.Proof
                  (Control.tag
                     (Account_update.Fee_payer.Authorization.to_control
                        fee_payer.authorization ) ))
            then Some (find_vk account_id)
            else None
          in
          (fee_payer, vk)
        in
...
let all_account_updates ({ fee_payer; account_updates; memo = _ } : t) :
    ( ( Account_update.t
  ...
  let fee_payer_digest = Call_forest.Digest.Account_update.create fee_payer in
  let elt =
    { Call_forest.Tree.account_update = fee_payer
    ; account_update_digest = fee_payer_digest
    ; calls = account_updates
    }
  in
