...
let every_vesting_period_minimum_balance_decreases_by_vesting_increment () =
  Quickcheck.test
    ...
    in
    let%map global_slot = Global_slot.gen_incl timing_rec.cliff_time max_slot in
...
let minimum_balance_checked_equal_to_unchecked () =
  ...
  Quickcheck.test
    ...
    let max_slot =
      ...
      / Amount.to_uint64 timing.vesting_increment
      * UInt64.of_uint32 (Global_slot.to_uint32 timing.vesting_period)
      |> UInt64.to_uint32
      |> UInt32.add (Global_slot.to_uint32 timing.cliff_time)
      |> Global_slot.of_uint32
    in
    let%map global_slot = Global_slot.(gen_incl zero max_slot) in
    (timing, global_slot))
    ~f:(fun (timing, global_slot) ->
