(** Testing
    -------
    Component:  Mina base
    Invocation: dune exec src/lib/mina_base/test/account/accounts.exe
    Subject:    Test basic accounts with timing.
 *)
...
let test_fine_tuning_of_the_account_generation () =
...
let liquid_balance_in_untimed_account_always_equal_to_balance () =
  Quickcheck.test
    (let open Quickcheck.Generator.Let_syntax in
...
let minimum_balance_checked_equal_to_unchecked () =
  ...
  in
  let global_slot_span_var span = Global_slot_span.Checked.constant span in
  Quickcheck.test
    (let open Quickcheck.Generator.Let_syntax in
    let%bind timing = gen_timing in
    let max_slot =
...
let () =
  Alcotest.run "Test account's timing."
    [ ( "accounts"
      , [ Alcotest.test_case "Test fine-tuning of the account generation."
            `Quick test_fine_tuning_of_the_account_generation
        ; Alcotest.test_case
            "Minimum balance never changes before the cliff time." `Quick
            minimum_balance_never_changes_before_the_cliff_time
        ; Alcotest.test_case
            "Cliff amount is immediately released at cliff time." `Quick
            cliff_amount_is_immediately_released_at_cliff_time
        ; Alcotest.test_case "Minimum balance never increases over time." `Quick
            minimum_balance_never_increases_over_time
        ; Alcotest.test_case
            "Final vesting slot is the first when minimum balance is 0." `Quick
            final_vesting_slot_is_the_first_when_minimum_balance_is_0
        ; Alcotest.test_case
        ...
        ; Alcotest.test_case
            "Incremental balance between slots before cliff is 0." `Quick
            incremental_balance_between_slots_before_cliff_is_0
        ; Alcotest.test_case
            "Incremental balance between slots after vesting finished is 0."
            `Quick incremental_balance_between_slots_after_vesting_finished_is_0
        ; Alcotest.test_case
            "Incremental balance where end is before start is 0." `Quick
            incremental_balance_where_end_is_before_start_is_0
        ; Alcotest.test_case
        ...
        ; Alcotest.test_case
            "Liquid balance in untimed account always equal to balance." `Quick
            liquid_balance_in_untimed_account_always_equal_to_balance
        ; Alcotest.test_case
            "Liquid balance is balance - minimum balance at given slot." `Quick
            liquid_balance_is_balance_minus_minimum_balance_at_given_slot
        ; Alcotest.test_case "Minimum balance checked equal to unchecked."
            `Quick minimum_balance_checked_equal_to_unchecked
        ] )
    ]
