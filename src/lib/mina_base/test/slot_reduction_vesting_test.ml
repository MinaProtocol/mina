...
let gen_large_period_timing =
  (* At 90s slot time, this slot will occur 12000 years after slot 0. We want
     this and the minimum vesting end slot to be large, to accommodate the long
     period. *)
...
let gen_slot_during_vesting ~(timing : Account_timing.as_record) =
  ...
  let right_after_cliff =
    Global_slot_since_genesis.(
      (* If cliff is too big, just exclude this case entirely *)
...
let gen_slot_after_vesting ~(timing : Account_timing.as_record) =
  let vesting_end = final_vesting_slot timing in
...
module Vesting_record = struct
  ...
  type t =
    (unit, UInt64.t, UInt64.t, UInt64.t, UInt64.t) Account.Timing.As_record.t
  [@@deriving equal]

  (** A form of [Account.Timing.As_record.t] with all fields stored as their
      string representations, to make [sexp_of] operations in tests easier. *)
  type as_strings =
    (unit, string, string, string, string) Account.Timing.As_record.t
  [@@deriving sexp_of]

  let to_strings (t : t) : _ Account_timing.As_record.t =
    { is_timed = ()
    ; initial_minimum_balance = t.initial_minimum_balance |> UInt64.to_string
    ; cliff_time = t.cliff_time |> UInt64.to_string
    ; cliff_amount = t.cliff_amount |> UInt64.to_string
    ; vesting_period = t.vesting_period |> UInt64.to_string
    ; vesting_increment = t.vesting_increment |> UInt64.to_string
    }
  ...
  let of_record (t : Account_timing.as_record) : t =
    { is_timed = ()
  ...
  (** Convert to a regular [Account_timing.as_record] by clamping all the values
      that might be out of range, as specified in the MIP *)
  let to_record (t : t) : Account_timing.as_record =
    { is_timed = true
  ...
  (** True if an account is actively vesting, as defined by the slot reduction MIP *)
...
let vesting_before_vesting_end () =
  Quickcheck.test gen_actively_vesting_at_hardfork
    ~sexp_of:(fun (timing, global_slot) ->
      [%sexp_of: Vesting_record.as_strings * Global_slot_since_genesis.t]
        ( Vesting_record.to_strings @@ Vesting_record.of_record timing
        , global_slot ) )
...
let funds_unlocked_at_slot ~global_slot (t : Account_timing.as_record) =
  let prev_slot =
...
let no_odd_vesting_discrepancies () =
  Quickcheck.test
    ...
    ~sexp_of:(fun (timing, adjusted_timing, test_slot, hardfork_slot) ->
      [%sexp_of:
        Vesting_record.as_strings
        * Vesting_record.as_strings
        * Global_slot_since_genesis.t
        * Global_slot_since_genesis.t]
        Vesting_record.
          ( to_strings @@ of_record timing
          , to_strings @@ of_record adjusted_timing
          , test_slot
          , hardfork_slot ) )
