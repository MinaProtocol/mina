...
(** This module defines a weight for each command component *)
module Weight = struct
  let proved :
      (Party.Predicated.Proved.t, Control.t) Party.Authorized.Poly.t -> int =
   fun _ -> 1

  let empty (_ : Party.Authorized.Empty.t option) : int = 1

  let signed :
      (Party.Predicated.Signed.t, Signature.t) Party.Authorized.Poly.t -> int =
   fun _ -> 1

  let proved_empty :
         ( (Party.Predicated.Proved.t, Control.t) Party.Authorized.Poly.t
         , Party.Authorized.Empty.t option )
         Inner.t
      -> int =
   fun { one; two; _ } -> proved one + empty two

  let proved_signed :
         ( (Party.Predicated.Proved.t, Control.t) Party.Authorized.Poly.t
         , (Party.Predicated.Signed.t, Signature.t) Party.Authorized.Poly.t )
         Inner.t
      -> int =
   fun { one; two; _ } -> proved one + signed two
  ...
  let proved_proved :
         ...
         Inner.t
      -> int =
   fun { one; two; _ } -> proved one + proved two
  ...
  let signed_signed :
         ...
         Inner.t
      -> int =
   fun { one; two; _ } -> signed one + signed two
  ...
  let signed_empty :
         ...
         Inner.t
      -> int =
   fun { one; two; _ } -> signed one + empty two
end

(** compute the weight of a snapp command *)
let weight : t -> int =
  let open Weight in
  function
  | Proved_empty proved_empty_inner ->
      proved_empty proved_empty_inner
  | Proved_signed proved_signed_inner ->
      proved_signed proved_signed_inner
  | Proved_proved proved_proved_inner ->
      proved_proved proved_proved_inner
  | Signed_signed signed_signed_inner ->
      signed_signed signed_signed_inner
  | Signed_empty signed_empty_inner ->
      signed_empty signed_empty_inner
