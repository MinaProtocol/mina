module type Base_ledger_intf = sig
  ...
  val foldi :
    t -> init:'accum -> f:(Addr.t -> 'accum -> Account.t -> 'accum) -> 'accum
  ...
  val token_owner :
    t -> Token_id.t -> Signature_lib.Public_key.Compressed.t option

  val token_owners : t -> Account_id.Set.t

  val tokens : t -> Signature_lib.Public_key.Compressed.t -> Token_id.Set.t
  ...
  val set_batch : t -> (Location.t * Account.t) list -> unit

  val get_at_index_exn : t -> index -> Account.t

  val set_at_index_exn : t -> index -> Account.t -> unit

  val index_of_account_exn : t -> Account_id.t -> index
...
module Make : functor
  ...
  (Dest : sig
     ...
     module Addr : sig
       ...
       module Table : sig
         ...
         type ('a, 'b) hashtbl = ('a, 'b) Source.Addr.Table.hashtbl
     ...
     val get_all_accounts_rooted_at_exn :
       t -> Addr.t -> (Addr.t * Account.t) list
     ...
     val get_batch :
       t -> Location.t list -> (Location.t * Account.t option) list
  ...
  val transfer_accounts :
    src:Source.t -> dest:Dest.t -> Dest.t Core_kernel.Or_error.t
end

module From_sparse_ledger : functor (Dest : Base_ledger_intf) -> sig
  val transfer_accounts :
    src:Sparse_ledger.t -> dest:Dest.t -> Dest.t Core_kernel.Or_error.t
end
