...
module Poly = struct
  ...
  module Stable = struct
    module V2 = struct
      type ('controller, 'version) t =
        ...
        ; set_verification_key : 'controller * 'version
  ...
  let to_input controller version t =
    ...
    Stable.Latest.Fields.fold ~init:[] ~edit_state:(f controller)
      ...
      ~set_verification_key:
        (f (fun (c, v) ->
             Random_oracle.Input.Chunked.append (controller c) (version v) ) )
...
module Stable = struct
  module V2 = struct
    type t =
      (Auth_required.Stable.V2.t, Protocol_version.Stable.V2.t) Poly.Stable.V2.t
...
let gen ~auth_tag : t Quickcheck.Generator.t =
  ...
  let%bind protocol_version = Protocol_version.gen in
  ...
  return
    ...
    ; set_delegate
    ; set_permissions
    ; set_verification_key = (set_verification_key, protocol_version)
    ; set_zkapp_uri
    ; edit_action_state
    ; set_token_symbol
    ; increment_nonce
...
module Checked = struct
  type t =
    (Auth_required.Checked.t, Protocol_version.Checked.t) Poly.Stable.Latest.t

  let to_input (x : t) =
    Poly.to_input Auth_required.Checked.to_input
      Protocol_version.Checked.to_input x
  ...
  let if_ b ~(then_ : t) ~(else_ : t) : t Snark_params.Tick.Checked.t =
    ...
    in
    let%map protocol_version =
      Protocol_version.Checked.if_ b
    ...
    ; set_delegate
    ; set_permissions
    ; set_verification_key = (controller, protocol_version)
    ; set_zkapp_uri
    ; edit_action_state
    ; set_token_symbol
    ; increment_nonce
  ...
  let constant (t : Stable.Latest.t) : t =
    ...
    Poly.Fields.map ~edit_state:a ~send:a ~receive:a ~set_delegate:a
      ...
      ~set_verification_key:(fun f ->
        let controller, version = get f t in
        ( Auth_required.Checked.constant controller
        , Protocol_version.Checked.constant version ) )
...
let typ =
  ...
  Typ.of_hlistable
    ...
    ; Auth_required.typ
    ; Auth_required.typ
    ; Typ.tuple2 Auth_required.typ Protocol_version.typ
    ; Auth_required.typ
    ; Auth_required.typ
    ; Auth_required.typ
    ; Auth_required.typ
...
let to_input (x : t) =
  Poly.to_input Auth_required.to_input Protocol_version.to_input x
...
let user_default : t =
  ...
  ; set_verification_key = (Signature, Protocol_version.current)
...
let empty : t =
  ...
  ; set_verification_key = (None, Protocol_version.current)
  ; set_zkapp_uri = None
...
module As_record = struct
  ...
  let deriver obj =
    ...
    Fields.make_creator obj ~auth:!.auth_required
      ~version:!.Protocol_version.deriver
    |> finish "VerificationKeyPermission" ~t_toplevel_annots
end

let to_record (auth, version) = { As_record.auth; version }

let of_record { As_record.auth; version } = (auth, version)
