  ...
  module Checked = struct
    ...
    let eval_proof ({ constant; signature_necessary; signature_sufficient } : t)
      ...
      *)
      ...
      let b = Boolean.((not signature_necessary) &&& not impossible) in
      let _p =
        let open Pickles.Impls.Step.As_prover in
        as_prover (fun _ ->
            let s = read Boolean.typ signature_sufficient in
            let c = Pickles.Impls.Step.As_prover.read Boolean.typ constant in
            let result = Pickles.Impls.Step.As_prover.read Boolean.typ b in
            printf "Proof: constant %b signature_verifies %b Result %b\n%!" s c
              result)
      in
      b
