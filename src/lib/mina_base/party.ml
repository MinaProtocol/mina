  ...
  let to_input
    ...
    List.reduce_exn ~f:append
      ...
      ; Set_or_keep.to_input delegate
          ~dummy:(Zkapp_predicate.Eq_data.Tc.public_key ()).default
    ...
    module V1 = struct
      type t =
            Poly(Public_key.Compressed.Stable.V1)(Token_id.Stable.V1)
              ...
              (Int)
              (Bool)
              (Zkapp_predicate.Protocol_state.Stable.V1)
            .t
            (* Opaque to txn logic *) =
        ...
        ; call_depth : int
        ; protocol_state : Zkapp_predicate.Protocol_state.Stable.V1.t
  ...
  *)
  module Fee_payer = struct
    ...
    let deriver obj =
      ...
      Fields.make_creator obj ~public_key:!.public_key ~update:!.Update.deriver
        ...
        ~protocol_state:!.Zkapp_predicate.Protocol_state.deriver
  ...
  module Checked = struct
    ...
    type t =
          Poly(Type_of_var(Public_key.Compressed))(Token_id.Checked)
            ...
            (Int_as_prover_ref)
            (Type_of_var(Boolean))
            (Zkapp_predicate.Protocol_state.Checked)
          .t =
      ...
      ; protocol_state : Zkapp_predicate.Protocol_state.Checked.t
    ...
    let to_input
      ...
      List.reduce_exn ~f:Random_oracle_input.Chunked.append
        ...
        ; Zkapp_predicate.Protocol_state.Checked.to_input protocol_state
        ; Random_oracle_input.Chunked.packed
  ...
  let typ () : (Checked.t, t) Typ.t =
    Typ.of_hlistable
      ...
      ; Field.typ
      ; Typ.Internal.ref ()
      ; Zkapp_predicate.Protocol_state.typ
      ; Impl.Boolean.typ
      ]
      ~var_to_hlist:Checked.to_hlist ~var_of_hlist:Checked.of_hlist
      ~value_to_hlist:to_hlist ~value_of_hlist:of_hlist
  ...
  let to_input
    ...
    List.reduce_exn ~f:Random_oracle_input.Chunked.append
      ...
      ; Random_oracle_input.Chunked.field call_data
      ; Zkapp_predicate.Protocol_state.to_input protocol_state
...
module Predicate = struct
  ...
  module Stable = struct
    module V1 = struct
      type t =
        | Full of Zkapp_predicate.Account.Stable.V2.t
  ...
  let to_full = function
    | Full s ->
        s
    | Nonce n ->
        { Zkapp_predicate.Account.accept with
          nonce = Check { lower = n; upper = n }
        }
    | Accept ->
        Zkapp_predicate.Account.accept

  let of_full (p : Zkapp_predicate.Account.t) =
    let module A = Zkapp_predicate.Account in
  ...
  let deriver obj =
    ...
    iso_record ~of_record:of_full ~to_record:to_full
      Zkapp_predicate.Account.deriver obj
  ...
  let digest (t : t) =
    ...
    in
    to_full t |> Zkapp_predicate.Account.to_input |> digest

  module Checked = struct
    type t = Zkapp_predicate.Account.Checked.t
    ...
    let digest (t : t) =
      let digest x =
        Random_oracle.Checked.(
          hash ~init:Hash_prefix_states.party_predicate (pack_input x))
      in
      Zkapp_predicate.Account.Checked.to_input t |> digest
  end

  let typ () : (Zkapp_predicate.Account.Checked.t, t) Typ.t =
    Typ.transport (Zkapp_predicate.Account.typ ()) ~there:to_full
      ~back:(fun s -> Full s)
end
  ...
  module Proved = struct
    ...
    module Stable = struct
      module V1 = struct
        type t =
          ( Body.Stable.V1.t
          , Zkapp_predicate.Account.Stable.V1.t )
          Poly.Stable.V1.t
        [@@deriving sexp, equal, yojson, hash, compare]
    ...
    module Digested = struct
      type t = (Body.Digested.t, Zkapp_predicate.Digested.t) Poly.t
    ...
    module Checked = struct
      type t = (Body.Checked.t, Zkapp_predicate.Account.Checked.t) Poly.t
    end
  end
...
let protocol_state (t : t) : Zkapp_predicate.Protocol_state.t =
