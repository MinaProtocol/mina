...
let%test_module "test" =
  ( module struct
    ...
    let create_and_apply lb logger txns stmt_to_work =
      let open Deferred.Or_error.Let_syntax in
      let diff =
        Sl.create_diff lb ~self:self_pk ~logger ~transactions_by_fee:txns
          ~get_completed_work:stmt_to_work
      in
      let%map _, `Ledger_proof ledger_proof =
        Sl.apply lb (Test_input1.Staged_ledger_diff.forget diff) ~logger
      in
    ...
    let%test_unit "Max throughput" =
      ...
      let lb = Sl.create ~ledger:initial_ledger in
      ...
      Quickcheck.test g ~trials:50 ~f:(fun i ->
          Async.Thread_safe.block_on_async_exn (fun () ->
              ...
              let materialized_ledger =
                Or_error.ok_exn @@ Sl.snarked_ledger lb ~snarked_ledger_hash
              in
