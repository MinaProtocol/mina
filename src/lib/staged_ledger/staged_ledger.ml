...
module T = struct
  ...
  module Resources = struct
    ...
    let coinbase_ft (cw : Transaction_snark_work.t) =
      (* Here we could have not added the fee transfer if prover=self but
      adding it to preserve that information in the staged_ledger_diff. It will
      be checked in apply_diff before including the fee transfer*)
