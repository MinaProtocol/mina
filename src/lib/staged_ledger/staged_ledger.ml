...
struct
  ...
  module Staged_ledger_error = struct
    type t =
      ...
      | Insufficient_fee of Fee.t * Fee.t
    ...
    let to_string = function
      ...
      | Insufficient_fee (f1, f2) ->
          Format.asprintf
            !"Transaction fee %{sexp: Fee.t} does not suffice proof fee \
              %{sexp: Fee.t} \n"
            f1 f2
  ...
  let create_fee_transfers completed_works delta public_key coinbase_fts =
    let open Result.Let_syntax in
    let singles =
      (if Fee.(equal zero delta) then [] else [(public_key, delta)])
    ...
    Or_error.try_with (fun () ->
        List.fold coinbase_fts ~init:singles_map ~f:(fun accum single ->
            ...
            | None ->
                accum
            | Some fee ->
                let new_fee = Option.value_exn (Fee.sub fee (snd single)) in
                if new_fee > Fee.zero then
  ...
  *)
  ...
  let fee_remainder (user_commands : User_command.With_valid_signature.t list)
    ...
    in
    let total_work_fee =
      Option.value ~default:Fee.zero (Fee.sub work_fee coinbase_fee)
    in
    Option.value_map
      ~default:
        (Error (Staged_ledger_error.Insufficient_fee (budget, total_work_fee)))
      ~f:(fun x -> Ok x)
      (Fee.sub budget total_work_fee)
