...
module Update_coinbase_stack_and_get_data_result = struct
  [%%versioned
  ...
  let read_all_proofs_from_disk ~ledger_depth : t -> Stable.Latest.t =
   fun ((a, witness, b, c, d), maps) ->
    ( a
    , List.map
        ~f:
          Transaction_snark_scan_state.Transaction_with_witness
    ...
    , c
    , d
    , Ledger.Mask_maps.to_stable ~ledger_depth maps )

  let write_all_proofs_to_disk ~ledger_depth ~proof_cache_db :
      Stable.Latest.t -> t =
   fun (a, witness, b, c, d, maps) ->
    ( ( a
      , List.map
      ...
      , c
      , d )
    , Ledger.Mask_maps.of_stable ~ledger_depth maps )
end
...
module T = struct
  ...
  let apply_diff ?(skip_verification = false) ~logger ~constraint_constants
      ...
      ?cached_update_coinbase_stack_and_get_data_result t pre_diff_info =
    let open Deferred.Result.Let_syntax in
    let max_throughput =
  ...
  let apply ?skip_verification ~constraint_constants ~global_slot
      ...
      ~zkapp_cmd_limit_hardcap ~signature_kind
      ?(transaction_pool_proxy = Check_commands.dummy_transaction_pool_proxy)
