...
module Node = struct
  ...
  let graphql_client ~logger t= 
    Test_graphql.create ~logger_metadata:[] ~uri:(graphql_uri t) ~enabled:true ~logger    
...
type signed_command_result = Test_graphql.signed_command_result
