...
type 'a final_result = [`Failed | `Applied of 'a] Ivar.t
...
module Cached = struct
  module type S = sig
    ...
    val final_result : ('t1, 't2) t -> 't2 final_result
...
module Cache = struct
  module type S = sig
    ...
    val final_result : 'elt t -> 'elt -> 'elt final_result Option.t
...
module Transmuter_cache = struct
  module type S = sig
    ...
    val final_result : t -> source -> target final_result Option.t
