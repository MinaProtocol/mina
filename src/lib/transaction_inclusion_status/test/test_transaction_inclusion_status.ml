...
let test_unknown_status_when_no_frontier () : unit Deferred.t =
  Quickcheck.async_test ~trials:1 gen_user_command ~f:(fun user_command ->
      ...
      in
      assert (State.equal State.Unknown status) )
...
let () =
  Async.Thread_safe.block_on_async_exn (fun () ->
      let tests =
        [ ( "transaction_status"
          , [ Alcotest_async.test_case "unknown status when no frontier" `Quick
                test_unknown_status_when_no_frontier
            ] )
        ]
      in
      Alcotest_async.run "Transaction_inclusion_status" tests )
