...
module Mat = Snarky_bn382.Fp.Constraint_matrix
...
let choose_best base opts terms =
  ...
  let best f xs =
    List.min_elt xs ~compare:(fun (_, wt1) (_, wt2) ->
        Int.compare (Weight.norm (base +. f wt1)) (Weight.norm (base +. f wt2))
    )
    |> Option.value_exn
  in
...
let add_constraint ?label:_ t
  ...
  | Boolean x ->
      ...
      let x_minus_1_weight = x_weight + if x_has_constant_term then 0 else 1 in
      choose_best
        (* x * x = x
             x * (x - 1) = 0 *)
        ...
        (function
          | `x_x_x -> (x, x, x) | `x_xMinus1_0 -> (x, decr_constant_term x, [])
          )
  | Equal (x, y) ->
      (* x * 1 = y
           y * 1 = x
