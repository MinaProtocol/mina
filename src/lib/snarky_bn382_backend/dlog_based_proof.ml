...
let of_backend (t : Snarky_bn382.Fq_proof.t) : t =
  ...
  let proof =
    ...
    let lr =
      let v = lr t in
      Array.init (Snarky_bn382.G.Affine.Pair.Vector.length v)
        (fun i -> gpair v (fun v -> Snarky_bn382.G.Affine.Pair.Vector.get v i))
    in
  ...
  let evals =
    ...
    |> Tuple_lib.Triple.map ~f:(fun e ->
        ...
        in
        let fq = fq e in
        { Pickles_types.Dlog_marlin_types.Evals.
          w_hat=fq w
        ; z_hat_a= fq za
        ; z_hat_b= fq zb
        ; h_1= fq h1
