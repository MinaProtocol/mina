...
module Any_ledger = Mina_ledger.Ledger.Any_ledger
...
module Make_str (A : Wire_types.Concrete) = struct
  ...
  let compute_delegatee_table_any_ledger keys ledger =
    O1trace.sync_thread "compute_delegatee_table_any_ledger" (fun () ->
        compute_delegatee_table keys ~iter_accounts:(fun f ->
            Mina_ledger.Ledger.Any_ledger.M.iteri ledger ~f:(fun i acct -> f i acct) ) )
    ...
    module Local_state = struct
      module Snapshot = struct
        module Ledger_snapshot = struct
          ...
          let compute_delegatee_table keys ledger =
            ...
            | Any_ledger ledger ->
                compute_delegatee_table_any_ledger keys ledger

          let close = function
            | Genesis_epoch_ledger _ ->
                ()
            | Any_ledger ledger ->
                Mina_ledger.Ledger.Any_ledger.M.close ledger
          ...
          let ledger_subset keys ledger =
            ...
            | Any_ledger db_ledger ->
                let ledger = Ledger.of_any_ledger db_ledger in
                let subset_ledger =
                  Sparse_ledger.of_ledger_subset_exn ledger keys
                in
                ignore
      ...
      let create_epoch_ledger ~location ~context:(module Context : CONTEXT)
        ...
        if Sys.file_exists location then (
          ...
          Snapshot.Ledger_snapshot.Any_ledger
            (Mina_ledger.Ledger.create_unmasked_ledger ~directory_name:location
      ...
      let reset_snapshot (t : t) id ledger =
        let delegatee_table =
          compute_delegatee_table_any_ledger
        ...
        | Staking_epoch_snapshot ->
            !t.staking_epoch_snapshot <-
              { delegatee_table
              ; ledger = Snapshot.Ledger_snapshot.Any_ledger ledger
              }
        | Next_epoch_snapshot ->
            !t.next_epoch_snapshot <-
              { delegatee_table
  ...
  module Hooks = struct
      ...
      O1trace.thread "sync_local_state" (fun () ->
          ...
          let sync { snapshot_id; expected_root = target_ledger_hash } =
            ...
            then (
              ...
              | Any_ledger next_epoch_ledger ->
                  let ledger =
                    Mina_ledger.Ledger.Any_ledger.M.create_checkpoint
                      next_epoch_ledger
                      ~directory_name:
                  ...
                  in
                  set_snapshot local_state Staking_epoch_snapshot
                    { ledger = Ledger_snapshot.Any_ledger ledger
                    ; delegatee_table =
              ...
              *)
              let%bind.Deferred.Or_error db_ledger =
                let db_ledger_of_snapshot snapshot snapshot_location =
                  O1trace.sync_thread "db_ledger_of_snapshot" (fun () ->
                      ...
                      | Ledger_snapshot.Any_ledger ledger ->
                          Ok ledger
              ...
              match%bind
                Mina_ledger.Sync_ledger.Any_ledger.fetch sync_ledger target_ledger_hash
                  ~data:() ~equal:(fun () () -> true)
              with
              | `Ok ledger ->
                  [%log info]
    ...
    let frontier_root_transition (prev : Consensus_state.Value.t)
      ...
      let snarked_ledger_hash =
        Mina_ledger.Ledger.Any_ledger.M.merkle_root snarked_ledger
      in
      if
        not
          (Epoch.equal
      ...
      then (
        ...
        then (
          ...
          !local_state.next_epoch_snapshot <-
            { ledger =
                Local_state.Snapshot.Ledger_snapshot.Any_ledger
                  (Mina_ledger.Ledger.Any_ledger.M.create_checkpoint
                     snarked_ledger
                     ~directory_name:
