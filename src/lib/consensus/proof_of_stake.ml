...
module Make_str (A : Wire_types.Concrete) = struct
  ...
  module Data = struct
    ...
    module Vrf = struct
      ...
      let get_vrf_evaluation
        ...
        let label =
          Stdlib.(
            "get_vrf_evaluation: " ^ __FILE__ ^ ":" ^ string_of_int __LINE__)
        in
        ...
        in
        let%bind () =
          with_label "Account is for the default token" (fun () ->
              make_checked (fun () ->
                  Token_id.(
        ...
        in
        let%bind () =
          with_label "Block stake winner matches account pk" (fun () ->
              Public_key.Compressed.Checked.Assert.equal block_stake_winner
                account.public_key )
        in
        let%bind () =
          with_label "Block creator matches delegate pk" (fun () ->
              Public_key.Compressed.Checked.Assert.equal block_creator
                account.delegate )
        in
        let%bind delegate =
          with_label "Decompress delegate pk" (fun () ->
      ...
      module Checked = struct
        let check
          ...
          let open Snark_params.Tick in
          let label =
            Stdlib.("check: " ^ __FILE__ ^ ":" ^ string_of_int __LINE__)
          in
          let%bind () = with_label label (fun _ -> Checked.return ()) in
    ...
    module Min_window_density = struct
      ...
      *)
      ...
      module Checked = struct
        let update_min_window_density ~(constants : Constants.var)
          ...
          let label =
            Stdlib.(
              "update_min_window_density: " ^ __FILE__ ^ ":"
    ...
    *)
    module Consensus_state = struct
      ...
      let update_var (previous_state : var)
        ...
        let open Snark_params.Tick in
        let label =
          Stdlib.("update_var: " ^ __FILE__ ^ ":" ^ string_of_int __LINE__)
        in
        let%bind () = with_label label (fun _ -> Checked.return ()) in
        let%bind constants =
  ...
  module Hooks = struct
    ...
    module Make_state_hooks
      ...
      include struct
        let next_state_checked ~constraint_constants
