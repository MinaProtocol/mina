...
let max (t1:t) (t2:t) =
  (* slots_per_epoch assumed same for t1, t2 *)
  let slot_number = Mina_numbers.Global_slot.max t1.slot_number t2.slot_number
  in { t1 with slot_number }
...
let diff ~(constants : Constants.t) (t : t) (other_epoch, other_slot) =
  ...
  let epoch, slot = to_epoch_and_slot t in
  let old_epoch =
    epoch - other_epoch - (UInt32.of_int @@ if UInt32.compare other_slot slot > 0 then 1 else 0)
  in
  let old_slot =
