...
(* Sum balances of default-token accounts whose delegate is non-empty: those
   are the accounts that participate in stake at genesis. Accounts with
   delegate = None or delegate = Public_key.Compressed.empty are opted out
   per MIP-0010 (unstaking). *)
let genesis_ledger_total_stake ~ledger =
  Mina_ledger.Ledger.foldi ~init:Currency.Amount.zero (Lazy.force ledger)
    ~f:(fun _addr sum (account : Mina_base.Account.t) ->
      ...
      let is_staked =
        not
          (Signature_lib.Public_key.Compressed.equal
             (Mina_base.Account.delegate_opt account.delegate)
             Signature_lib.Public_key.Compressed.empty )
      in
      if is_default_token && is_staked then
        Currency.Amount.add sum (Currency.Balance.to_amount @@ account.balance)
        |> Base.Option.value_exn ?here:None ?error:None
