...
module Tick_backend : module type of Crypto_params.Tick_backend

module Tock_backend : module type of Crypto_params.Tock_backend

module Snarkette_tick : module type of Crypto_params.Snarkette_tick

module Snarkette_tock : module type of Crypto_params.Snarkette_tock

module Scan_state_constants : module type of Scan_state_constants
...
module Tock0 : sig
  include module type of Crypto_params.Tock0

  module Snarkable : Snarkable with module Impl := Crypto_params.Tock0
end

module Tick0 : sig
  include module type of Crypto_params.Tick0

  module Snarkable : Snarkable with module Impl := Crypto_params.Tick0
end

module Wrap_input = Crypto_params.Wrap_input

module type Inner_curve_scalar = sig
  module Impl : Snarky.Snark_intf.S

  module Other_impl : Snarky.Snark_intf.S

  type var = Impl.Boolean.var Bitstring.Lsb_first.t
  ...
  module Checked : sig
    val equal :
         Impl.Boolean.var Bitstring.Lsb_first.t
      -> Impl.Boolean.var Bitstring.Lsb_first.t
      -> (Impl.Boolean.var, 'a) Impl.Checked.t

    val to_bits : 'a -> 'a

    module Assert : sig
      val equal : var -> var -> (unit, 'a) Impl.Checked.t
    end
  end
end

module type Inner_curve_aux = sig
  module Impl : Snarky.Snark_intf.S
  ...
  module Scalar :
    ...
    with module Impl := Impl
     and module Other_impl := Other_impl
end

(*
module Tock : sig
  include module type of Tock0 with module Proof := Tock0.Proof

  module Fq : Snarky_field_extensions.Intf.S with module Impl = Tock0

  module Inner_curve : sig
    include module type of Tock_backend.Inner_curve

    val t_of_sexp : Sexp.t -> t
    val sexp_of_t : t -> Sexp.t

    include Inner_curve_aux with module Impl = Tock0 and module Other_impl = Tick0

    val ctypes_typ : t Ctypes_static.typ

    val scale : t -> Scalar.t -> t

    module Checked : sig
      module Scalable_inner_curve : sig
        include module type of Tock_backend.Inner_curve
        val scale : t -> Scalar.t -> t
      end

      include Snarky_curves.Weierstrass_checked_intf
              with 
                module Impl := Tock0
                and type unchecked := Scalable_inner_curve.t
                and type t = Fq.t * Fq.t

      val add_known_unsafe : field Snarky.Cvar.t * field Snarky.Cvar.t ->
                             T.t ->
                             ([ `I_thought_about_this_very_carefully of t ], 'a, field) Snarky.Checked.t
    end
  ...
  module Pairing : sig
    module T : sig
      val conv_field : Snarkette_tock.Fq.t -> field

      module Impl = Tock0
      open Snarky_field_extensions.Field_extensions
      module Fq = Fq

      val non_residue : field

      module Fqe : sig
        module Params : sig
          val non_residue : field

          val mul_by_non_residue : Fq.t -> Fq.t
        end
