...
(* not really part of the networking layer, but
   networking-related
*)
module Find_ip = Find_ip

(* Gossip_net.Libp2p shadowed below, grab these while you can *)
module Gossip_net_libp2p = Gossip_net.Libp2p

type gossip_net_libp2p_config = Gossip_net.Libp2p.Config.t
...
let%test_module "Mina_networking tests" =
  ( module struct
    let (_ : _) =
      (* let _block_reader, block_sink =
         ...
         in
           let _sinks = (block_sink, tx_remote_sink, snark_remote_sink) in *)
      Async.Thread_safe.block_on_async_exn (fun () ->
          ...
          let log_gossip_heard : Config.log_gossip_heard =
          ...
          let%bind creatable_gossip_net =
            ...
            let gossip_net_params : gossip_net_libp2p_config =
          ...
          in
          let _mina_networking = create config in
          return () )
  end )
