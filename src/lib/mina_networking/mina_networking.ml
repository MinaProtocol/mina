...
module Rpcs = struct
  ...
  *)
  ...
  module Get_node_status_data = struct
    module Node_status_data = struct
      [%%versioned
    ...
    module Master = struct
      let name = "get_node_status_data"

      module T = struct
        type query = unit [@@deriving sexp, to_yojson]

        type response = Node_status_data.t Or_error.t
      end
    ...
    let response_to_yojson response =
      match response with
      | Ok status ->
          Node_status_data.Stable.V1.to_yojson status
      | Error err ->
    ...
    module V1 = struct
      module T = struct
        ...
        type response =
          Node_status_data.Stable.V1.t Core_kernel.Or_error.Stable.V1.t
        [@@deriving bin_io, version {rpc}]
...
let create (config : Config.t)
    ...
    ~(get_node_status_data :
          Rpcs.Get_node_status_data.query Envelope.Incoming.t
       -> Rpcs.Get_node_status_data.response Deferred.t)
    ~(get_transition_chain_proof :
  ...
  in
  (* The node status data RPC is implemented directly in go, serving a string which
  ...
  Clock.every' (Time.Span.of_min 1.) (fun () ->
      match%bind
        get_node_status_data {data= (); sender= Local; received_at= fake_time}
      with
      | Error _ ->
          Deferred.unit
      | Ok data ->
          Gossip_net.Any.set_node_status_data gossip_net
            ( Rpcs.Get_node_status_data.Node_status_data.to_yojson data
...
include struct
  ...
  let get_peer_node_status_data t peer =
    let open Deferred.Or_error.Let_syntax in
    let%bind s = get_peer_node_status_data t.gossip_net peer in
    Or_error.try_with (fun () ->
        match
          Rpcs.Get_node_status_data.Node_status_data.of_yojson
