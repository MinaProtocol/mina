...
module Impl : Worker_impl_intf.Worker_impl = struct
  ...
  let log_base_snark ~logger f ~statement ~spec ~all_inputs =
    match%map.Deferred
  ...
  let perform_regular_work ~(m : (module Transaction_snark.S) option) ~logger
      ~cache ~regular ~sok_digest =
          ...
          [%log error] "SNARK worker failed: $error"
            ~metadata:
              ...
              ; ( "spec"
                ...
                , `String
                    (Sexp.to_string
                       (Rpcs_types.Regular_work_single.sexp_of_t regular) ) )
              ] ;
          Error e
      | Ok res ->
  ...
  let perform_single ({ m; cache; proof_level } : Worker_state.t) ~message =
    ...
    fun (single : Rpcs_types.Wire_work.Single.Spec.t) ->
      ...
      | Genesis_constants.Proof_level.Full -> (
          match single with
          | Regular regular ->
              perform_regular_work ~m ~logger ~regular ~cache ~sok_digest
          | _ ->
              failwith "TODO: prod full zkapp proof" )
      | Check | No_check ->
          (* Use a dummy proof. *)
          let stmt =
            match single with
            | Regular regular -> (
                ...
                | Merge (stmt, _, _) ->
                    stmt )
            | _ ->
                failwith "TODO: prod check/nocheck zkapp proof"
          in
