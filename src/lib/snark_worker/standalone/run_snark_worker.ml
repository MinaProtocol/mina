...
module Impl : Snark_worker.Impl.intf = Snark_worker.Impl.Prod
...
let perform (s : Impl.Worker_state.t) ~public_key ~fee
    (instances : Work.Selector.Single_spec.t One_or_two.t) =
  One_or_two.Deferred_result.map_biased instances ~f:(fun one_or_two w ->
      let open Deferred.Or_error.Let_syntax in
      let%map proof, time =
        Impl.perform_single s
          ~message:(Mina_base.Sok_message.create ~fee ~prover:public_key)
          (Regular (w, { one_or_two; pair_uuid = None }))
      in

      let work_tag =
        match w with Transition _ -> `Transition | Merge _ -> `Merge
      in
      let proof_cached =
  ...
  |> Deferred.Or_error.map
       ~f:(fun proofs_and_time : _ Work.Work.Result_without_metrics.t ->
         { proofs = One_or_two.map proofs_and_time ~f:fst
         ; statements =
             One_or_two.map instances ~f:Work.Work.Single.Spec.statement
         ; prover = public_key
         ; fee
         } )
  ...
  Command.async ~summary:"Run snark worker directly"
    (let%map_open spec_json =
     ...
     fun () ->
         ...
         let spec_of_json json =
           match
             ...
             |> One_or_two.of_yojson
                  (Work.Work.Single.Spec.of_yojson
       ...
       let spec =
         One_or_two.map
           ~f:(Work.Selector.Single_spec.cache ~proof_cache_db)
           spec
       in
       let public_key =
         Option.value
       ...
       let fee =
         Option.value
           ...
           snark_work_fee
       in
       match%bind perform ~fee ~public_key worker_state spec with
       | Ok result -> (
           let result =
             Work.Work.Result_without_metrics.map
               ~f_proof:Ledger_proof.Cached.read_proof_from_disk result
           in
           Caml.Format.printf
