    ...
    module Stable = struct
      ...
      module Tests = struct
        let%test "in_snark protocol constants serialization v1" =
          ...
          let known_good_hash =
            "\x75\xC8\xFD\x04\xAD\x91\x6A\xEC\x3E\x3D\x5C\xB7\x6A\x45\x2B\x11\x6B\x3D\x4D\x09\x12\xA0\xA4\x85\xE9\xFB\x8E\x3D\x24\x0E\x21\x0C"
          in
    ...
    module Stable = struct
      ...
      module Tests = struct
        let%test "out_of_snark protocol constants serialization v1" =
          ...
          (*from the print statement in Serialization.check_serialization*)
          let known_good_hash =
            "\x7C\x95\x30\x55\xD2\xE6\x54\x38\x27\xA0\x66\xA0\xC2\xAB\x96\xDF\xF0\x49\xB8\xEF\x26\xD3\x1C\xE0\xFA\x8C\xD1\xF0\x10\xD1\x59\x24"
          in
          Serialization.check_serialization (module V1) t known_good_hash
      end
    end]
