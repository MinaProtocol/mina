          ...
          [ ( "decision"
            , `String
                ...
                | `Not_broadcasted ->
                    "not_broadcasted" ) )
          ; ( "valid_commands"
            , `List (List.map ~f:User_command.Wire.to_yojson valid_commands) )
          ; ( "invalid_commands"
            , `List
...
let setup_and_submit_zkapp_commands t
    (zkapp_commands : Zkapp_command.Wire.t list) =
  let open Participating_state.Let_syntax in
  (* hack to get types to work out *)
  let%map () = return () in
  let open Deferred.Let_syntax in
...
let setup_and_submit_zkapp_command t (zkapp_command : Zkapp_command.Wire.t) =
...
module Receipt_chain_verifier = Merkle_list_verifier.Make (struct
  type proof_elem = User_command.Wire.t
...
let verify_payment t (addr : Account_id.t) (verifying_txn : User_command.Wire.t)
    (init_receipt, proof) =
  let open Participating_state.Let_syntax in
  let%map account = get_account t addr in
  ...
  in
  if List.exists proof ~f:(fun txn -> User_command.Wire.equal verifying_txn txn)
  then Ok ()
  else
    Or_error.errorf
      !"Merkle list proof does not contain payment %{sexp:User_command.Wire.t}"
      verifying_txn
