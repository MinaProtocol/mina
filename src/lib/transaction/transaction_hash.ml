...
let%test_module "Transaction hashes" =
  ( module struct
    ...
    (* To regenerate:
       * Use the generate_zkapp_txn script:
         1. Build: dune build scripts/generate_zkapp_txn/generate_zkapp_txn.exe
         2. Run: ./_build/default/scripts/generate_zkapp_txn/generate_zkapp_txn.exe
         This will output both the transaction ID and expected hash.
    *)
    ...
    let%test "zkApp v1 hash from transaction id" =
      ...
      in
      let expected_hash =
        "5Ju9x9Wq7Thdv5m2bd6FP5cYvS8TU8CMi9JwezaafbVtNCZjPTn1"
      in
      run_test ~transaction_id ~expected_hash
  end )
