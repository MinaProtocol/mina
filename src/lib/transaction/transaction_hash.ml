...
let to_string, of_string =
  let module Base58_check = Codable.Make_base58_check (struct
  ...
  (* distinguish current transaction hashes from those pre-Berkeley hard fork *)
  let byte = 'B' in
  let to_string txn_hash =
    let b58 = Base58_check.to_base58_check txn_hash in
    sprintf "%c%s" byte b58
  in
  let of_string s =
    if not (Char.equal byte s.[0]) then
      failwithf "Expected transaction hash to start with '%c'" byte ()
    else
...
let%test "signed command v1 hash from transaction id" =
  ...
  in
  (* N.B.: this is the new-style hash, computed by converting the V1 signed command to a V2 signed command, then hashing *)
  let expected_hash =
    "BCkpa7jFWhqLwA8KMNkYEdNiaokTe9tV6Bk2RgZFLhxGjbL1vrXPBc"
  in
  let hash =
...
let%test "signed command v2 hash from transaction id" =
  ...
  in
  let expected_hash =
    "BCkpZcAHDStkSGLD8Whb4vjk4qMrf79TL8gKUT3sC8PUHpKmV8mpMx"
  in
  let hash =
    match hash_of_transaction_id transaction_id with
    | Ok hash ->
...
let%test "zkApp v1 hash from transaction id" =
  ...
  in
  let expected_hash =
    "BCkpZDbwVFhQMVqU7xjeuYfM67whbSo3a5UjVMreTFmxTsdQFj8Rvk"
  in
  let hash =
    match hash_of_transaction_id transaction_id with
    | Ok hash ->
