...
let () =
  Async.Thread_safe.block_on_async_exn (fun () ->
    ...
    let module Transaction_snark_instance = Transaction_snark.Make (struct 
      let constraint_constants = Genesis_constants_compiled.compiled_config.constraint_constants
    ...
    let module Blockchain_snark_instance = Blockchain_snark.Blockchain_snark_state.Make (struct
      let constraint_constants = Genesis_constants_compiled.compiled_config.constraint_constants
      let proof_level = Genesis_constants_compiled.compiled_config.proof_level
    ...
    let () =
      Format.eprintf "Generated blockchain snark circuit in %s.@."
        (Time.Span.to_string_hum (Time.diff after before)) in
