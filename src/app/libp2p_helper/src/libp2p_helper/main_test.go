...
func TestLib2pMetrics(t *testing.T) {
	...
	select {
	case <-time.After(testTimeout):
		t.Fatal("B did not receive the first message from A")
	case <-done:
	}
	...
	select {
	case <-time.After(testTimeout):
		t.Fatal("B did not receive the second message from A")
	case <-done:
	}

	time.Sleep(time.Second) // Wait for metrics to be reported.
