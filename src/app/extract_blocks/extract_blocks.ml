...
let main ~archive_uri ~start_state_hash_opt ~end_state_hash_opt ~all_blocks ~output_folder 
  ~network ~include_block_height_in_name () =
  ...
  | Ok pool ->
      ...
      let%map () =
        Deferred.List.iter blocks_with_accounts_created ~f:(fun block ->
            ...
            let network_prefix = network |> Option.map ~f:(fun network -> network ^ "-") |> Option.value ~default:"" in
            let height_prefix = if include_block_height_in_name then 
                (Unsigned.UInt32.to_string block.height) ^ "-"
              else 
                ""
              in
            let output_file =
              output_folder ^ "/" ^ network_prefix ^ height_prefix ^ State_hash.to_base58_check block.state_hash ^ ".json"
            in
    ...
    run
      ...
      async
        ...
        (let%map archive_uri =
         ...
         and output_folder = 
            Param.flag "--output-folder" 
            ~doc:"Output folder for blocks jsons"
         ...
         and include_block_height_in_name = 
            ...
            ~doc:"Includes block height in block name"
            Param.no_arg
           
         in
         main ~archive_uri ~start_state_hash_opt ~end_state_hash_opt ~all_blocks ~output_folder ~network ~include_block_height_in_name
        )))
