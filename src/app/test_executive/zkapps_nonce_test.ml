...
module Make (Inputs : Intf.Test.Inputs_intf) = struct
  ...
  let send_padding_transactions ~fee ~logger ~n nodes =
    ...
    repeat_seq ~n ~f:(fun () ->
        Graphql_requests.must_send_online_payment ~logger (Network.Node.get_ingress_uri sender) ~sender_pub_key
    ...
    let%bind () =
      section_hard
        "Send a zkapp commands with fee payer nonce increments and nonce \
         preconditions"
        (send_zkapp_batch ~logger (Network.Node.get_ingress_uri node)
    ...
    let%bind () =
      section_hard
        ...
        (let%bind { nonce = fish1_nonce; _ } =
           Graphql_requests.get_account_data ~logger (Network.Node.get_ingress_uri node)
    ...
    let%bind () =
      section_hard
        ...
        (let%bind pooled_zkapp_commands =
           Graphql_requests.get_pooled_zkapp_commands ~logger (Network.Node.get_ingress_uri node) ~pk:fish1_pk
           |> Deferred.bind ~f:Malleable_error.or_hard_error
         in
