...
let main inputs =
  ...
  let%bind monitor_test_result =
    ...
    Monitor.try_with ~here:[%here] ~extract_exn:false (fun () ->
        ...
        (* let seed_node_set =
             ref
               (String.Set.of_list (seed_nodes |> List.map ~f:(fun n -> Engine.Network.Node.id n)))
           in *)
        ...
        in
        (* let%bind () =
           Dsl.Event_router.on ( event_router) Event_type.Node_initialization
                  ~f:(fun node () ->
                    if
                      String.Set.exists !seed_node_set ~f:(fun n ->
                          String.equal n (Engine.Network.Node.id node))
                    then (
                      seed_node_set := String.Set.remove !seed_node_set (Engine.Network.Node.id node) ;
                      if String.Set.is_empty !seed_node_set then `Stop `Success
                      else `Continue )
                    else `Continue)
                |> Dsl.Event_router.await_with_timeout event_router
