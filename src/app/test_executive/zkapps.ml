  ...
  let run network t =
    ...
    let%bind parties_create_account =
      ...
      let (parties_spec : Transaction_snark.For_tests.Spec.t) =
        ...
        ; zkapp_update = Party.Update.dummy
      ...
      in
      return
      @@ Transaction_snark.For_tests.deploy_zkapp ~constraint_constants
           parties_spec
    in
    ...
    in
    let%bind.Deferred zkapp_update_all, parties_update_all =
      let open Mina_base in
      ...
      let%map.Deferred parties_update_all =
        Transaction_snark.For_tests.update_states ~constraint_constants
          parties_spec
      in
      (zkapp_update, parties_update_all)
    in
    ...
    let wait_for_zkapp parties =
      let%map () =
        ...
        @@ Wait_condition.zkapp_to_be_included_in_frontier ~parties
      in
      [%log info] "Zkapps transaction included in transition frontier"
    in
    let%bind () =
      section "Send a zkapp to create zkapp accounts"
    ...
    in
    let%bind () =
      section "Send a zkapp to update permissions"
        (send_zkapp ~logger node parties_update_permissions)
    in
    let%bind () =
      section
        "Wait for zkapp to update permissions to be included in transition \
         frontier"
    ...
    let%bind () =
      section "Send a zkapp to update all fields"
        (send_zkapp ~logger node parties_update_all)
    in
    let%bind () =
      section
        "Wait for zkapp to update all fields to be included in transition \
         frontier"
    ...
    in
    let%bind () =
      section "Send a zkapp with an invalid nonce"
        (send_invalid_zkapp ~logger node parties_invalid_nonce "Invalid_nonce")
    in
    let%bind () =
      section "Send a zkapp with an invalid signature"
