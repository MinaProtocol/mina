...
module Make (Inputs : Intf.Test.Inputs_intf) = struct
  ...
  let send_padding_transactions ~fee ~logger ~n nodes =
    ...
    repeat_seq ~n ~f:(fun () ->
      Integration_test_lib.Graphql_requests.must_send_online_payment ~logger (Network.Node.get_ingress_uri sender) ~sender_pub_key
  ...
  let send_payment_from_zkapp_account ?expected_failure
    ...
    | None ->
        ...
        Integration_test_lib.Graphql_requests.must_send_payment_with_raw_sig ~logger
          ...
          ~nonce ~memo ~valid_until ~raw_signature node_uri
  ...
  let run network t =
    ...
    let%bind () =
      section_hard "Send a zkApp transaction to create zkApp accounts"
        (send_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_create_accounts)
    in
    let%bind () =
      section_hard
    ...
    let%bind () =
      ...
      section_hard "Send a valid payment from zkApp account"
        (send_payment_from_zkapp_account ~constraint_constants ~node_uri:(Network.Node.get_ingress_uri node) ~logger
           sender nonce )
    in
    let%bind () =
      section_hard "Send a zkApp transaction to update permissions"
        (send_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_update_permissions)
    in
    let%bind () =
      section_hard
    ...
    let%bind () =
      ...
      section_hard "Send an invalid payment from zkApp account"
        (send_payment_from_zkapp_account ~constraint_constants ~logger sender
           nonce ~node_uri:(Network.Node.get_ingress_uri node)
           ~expected_failure:
      ...
      section_hard "Verify that updated permissions are in ledger accounts"
        (Malleable_error.List.iter zkapp_account_ids ~f:(fun account_id ->
             ...
             let%bind ledger_permissions =
               get_account_permissions ~logger (Network.Node.get_ingress_uri node) account_id
             in
    ...
    let%bind () =
      section_hard "Send a zkapp with an insufficient fee"
        (send_invalid_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_insufficient_fee
           "Insufficient fee" )
    in
    (* Won't be accepted until the previous transactions are applied *)
    let%bind () =
      section_hard "Send a zkApp transaction to update all fields"
        (send_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_update_all)
    in
    let%bind () =
      section_hard "Send a zkapp with an invalid proof"
        (send_invalid_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_invalid_proof
           "Verification_failed" )
    in
    let%bind () =
      section_hard "Send a zkapp with an insufficient replace fee"
        (send_invalid_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_insufficient_replace_fee
           "Insufficient_replace_fee" )
    in
    let%bind () =
      section_hard "Send a zkApp transaction with an invalid nonce"
        (send_invalid_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_invalid_nonce
           "Invalid_nonce" )
    in
    let%bind () =
      section_hard
        "Send a zkApp transaction with insufficient_funds, fee too high"
        (send_invalid_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_insufficient_funds
           "Insufficient_funds" )
    in
    let%bind () =
      section_hard "Send a zkApp transaction with an invalid signature"
        (send_invalid_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_invalid_signature
           "Verification_failed" )
    in
    let%bind () =
      section_hard "Send a zkApp transaction with a nonexistent fee payer"
        (send_invalid_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_nonexistent_fee_payer
    ...
    let%bind () =
      section_hard "Send a zkApp transaction to mint token"
        (send_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_mint_token)
    in
    let%bind () =
      section_hard "Send a zkApp transaction to mint 2nd token"
        (send_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_mint_token2)
    in
    let%bind () =
      section_hard "Send a zkApp transaction to transfer tokens"
        (send_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_token_transfer)
    in
    let%bind () =
      section_hard "Send a zkApp transaction to transfer tokens (2)"
        (send_zkapp ~logger (Network.Node.get_ingress_uri node) zkapp_command_token_transfer2)
    in
    let%bind () =
      ...
      section_hard "Verify zkApp transaction updates in ledger"
        (Malleable_error.List.iter zkapp_account_ids ~f:(fun account_id ->
             ...
             let%bind ledger_update =
               get_account_update ~logger (Network.Node.get_ingress_uri node) account_id
             in
             if
