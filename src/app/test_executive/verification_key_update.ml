  ...
  let run network t =
    ...
    let%bind () =
      section "Send a zkApp to create a zkApp account"
        (send_zkapp ~logger (Network.Node.get_ingress_uri whale1) zkapp_command_create_account)
    in
    let%bind () =
      section
    ...
    let%bind () =
      section
        ...
        (send_zkapp ~logger (Network.Node.get_ingress_uri whale1) zkapp_command_update_vk1)
    in
    ...
    let%bind () =
      section
        "Send zkApp to update to a new verification key v2 and then refers to \
         the old key v1"
        (send_invalid_zkapp ~logger (Network.Node.get_ingress_uri whale1) zkapp_command_update_vk2_refers_vk1
           "Verification_failed" )
    in
    let%bind () =
      section
        "Send zkApp to update to a new verification key v2 and then refers to \
         that"
        (send_zkapp ~logger (Network.Node.get_ingress_uri whale1) zkapp_command_update_vk2)
    in
    let%bind () =
      section
