...
type _unused = unit constraint t = t
...
type _ Request.t +=
  | Get_path : Domaccount.Index.t -> path Request.t
  | Get_element : Domaccount.Index.t -> (Domaccount.t * path) Request.t
  | Set : Domaccount.Index.t * Domaccount.t -> unit Request.t
  | Find_index : Domain.t -> Account.Index.t Request.t
...
*)
let%snarkydef modify_domaccount ~depth t aid
    ~(filter : Domaccount.var -> ('a, _) Checked.t) ~f =
  let%bind addr =
    request_witness
      (Domaccount.Index.Unpacked.typ ~ledger_depth:depth)
      As_prover.(map (read Domain.typ aid) ~f:(fun s -> Find_index s))
  in
  handle
    (Merkle_tree.modify_req ~depth (var_to_hash_packed t) addr
       ~f:(fun domaccount ->
         let%bind x = filter domaccount in
         f x domaccount ))
    reraise_merkle_requests
  >>| var_of_hash_packed
...
(*
   ...
   - returns a root [t'] of a tree of depth [depth] which is [t] but with the
     account [f account] at path [addr].
*)
let%snarkydef modify_domaccount_send ~depth t aid ~is_writeable ~f =
  modify_domaccount ~depth t aid
    ~filter:(fun domaccount ->
      let%bind account_already_there =
        Domain.Checked.equal (Domaccount.identifier_of_var domain) aid
      in
      let%bind account_not_there =
        Domain.Checked.equal domaccount.domain
          Domain.(var_of_t empty)
      in
