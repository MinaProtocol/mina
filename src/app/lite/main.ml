...
let drop_top_of_tree ~desired_layers =
  ...
  let rec drop n t =
    if n = 0 then t
      ...
      | Node (_, Hash _, t') | Node (_, t', Hash _) ->
          drop (n - 1) t'
  ...
  module Record = struct
    ...
    let render ?tooltip ?(grouping = `Separate) {style; rows; heading} width =
      ...
      let grouping =
        match grouping with
        | `Thin_together ->
            Style.of_class "b-silver ph2 pv2 br3"
        | `Together ->
            grouping_style
        | `Separate ->
            Style.empty
  ...
  let mobile_update_toggle _ =
    ( if js_is_mobile_small () then
      let target' =
        ...
        | Tooltip_stage.None, _ ->
            target
        | x, y when Tooltip_stage.equal x y ->
            Tooltip_stage.None
        | _ ->
            target
      in
  ...
  fun state (tree0 : ('hash, 'account) Sparse_ledger_lib.Sparse_ledger.tree)
    ...
    let specs =
      let rec go acc layer index
        ...
        match t with
        | Account account ->
            finish (Spec.Account {account; pos})
        | Hash _h ->
            finish hash_spec
        | Node (_h, Hash _, r) ->
...
let state_html state =
  ...
  let proof_style =
    ...
    match verification with
    | `Complete (Ok _) ->
        "grass-gradient b-grass"
    | `Complete (Error _) ->
        "proof"
    | `Pending _ ->
        "proof verifying shake shake-constant"
  in
  let proof_style = Style.(proof_style + of_class "wb") in
  let state_hash =
    match verification with
    | `Complete (Ok {state_hash}) ->
        field_to_base64 state_hash
    | `Complete (Error _e) ->
        "Could not verify SNARK"
    | `Pending _ ->
