let proof_tooltip =
  ...
  ; `Text
      " proof, which serves as a cryptographic certificate of the protocol \
       state's integrity.\n\
       This certificate, called a \"succinct blockchain\", takes \
       the place of the GBs long, ever-growing blockchains used for \
...
let state_tooltip =
  [ `Text
      "The zk-SNARK proof serves as a succinct blockchain,\n\
       validating the entire protocol state just as a heavy \
       blockchain does in existing cryptocurrencies."
  ; `New_line
  ...
  ; `Text
      " of two versions of the database of accounts.\n\
       Changes to accounts are reflected immediately in the staged \
       ledger hash.\n\
       The locked ledger hash is set from the staged ledger hash \
       periodically,\n\
...
let account_tooltip =
  [ `Text
      "Once a client has a protocol state and a succinct blockchain \
       ...
       Namely, they need a merkle-path from the protocol state's \
       ledger hash to their account."
  ; `New_line
  ; `Text
      "The succinct blockchain, protocol state, merkle-path, and account \
       information are altogether just a few kilobytes,\n\
       so Coda can provide a full proof of the state of an \
       account with just this tiny amount of data." ]
