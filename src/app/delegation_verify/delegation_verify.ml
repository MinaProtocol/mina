...
let instantiate_verify_functions ~logger = function
  ...
  | Some config_file ->
      ...
      let%map.Deferred precomputed_values =
        ...
        | Ok ((precomputed_values : Precomputed_values.t), _) ->
    ...
    Command.Let_syntax.(
      ...
      fun () ->
        ...
        let module V = Make_verifier (struct
          ...
          let verify_blockchain_snarks x =
            Deferred.map ~f:(function
              | true ->
                  Or_error.return ()
              | false ->
                  Or_error.error_string "blockchain snarks verification failed" )
            @@ verify_blockchain_snarks x

          let verify_transaction_snarks x =
            Deferred.map ~f:(function
              | true ->
                  Or_error.return ()
              | false ->
                  Or_error.error_string "transaction snarks verification failed" )
            @@ verify_transaction_snarks x
        end) in
        let open Deferred.Let_syntax in
        match%bind
