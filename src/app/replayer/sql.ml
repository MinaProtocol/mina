...
module User_command = struct
  ...
  let query =
    Caqti_request.collect Caqti_type.int typ
      {sql| SELECT type,fee_payer_id, source_id,receiver_id,fee,fee_token_id,token_id,amount,valid_until,memo,nonce,
...
module Internal_command = struct
  ...
  let typ =
    Mina_caqti.Type_spec.custom_type ~to_hlist ~of_hlist
      Caqti_type.[ string; int; int64; int; int; int64; int64; int64; int; int ]
  ...
  *)
  let query =
    Caqti_request.collect Caqti_type.int typ
      {sql| SELECT type,receiver_id,fee,token_id,
