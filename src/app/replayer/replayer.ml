...
let main ~input_file ~runtime_config ~output_file_opt ~migration_mode
  ...
  let load_ledger genesis_ledger =
    let%map packed_ledger =
      match%bind
        Genesis_ledger_helper.Ledger.load ~proof_level
          ...
          ~logger ~constraint_constants genesis_ledger
      with
      | Error e ->
          [%log fatal] "Could not load accounts from input: %s"
    ...
    | Error msg ->
        failwith
          (sprintf "Could not parse JSON in input file \"%s\": %s" input_file
             msg )
  in
  let%bind ledger = load_ledger input.genesis_ledger in
  ...
  | Some config_file ->
      ...
      in
      let%map genesis_ledger =
        load_ledger (Option.value_exn runtime_config.ledger)
      in
      let checkpoint_ledger_hash = Ledger.merkle_root ledger in
      let genesis_ledger_hash = Ledger.merkle_root genesis_ledger in
      if Ledger_hash.equal checkpoint_ledger_hash genesis_ledger_hash then
        [%log info]
          "Ledger in checkpoint file is the same as the ledger in genesis \
           ledger: %s"
          (Ledger_hash.to_base58_check genesis_ledger_hash)
      else
        [%log error]
          ~metadata:
            [ ( "genesis_ledger_hash"
              , `String (Ledger_hash.to_base58_check genesis_ledger_hash) )
            ; ( "checkpoint_ledger_hash"
              , `String (Ledger_hash.to_base58_check checkpoint_ledger_hash) )
            ]
          "Ledger in checkpoint file is different from the ledger in genesis \
           ledger"
  | None -> (
          ...
          *)
          ...
          if Int64.equal input.start_slot_since_genesis 0L then
            ...
            if Int64.Table.mem global_slot_hashes_tbl Int64.zero then
              ...
              [%log fatal]
                "Block chain leading to target state hash does not include \
                 genesis block" ;
              Core_kernel.exit 1 ) ;
          ...
          in
          [%log info]
            "Obtained %d user command ids, %d internal command ids, and %d \
             zkApp command ids"
          ...
          let%bind unsorted_internal_cmds_list =
            Deferred.List.map internal_cmd_ids ~f:(fun id ->
                ...
                with
                | Ok [] ->
                    failwithf "Could not find any internal commands with id: %d"
                      id ()
                | Ok internal_cmds ->
                    internal_cmds
                | Error msg ->
                    failwithf
                      "Error querying for internal commands with id %d, error \
                       %s"
                      id (Caqti_error.show msg) () )
          in
          let unsorted_internal_cmds =
            List.concat unsorted_internal_cmds_list
          in
                  ...
                  | _ ->
                      failwith
                        "Two internal commands have the same global slot since \
                         genesis %Ld, sequence no %d, and secondary sequence \
                         no %d, but are not a coinbase and fee transfer via \
                         coinbase"
                else cmp )
          in
          ...
          let filtered_zkapp_cmds =
            List.filter unsorted_zkapp_cmds
              ~f:(fun (cmd : Sql.Zkapp_command.t) ->
          ...
          let%bind genesis_snarked_ledger_hash =
            let%map hash_str =
              query_db ~f:(fun db ->
                  Sql.Block.genesis_snarked_ledger db
                    input.start_slot_since_genesis )
            in
            ...
            Option.iter !checkpoint_target ~f:(fun target ->
                ...
                | Some interval ->
                    ...
                    if Int64.( <= ) new_target max_canonical_slot then (
                      [%log info] "Checkpoint target was %Ld, setting to %Ld"
                        target new_target ;
          ...
          let rec apply_commands ~last_global_slot_since_genesis ~last_block_id
            ...
            let check_account_accessed state_hash =
              ...
              in
              [%log info]
                "Verifying that accounts created are also deemed accessed in \
                 block with global slot since genesis %Ld"
                last_global_slot_since_genesis ;
              ...
              List.iter accounts_created_db
                ~f:(fun { account_identifier_id = acct_id_created; _ } ->
                  ...
                  then (
                    [%log error]
                      "Created account not present in accessed accounts"
                      ~metadata:
            ...
            let run_transactions_on_slot_change ?(last_block = false) block_txns
              ...
              | None ->
                  ...
                  then (
                    ...
                    [%log fatal]
                      "Missing ledger hash information for last global slot, \
                       which is not the start slot" ;
                    Core.exit 1 )
              | Some (state_hash, ledger_hash, snarked_hash) ->
                  let write_checkpoint_file ~checkpoint_output_folder_opt
                    ...
                    if last_block then write_checkpoint ()
                      ...
                      | None ->
                          Deferred.unit
                      | Some target ->
                          if Int64.(last_global_slot_since_genesis >= target)
                          then ( incr_checkpoint_target () ; write_checkpoint ()
                            )
                          else Deferred.unit
                  in
                  let rec count_txns ~signed_count ~zkapp_count
                      ~fee_transfer_count ~coinbase_count = function
                    | [] ->
                        [%log info]
                        ...
                        | Mina_transaction.Transaction.Command cmd -> (
                            ...
                            | User_command.Signed_command _ ->
                                count_txns ~signed_count:(signed_count + 1)
                                  ~zkapp_count ~fee_transfer_count
                                  ~coinbase_count txns
                            | Zkapp_command _ ->
                                count_txns ~signed_count
                                  ~zkapp_count:(zkapp_count + 1)
                  ...
                  let run_transactions () =
                    count_txns ~signed_count:0 ~zkapp_count:0
                      ~fee_transfer_count:0 ~coinbase_count:0 block_txns ;
                    ...
                    let apply_transaction_phases txns =
                      let%bind phase_1s =
                        Deferred.List.mapi txns ~f:(fun n txn ->
                            ...
                            | Ok partially_applied ->
                                ...
                                in
                                let%map () =
                                  update_next_epoch_data ~logger pool
                                    ~last_block_id ~ledger ~next_epoch_ledger
                                    ~next_seed
                                in
                                partially_applied
                            | Error err ->
                                [%log error]
                                  "Error during Phase 1 application of \
                                   transaction %d (0-based) in block with \
                                  ...
                                  ~metadata:
                                    ...
                                    ; ( "transaction"
                                      , Mina_transaction.Transaction.to_yojson
                                          txn )
                                    ; ( "error"
                      ...
                      Deferred.List.map phase_1s ~f:(fun partial ->
                          ...
                          | Error err ->
                              (* must be a zkApp *)
                              ( match partial with
                              | Ledger.Transaction_partially_applied
                                .Zkapp_command zk_partial ->
                                  let cmd = zk_partial.command in
                                  [%log error]
                                    "Error during Phase 2 application of \
                                     partially-applied zkApp in block with \
                                     state hash $state_hash"
                                    ~metadata:
                                      [ ( "state_hash"
                                        , State_hash.to_yojson state_hash )
                ...
                then
                  ...
                  if List.is_empty block_txns then (
                    [%log info]
                      "No transactions to run for block with state hash \
                       $state_hash"
                      ~metadata:
                        [ ("state_hash", State_hash.to_yojson state_hash)
                        ; ( "global_slot_since_genesis"
                        ...
                        let accounts_accessed =
                          List.concat_map transactions_applied
                            ~f:(fun txn_applied ->
                              ...
                              .account_access_statuses txn status )
                        ...
                        Deferred.List.iter accounts_accessed ~f:(fun acct_id ->
                            ...
                            query_db ~f:(fun db ->
                                Processor.Accounts_accessed.add_if_doesn't_exist
                                  db last_block_id ~logger (index, acct) )
                            |> Deferred.ignore_m )
                      else (
            ...
            | [], uc :: ucs, zkc :: zkcs -> (
                ...
                | `User_command ->
                    ...
                    in
                    let%bind txn =
                      user_command_to_transaction ~logger ~pool uc
                    in
                    apply_commands ~block_txns:(txn :: block_txns)
                      ~last_global_slot_since_genesis:
                        uc.global_slot_since_genesis ~last_block_id:uc.block_id
                      internal_cmds ucs zkapp_cmds
                | `Zkapp_command ->
                    let%bind block_txns =
                      check_for_complete_block
                        ~cmd_global_slot_since_genesis:
                    ...
                    in
                    let%bind txn =
                      zkapp_command_to_transaction ~logger ~pool zkc
                    in
                    apply_commands ~block_txns:(txn :: block_txns)
                      ~last_global_slot_since_genesis:
                        zkc.global_slot_since_genesis
            ...
            | ic :: ics, [], zkc :: zkcs -> (
                ...
                | `Internal_command ->
                    ...
                    apply_commands ~block_txns
                      ~last_global_slot_since_genesis:
                        ic.global_slot_since_genesis ~last_block_id:ic.block_id
                      ics' user_cmds zkapp_cmds
                | `Zkapp_command ->
                    let%bind block_txns =
                      check_for_complete_block
                        ~cmd_global_slot_since_genesis:
          ...
          if
            not
              (Int64.equal start_slot_since_genesis
                 input.start_slot_since_genesis )
          then
            [%log info]
          ...
          let%bind ( staking_epoch_ledger
                   , staking_seed
                   , next_epoch_ledger
                   , next_seed ) =
            apply_commands ~block_txns:[]
              ~last_global_slot_since_genesis:start_slot_since_genesis
              ~last_block_id:oldest_block_id sorted_internal_cmds
              sorted_user_cmds sorted_zkapp_cmds
          in
          match input.target_epoch_ledgers_state_hash with
          | None ->
              [%log info]
                "No target epoch ledger hash supplied, not writing output" ;
              Deferred.unit
    ...
    run
      ...
      Command.async ~summary:"Replay transactions from Mina archive database"
        (let%map input_file =
         ...
         and runtime_config =
           Param.flag "--verify-checkpoint-ledger"
             ~doc:
               "PATH path to the runtime config, this subcommand is intended \
                to verify the ledger from the final checkpoint file for \
                migration against the ledger referred by the new runtime \
                config file"
         ...
         in
         main ~input_file ~runtime_config ~output_file_opt ~migration_mode
