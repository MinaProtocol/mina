  ...
  don't_wait_for
    (let epsilon = 1.0 in
     let rec go () =
       ...
       if not (diff < Time.Span.to_sec acceptable_delay +. epsilon)
       then (
...
let start_checks log workers payment_reader start_reader
    testnet ~acceptable_delay =
...
let test log n should_propose snark_work_public_keys work_selection =
  ...
  start_checks log workers payment_reader start_reader
    testnet ~acceptable_delay;
  testnet
