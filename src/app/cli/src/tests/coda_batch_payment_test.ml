...
let main () =
  ...
  in
  let keypairs =
    let open Precomputed_values in
    List.map
      (Lazy.force (accounts precomputed_values))
      ~f:keypair_of_account_record_exn
  in
  let largest_account_sk, largest_account =
  ...
  let snark_work_public_keys i =
    if i = 0 then Some largest_account.public_key else None
  in
  let num_nodes = 3 in
  let%bind testnet =
  ...
  let%bind payments =
    Coda_worker_testnet.Payments.send_batch_consecutive_payments testnet
      ~node:0 ~keypairs
      ~sender:(Option.value_exn largest_account_sk)
      ~n:4
  in
  let%bind () =
