...
module T = struct
  ...
  type 'worker functions =
    ...
    ; best_tip_diff:
        ( 'worker
        , unit
        , User_command.t Protocols.Coda_pow.Best_tip_diff_view.t Pipe.Reader.t
        )
        Rpc_parallel.Function.t
    ; prove_receipt:
        ( 'worker
  ...
  struct
    ...
    let best_tip_diff_impl ~worker_state ~conn_state:() () =
      worker_state.coda_best_tip_diff ()
    ...
    let best_tip_diff =
      C.create_pipe ~f:best_tip_diff_impl ~bin_input:Unit.bin_t
        ~bin_output:
          [%bin_type_class:
    ...
    let init_worker_state
      ...
      let coda_process_payment cmd =
        let%map receipt = Run.send_payment log coda (cmd :> User_command.t) in
        receipt |> Participating_state.active_exn
      in
      ...
      let coda_strongest_ledgers () =
        ...
        return r.pipe
      in
      let coda_best_tip_diff () =
        let r, w = Linear_pipe.create () in
        don't_wait_for
          (Strict_pipe.Reader.iter (Main.best_tip_diff coda) ~f:(fun diff ->
               Linear_pipe.write w diff )) ;
        return r.pipe
      in
      { coda_peers= with_monitor coda_peers
      ; coda_strongest_ledgers= with_monitor coda_strongest_ledgers
      ; coda_best_tip_diff= with_monitor coda_best_tip_diff
