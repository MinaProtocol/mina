...
let generate_blocks logger =
  let generation_flags =
    let open Command.Let_syntax in
    let%map_open blocks_filename =
      flag "-blocks-filename" (required string)
        ~doc:"PATH The file to write the generated blocks to"
    and skip_slots =
      flag "-skip-slots" (required int)
        ~doc:"The number of slots to skip over after each successful block"
    in
    (blocks_filename, skip_slots)
  in
  Command.async ~summary:"Start coda daemon in block precomputation mode"
    (Command.Param.map2 generation_flags (setup_daemon logger)
       ~f:(fun (blocks_filename, skip_slots) setup_daemon () ->
         (* Enable updating the time offset. *)
         Block_time.Controller.enable_setting_offset () ;
         let%bind coda = setup_daemon () in
         let%bind () =
           Coda_lib.generate_precomputed_blocks coda ~filename:blocks_filename
             ~skip_slots
         in
         [%log info]
           "Daemon ready, starting block generation. Clients can now connect" ;
         Async.never () ))
    ...
    , Command.async ~summary:"Dump the registered structured events"
        ...
        fun () ->
          ...
          | None ->
              () ) ;
          Deferred.return ()) )
  ; ("compute-blocks", generate_blocks logger)
