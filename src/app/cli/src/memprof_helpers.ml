...
let clean () =
  ...
  let rec aux i j =
    if i >= sz then j
    else if Ephemeron.K1.check_key s.(i) then (
      s.(j) <- s.(i) ;
      aux (i + 1) (j + 1) )
    else aux (i + 1) j
  in
  ...
  else if 2 * !n_samples > Array.length s then (
...
let callback : sample_info Memprof.callback =
 fun info ->
...
type sortedSampleTree =
  | SSTC of
      sample_info list * int * (raw_backtrace_slot * sortedSampleTree) list
...
let rec sort_sampleTree (t : sampleTree) : sortedSampleTree =
  ...
  SSTC
    ...
    , n
    , List.sort
        (fun (_, SSTC (_, n1, _)) (_, SSTC (_, n2, _)) -> n2 - n1)
    ...
    List.iter (fun (li, SSTC (_sl, n, tl)) ->
        if min_samples <= n then (
          ...
          | Some {filename; line_number; start_char; end_char} ->
              Printf.fprintf chan "%7d | %s%s:%d %d-%d" n indent filename
                line_number start_char end_char
          | None ->
...
let dump () =
  ...
  let rec aux st i =
    if i >= sz then st
      ...
      match Ephemeron.K1.get_data s.(i) with
      | None ->
          aux st (i + 1)
      | Some s ->
          aux (add_sampleTree s st) (i + 1)
  in
