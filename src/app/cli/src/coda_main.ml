...
module type Main_intf = sig
  ...
  val best_ledger_builder : t -> Inputs.Staged_ledger.t
...
struct
  ...
  module Tip = struct
    ...
    let copy t = {t with ledger_builder= Staged_ledger.copy t.ledger_builder}
  end
end
...
module Run (Config_in : Config_intf) (Program : Main_intf) = struct
  ...
  let get_status t =
    ...
    ; ledger_merkle_root
    ; ledger_builder_hash=
        best_ledger_builder t |> Staged_ledger.hash
