...
let start_auto_hardfork_config_generation ~logger mina =
  ...
  | Cli_lib.Arg_type.Hardfork_handling.Migrate_exit -> (
      match
        ...
        , Mina_lib.scheduled_hard_fork_genesis_slot mina )
      with
      | Some slot_chain_end, Some slot_tx_end, Some _scheduled_genesis -> (
          ...
          [%log info] "Generating hardfork config in daemon config directory" ;
          let%bind result =
            Mina_lib.Hardfork_config.dump_reference_config
              ~breadcrumb_spec:`Stop_slot ~directory_name:config.conf_dir mina
          in
          match result with
          | Ok () ->
              [%log info]
