...
let create_ledger_and_transactions num_transactions :
  ...
  let random_transaction () : Signed_command.With_valid_signature.t =
    ...
    let fee = Currency.Fee.nanomina_unsafe (1 + Random.int 100) in
    let amount = Currency.Amount.nanomina_unsafe (1 + Random.int 100) in
  ...
  | `Two_from_same ->
      let a =
        txn keys.(0) keys.(1)
          (Currency.Amount.nanomina_unsafe 10)
