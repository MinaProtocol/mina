...
let run ~genesis_constants ~constraint_constants ~proof_level ~user_command_profiler ~zkapp_profiler num_transactions ~max_num_updates
  ...
  if use_zkapps then (
    let ledger, transactions =
      Async.Thread_safe.block_on_async_exn (fun () ->
          create_ledger_and_zkapps ~genesis_constants ~constraint_constants ?min_num_updates ~max_num_updates () )
    in
...
let dry ~genesis_constants ~constraint_constants ~proof_level ~max_num_updates ?min_num_updates num_transactions repeats preeval
    use_zkapps () =
  let zkapp_profiler ~verifier:_ _ _ =
    failwith "Can't check base SNARKs on zkApps"
  in
  Test_util.with_randomness 123456789 (fun () ->
      run ~genesis_constants ~constraint_constants ~proof_level ~user_command_profiler:(check_base_snarks ~genesis_constants ~constraint_constants) ~zkapp_profiler
        num_transactions ~max_num_updates ?min_num_updates repeats preeval
        use_zkapps )

let witness ~genesis_constants ~constraint_constants ~proof_level ~max_num_updates ?min_num_updates num_transactions repeats preeval
    use_zkapps () =
  let zkapp_profiler ~verifier:_ _ _ =
    failwith "Can't generate witnesses for base SNARKs on zkApps"
  in
  Test_util.with_randomness 123456789 (fun () ->
      run ~genesis_constants ~constraint_constants ~proof_level ~user_command_profiler:(generate_base_snarks_witness ~genesis_constants ~constraint_constants) ~zkapp_profiler
        num_transactions ~max_num_updates ?min_num_updates repeats preeval
        use_zkapps )

let main ~(genesis_constants : Genesis_constants.t) ~(constraint_constants : Genesis_constants.Constraint_constants.t) 
      ~proof_level ~max_num_updates ?min_num_updates num_transactions repeats preeval
    use_zkapps () =
  Test_util.with_randomness 123456789 (fun () ->
      ...
      run 
        ~genesis_constants ~constraint_constants ~proof_level ~user_command_profiler:(profile_user_command ~genesis_constants ~constraint_constants (module T))
        ~zkapp_profiler:(profile_zkapps ~constraint_constants) num_transactions ~max_num_updates
  ...
  Command.basic ~summary:"transaction snark profiler"
    (let%map_open n =
     ...
     if witness_only then
       witness ~genesis_constants ~constraint_constants ~proof_level ~max_num_updates ?min_num_updates num_transactions repeats
         preeval use_zkapps
     else if check_only then
       dry ~genesis_constants ~constraint_constants ~proof_level ~max_num_updates ?min_num_updates num_transactions repeats preeval
         use_zkapps
     else
       main ~genesis_constants ~constraint_constants ~proof_level ~max_num_updates ?min_num_updates num_transactions repeats preeval
         use_zkapps )
