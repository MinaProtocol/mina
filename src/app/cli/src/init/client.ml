...
let generate_libp2p_keypair_do privkey_path =
  ...
  Deferred.ignore_m
    ...
    Mina_stdlib.File_system.with_temp_dir "mina-generate-libp2p-keypair" ~f:(fun tmpd ->
        match%bind
...
let dump_libp2p_keypair_do privkey_path =
  ...
  Deferred.ignore_m
    ...
    let logger = Logger.null () in
    (* Using the helper only for keypair generation requires no state. *)
    Mina_stdlib.File_system.with_temp_dir "mina-dump-libp2p-keypair" ~f:(fun tmpd ->
        match%bind
          Mina_net2.create ~logger ~conf_dir:tmpd ~all_peers_seen_metric:false
            ~pids:(Child_processes.Termination.create_pid_table ())
            ~on_peer_connected:ignore ~on_peer_disconnected:ignore ()
