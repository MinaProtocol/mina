...
let%test_module "archive_unit_test" =
  ( module struct
    let runtime_config = Runtime_config.for_unit_tests

    let {Precomputed_values.genesis_ledger; base_proof; _} =
      Or_error.ok_exn
        (Async.Thread_safe.block_on_async_exn (fun () ->
             Precomputed_values.load_values ~logger:(Logger.null ())
               ~not_found:`Error ~runtime_config () ))
      ...
      Quickcheck.test ~trials:20
        ...
        @@ Quickcheck_lib.gen_imperative_list
             (Transition_frontier.For_tests.gen_genesis_breadcrumb
                ~genesis_ledger:Genesis_ledger.t ~runtime_config ~base_proof ())
