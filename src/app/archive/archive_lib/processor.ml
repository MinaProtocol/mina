...
module User_command = struct
  module Signed_command = struct
    ...
    type public_keys = {fee_payer_id: int; source_id: int; receiver_id: int}

    let add_public_keys_if_don't_exist (module Conn : CONNECTION)
        (t : Signed_command.t) =
      let open Deferred.Result.Let_syntax in
    ...
    let add_if_doesn't_exist ?(via = `Ident) (module Conn : CONNECTION)
      ...
      | None ->
          let%bind {fee_payer_id; source_id; receiver_id} =
            add_public_keys_if_don't_exist (module Conn) t
          in
          let valid_until =
...
module Block_and_signed_command = struct
  type t =
    ...
    ; fee_payer_balance_id: int option
  ...
  let add_with_status (module Conn : CONNECTION) ~block_id ~user_command_id
    ...
    let add_optional_balance id balance =
      ...
      | Some balance ->
          let%map balance_id =
            Balance.add (module Conn) ~public_key_id:id ~balance
          in
          Some balance_id
    in
    let%bind fee_payer_balance_id =
      add_optional_balance fee_payer_id fee_payer_balance
    in
          ...
          deferred_result_list_fold transactions ~init:0 ~f:(fun sequence_no ->
            ...
            | { Mina_base.With_status.status
              ; data= Mina_base.Transaction.Command command } ->
                ...
                in
                let%bind {fee_payer_id; source_id; receiver_id} =
                  User_command.Signed_command.add_public_keys_if_don't_exist
                    (module Conn)
            ...
            | {data= Fee_transfer fee_transfer_bundled; status} ->
                ...
                in
                (* TODO: Need a better name for fee_transfer_ids now that receiver_pk was added to it? *)
    ...
    let%bind () =
      deferred_result_list_fold internal_cmd_ids_and_seq_nos ~init:()
        ~f:(fun ()
           (internal_command_id, (sequence_no, secondary_sequence_no))
           ->
          (* TODO: Add receiver_balance column to Extensional.Internal_command
