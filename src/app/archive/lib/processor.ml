...
let added_transactions {port; _} added =
  ...
  let%bind queried_existing_user_commands =
    let graphql =
      Graphql_query.User_commands.Query_first_seen.make
        ...
        ()
    in
    let%map obj = Graphql_client.query graphql port in
    let list =
  ...
  let graphql =
    Graphql_query.User_commands.Insert.make
      ...
      ()
  in
  let%map _result = Graphql_client.query graphql port in
  ()

let compute_with_receipt_chains ~logger
    (user_commands :
  ...
  List.folding_map sorted_user_commands
    ...
    ~f:(fun acc_sender_receipt_chain_hash_map
      ...
      let udpated_sender_receipt_chain, new_receipt_chain =
        Option.value_map ~default:(acc_sender_receipt_chain_hash_map, None)
          (Map.find acc_sender_receipt_chain_hash_map sender)
          ~f:(fun previous_receipt_chain ->
            let new_receipt_chain =
              Receipt.Chain_hash.cons user_command_payload
                previous_receipt_chain
            in
            let updated_acc_receipt_chain_hash_map =
              Map.set acc_sender_receipt_chain_hash_map ~key:sender
            ...
            Logger.trace ~module_:__MODULE__ ~location:__LOC__ logger
              !"New receipt chain (%{sexp:Receipt.Chain_hash.t}) computed \
                from %{sexp:Receipt.Chain_hash.t}"
              new_receipt_chain previous_receipt_chain ;
            (updated_acc_receipt_chain_hash_map, Some receipt_chain_input) )
      in
...
let tag_with_first_seen
  ...
  let%map first_query_response =
    let%map result = Graphql_client.query graphql port in
...
let added_transition {port; logger}
  ...
  let encoded_block =
    Types.Blocks.serialize block_with_hash
      (compute_with_receipt_chains user_commands_with_time ~logger
         sender_receipt_chains_from_parent_ledger)
  ...
  in
  let%bind _ = Graphql_client.query graphql port in
