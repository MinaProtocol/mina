    ...
    let query_string =
      [%string
        {| 
         ...
         -- The ledger charges the creation fee once. Identical account updates
         -- share a single zkapp_account_update row, so the creating update can
         -- surface under more than one applied command in a block: attach the
         -- fee to the first such command only.
         LEFT JOIN accounts_created ac
