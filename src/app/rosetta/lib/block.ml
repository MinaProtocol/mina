    ...
    let query_string =
      [%string
        {| 
         ...
         -- The ledger charges the creation fee once, however many times the
         -- creating update is applied: attach it to the first occurrence only.
         -- An account-update id can repeat within a command's array (see the
         -- unnest above) and identical updates share one row across commands,
         -- so without this guard one creation is billed once per occurrence.
         LEFT JOIN accounts_created ac
