...
relevant_internal_commands AS (
  SELECT
    ic.id,
    ic.receiver_id
  FROM internal_commands ic
  INNER JOIN public_keys pk ON pk.id = ic.receiver_id
  WHERE pk.value = $1
),

relevant_user_commands AS (
  SELECT
    uc.id,
    uc.fee_payer_id,
    uc.source_id,
  ...
  INNER JOIN public_keys pk1 ON pk1.id = uc.fee_payer_id
  INNER JOIN public_keys pk2 ON pk2.id = uc.source_id
  INNER JOIN public_keys pk3 ON pk3.id = uc.receiver_id
  WHERE
    (pk1.value = $1 OR pk2.value = $1 OR pk3.value = $1) AND
    (uc.type = 'delegation' OR uc.type = 'payment') AND
    (uc.status = 'applied')
),
...
relevant_internal_block_balances AS (
  SELECT
    ...
    block_internal_command.receiver_balance AS balance
  FROM blocks_internal_commands block_internal_command
  INNER JOIN relevant_internal_commands internal_command
  ON block_internal_command.internal_command_id = internal_command.id
),
...
relevant_user_block_balances AS (
  SELECT
    ...
    CASE WHEN pk1.value = $1 THEN block_user_command.fee_payer_balance
         WHEN pk2.value = $1 THEN block_user_command.source_balance
         ELSE block_user_command.receiver_balance
    END AS balance
  FROM blocks_user_commands block_user_command

  INNER JOIN relevant_user_commands user_command ON block_user_command.user_command_id = user_command.id

  INNER JOIN public_keys pk1 ON pk1.id = user_command.fee_payer_id
  INNER JOIN public_keys pk2 ON pk2.id = user_command.source_id
  INNER JOIN public_keys pk3 ON pk3.id = user_command.receiver_id
),
     ...
     fun ~env req ->
      ...
      | Some partial_identifier ->
          ...
          ; balances=
              ...
              (* TODO: Double-check that token_id handling here is correct. *)
          ; metadata }
  end
