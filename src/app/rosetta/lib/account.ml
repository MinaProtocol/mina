    ...
    let run (module Conn : Caqti_async.CONNECTION) requested_block_height
      ...
      if has_canonical_height then
        ...
        | Some (_, _, _, None) | None ->
            let%map result =
              Conn.find_opt query_canonical_fallback
            ...
            Option.map result ~f:(fun (slot, balance, nonce) ->
                (slot, balance, Option.map ~f:Int64.(( + ) one) nonce) )
      else
        match%bind
        ...
        | Some (_, _, _, None) | None ->
            let%map result =
              Conn.find_opt query_pending_fallback
                (address, requested_block_height)
            in
            (* The nonce is returned from this user-command, so we need to add one from here to get the current nonce in the account. *)
            Option.map result ~f:(fun (_pk, slot, balance, nonce) ->
                (slot, balance, Option.map ~f:Int64.(( + ) one) nonce) )
  end
    ...
    | Some account_identifier_id ->
        ...
        let%bind total_balance =
          ...
          | None, Some _ ->
              ...
              failwith
                "LOOKUP BALANCE, NONCE IN ACCOUNTS_ACCESSED; timing_info isn't \
                 just genesis ledger any longer"
