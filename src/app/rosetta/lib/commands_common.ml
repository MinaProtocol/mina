  ...
  let%test_module "Zkapp_command_info.to_transaction metadata" =
    ( module struct
      ...
      (* Property-based test: for any random valid memo (generated by
         Signed_command_memo.gen) and for arbitrary fuzzed memo strings
         (potentially invalid base58-check), to_transaction must:
           - always return Ok
      ...
      let%test_unit "PBT: metadata invariants over random memos" =
        let base = List.nth_exn dummies 0 in
        let memo_input_gen =
