...
let verify_message_command =
  ...
  fun () ->
    ...
    (* Convert string to the same input format that String_sign used *)
    let string_to_input s =
      ...
      Random_oracle.Input.Legacy.
        { field_elements = [||]
        ; bitstrings = Array.of_list_map (String.to_list s) ~f:char_bits
        }
    in
    let m = string_to_input message in
    let inner_curve = Snark_params.Tick.Inner_curve.of_affine pk in
    let signature_kind = Mina_signature_kind.t_DEPRECATED in
    match Schnorr.Legacy.verify ~signature_kind signature inner_curve m with
    | true ->
        return ()
    | false ->
