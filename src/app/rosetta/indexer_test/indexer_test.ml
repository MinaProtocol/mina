...
module Offset_limit = struct
  ...
  let test pool =
    ...
    Quickcheck.async_test ~trials:5 generator ~f:(fun (value_1, value_2) ->
        ...
        let%bind transactions_1_result =
          Ops.to_transactions ~search_include_timestamp:false info_1
        in
        let transactions_1' =
          ok_or_failwith Rosetta_lib.Errors.show transactions_1_result
        in
        let%bind info_2 = run' value_2 pool in
        let%map transactions_2_result =
          Ops.to_transactions ~search_include_timestamp:false info_2
        in
        let transactions_2' =
