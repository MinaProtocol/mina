  ...
  let groups =
    fold l ~init:[] ~f:(fun acc x ->
        ...
        | [] ->
            [[x]]
        | current_group :: tl ->
            if break (hd_exn current_group) x then [x] :: current_group :: tl
              (* start new group *)
            else (x :: current_group) :: tl )
    (* extend current group *)
  in
  match groups with [] -> [] | l -> rev_map l ~f:rev

module Node_network_info = struct
  type t =
    {local_ip: string; external_ip: string; peer_ips: string list; name: string}
  [@@deriving yojson]
end
  ...
  |> Core.List.filter_map ~f:(function
       | name :: local_ip :: external_ip :: peers :: _ ->
           let peer_ips =
             ...
             |> Core.List.filter_map ~f:(fun x ->
                    match String.split_on_char ':' x with
                    | [ip; _port] ->
                        Some ip
                    | _ ->
                      Core.(eprintf "skipping %s" x) ;
                      None
               )
           in
...
module Block_received = struct
  type t =
    {state_hash: string; sender_ip: [`Local of string | `External of string]}
end

module Event = struct
  type t = [`Produced of Block_produced.t | `Received of Block_received.t]

  let state_hash : t -> string = function
    | `Produced {state_hash} ->
        state_hash
    | `Received {state_hash; _} ->
        state_hash
end
...
let hashtbl_of_alist xs =
  ...
  List.iter (fun (k, v) -> Hashtbl.add t k v) xs ;
  t

let to_json_for_vis
    (events :
      ( [`State_hash of string]
      * ([`Node_name of string] * Block_received.t) list )
      list) (network : Node_network_info.t list) : Yojson.Safe.t =
  let _by_name =
    hashtbl_of_alist
      (List.map (fun x -> (x.Node_network_info.name, x)) network)
  in
  let by_local_ip =
    hashtbl_of_alist
      (List.map (fun x -> (x.Node_network_info.local_ip, x)) network)
  in
  let by_external_ip =
    hashtbl_of_alist
      (List.map (fun x -> (x.Node_network_info.external_ip, x)) network)
  in
  let network_nodes, network_edges =
    let nodes : Yojson.Safe.t =
      `List
        (List.map
           (fun (node : Node_network_info.t) ->
             `Assoc [("id", `String node.name); ("label", `String node.name)]
             )
           network)
    in
    ...
    let xs =
      List.map
        (fun (`State_hash state_hash, events) ->
          let edges =
            Core.List.filter_map
              ~f:(fun (`Node_name node, e) ->
                match e with
                | {Block_received.sender_ip; state_hash= _} ->
                  Core.with_return (fun { return } ->
                    let sender =
                      let ip =
                        match sender_ip with `Local ip | `External ip -> ip
                      in
                      if Core.String.is_prefix ip ~prefix:"10." then
                        ( match Hashtbl.find_opt by_local_ip ip with
                          | None -> Core.(eprintf "sender not found: %s\n%!" ip); return None
                          | Some x -> x)
                      else Hashtbl.find by_external_ip ip
                    in
                    Some (
                    `Assoc
...
module Events = struct
  module Instant = struct
    type t = {time: string; podRealName: string} [@@deriving yojson]
  end

  module Instant_with_sender = struct
    type t = {time: string; podRealName: string; sender: string}
    [@@deriving yojson]
  end
  ...
  type t = Event.t list [@@deriving yojson]
end

open Core

let () =
  let network, events = (Sys.argv.(1), Sys.argv.(2)) in
  ...
  let events =
    List.map events ~f:(fun {stateHash; produced= _; received} ->
        ...
        , List.map received ~f:(fun {time= _; podRealName; sender} ->
              ( `Node_name podRealName
              , { Block_received.state_hash= stateHash
                ; sender_ip= `External sender } ) ) ) )
  in
  to_json_for_vis events network |> Yojson.Safe.to_file "graph.json"
