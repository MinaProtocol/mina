(** Testing
    ...
    Subject: Test zkapp initialize state
    Invocation: \
     dune exec src/app/zkapps_examples/test/initialize_state/initialize_state.exe
*)
...
let tag, _, p_module, Pickles.Provers.[ initialize_prover; update_state_prover ]
    =
...
module Update_state_account_update = struct
  let new_state =
    List.init Zkapp_state.max_size_int ~f:(fun _ -> Snark_params.Tick.Field.one)
...
let test_initialize_and_update () =
  let account =
    []
    |> Zkapp_command.Call_forest.cons_tree
...
let test_initialize_and_multiple_update () =
  let account =
    []
    |> Zkapp_command.Call_forest.cons_tree
         Update_state_account_update.account_update
...
let test_update_without_initialize_fails () =
  let account =
    []
    |> Zkapp_command.Call_forest.cons_tree
         Update_state_account_update.account_update
...
let test_double_initialize_fails () =
  let account =
    []
    |> Zkapp_command.Call_forest.cons_tree
         Initialize_account_update.account_update
...
let test_initialize_after_update_fails () =
  let account =
    []
    |> Zkapp_command.Call_forest.cons_tree
         Initialize_account_update.account_update
...
let test_initialize_without_deploy_fails () =
  let account =
...
let () =
  ...
  run "Initialize state test"
    [ ( "initialize_state"
      , [ test_case "Initialize" `Quick test_initialize
        ; test_case "Initialize_and_update" `Quick test_initialize_and_update
        ; test_case "Initialize_and_multiple_update" `Quick
            test_initialize_and_multiple_update
        ; test_case "Update_without_initialize_fails" `Quick
            test_update_without_initialize_fails
        ; test_case "Double_initialize_fails" `Quick
            test_double_initialize_fails
        ; test_case "Initialize_after_update_fails" `Quick
            test_initialize_after_update_fails
        ; test_case "Initialize_without_deploy_fails" `Quick
            test_initialize_without_deploy_fails
        ] )
    ]
