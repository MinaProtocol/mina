...
let main ~genesis_constants ~constraint_constants () =
  ...
  let%bind zkapp_command = Lazy.force @@ zkapp_command ~genesis_constants ~constraint_constants in
  print_heap_usage "Zkapp_command.t" @@ zkapp_command;
  let%bind zkapp_proof = Lazy.force @@ zkapp_proof ~genesis_constants ~constraint_constants in
  print_heap_usage "Pickles.Side_loaded.Proof.t" zkapp_proof ;
  let%bind verification_key = Lazy.force @@ verification_key ~constraint_constants in
  ...
  let%map scan_state_base_node_zkapp = Lazy.force @@ scan_state_base_node_zkapp ~genesis_constants ~constraint_constants in
