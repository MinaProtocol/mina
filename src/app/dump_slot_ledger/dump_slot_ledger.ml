...
let caqti_ledger_entry =
  ...
  let encode {
    ...
    token_symbol;
    timestamp
  } = Ok ((delegate_key, ledger_index, balance, nonce), (receipt_chain_hash, ledger_hash, token_symbol, timestamp)) 
  in 
  let decode (
    (delegate_key,
    ledger_index,
    ...
    Utils.(tup8 (option string) int string int string string (option string) string)
  end
...
let dump_slot slot postgres = 
  ...
  | Ok pool ->
    ...
    |sql} slot_string
    in 
    let%bind ledger = Caqti_async.Pool.use (fun (module Conn : Caqti_async.CONNECTION) ->
      Conn.collect_list entries () 
      >>| function 
      | Ok rows ->
        let () = List.iter rows ~f:( fun row ->
      ...
      | Error e ->
        [%log error]
        "Failed to obtain ledger entries from the database, see error: $error"
        ~metadata:[ ("error", `String (Caqti_error.show e)) ];
        Ok () 
    ) pool in 
    ...
    | Error e -> 
      [%log error]
      "Failed to obtain entries after the initial ledger query, see error: $error"
      ~metadata:[ ("error", `String (Caqti_error.show e)) ];
      Deferred.unit 

let command = 
  Command.async
  ~summary:"Prints out the ledger for a given slot for debugging purposes. 
  It requires an archive db url and a slot number"
  ( let%map_open.Command postgres =
    Cli_lib.Flag.Uri.Archive.postgres 
    and  slot = 
