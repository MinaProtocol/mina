...
let pre_fork_validations ~mainnet_archive_uri ~migrated_archive_uri () =
  ...
  let mainnet_pool =
    Caqti_async.connect_pool   ~pool_config:
    Caqti_pool_config.(
      merge_left (default_from_env ()) (create ~max_size:128 ())) mainnet_archive_uri
  in
  let migrated_pool =
    Caqti_async.connect_pool   ~pool_config:
    Caqti_pool_config.(
      merge_left (default_from_env ()) (create ~max_size:128 ())) migrated_archive_uri
  in
