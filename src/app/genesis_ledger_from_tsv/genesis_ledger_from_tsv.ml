...
let amount_geq_min_balance ~amount ~initial_min_balance =
  let amount = Currency.Amount.mina_of_string_exn amount in
  let initial_min_balance =
    Currency.Amount.mina_of_string_exn initial_min_balance
  in
...
let runtime_config_account ~logger ~wallet_pk ~amount ~initial_min_balance
  ...
  let balance = Currency.Balance.mina_of_string_exn amount in
  let initial_minimum_balance =
    (* if omitted in the TSV, use balance *)
    if String.is_empty initial_min_balance then balance
    else Currency.Balance.mina_of_string_exn initial_min_balance
  in
  let cliff_time =
    let num_slots_float =
  ...
  in
  let cliff_amount = Currency.Amount.mina_of_string_exn cliff_amount in
  let vesting_period =
    ...
    | _ ->
        failwithf "Expected unlock frequency to be 0 or 1, got %s"
          unlock_frequency ()
  in
  let vesting_increment = Currency.Amount.mina_of_string_exn unlock_amount in
  let no_vesting =
    ...
    *)
    let initial_minimum_balance =
      if String.is_empty initial_min_balance then
        Currency.Balance.mina_of_string_exn amount
      else Currency.Balance.mina_of_string_exn initial_min_balance
    in
    let cliff_amount = Currency.Amount.mina_of_string_exn cliff_amount in
    let unlock_amount = Currency.Amount.mina_of_string_exn unlock_amount in
    if
      Currency.Amount.equal cliff_amount Currency.Amount.zero
