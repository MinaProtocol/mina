        ...
        Command::Node(node_cmd) => match node_cmd {
                ...
                match supervisor::rpc_call(
                    ...
                    Ok(_) => {
                        ...
                        if cmd.node_args.raw_output {
                            println!(
                                "Node '{node_id}' on network '{network_id}' has been started."
                            );
                        } else {
                            println!(
                                "{}",
            ...
            NodeCommand::Stop(cmd) => {
                ...
                match supervisor::rpc_call(
                    ...
                    Ok(_) => {
                        if cmd.raw_output {
                            println!(
                                "Node '{node_id}' on network '{network_id}' has been stopped."
                            );
                        } else {
                            println!(
                                "{}",
            ...
            NodeCommand::DumpPrecomputedBlocks(cmd) => {
                ...
                match supervisor::rpc_call(
                    ...
                    Ok(result) => {
                        let blocks = result["precomputed_blocks"]
                            .as_str()
                            .unwrap_or_default()
                            .to_string();
                        if cmd.raw_output {
