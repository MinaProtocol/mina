...
async fn launch_docker(
    ...
    docker
        ...
        .await
        .map_err(|e| {
            std::io::Error::other(format!("create_container '{}' failed: {e}", node.name))
        })?;
    ...
    docker
        ...
        .await
        .map_err(|e| {
            std::io::Error::other(format!("start_container '{}' failed: {e}", node.name))
        })?;
...
fn register_unit(
    ...
    info!("supervisor: started '{name}' (pid {pid:?})");
    {
        ...
        st.nodes
            .insert(name.to_string(), NodeStatus::Running { pid });
    ...
    tokio::spawn(async move {
        ...
        guard
            .nodes
            .insert(name.clone(), NodeStatus::Exited { code });
