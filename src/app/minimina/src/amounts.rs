//! # Amounts Module
//!
//! Pure parsing helpers ported from the mina-local-network Python rewrite
//! (`mln/amounts.py`). Provides:
//!
...
//!
//! These functions have no in-crate consumer yet (balances currently enter
//! minimina only via a verbatim `--genesis-ledger` JSON file), so the module is
//! marked `#![allow(dead_code)]` until a real caller lands.
#![allow(dead_code)]
...
///
/// Accepts `<int>mina`, a fractional `<int>.<frac>mina` (e.g. `0.25mina`), or
/// `<int>nanomina`. Fractional mina is converted exactly via integer arithmetic;
/// more than 9 fractional digits is sub-nanomina precision and rejected, since
/// nanomina is the smallest representable unit.
///
/// Grammar: `^(\d+)(?:\.(\d+))?mina$|^(\d+)nanomina$`.
pub fn amount_dsl_to_nanomina(amount_str: &str) -> Result<u64, AmountParseError> {
    ...
    if let Some(digits) = amount_str.strip_suffix("nanomina") {
        ...
        return digits.parse::<u64>().map_err(|_| {
            AmountParseError::new(format!("Amount {amount_str:?} is out of range for u64 nanomina"))
        });
    }
    ...
    if let Some(number) = amount_str.strip_suffix("mina") {
        ...
        let frac_nanomina: u64 = match frac_str {
            Some(frac) => {
                ...
                padded.parse::<u64>().expect("padded fractional digits parse")
            }
            None => 0,
        };
...
pub fn convert_balance_to_decimal_mina(balance_str: &str) -> Result<String, AmountParseError> {
    Ok(nanomina_to_decimal_mina(amount_dsl_to_nanomina(balance_str)?))
}
...
pub fn parse_iso_duration(dur_str: &str) -> Result<f64, AmountParseError> {
    let invalid =
        || AmountParseError::new(format!("Cannot parse ISO duration: {dur_str:?}"));
...
mod tests {
    ...
    fn dsl_max_precision_boundary() {
        ...
        assert_eq!(amount_dsl_to_nanomina("1.000000001mina").unwrap(), 1_000_000_001);
    }
