...
module Hash_json = struct
  type epoch_data = {
    staking_hash: string;
    next_hash: string option
  }
  [@@deriving to_yojson]

  type t = {
    genesis_hash: string; 
    epoch_data: epoch_data option
  }
  [@@deriving to_yojson]
end
...
let main ~config_file ~genesis_dir ~proof_level ~hash_output_file () =
  ...
  in
Option.value_map hash_output_file ~default:Deferred.unit
~f:(fun path ->
    let ledger_hash ledger = Mina_base.State_hash.to_base58_check @@ Mina_ledger.Ledger.merkle_root @@ Lazy.force ledger in
    let genesis_hash = ledger_hash @@ Genesis_ledger.Packed.t precomputed_values.genesis_ledger in
    let epoch_data = Option.map precomputed_values.genesis_epoch_data ~f:(fun data ->
      {
        Hash_json.staking_hash=ledger_hash data.staking.ledger;
        next_hash= Option.map data.next ~f:(fun data -> ledger_hash data.ledger)
        }) in
        Async.Writer.save path
          ~contents:(Yojson.Safe.to_string
                        (Hash_json.to_yojson
                          {genesis_hash; epoch_data})))
           ...
           flag "--proof-level"
             ...
             and hash_output_file =
              flag "--hash-output-file"
                (optional string)
                ~doc:"PATH path to the file where the hashes of the ledgers are to be saved"
         in
