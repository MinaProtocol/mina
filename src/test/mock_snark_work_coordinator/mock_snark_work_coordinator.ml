    ...
    if List.length held_proofs >= proof_batch_size then (
      ...
      let batch_processed =
        if List.length held_proofs > 0 then (
          let output_file =
            Printf.sprintf "%s/proof_batch.%d.json" output_folder
              batch_processed
          in
      ...
      | `Eof ->
          ...
          exit 1
      | `Ok ({ spec_with_proof; fee; prover } : Work.Result.Combined.t) ->
          let new_proofs = spec_with_proof |> One_or_two.map ~f:Tuple2.get2 in
          let stmts =
            spec_with_proof
          ...
          if not (Sok_message.equal input_sok_message result_sok_message) then (
            [%log fatal]
              ...
              ~metadata:
                ...
                ; ("stmts", Transaction_snark_work.Statement.to_yojson stmts)
                ] ;
            exit 1 )
          else (
            [%log info] "Collected proof, $txn_proofs_left remaining"
              ~metadata:[ ("txn_proofs_left", `Int (txn_proofs_left - 1)) ] ;
  ...
  Command.async ~summary:"Mock coordinator test"
    ...
    and reassignment_timeout =
      flag "--reassignment-timeout"
        (optional_with_default 420.0 float)
