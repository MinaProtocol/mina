...
let command_convert_old =
  Command.basic
    ~summary:
      "Convert dumped spec&proof to input understandable by internal verifier"
    (let%map_open.Command proof =
       flag "--proof" (required string)
         ~doc:"Path to proof generated by snark worker"
     and output =
       flag "--output-file" (required string) ~doc:"File of converted data"
     in
     fun () ->
       let Work.Work.Result_without_metrics.{ proofs; prover; fee; _ } =
         Yojson.Safe.from_file proof
         |> Work.Work.Result_without_metrics.of_yojson Ledger_proof.of_yojson
         |> Result.ok_or_failwith
       in
       let sok_message = Sok_message.create ~fee ~prover in
       One_or_two.to_list proofs
       |> List.map ~f:(fun pf -> (pf, sok_message))
       |> [%derive.to_yojson: (Ledger_proof.t * Sok_message.t) list]
       |> Yojson.Safe.to_file output )

let command_verifier =
  Command.basic ~summary:"Verifier"
    (let%map_open.Command proof =
       flag "--proof" (required string)
         ~doc:"Path to proof generated by snark worker"
     and dumped_spec =
       flag "--dumped-spec" (required string) ~doc:"Path to dumped spec"
     in
     fun () ->
       ...
       let Work.Spec.Dumped.{ prover; fee; _ } =
       ...
       in
       let verification_inputs =
         One_or_two.to_list result.proofs
         |> List.map ~f:(fun proof -> (proof, sok_message))
       in
       let k () =
         match%map
           Verifier.verify_transaction_snarks verifier verification_inputs
...
let command_test_coordinator =
  Command.basic ~summary:"Mock coordinator test"
    (let%map_open.Command dumped_spec_path =
     ...
     and save_proof =
       flag "--save-proofs" (optional string) ~doc:"Path to save proof"
     in
     fun () ->
       let Genesis_proof.{ constraint_constants; _ } =
         Lazy.force Precomputed_values.for_unit_tests
       ...
       let k () =
         ...
         Mock_coordinator.start ~predefined_specs ~partitioner ~logger
           ...
           ~completed_snark_work_sink ~sok_message:input_sok_message ?save_proof
...
let () =
  Command_unix.run
    (Command.group ~summary:"Mock coordinator"
       [ ("test", command_test_coordinator)
       ; ("verifier", command_verifier)
       ; ("convert", command_convert_old)
       ] )
