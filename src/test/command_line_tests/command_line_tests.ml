...
module AutoHardforkConfigGeneration = struct
  ...
  let validate_generated_config ~conf_dir =
    let open Deferred.Or_error.Let_syntax in
    (* Define file paths *)
    let daemon_json = conf_dir ^/ "daemon.json" in
    let daemon_backup = conf_dir ^/ "daemon.berkeley.json" in
    let fork_validation_config =
      conf_dir ^/ "fork_validation" ^/ "legacy" ^/ "daemon.json"
    in
    let%bind () =
    ...
    in
    let%bind () =
      assert_file_exists ~path:daemon_backup
        ~error_msg:"daemon.berkeley.json backup was not created"
    in
    let%bind () =
      assert_file_exists ~path:fork_validation_config
        ~error_msg:"fork_validation/legacy/daemon.json was not created"
    in
    (* All files exist, now validate config contents *)
    let expected_fork_slot = slot_chain_end + hard_fork_genesis_slot_delta in
    (* Read and parse configs using Runtime_config *)
    let%bind daemon_config =
      ...
      |> Deferred.return
    in
    let%bind daemon_backup_config =
      Yojson.Safe.from_file daemon_backup
      |> Runtime_config.of_yojson
      |> Result.map_error ~f:Error.of_string
      |> Deferred.return
    in
    ...
    in
    (* Extract genesis timestamps *)
    let%bind old_genesis =
      of_option daemon_backup_config.genesis
        ~error:"Backup config missing genesis field"
    in
    let%bind old_genesis_timestamp =
      of_option old_genesis.genesis_state_timestamp
        ~error:"Backup config missing genesis_state_timestamp"
    in
  ...
  let generate_hardfork_config daemon output =
    ...
    in
    (* Create daemon config with hardfork parameters *)
    let daemon_config : Runtime_config.Daemon.t =
    ...
    in
    let proof_config : Runtime_config.Proof_keys.t =
      { level = None
      ; sub_windows_per_window = None
      ; ledger_depth = None
      ; work_delay = None
      ; block_window_duration_ms = Some slot_duration_ms
    ...
    in
    (* Set genesis timestamp to a few minutes in the future, like 2-node test *)
    ...
    in
    let genesis_config : Runtime_config.Genesis.t =
      { k = None
      ; delta = None
    ...
    in
    let runtime_config =
      Runtime_config.make ~ledger ~daemon:daemon_config ~proof:proof_config
  ...
  let test_case (test : t) =
    ...
    let%bind block_producer_kp = generate_hardfork_config daemon ledger_file in
    ...
    let activated = conf_dir ^/ "activated" in
    ...
    | `Success -> (
        ...
        | `Result (Ok ()) -> (
            ...
            match%bind.Deferred validate_generated_config ~conf_dir with
            | Ok () ->
                Deferred.Or_error.return Mina_automation_fixture.Intf.Passed
            | Error err ->
                Deferred.Or_error.return
