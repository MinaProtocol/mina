...
module Other = Extend(Camlsnark.Snark.Make(Other_curve))

module Main = struct
  module Main0 = Extend(Camlsnark.Snark.Make(Main_curve))

  include (Main0 : module type of Main0 with module Field := Main0.Field)

  module Field = struct
    include Main0.Field

    include Field_bin.Make(Main0.Field)(Main_curve.Bigint.R)
