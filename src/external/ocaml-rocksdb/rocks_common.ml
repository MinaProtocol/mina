...
let rec keep_alive o =
  if Sys.opaque_identity (int_of_string "0") <> 0 then
    keep_alive (Sys.opaque_identity o)

module Views = struct
  open Unsigned

  let bool_to_int =
    view ~read:(fun i -> i <> 0) ~write:(function true -> 1 | false -> 0) int
  ...
  let int_to_size_t = view ~read:Size_t.to_int ~write:Size_t.of_int size_t

  let int_to_uint_t = view ~read:UInt.to_int ~write:UInt.of_int uint

  let int_to_uint32_t = view ~read:UInt32.to_int ~write:UInt32.of_int uint32_t

  let int_to_uint64_t = view ~read:UInt64.to_int ~write:UInt64.of_int uint64_t
end
...
type t = { ptr : unit ptr; mutable valid : bool }
...
let t : t typ =
  view
    ...
    ~write:(fun { ptr; valid } ->
      if valid then ptr else raise OperationOnInvalidObject )
    (ptr void)

let make_destroy t destructor =
  let inner = foreign destructor (t @-> returning void) in
  fun t ->
    inner t ;
    t.valid <- false
...
module CreateConstructors (T : RocksType) = struct
  ...
  let create_no_gc = foreign T.constructor (void @-> returning t)
