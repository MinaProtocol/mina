...
module type TRANSACTION = sig
  ...
  val txnbegin :
    ?wopts:WriteOptions.t -> ?txnopts:TransactionOptions.t -> db -> t
  ...
  val get_string :
    ?pos:int -> ?len:int -> ?opts:ReadOptions.t -> t -> string -> string option
  ...
  val delete :
    ?pos:int -> ?len:int -> ?opts:WriteOptions.t -> t -> bigarray -> unit

  val delete_string :
    ?pos:int -> ?len:int -> ?opts:WriteOptions.t -> t -> string -> unit
...
module type ROCKS = sig
  ...
  val open_transactiondb :
    ?opts:Options.t -> ?txnopts:TransactionDbOptions.t -> string -> t
  ...
  val multi_get :
    ?opts:ReadOptions.t -> t -> bigarray list -> bigarray option list
  ...
  val checkpoint_create :
    t -> ?log_size_for_flush:int -> dir:string -> unit -> unit
