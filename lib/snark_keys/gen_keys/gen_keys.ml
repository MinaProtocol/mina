...
let load_transaction_snark_keys_expr ~loc tx_keys_location tx_keys_checksum =
  ...
  [%expr
    ...
    Transaction_snark.Keys.load
      (Transaction_snark.Keys.Location.of_string
         [%e
           estring (Transaction_snark.Keys.Location.to_string tx_keys_location)])
    >>| fun (keys, checksum) ->
    assert (
...
let load_blockchain_snark_keys_expr ~loc bc_keys_location bc_keys_checksum =
  let module E = Ppxlib.Ast_builder.Make (struct
    let loc = loc
  end) in
  let open E in
  [%expr
    ...
    Blockchain_snark.Blockchain_transition.Keys.load
      (Blockchain_snark.Blockchain_transition.Keys.Location.of_string
         [%e
           estring
             (Blockchain_snark.Blockchain_transition.Keys.Location.to_string
                bc_keys_location)])
    >>| fun (keys, checksum) ->
    assert (
      String.equal (Md5_lib.to_hex checksum)
...
module Dummy = struct
  module Transaction_keys = struct
    let expr ~loc = [%expr Async.return Transaction_snark.Keys.dummy]
  end
...
let main () =
  ...
  let%bind blockchain_expr, transaction_expr =
    ...
    | true ->
        return
          (Dummy.Blockchain_keys.expr ~loc, Dummy.Transaction_keys.expr ~loc)
    | false ->
        ...
        , load_transaction_snark_keys_expr ~loc tx_keys_location
            tx_keys_checksum )
  in
  let fmt =
