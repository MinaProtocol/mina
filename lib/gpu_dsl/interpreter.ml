...
module Value = struct
  ...
  let rec sexp_of_struct : type a. a Type.struct_spec -> a Struct.t -> Sexp.t list =
    fun spec s ->
      match spec, s with
      | Type.[], Struct.[] -> []
      | Type.(::) (t, spec), Struct.(::) (x, s) ->
...
let eval_op (type a) (s : State.t) ({op; label} : a Op.Value.t) : State.t * a Id.t =
  ...
  | Mul (x, y) ->
    ...
    let id = Id.Id (Type.Arith_result, label) in
  ...
  | Or (x, y) ->
    let x = State.get_exn s x in
    let y = State.get_exn s y in
    let id = Id.Id (Type.bool, label) in
...
let ptr_value_id ptr =
  Id.Id (Type.pointer_elt (Id.typ ptr), Id.label ptr)
