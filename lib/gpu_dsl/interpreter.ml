...
module Value = struct
  ...
  and sexp_of_t (T (typ, x)) =
    ...
    | Struct spec ->
        [%sexp_of : unit sexp_opaque] ()
        (* TODO: Sexp.List (sexp_of_struct spec x) *)
...
module State = struct
  ...
  let create_id s typ lab = Id.Id (typ, name_in_scope s lab, 0)
...
let rec eval : type a. State.t -> a T.t -> State.t * a =
 fun s t ->
  ...
  | Load (ptr, lab, k) ->
      let (Pointer.Pointer loc) = State.get_exn s ptr in
      let typ =
        match Id.typ ptr with Type.Pointer t -> t | _ -> assert false
      in
