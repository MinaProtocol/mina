module type S = sig
  ...
  module Ledger_builder_diff : sig type t [@@deriving sexp, bin_io] end

  type t [@@deriving sexp, bin_io, compare, eq]

  val create :
    protocol_state:Protocol_state.value -> protocol_state_proof:Proof.t ->ledger_builder_diff:Ledger_builder_diff.t-> t
...
end)(Protocol_state : Protocol_state.S) :
