...
let two_to_the i = two_to_the i |> Bigint.of_bignum_bigint |> Bigint.to_field
...
let floor_divide ~numerator:(`Two_to_the (b: int) as numerator) y y_unpacked =
  ...
  let%bind z =
    exists Typ.field
      ~request:
        As_prover.(map (read_var y) ~f:(fun y -> Floor_divide (numerator, y)))
      ~compute:
        As_prover.(
          map (read_var y) ~f:(fun y ->
              Bigint.to_field
                (Tick_curve.Bigint.R.div
                   (Bigint.of_field (two_to_the b))
                   (Bigint.of_field y)) ))
  in
