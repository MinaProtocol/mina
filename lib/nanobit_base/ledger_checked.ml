...
let modify_account t pk ~f =
  with_label __LOC__ begin
    ...
    in
    handle
      (Merkle_tree.modify_req ~depth (Ledger_hash.var_to_hash_packed t) addr ~f:(fun account ->
...
(* [create_account t pk] implements the following spec:
  ...
  [{ public_key = pk; balance = Account.Balance.zero; nonce =
    Account_nonce.zero }] at [addr] instead of
...
let create_account t pk =
  ...
  let%bind () =
    Merkle_tree.implied_root
      ...
      path
    >>| Ledger_hash.var_of_hash_packed
    >>= Ledger_hash.assert_equal t
  in
  let account : Account.var =
    { public_key = pk
    ; balance = Currency.Balance.(var_of_t zero)
    ; nonce = Account_nonce.(Unpacked.var_of_value zero)
    ; receipt_chain = Receipt_chain.Tail.Checked.empty
    }
  in
