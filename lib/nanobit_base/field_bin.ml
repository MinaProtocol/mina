...
struct
  ...
  let ( { Bin_prot.Type_class.reader= bin_reader_t
    ...
    Bin_prot.Type_class.cnv Fn.id to_bigstring of_bigstring Bigstring.bin_t
  ...
  let%test_unit "field_bigstring_self_inverse" =
    let assert_str b s = if b then failwith s else () in
    let rec go i =
      if i = 0 then ()
      else
        let x = Field.random () in
        assert_str
          (x = of_bigstring (to_bigstring x))
          (Printf.sprintf "Failed on iteration %d" i) ;
        go (i - 1)
    in
    go 1000
end
