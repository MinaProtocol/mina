...
module Undo = struct
  type transaction = 
    { transaction : Transaction.t; previous_receipt_chain_hash : Receipt.Chain_hash.t }
  [@@deriving sexp]
...
(* someday: It would probably be better if we didn't modify the receipt chain hash
   ...
   we don't for now. *)
let apply_transaction_unchecked ledger ({payload; sender} as transaction: Transaction.t) =
  ...
  in
  let undo =
    { Undo.transaction; previous_receipt_chain_hash = sender_account.receipt_chain_hash }
  in
  ...
  then begin
    ...
    set ledger sender { sender_account_without_balance_modified with balance= sender_balance' };
...
let undo_transaction ledger { Undo.transaction={ payload; sender }; previous_receipt_chain_hash } =
  let sender = Public_key.compress sender in
  let {Transaction.Payload.fee; amount; receiver; nonce} = payload in
  let open Or_error.Let_syntax in
  let%bind sender_account = get' ledger "sender" sender in
...
let undo : t -> Undo.t -> unit Or_error.t =
  fun ledger undo ->
    ...
    match undo with
    | Fee_transfer t ->
      undo_fee_transfer ledger t
    | Transaction u ->
      undo_transaction ledger u

let apply_super_transaction ledger (t : Super_transaction.t) =
  match t with
  | Transaction txn ->
    Or_error.map (apply_transaction ledger txn) ~f:(fun u -> Undo.Transaction u)
  | Fee_transfer t ->
    Or_error.map (apply_fee_transfer ledger t) ~f:(fun () -> Undo.Fee_transfer t)
...
let merkle_root_after_transactions t ts =
  let undos_rev =
    List.rev_map ts ~f:(fun txn ->
        Or_error.ok_exn (apply_transaction t txn))
  in
