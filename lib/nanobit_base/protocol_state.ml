...
let%test_module "Protocol_state tests" =
  ...
  (module struct
    ...
    module Consensus_state = struct
      ...
      let to_hlist {amount; ledger_hash} =
        H_list.[amount; ledger_hash]

      let of_hlist : (unit, 'amount -> 'ledger_hash -> unit) H_list.t -> ('amount, 'ledger_hash) t =
        fun H_list.[amount; ledger_hash] ->
          {amount; ledger_hash}

      let data_spec =
        let open Snark_params.Tick.Data_spec in
        [Amount.typ; Ledger_hash.typ]
      ...
      let fold {amount; ledger_hash} =
        let open Fold in
        Amount.fold amount +> Ledger_hash.fold ledger_hash
    ...
    let%test_unit "profile hashes" =
      ...
      in
      for i = 0 to 100 do
        ignore (Protocol_state.hash state)
      done
      (*
      let gen =
