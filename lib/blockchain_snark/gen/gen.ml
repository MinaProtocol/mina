...
let expr_of_t ~loc
  ...
  in
  [%expr
    { Nanobit_base.Blockchain_state.next_difficulty = [%e e "Target" (module Target) next_difficulty]
    ; previous_state_hash = [%e e "State_hash" (module State_hash) previous_state_hash]
    ; ledger_builder_hash = [%e e "Ledger_builder_hash" (module Ledger_builder_hash) ledger_builder_hash]
...
let negative_one =
  ...
  ; previous_state_hash= State_hash.of_hash Snark_params.Tick.Pedersen.zero_hash
...
let genesis_block, zero =
  ...
  let block =
    let open Block in
    { header=
        {Block.Header.nonce= Nonce.of_int 0; time = genesis_time}
    ; body=
  ...
  let rec find_ok_nonce i =
    if Or_error.is_ok (Proof_of_work.create zero i)
    then i
...
let genesis_block_expr ~loc =
  ...
  [%expr
    { Nanobit_base.Block.header =
        { Nanobit_base.Block.Header.nonce = Nanobit_base.Block.Nonce.of_int
                    [%e eint (Unsigned.UInt64.to_int genesis_block.header.nonce)]
        ; time =
            Nanobit_base.Block_time.t_of_sexp
              [%e Ppx_util.expr_of_sexp ~loc (Block_time.sexp_of_t genesis_block.header.time)]
        }
    ; body =
        { Nanobit_base.Block.Body.proof = None
        ; ledger_builder_hash = Nanobit_base.Ledger_builder_hash.dummy
        ; target_hash = Nanobit_base.Ledger.merkle_root Nanobit_base.Genesis_ledger.ledger
        }
    }
  ]
