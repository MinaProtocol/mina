...
module Keys = struct
  type t = {step: Tick.Keypair.t; wrap: Tock.Keypair.t}

  let dummy =
    { step=
        Tick.Keypair.create ~vk:Dummy_values.Tick.verification_key
          ~pk:Dummy_values.Tick.proving_key
    ; wrap=
        Tock.Keypair.create ~vk:Dummy_values.Tock.verification_key
  ...
  let checksum ~step ~wrap =
    Md5.digest_string
      ("Blockchain_transition" ^ Md5.to_hex step ^ Md5.to_hex wrap)
