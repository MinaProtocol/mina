...
module Keys = struct
  ...
  module Proving = struct
    ...
    let dummy =
      { step= Dummy_values.Tick.proving_key
  ...
  module Verification = struct
    ...
    let checksum ~step ~wrap =
      Md5.digest_string
        ("Blockchain_transition_verification" ^ Md5.to_hex step ^ Md5.to_hex wrap)
  ...
  let dummy =
    { proving = Proving.dummy
    ; verification = Verification.dummy
    }
  ...
  module Location = struct
    module T = struct
      type t = { proving : Proving.Location.t; verification : Verification.Location.t }
      [@@deriving sexp]
    end
  ...
  module Keys = struct
    ...
    let step_cached =
      ...
      Cached.Spec.create ~load ~directory:Cache_dir.cache_dir
        ...
        ~input:(Tick.constraint_system ~exposing:(Step_base.input ()) Step_base.main)
    ...
    let cached () =
      ...
      let checksum : Checksum.t =
        { proving =
            Proving.checksum
              ~step:step_pk.checksum
              ~wrap:wrap_pk.checksum
        ; verification =
            Verification.checksum
              ~step:step_vk.checksum
