...
module Make (Inputs : Inputs.S) : sig
          ...
          Parallel_scan.State.fold_chronological state ~init:None ~f:
            (fun acc_statement job ->
              ...
              | Base (Some {transaction; statement; witness}) ->
                  let source = Frozen_ledger_hash.of_ledger_hash @@ Sparse_ledger.merkle_root witness in
                  let after =
                    ...
                    |> ok_or_return
                  in
                  let target = Frozen_ledger_hash.of_ledger_hash @@ Sparse_ledger.merkle_root after in
  ...
  let of_aux_and_ledger ~snarked_ledger_hash ~public_key ~ledger ~aux =
    ...
    let%map () =
      ...
      | Ok {fee_excess; source; target; supply_increase= _; proof_type= _} ->
          ...
          and () =
            check
              (Frozen_ledger_hash.equal (Ledger.merkle_root ledger |> Frozen_ledger_hash.of_ledger_hash) target)
  ...
  let apply_super_transaction_and_get_statement ledger s =
    ...
    let source = Ledger.merkle_root ledger |> Frozen_ledger_hash.of_ledger_hash in
