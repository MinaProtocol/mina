...
let main clients =
  let echo_ports =
    List.map clients ~f:(fun client ->
        List.nth_exn client.Testbridge.Main.Client.exposed_tcp_ports 0 )
  in
  let call_ports =
    List.map clients ~f:(fun client ->
        List.nth_exn client.Testbridge.Main.Client.internal_tcp_addrs 0 )
  in
  printf "starting host: %s\n"
    (Sexp.to_string_hum ([%sexp_of : Testbridge.Main.Client.t list] clients)) ;
  printf "waiting for clients...\n" ;
  let%bind () =
    Deferred.List.iter ~how:`Parallel echo_ports ~f:(fun port ->
        Testbridge.Kubernetes.call_retry Rpcs.Ping.rpc port () ~retries:4
          ~wait:(sec 2.0) )
  in
  let%bind () =
    Deferred.List.all_unit
      (List.mapi echo_ports ~f:(fun i port ->
           ...
           Testbridge.Kubernetes.call_exn Rpcs.Set.rpc port
             ("value-" ^ Int.to_string i ^ "-" ^ id) ))
  in
  let%bind fetch_results =
    Deferred.List.map ~how:`Parallel
      (List.zip_exn echo_ports (List.rev call_ports))
      ~f:(fun (port, call_port) ->
        Testbridge.Kubernetes.call_exn Rpcs.Fetch.rpc port call_port )
  in
    ...
    (let%map_open container_count =
     ...
     and containers_per_machine =
       flag "containers-per-machine" ~doc:"containers per machine"
         (required int)
     and image_host =
       flag "image-host" ~doc:"location of docker repository" (required string)
     in
     fun () ->
       let open Deferred.Let_syntax in
       let%bind clients =
         Testbridge.Main.create ~image_host ~project_dir:"../client"
           ~to_tar:["."] ~pre_cmds:[] ~post_cmds:[]
           ~launch_cmd:("bash", ["/app/testbridge-launch.sh"])
           ~container_count ~containers_per_machine ~external_tcp_ports:[8000]
           ~internal_tcp_ports:[8001] ~internal_udp_ports:[]
       in
       main clients)
  |> Command.run
