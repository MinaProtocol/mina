...
module Node = struct
  type t = {ready: bool; hostname: string} [@@deriving sexp]
end

module Pod = struct
  type status = [`Running | `Pending | `Terminating] [@@deriving sexp]
...
module Endpoint_port = struct
  type protocol = [`TCP | `UDP] [@@deriving sexp]

  type t = {port: int; protocol: protocol}
end

module Service = struct
  type protocol = [`TCP | `UDP] [@@deriving sexp]

  type t = {node_port: int; port: int; protocol: protocol} [@@deriving sexp]
end

let get_nodes () =
  let%map stdout =
    Process.run_exn ~prog:"kubectl" ~args:["get"; "nodes"; "-o"; "json"] ()
  in
  ...
  List.map items ~f:(fun item ->
      let hostname =
        item |> member "metadata" |> member "labels"
        |> member "kubernetes.io/hostname"
        |> to_string
      in
      let ready =
        let conditions =
          item |> member "status" |> member "conditions" |> to_list
        in
        List.exists conditions ~f:(fun condition ->
...
let wait_for_nodes nodes_needed =
  let rec go () =
    ...
    if nodes_have >= nodes_needed then return nodes
    else
      let%bind () = after (sec 1.)
      and () =
        return (printf "have %d / %d nodes\n" nodes_have nodes_needed)
      in
      go ()
  in
  go ()
...
let get_pods () =
  let%map stdout =
    Process.run_exn ~prog:"kubectl" ~args:["get"; "pods"; "-o"; "json"] ()
  in
  let json = Yojson.Basic.from_string stdout in
  let open Yojson.Basic.Util in
  let items = json |> member "items" |> to_list in
  let pods =
    List.map items ~f:(fun item ->
        let containers =
          item |> member "spec" |> member "containers" |> to_list
        in
        let container = List.nth_exn containers 0 in
        let deleting =
          item |> member "metadata" |> member "deletionTimestamp"
          |> to_string_option
        in
        let phase =
          if deleting <> None then `Terminating
          else
        ...
        let pod_name =
          item |> member "metadata" |> member "name" |> to_string
        in
        let node_selector =
          item |> member "spec" |> member "nodeSelector"
          |> member "kubernetes.io/hostname"
          |> to_string
        in
        match
          item |> member "spec" |> member "nodeName" |> to_string_option
        with
        | None ->
            { Pod.container_name
            ; Pod.pod_name
...
let run_pod ~pod_name ~pod_image ~node_hostname ~tcp_ports ~udp_ports =
  ...
  let wrap_assocs names json =
    List.fold ~init:json (List.rev names) ~f:(fun json name ->
        wrap_assoc name json )
  in
  let pod_overrides =
    let tcp_ports_json =
      List.map tcp_ports ~f:(fun port ->
          `Assoc [("containerPort", `Int port); ("protocol", `String "TCP")] )
    in
    let udp_ports_json =
      List.map udp_ports ~f:(fun port ->
          `Assoc [("containerPort", `Int port); ("protocol", `String "UDP")] )
    in
    let ports_json = List.concat [udp_ports_json; tcp_ports_json] in
    let container_json =
      `Assoc
        [ ("image", `String pod_image)
        ; ("name", `String pod_name)
        ; ("ports", `List ports_json) ]
    in
    let json =
      wrap_assocs ["spec"; "template"; "spec"]
        (`Assoc
          [ ("containers", `List [container_json])
          ; ( "nodeSelector"
            , wrap_assoc "kubernetes.io/hostname" (`String node_hostname) ) ])
    in
    json |> to_string
  in
  let prog = "kubectl" in
  (*printf "%s\n" pod_overrides;*)
  let args =
    ["run"; pod_name; "--image=" ^ pod_image; "--overrides=" ^ pod_overrides]
  in
...
let wait_for_pods pods_needed =
  let rec go () =
    ...
    if pods_have >= pods_needed then return pods
    else
      let%bind () = after (sec 1.)
...
let get_endpoints pods =
  let%map stdout =
    Process.run_exn ~prog:"kubectl" ~args:["get"; "endpoints"; "-o"; "json"] ()
  in
  let json = Yojson.Basic.from_string stdout in
  let open Yojson.Basic.Util in
  let items = json |> member "items" |> to_list in
  ...
  let pod_internal_ports =
    List.map pod_services ~f:(fun (services, ip) ->
        List.map ports ~f:(fun port ->
            let service =
              List.find_exn services ~f:(fun service -> service.port = port)
            in
...
let forward_ports pods ports =
  ...
  Deferred.List.map pods ~f:(fun pod ->
      Deferred.List.map ports ~f:(fun port ->
          let args =
            ["port-forward"; pod.Pod.pod_name; "0" ^ ":" ^ Int.to_string port]
          in
          ...
          let port_token =
            List.nth_exn (String.split hostport_token ~on:':') 1
          in
...
let call_pipe rpc port query ~f =
  Tcp.with_connection
    (Tcp.Where_to_connect.of_host_and_port
       (Host_and_port.of_string ("127.0.0.1:" ^ Int.to_string port)))
    (fun _ r w ->
...
let call_pipe_exn rpc port query ~f =
  call_pipe rpc port query ~f:(fun res ->
...
let call_exn rpc port query =
  let%map res = call rpc port query in
  match res with Ok msg -> msg | Error e -> failwith (Error.to_string_hum e)

let call_retry rpc port query ~retries ~wait =
  let rec go remaining_tries =
    let%bind res = call rpc port query in
    match res with
    | Ok msg -> return msg
    | Error e ->
        ...
        else failwith (Error.to_string_hum e)
  in
  go retries
