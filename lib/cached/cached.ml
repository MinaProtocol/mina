...
module Projection = struct
  type 'a t = T: string * ('a -> 's) * 's Storage.Disk.Controller.t -> 'a t
end

module T = struct
  module Env = struct
    type 'e t = {base_path: string; value: 'e Lazy.t}
  end

  type ('a, 'e) t = 'e Env.t -> 'a Deferred.t

  let bind t ~f e = Deferred.bind (t e) ~f:(fun x -> f x e)
...
let component ~label ~f bin_t =
  ...
  let controller =
    Storage.Disk.Controller.create ~parent_log:(Logger.create ()) bin_t
  in
  match%bind lift_deferred (load_with_checksum controller path) with
  | Error `Checksum_no_match -> failwith "Checksum failure"
  | Error ((`IO_error _ | `No_exist) as err) ->
      ...
      let%map checksum =
        lift_deferred (store_with_checksum controller path value)
      in
      {path; value; checksum}
  | Ok {data; checksum} ->
      Core.printf "All ok!\n%!" ;
