...
struct
  ...
  module Verification_key : sig
    ...
    val characterizing_elts_up_to_sign : Libsnark.Verification_key.t -> Other_curve.Field.Vector.t
    val sign_elts : Libsnark.Verification_key.t -> Other_curve.Field.Vector.t
    val characterizing_bits : Libsnark.Verification_key.t -> bool list
    ...
    let characterizing_elts_up_to_sign =
      let stub =
        foreign (with_prefix Prefix.prefix "gm_verification_key_characterizing_elts_up_to_sign")
          (Libsnark.Verification_key.typ @-> returning Other_curve.Field.Vector.typ)
      in
      fun t ->
        let v = stub t in
        Caml.Gc.finalise Other_curve.Field.Vector.delete v;
        v
    ...
    let characterizing_bits vk =
      let open Other_curve in
      let unpack x =
        let n = Bigint.R.of_field x in
        List.init Other_curve.Field.size_in_bits ~f:(fun i ->
          Bigint.R.test_bit n i)
      in
      let bs1 =
        let v = characterizing_elts_up_to_sign vk in
        List.init (Field.Vector.length v) ~f:(fun i ->
          unpack (Field.Vector.get v i))
        |> List.concat
      in
      let bs2 =
        let v = sign_elts vk in
        List.init (Field.Vector.length v) ~f:(fun i ->
          Bigint.R.test_bit
            (Bigint.R.of_field (Field.Vector.get v i)) 0)
      in
      bs1 @ bs2
  ...
  module All_in_one = struct
    ...
    let choose_verification_key_bits_and_proof_and_check_result input get_witness =
      let open Let_syntax in
      let%bind t = create input get_witness in
      let%map bs1 =
...
module Mnt4
  ...
  Make (Impl)
    ...
    (Libsnark.Mnt4.Default)
    (Libsnark.Mnt6.Default)
    (Info)
...
module Mnt6
  ...
  Make (Impl) (struct let prefix = "camlsnark_mnt6" end)
    (Libsnark.Mnt6.Default) (Libsnark.Mnt4.Default) (Info)
