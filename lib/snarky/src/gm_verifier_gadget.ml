...
struct
  ...
  module Verification_key : sig
    ...
    val characterizing_vars_up_to_sign : t -> Libsnark.Linear_combination.Vector.t
    val sign_vars : t -> Libsnark.Linear_combination.Vector.t

    val characterizing_elts_up_to_sign : Other_curve.Verification_key.t -> Field.Vector.t
    ...
    let prefix =
      with_prefix Prefix.prefix
        "r1cs_se_ppzksnark_verification_key_variable"
    ...
    let characterizing_elts_up_to_sign =
      let stub =
        foreign (with_prefix Prefix.prefix "gm_verification_key_characterizing_elts_up_to_sign")
    ...
    let sign_elts =
      let stub =
        foreign (with_prefix Prefix.prefix "gm_verification_key_sign_elts")
          (Other_curve.Verification_key.typ @-> returning Field.Vector.typ)
      in
      fun t ->
        let v = stub t in
    ...
    let characterizing_vars_up_to_sign =
      let stub =
        foreign (func_name "characterizing_vars_up_to_sign")
  ...
  module Verification_key_data = struct
    type 'field t_ =
      { characterizing_up_to_sign : 'field list
      ; sign : 'field list
      }
    [@@deriving sexp]
    ...
    let characterizing_length, sign_length =
      ...
      let g2_count = 3 in
      let gt_count = 1 in
      ( g1_count * 1 + g2_count * Info.fqe_size_in_field_elements + gt_count * Info.fqe_size_in_field_elements
    ...
    let typ =
      ...
      Typ.of_hlistable
        ...
        ~var_to_hlist:(fun { characterizing_up_to_sign; sign } -> [characterizing_up_to_sign; sign])
        ~value_to_hlist:(fun { characterizing_up_to_sign; sign } -> [characterizing_up_to_sign; sign])
        ~var_of_hlist:(fun [ characterizing_up_to_sign; sign ] -> {characterizing_up_to_sign; sign})
        ~value_of_hlist:(fun [ characterizing_up_to_sign; sign ] -> {characterizing_up_to_sign; sign})
    ...
    let of_verification_key vk : t =
      ...
      ; sign = 
          List.init (V.length signs) ~f:(fun i -> V.get signs i)
      }
    ...
    let to_bits ({ characterizing_up_to_sign; sign } as t : t) = 
      ...
      let bs1 =
        ...
        List.concat_map characterizing_up_to_sign ~f:(fun x ->
          Field.unpack x)
      and bs2 =
        (* but not here *)
    ...
    module Checked = struct
      let constant ({ sign; characterizing_up_to_sign } : t) : var =
        ...
        ; characterizing_up_to_sign = List.map characterizing_up_to_sign ~f:Field.Checked.constant
        }
      ...
      module Assert = struct
        let equal t1 t2 =
          ...
          let%map () = check t1.characterizing_up_to_sign t2.characterizing_up_to_sign
          and () = check t1.sign t2.sign
          in
          ()
      end
  ...
  module Proof : sig
    ...
    let prefix =
      with_prefix Prefix.prefix
        "r1cs_se_ppzksnark_proof_variable"
  ...
  module Online_verifier = struct
    ...
    let prefix = with_prefix Prefix.prefix "r1cs_se_ppzksnark_online_verifier_gadget"
    ...
    let create_stub =
      foreign (func_name "create")
        ( Pb.typ
          @-> Preprocessed_verification_key.typ
          @-> Pb.Variable_array.typ
          @-> int
          @-> Proof.typ
          @-> Pb.Variable.typ
    ...
    let generate_constraints =
      let stub =
        foreign (func_name "generate_r1cs_constraints")
          (gadget_typ @-> returning void)
      in
      fun {gadget; _} -> stub gadget

    let generate_witness_stub =
      foreign (func_name "generate_r1cs_witness")
        (gadget_typ @-> returning void)
  ...
  module All_in_one = struct
    module T = struct
      ...
      let prefix = with_prefix Prefix.prefix "r1cs_se_ppzksnark_verifier_gadget"
      let func_name = with_prefix prefix
      ...
      let conv_term conv_back term =
        ...
        let var =
          Pb.Variable.of_int
            (Libsnark.Var.index (Term.var term))
        in
      ...
      let conv_lc conv_back lc =
        ...
        if Int.equal n 0
        then Field.Checked.constant Field.zero
        else begin
          let rec go i acc =
            if Int.equal i n
            then acc
              ...
              let acc =
                Field.Checked.add
                  (conv_term conv_back term)
                  acc
              in
              go (i + 1) acc
          in
          go 1
            (conv_term conv_back (Term.Vector.get terms 0))
        end

      let conv_lc_vector conv_back lcs =
        let module V = Libsnark.Linear_combination.Vector in
        List.init (V.length lcs) ~f:(fun i ->
          conv_lc conv_back (V.get lcs i))
      ;;
    ...
    let choose_verification_key_data_and_proof_and_check_result input get_witness =
      let open Let_syntax in
      let%map t = create input get_witness in
      assert (List.length t.vk_characterizing_vars_up_to_sign
              = Verification_key_data.characterizing_length);
      assert (List.length t.vk_sign_vars = Verification_key_data.sign_length);
      ( { Verification_key_data.characterizing_up_to_sign = t.vk_characterizing_vars_up_to_sign
        ; sign = t.vk_sign_vars
        }
      , t.result )
      (*
      let%map bs1 =
        Checked.List.map t.vk_characterizing_vars_up_to_sign ~f:(fun x ->
          Field.Checked.choose_preimage_var x ~length:Field.size_in_bits)
      and bs2 =
        Checked.List.map t.vk_sign_vars ~f:(fun x ->
          let%bind () = Field.Checked.Assert.non_zero x in
          Field.Checked.unpack_full x
          >>| Bitstring_lib.Bitstring.Lsb_first.to_list
          >>| List.hd_exn)
      in
      (List.concat bs1 @ bs2, t.result) *)
    ;;
  end
end
...
module Mnt4
  ...
  Make (Impl)
    (struct let prefix = "camlsnark_mnt4" end)
    (Libsnark.Mnt4.GM)
    (Libsnark.Mnt6.GM)
    (struct
      include Info
...
module Mnt6
  ...
  Make (Impl) (struct let prefix = "camlsnark_mnt6" end)
    (Libsnark.Mnt6.GM) (Libsnark.Mnt4.GM)
    (struct
      include Info
