...
module Make
  ...
  module Bits (M : sig val length_in_bits : int end) = struct
    include M
    type t = bool list
    type var = Boolean.var list
  ...
  module Digest = Bits(struct let length_in_bits = 256 end)

  module Block = struct
    include Bits(struct let length_in_bits = 512 end)
    type 'a t_ = 'a list
    ...
    let list_to_padded_blocks ~padding bits =
      let rec go blocks curr curr_length bs =
        ...
        | [] ->
          ...
          List.rev
            (List.rev_append curr (List.init padding_length ~f:(fun _ -> padding))
             :: blocks)
        | b :: bs ->
          if Int.equal curr_length length_in_bits
          then go (List.rev curr :: blocks) [b] 1 bs
  ...
  module State = struct
    include Bits(struct let length_in_bits = 256 end)
    ...
    module Checked = struct
      ...
      module Digest_variable = struct
        ...
        let create =
          let stub =
            foreign (func_name "create") (Protoboard.typ @-> int @-> returning typ)
          in
          fun pb n ->
            let t = stub pb n in
        ...
        let bits =
          let stub =
            foreign (func_name "bits") (typ @-> returning Protoboard.Variable_array.typ)
          in
          fun t ->
            let bs = stub t in
      ...
      module Compression_gadget = struct
        ...
        let prefix = with_prefix Prefix.prefix "sha256_compression_function_gadget"
        let func_name = with_prefix prefix
      ...
      module T = struct
        ...
        type t = { gadget : Compression_gadget.t; output_unconstrained : Impl.Field.var list }
        ...
        let generate_constraints t = Compression_gadget.generate_constraints t.gadget
        let generate_witness t _input () =
          As_prover.(map (return ()) ~f:(fun () ->
