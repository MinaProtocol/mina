...
module Make (Field : Field_intf.S) (Var : Comparable.S) = struct
  ...
  let eval context t0 =
    ...
    let rec go scale = function
      | Constant c ->
        res := Field.add !res (Field.mul scale c)
      | Var v ->
        res := Field.add !res (Field.mul scale (context v))
      | Scale (s, t) ->
        go (Field.mul s scale) t
      | Add (t1, t2) ->
        go scale t1;
        go scale t2
    in
  ...
  let linear_combination (terms : (Field.t * t) list) : t =
    List.fold terms ~init:(constant Field.zero) ~f:(fun acc (c, t) -> add acc (scale t c))
