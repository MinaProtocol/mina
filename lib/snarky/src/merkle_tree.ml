...
module Free_hash = struct
  ...
  let rec run t ~hash ~compress =
    ...
    | Compress (l, r) -> compress (run ~hash ~compress l) (run ~hash ~compress r)
  ;;
end
...
type ('hash, 'a) non_empty_tree =
  ...
  | Leaf of 'hash * 'a
and ('hash, 'a) tree =
  | Non_empty of ('hash, 'a) non_empty_tree
  | Empty
[@@deriving sexp]
    ...
    then
      ...
      | Non_empty (Node (h, t_l, t_r)) ->
        ...
        then
          ...
          Node (compress (non_empty_hash t_l') (tree_hash ~default t_r), Non_empty t_l', t_r)
        else
          let t_r' = go mask' t_r in
          Node (compress (tree_hash ~default t_l) (non_empty_hash t_r'), t_l, Non_empty t_r')
...
let recompute_hashes { tree; depth; count; hash; compress } =
  let h = let default = hash None in fun t -> tree_hash ~default t in
  let rec go = function
...
let implied_free_root addr0 x path0 =
  implied_root ~compress:(fun a b -> Free_hash.Compress (a, b)) addr0 (Hash_value x) path0
;;
...
= struct
  ...
  let update ~(depth : int) ~root ~prev ~next addr0
    ...
    =
    let open Let_syntax in
    with_label "Merkle_tree.Checked.update" begin
      let%bind prev_entry_hash = Elt.hash prev
      and next_entry_hash = Elt.hash next
      and prev_path =
