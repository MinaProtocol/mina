...
module Make_basic (Backend : Backend_intf.S) = struct
  ...
  module Field0 = struct
    ...
    include Sexpable.Of_sexpable (Bignum_bigint)
              (struct
                type nonrec t = t
    ...
    include Binable.Of_binable (Bignum_bigint)
              (struct
                type nonrec t = t
  ...
  module Checked = struct
    ...
    let two_to_the n =
      let rec go acc i =
        if i = 0 then acc else go Field.Infix.(acc + acc) (i - 1)
      in
      go Field.one n
    ...
    let chunk_bitstrings_for_equality (t1: Boolean.var list)
        (t2: Boolean.var list) =
    ...
    module Assert = struct
      ...
      let equal = assert_equal ~label:"Checked.Assert.equal"
