...
module Make_basic (Backend : Backend_intf.S) = struct
  ...
  module Field = struct
    ...
    module Checked = struct
      ...
      let lt_bitstring_value =
        ...
        in
        fun
          (xs : Boolean.var Bitstring_lib.Bitstring.Msb_first.t)
  ...
  let%test_unit "lt_bitstring_value" =
    let gen =
      ...
      and y = list_with_length length bool
      in
      (x, y)
    in
    Quickcheck.test gen ~f:(fun (x, y) ->
      ...
      let ((), lt) =
        Checked.run_and_check
          (Checked.map ~f:(As_prover.read Checked.Boolean.typ)
