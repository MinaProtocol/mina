...
module Make_GM_verification_key_accessors
    (Prefix : sig val prefix : string end)
    (Gm_verification_key : Foreign_intf)
    (G1 : sig
...
= struct
  ...
  let func name ret delete =
    let stub =
      foreign (func_name name)
        (Gm_verification_key.typ @-> returning ret)
    in
    fun vk ->
      let r = stub vk in
...
= struct
  ...
  let to_coords =
    ...
    in
    let to_affine =
      foreign (func_name "to_affine_coordinates")
        (typ @-> returning void)
    in
...
module Mnt4 = struct
  ...
  module GM_verification_key_accessors =
    Make_GM_verification_key_accessors
      (Prefix)
      (GM.Verification_key)
      (Group)
      (G2)
