...
module Call_ring = struct
  ...
  let window = 8
  let create name = (name, ref 0, Array.create ~len:window 0)

  let report (name, epoch, cs) =
    Printf.printf !"$$$ %s@%d: [%s]\n%!"
      name
      !epoch
      (String.concat ~sep:","
         (Array.to_list (Array.map cs ~f:string_of_int)))

  let record (name, epoch, cs) =
    let now = Time.now () in
    let span_since_genesis = Time.diff now genesis in
    let mins_since_genesis = int_of_float (Time.Span.to_min span_since_genesis) in
