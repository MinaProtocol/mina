  ...
  let%test_unit "time_queue_handles_in_order" =
    Async.Thread_safe.block_on_async_exn (fun () ->
      let tick_assert_sees t actions =
        ...
        let%map () = tick_forwards t ~f:(fun next ->
          Char.Table.add_exn table ~key:next ~data:();
          return ()
        ) in
        if Char.Table.length table <> List.length actions then
          failwithf !"Char length %d and action length %d ; actions %{sexp:char List.t} ; table %{sexp:char List.t}\n%!" (Char.Table.length table) (List.length actions) actions (Char.Table.keys table) ();
...
= struct
  ...
  let send t ~recipient message : unit Deferred.Or_error.t =
    ...
    | Some (r, w) ->
        Time_queue.handle_in_future t.q ~after:(Message_delay.delay message) (Msg (message, recipient));
...
= struct
    ...
    module Timer_transport = Fake_timer_transport(Message)(Message_delay)(Trivial_peer)
    ...
    let rec loop t ~stop =
      ...
      in
      let choose3
        (a : 'a Deferred.t) (a_imm : 'a option)
        (b : 'b Deferred.t) (b_imm : 'b option)
        (c : 'c Deferred.t) (c_imm : 'c option) :
          ('a option * 'b option * 'c option) Deferred.t =
        Deferred.any
          [ a >>| (fun a ->
              (Some a, merge (Deferred.peek b) b_imm, merge (Deferred.peek c) c_imm))
          ; b >>| (fun b ->
              (merge (Deferred.peek a) a_imm, Some b, merge (Deferred.peek c) c_imm))
          ; c >>| (fun () ->
              (merge (Deferred.peek a) a_imm, merge (Deferred.peek b) b_imm, Some ()))
          ]
      in
      let node_ready : MyNode.t Deferred.t =
        ...
        any_ready >>| (fun n ->
          let maybe_real =
            List.fold (Identifier.Table.data t.nodes) ~init:None ~f:(fun acc x ->
              match acc, x with
              | Some _, _ -> acc
      ...
      in
      let ticks_available =
        Timer_transport.actions_ready t.timer
      in
      let chosen =
          choose3
            stop            None
            node_ready      node_ready_imm
            ticks_available None
      in
...
let%test_module "Distributed_dsl" = (module struct
  ...
  module Message = struct type t = Msg of int end
  ...
  module Timer_label = struct
    type label = Timeout_message | Spawn_msg [@@deriving enum, sexp, compare, hash]
    module T = struct
      type t = label [@@deriving compare, hash, sexp]
    end
    include T
  ...
  module Condition_label = struct
    type label = Init | Wait_timeout | Bigger_than_five | Failure_case [@@deriving enum, sexp, compare, hash]
    module T = struct
      type t = label [@@deriving compare, hash, sexp]
    end
    include T
