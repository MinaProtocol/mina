  ...
  List.iter p ~f:(fun chain ->
      printf "%s, "
        (Md5.to_hex
           (Md5.digest_string
              (Sexp.to_string_hum
                 ([%sexp_of : State_hash.t] (Blockchain_state.hash chain.state)))))
  ) ;
  printf "\n"

module Linked_blockchain = struct
  type elt = {edges: elt Digest.Table.t; node: Blockchain.t}

  type t = {_roots: elt Digest.Table.t; _all_nodes: elt Digest.Table.t}

  let create () =
    {_roots= Digest.Table.create (); _all_nodes= Digest.Table.create ()}

  let roots t = Digest.Table.data t._roots

  let all_nodes t = Digest.Table.data t._all_nodes
  ...
  let fixup_roots t =
    ...
    List.iter
      (List.join
         (List.map (roots t) (fun root -> Digest.Table.data root.edges)))
      ~f:go ;
  ...
  let add t chain = _add t chain ; reconnect t

  let deepest_path t =
    let compare_right (_, r) (_, r') = Int.compare r r' in
    let rec go (acc, count) e =
      List.max_elt
        (List.map (Digest.Table.data e.edges) ~f:(fun e ->
             go (e.node :: acc, count + 1) e ))
        compare_right
    ...
    | 0 | 1 ->
        ...
        dump_path_hashes ~tag:"all_nodes"
          (List.map (all_nodes t) ~f:(fun e -> e.node)) ;
        p
    | _ -> p
end
  ...
  printf "** Killing %s\n"
    (Sexp.to_string_hum
       ([%sexp_of : Host_and_port.t list]
          (List.map to_kill ~f:(fun n ->
               n.Nanobit_args.nanobit
                 .Nanobit_testbridge.Nanobit.membership_addr )))) ;
  let%map () =
    Deferred.List.iter to_kill ~f:(fun na ->
        Nanobit_testbridge.stop na.Nanobit_args.nanobit )
  in
...
let main nanobits =
  printf "host started: %s\n"
    (Sexp.to_string_hum
       ([%sexp_of : Nanobit_testbridge.Nanobit.t list] nanobits)) ;
  printf "initing nanobits...\n" ;
  let%bind args =
    Nanobit_testbridge.run_main_fully_connected nanobits ~should_mine:true
  in
  printf "init'd nanobits: %s\n"
    (Sexp.to_string_hum
       ([%sexp_of : Nanobit_testbridge.Rpcs.Main.query list] args)) ;
  printf "starting test\n" ;
  let alive_nanobits, dead_nanobits =
    ( List.map2_exn nanobits args ~f:(fun nanobit args ->
          {Nanobit_args.nanobit; args} )
    , [] )
  in
  ...
  don't_wait_for
    (let rec go last_sca_length =
       ...
       let () = if not pass then printf "no new block in 45 seconds" in
       assert pass ;
       go !sca_length
     in
     go !sca_length) ;
  let check_sca (chains: Linked_blockchain.t list) =
    ...
    let shortest_path =
      Option.value_exn
        (List.min_elt
           (chains |> List.map ~f:Linked_blockchain.deepest_path)
           by_length)
    in
    ...
    if List.length shortest_path < 1 then ()
    else
      let longest_path =
        Option.value_exn
          (List.max_elt
      ...
      let newest_match =
        List.findi oldest_to_newest ~f:(fun idx (s, l) ->
            not
              (State_hash.equal
                 (Blockchain_state.hash s.state)
                 (Blockchain_state.hash l.state)) )
      ...
      printf
        "got blockchain longest_path:%d shortest_path:%d anc_age:%d \
         newest_match:%d pass:%b\n"
  ...
  let resync idx (states: Linked_blockchain.t list) (new_chain: Blockchain.t) =
    printf "RESYNCING %d with %s (prior: %s)\n" idx
      (Md5.to_hex
         (Md5.digest_string
            (Sexp.to_string_hum
               ([%sexp_of : State_hash.t]
                  (Blockchain_state.hash new_chain.state)))))
      (Md5.to_hex
         (Md5.digest_string
            (Sexp.to_string_hum
               ([%sexp_of : State_hash.t] new_chain.state.previous_state_hash)))) ;
    let len = List.length states in
    let others =
      List.slice states 0 idx @ List.slice states (Int.min (idx + 1) len) len
    in
    (* All the deepest blockchain paths for the other nodes *)
    let other_paths = List.map others ~f:Linked_blockchain.deepest_path in
    (* The deepest most blockchain for each other node *)
    let deepest_tails =
      List.map ~f:List.rev other_paths |> List.map ~f:List.hd
    in
      ...
      List.map deepest_tails ~f:(fun tail_opt ->
          ...
          | Some tail ->
              ...
              || (* Or they ALMOST have this block *)
                 State_hash.equal new_chain.state.previous_state_hash
                   (Blockchain_state.hash tail.state)
          | None -> false )
    in
    (* Grab the longest path that meets the mask *)
    let best_path =
      List.map2_exn other_paths mask ~f:(fun p b -> if b then p else [])
      |> List.max_elt ~compare:(fun p p' ->
             Int.compare (List.length p) (List.length p') )
      |> Option.value_exn
    in
    ...
    printf "Length new:%d old:%d\n" (List.length new_deepest)
      (List.length old_deepest)
  in
  let update_chain states nanobit i (blockchain: Blockchain.t) =
    let () = if i = 0 then printf "NODE 0 got an update\n" else () in
    printf "got blockchain idx:%d %s %s %s\n" i
      (Sexp.to_string_hum
         ([%sexp_of : Host_and_port.t]
            nanobit.Nanobit_testbridge.Nanobit.membership_addr))
      (Sexp.to_string_hum
         ([%sexp_of : Ledger_hash.t] blockchain.state.ledger_hash))
      (Md5.to_hex
         (Md5.digest_string
            (Sexp.to_string_hum
               ([%sexp_of : State_hash.t] blockchain.state.previous_state_hash)))) ;
    ...
    else check_sca adjusted_states
  in
  let states =
    List.init (List.length nanobits) ~f:(fun n -> Linked_blockchain.create ())
  in
  let listen_for_blocks i nanobit =
    Deferred.ignore
      (Nanobit_testbridge.get_strongest_blocks nanobit ~f:(fun pipe ->
           let%map () =
  ...
  don't_wait_for
    (let rec go () : unit Deferred.t =
       ...
       let%bind alive_nanobits, dead_nanobits =
         kill_nanobit0 alive_nanobits dead_nanobits
       in
       let%bind () = after (sec 10.) in
       let%bind alive_nanobits, dead_nanobits =
         start_nanobit0 alive_nanobits dead_nanobits
       in
