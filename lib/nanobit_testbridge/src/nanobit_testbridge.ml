...
let get_peers nanobit =
  Testbridge.Kubernetes.call_exn Rpcs.Get_peers.rpc nanobit.Nanobit.bridge_port
    ()
...
let main nanobit args =
  Testbridge.Kubernetes.call_exn Rpcs.Main.rpc nanobit.Nanobit.bridge_port args
...
let run_main_fully_connected ?(should_mine= false) nanobits =
  ...
  | first_args :: rest_args ->
      ...
      let%map () =
        Deferred.List.iter ~how:`Parallel (List.zip_exn nanobits full_args) ~f:
          (fun (nanobit, args) -> main nanobit args )
      in
      next_args
       ...
       let%bind clients =
         Testbridge.Main.create ~image_host ~project_dir:"../../../"
           ...
           ~post_cmds:
             ...
             ; ("chmod", ["-R"; "+w"; "/app/app/kademlia-haskell"])
             ; ("rm", ["-rf"; "/app/app/kademlia-haskell/result"]) ]
           ~container_count ~containers_per_machine ~external_tcp_ports:[8010]
           ~internal_tcp_ports:[8001] ~internal_udp_ports:[8000]
       in
       let nanobits =
