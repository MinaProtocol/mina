...
let lists_same xs ys =
  List.for_all xs ~f:(fun x ->
      List.exists ys ~f:(fun y -> Host_and_port.(y = x)) )
  && List.for_all ys ~f:(fun y ->
         List.exists xs ~f:(fun x -> Host_and_port.(x = y)) )
...
let check_alive_nanobits alive_nanobits =
  ...
  in
  let peers =
    List.map2_exn alive_nanobits peers ~f:(fun alive_peer peers ->
        alive_peer.Nanobit_testbridge.Nanobit.membership_addr
        :: Option.value_exn peers )
  in
  List.for_all peers ~f:(fun peers ->
      lists_same
        (List.map alive_nanobits ~f:(fun nanobit ->
             nanobit.Nanobit_testbridge.Nanobit.membership_addr ))
        peers )
...
let main nanobits =
  ...
  let%bind alive_nanobits, dead_nanobits =
    Deferred.List.fold
      ...
      (pattern (List.length nanobits))
      ~f:(fun (alive_nanobits, dead_nanobits) cmd ->
        match cmd with
        | `Kill n ->
            printf "kill %d\n" n ;
            let%bind alive_nanobits, dead_nanobits =
              kill_nanobits n alive_nanobits dead_nanobits
            in
            let%map () =
              check (List.map alive_nanobits ~f:(fun na -> na.nanobit))
            in
            (alive_nanobits, dead_nanobits)
        | `Start n ->
            printf "start %d\n" n ;
            let%bind alive_nanobits, dead_nanobits =
              start_nanobits n alive_nanobits dead_nanobits
            in
            let%map () =
              check (List.map alive_nanobits ~f:(fun na -> na.nanobit))
            in
  ...
  let%bind _ =
    kill_nanobits (List.length alive_nanobits) alive_nanobits dead_nanobits
  in
  printf "done killing\n" ; Async.never ()
