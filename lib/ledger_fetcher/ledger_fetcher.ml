...
module Make (Inputs : Inputs_intf) :
  Minibit.Ledger_fetcher_intf
  with type ledger_hash := Inputs.Ledger_hash.t
...
struct
  ...
  module Config = struct
    type t =
      ...
      ; ledger_transitions:
          (Ledger_hash.t * Transaction.With_valid_signature.t list * State.t)
  ...
  module State = struct
    ...
    include Bin_prot.Utils.Make_binable (struct
      ...
      let to_binable ({strongest_ledgers; hash_to_ledger}: t) : Binable.t =
        {strongest_ledgers= Heap.to_list strongest_ledgers; hash_to_ledger}

      let of_binable ({Binable.strongest_ledgers; hash_to_ledger}: Binable.t) :
          t =
  ...
  let get t h : Ledger.t Deferred.Or_error.t =
    ...
    | Error e ->
        Logger.debug t.log "Didn't get ledger locally, trying remotely %s"
          (Error.to_string_hum e) ;
        let%bind net = t.net in
        let open Deferred.Or_error.Let_syntax in
        let%map ledger, state =
          Net.Ledger_fetcher_io.get_ledger_at_hash net h
        in
        add t h ledger state ; ledger
    | Ok (l, s) -> Deferred.Or_error.return l

  let initialize t =
    if Heap.length t.state.strongest_ledgers = 0 then
  ...
  let strongest_ledgers t =
    ...
    Linear_pipe.filter_map t.all_new_states_reader ~f:(fun (ledger, state) ->
        ...
        if Strength.( < ) !best_strength new_strength then (
  ...
  let create (config: Config.t) =
    ...
    let storage_controller =
      Store.Controller.create ~parent_log:config.parent_log
    ...
    Linear_pipe.write_or_exn ~capacity:1 all_new_states_writer
      all_new_states_reader initial_strong_state ;
    don't_wait_for
      (Linear_pipe.iter config.ledger_transitions ~f:
         (fun (h, transactions, state) ->
           ...
           Logger.debug t.log
             !"Trying to get ledger with hash %{sexp: Ledger_hash.t}"
             h ;
           Logger.debug t.log
             !"All ledger hashes we know about are: %{sexp: Ledger_hash.t list}"
             (Ledger_hash.Table.keys t.state.hash_to_ledger) ;
           match%bind get t h with
           | Error e ->
               Logger.warn t.log
                 "Failed to keep-up with transactions (can't get ledger %s). \
                  Not mining until we get more data!"
                 (Error.to_string_hum e) ;
               return ()
           | Ok unsafe_ledger ->
               ...
               List.iter transactions ~f:(fun transaction ->
                   ...
                   | Error e ->
                       Logger.warn t.log
                         "Failed to apply a transaction %s ;; mining without \
                          this transaction"
                         (Error.to_string_hum e)
                   | Ok () -> () ) ;
           ...
           * new tip is received *)
               ...
               Linear_pipe.write_or_exn ~capacity:1 all_new_states_writer
                 all_new_states_reader next_state ;
