...
module Transition = struct
  let to_tagged_transaction : Transaction.t -> Tagged_transaction.t = function
    | Fee_transfer t -> Fee_transfer.to_tagged_transaction t
    | Valid_payment t -> (Normal, (t :> Payment.t))
...
module Base = struct
  ...
  *)
  ...
  let apply_tagged_transaction (type shifted)
      ...
      root ((tag, {sender; signature; payload}) as txn: Tagged_transaction.var) =
    with_label __LOC__
        ...
        let%bind root =
          ...
          Frozen_ledger_hash.modify_account_send root
            ...
            sender_compressed ~f:(fun account ->
              with_label __LOC__
                (let%bind next_nonce =
                   Account_nonce.increment_if_var account.nonce
                 ...
                 let%bind receipt_chain_hash =
                   ...
                   let%bind r =
                     Receipt_chain_hash.Checked.cons ~payload:payload_section
                       current
                   in
                   Receipt_chain_hash.Checked.if_
...
let check_payment ~sok_message ~source ~target t handler =
  check_transition ~sok_message ~source ~target (Valid_payment t) handler
...
struct
  ...
  let of_payment ~sok_digest ~source ~target payment handler =
    of_transition ~sok_digest ~source ~target (Valid_payment payment) handler
...
let%test_module "transaction_snark" =
  ( module struct
    ...
    let of_transaction' sok_digest ledger transaction handler =
      ...
      let target =
        Ledger.merkle_root_after_payment_exn ledger transaction
        |> Frozen_ledger_hash.of_ledger_hash
      in
      of_payment ~sok_digest ~source ~target transaction handler
