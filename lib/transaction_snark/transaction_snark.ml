  ...
  let apply_tagged_transaction (type shifted)
    ...
    with_label __LOC__
      ( if not Insecure.transaction_replay then
        ...
        in
        (root, signed_fee_of_signed_amount excess, supply_increase) )
...
module Verification = struct
  ...
  struct
    ...
    (* The curve pt corresponding to H(merge_prefix, _digest, _, _, _, Fee.Signed.zero, wrap_vk)
...
let check_merge ?(wrap_vk= Dummy_values.Tock.verification_key) ~sok_message
  ...
  in
  let supply_increase =
    Option.value_exn (Amount.add proof1.supply_increase proof2.supply_increase)
  in
  let result =
  ...
  let top_hash =
    merge_top_hash wrap_vk_bits ~sok_digest ~state1:result.source
      ~state2:result.target ~fee_excess ~supply_increase
  in
  let open Tick in
  let main =
    Checked.map
