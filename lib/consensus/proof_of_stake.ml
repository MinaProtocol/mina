...
module Make (Inputs : Inputs_intf) : Mechanism.S = struct
  ...
  module Epoch = struct
    ...
    let size = UInt32.of_int (3 * Inputs.probable_slots_per_transition_count * Inputs.unforkable_transition_count)
  ...
  module Consensus_transition_data = struct
    ...
    type var = (Epoch.Unpacked.var, Epoch.Slot.Unpacked.var, Amount.var, Sha256.Digest.var) t
    ...
    let to_hlist {epoch; slot; total_currency_diff; proposer_vrf_result} =
      Nanobit_base.H_list.[epoch; slot; total_currency_diff; proposer_vrf_result]

    let of_hlist :
           (unit, 'epoch -> 'slot -> 'amount -> 'vrf_result -> unit) Nanobit_base.H_list.t
        -> ('epoch, 'slot, 'amount, 'vrf_result) t =
      fun Nanobit_base.H_list.([epoch; slot; total_currency_diff; proposer_vrf_result]) ->
        {epoch; slot; total_currency_diff; proposer_vrf_result}

    let data_spec =
      let open Snark_params.Tick.Data_spec in
      [Epoch.Unpacked.typ; Epoch.Slot.Unpacked.typ; Amount.typ; Sha256.Digest.typ]
    ...
    let fold {epoch; slot; total_currency_diff; proposer_vrf_result} =
      ...
      Epoch.Bits.fold epoch
      +> Epoch.Slot.Bits.fold slot
    ...
    let var_to_bits {epoch; slot; total_currency_diff; proposer_vrf_result} =
      ...
      @ (total_currency_diff |> Amount.var_to_bits |> Bitstring_lib.Bitstring.Lsb_first.to_list)
      @ proposer_vrf_result

    let bit_length =
      Epoch.length_in_bits + Epoch.Slot.length_in_bits + Amount.length + Sha256.Digest.length_in_bits
  end
  ...
  module Consensus_state = struct
    ...
    let update previous_state transition_data =
      let open Or_error.Let_syntax in
      let open Consensus_transition_data in
      let%map total_currency =
        Amount.add previous_state.total_currency transition_data.total_currency_diff
        ...
        if transition_data.epoch = previous_state.current_epoch then
          ...
          let next_epoch_seed =
            Snark_params.Tick.Pedersen.digest_fold
              Nanobit_base.Hash_prefix.epoch_seed
              (Epoch_seed.fold previous_state.next_epoch_seed +> List.fold transition_data.proposer_vrf_result)
            |> Epoch_seed.of_hash
          in
          (previous_state.epoch_seed, next_epoch_seed)
        else
    ...
    let to_hlist {length; current_epoch; current_slot; total_currency; epoch_seed; next_epoch_seed} =
      Nanobit_base.H_list.[length; current_epoch; current_slot; total_currency; epoch_seed; next_epoch_seed]

    let of_hlist :
           ( unit
           , 'length -> 'epoch -> 'slot -> 'amount -> 'epoch_seed -> 'epoch_seed -> unit )
     ...
     fun Nanobit_base.H_list.([ length
                              ...
                              ; epoch_seed
                              ; next_epoch_seed ]) ->
       {length; current_epoch; current_slot; total_currency; epoch_seed; next_epoch_seed}
    ...
    let var_to_bits {length; current_epoch; current_slot; total_currency; epoch_seed; next_epoch_seed} =
      ...
      @ epoch_seed_bits
      @ next_epoch_seed_bits

    let fold {length; current_epoch; current_slot; total_currency; epoch_seed; next_epoch_seed} =
      let open Nanobit_base.Util in
  ...
  let update (previous_state: Consensus_state.value) (transition: Snark_transition.value) =
    Consensus_state.update previous_state (Snark_transition.consensus_data transition)
  ...
  let generate_transition ~previous_protocol_state ~blockchain_state ~time
    ...
    let consensus_transition_data =
      ...
      { epoch
      ; slot 
      ; total_currency_diff= Inputs.coinbase
      ; proposer_vrf_result }
    in
    let consensus_state = Or_error.ok_exn @@ Consensus_state.update previous_consensus_state consensus_transition_data in
    let protocol_state =
