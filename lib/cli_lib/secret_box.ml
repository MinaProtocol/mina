...
let encrypt ~(password:Bytes.t) ~(plaintext:Bytes.t) =
  ...
  let {Password_hash.mem_limit; ops_limit} as diff = Password_hash.moderate in
...
(** warning: this will zero [password] *)
let decrypt_exn ~(password:Bytes.t) {box_primitive; pw_primitive; nonce; pwsalt; pwdiff= (mem_limit, ops_limit); ciphertext} =
