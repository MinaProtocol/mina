...
let read_password_exn prompt : Bytes.t Async.Deferred.t =
  ...
  let old_termios = Terminal_io.tcgetattr stdin in
  let isatty = isatty stdin in
  let () = if isatty then Terminal_io.tcsetattr ~mode:Terminal_io.TCSANOW
    {old_termios with c_echo= false; c_echonl= true} stdin in
  Writer.write (Lazy.force Writer.stdout) prompt;
  let%map pwd = Reader.read_line (Lazy.force Reader.stdin) in
    if isatty then Terminal_io.tcsetattr ~mode:Terminal_io.TCSANOW old_termios stdin ;
    match pwd with
