...
module type S = sig
  module G1 : sig type t end
  module G2 : sig type t end
  module Fq_target : sig type t end
...
= struct
  ...
  module G2_precomputation = struct
    ...
    let doubling_step_for_flipped_miller_loop
      ...
      let next =
        ...
        let z = Fq_twist.(square (current.y + current.z) - c - square current.z) in
    ...
    let create (q : G2.t) =
      ...
      let rec go found_one r dbl_coeffs add_coeffs i  =
        if i < 0
        then (r, dbl_coeffs, add_coeffs)
        else
          let bit = N.test_bit Info.loop_count i in
          if not found_one
          then go (found_one || bit) r dbl_coeffs add_coeffs (i - 1)
          else begin
            let r, dc = doubling_step_for_flipped_miller_loop r in
            let dbl_coeffs = dc :: dbl_coeffs in
            if bit
            then begin
              let r, ac = mixed_addition_step_for_flipped_miller_loop qx qy qy2 r in
              let add_coeffs = ac :: add_coeffs in
              go found_one r dbl_coeffs add_coeffs (i - 1)
            end else go found_one r dbl_coeffs add_coeffs (i - 1)
          end
      in
        ...
        then add_coeffs
          ...
          let _r, ac =
            mixed_addition_step_for_flipped_miller_loop
              minus_R_affine_X minus_R_affine_Y minus_R_affine_Y2 r
          in
          ac :: add_coeffs
      in
      { qx
  ...
  let miller_loop (p : G1_precomputation.t) (q : G2_precomputation.t) =
    ...
    if Info.is_loop_count_neg then begin
      let add_idx = !add_idx_r in
      incr add_idx_r;
      let ac = q.add_coeffs.(add_idx) in
      let g_RnegR_at_P = Fq_twist.(ac.c_RZ * p.py_twist,
                                   negate (q.qy_over_twist * ac.c_RZ + l1_coeff * ac.c_L1))
      in
  ...
  let unreduced_pairing p q =
    miller_loop
      (G1_precomputation.create p)
      (G2_precomputation.create q)
  ...
  let final_exponentiation_last_chunk elt elt_inv =
    ...
    let w0_part =
      if Info.final_exponent_last_chunk_is_w0_neg then
        cyclotomic_exp
          elt_inv Info.final_exponent_last_chunk_abs_of_w0
      else
