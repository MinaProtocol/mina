...
= struct
  ...
  let rec extended_euclidean a b =
    ...
    else match (extended_euclidean b (a % b)) with
      (d, x, y) -> d, y, x - a//b*y;;
...
let find_wnaf (type t) 
  (module N : Nat_intf.S with type t = t)
  window_size
  scalar
  =
...
= struct
  ...
  let ( * ) (a1, b1, c1) (a2, b2, c2) =
    ...
    let b = Fp.(b1 * b2) in
    let c = Fp.(c1 * c2) in
    Fp.( a + non_residue*((b1+c1)*(b2+c2)-b-c),
...
= struct
  ...
  let mul_by_2345 (a1, b1) (a2, b2) =
    ...
    let a = Fp.(a1_1 * a2_2 * non_residue, a1_2 * a2_2 * non_residue, a1_0 * a2_2) in
    let b = Fp3.(b1 * b2) in
  ...
  let cyclotomic_exp x exponent =
    ...
    let rec go found_nonzero res i =
      ...
      then res
        ...
        let res = 
          if found_nonzero then cyclotomic_square res else res
        in
