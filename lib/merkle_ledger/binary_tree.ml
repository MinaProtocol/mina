...
module Make (Account: sig type t end)
(Hash: Intf.Hash with type account := Account.t)
(Depth : sig val depth: int end) = struct
    
  type t = Node of {hash: Hash.t; left: t; right: t} | Leaf of Hash.t [@@deriving sexp]
  ...
  let set_accounts (list: Account.t list) = 
    let rec go (list: Hash.t list) num_nodes =
        if num_nodes = 1 then
            ...
            let (right_tree, remaining_nodes, right_height) = go right_list (num_nodes / 2) in
            assert (left_height = right_height);
            let hash = Hash.merge ~height:(left_height + 1) (get_hash left_tree) (get_hash right_tree) in
            (Node {hash; left=left_tree; right=right_tree}, remaining_nodes, left_height + 1)
    in
    ...
    let (tree, remaining_nodes, _) = go ( (List.map ~f:Hash.hash_account list) @ empty_hashes) max_num_accts in
    assert (remaining_nodes = []);
    tree

  let rec get_inner_hash_at_addr_exn =
    function
      Leaf hash -> (function 
                    [] -> hash
                    | _::_ -> failwith "Could not traverse beyond a leaf")
