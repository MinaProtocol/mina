  ...
  module Context = struct
    ...
    let typ =
      ...
      Impl.Typ.of_hlistable [Message.typ; Public_key.typ]
        ~var_to_hlist:(fun {Context.message; public_key} -> [message; public_key])
        ~var_of_hlist:(fun [message; public_key] -> {message; public_key})
        ~value_to_hlist:(fun {Context.message; public_key} -> [message; public_key])
...
module Bigint_scalar
  ...
  let%test_unit "of_bits . to_bits = identity" =
    Quickcheck.test gen ~f:(fun x ->
      assert (equal x (of_bits (to_bits x))))
  ...
  (module (struct
    ...
    module Curve = Snarky.Curves.Edwards.Make(Impl)(Scalar)
        (struct
          ...
          let cofactor = Bigint.(of_int 8 * of_int 5 * of_int 7 * of_int 399699743)
    ...
    module Group = struct
      ...
      let%test_unit "inv works" =
        Quickcheck.test gen ~f:(fun x ->
          ...
          if not (equal res identity)
          then failwithf !"inv failured, x = %{sexp:t}, x + inv x = %{sexp:t}, expected %{sexp:t}"
                 x res identity ())

      let%test_unit "scaling associates" =
        let open Quickcheck in
        test ~trials:50 (Generator.tuple2 Scalar.gen Scalar.gen) ~f:(fun (a, b) ->
          assert (equal (scale generator (Scalar.mul a b)) (scale (scale generator a) b)))
      ...
      let%test_unit "modulus-not-identity" =
        ...
        Quickcheck.test ~trials:50
          (Quickcheck.Generator.tuple2 non_identity_pt Scalar.(gen_incl one (modulus - one)))
          ~f:(fun (x, s) ->
          assert (not (equal (scale x s) Curve.identity)))
      ...
      module Checked = struct
        ...
        let to_bits ((x, y) : var) =
          ...
          let%map x = Field.Checked.choose_preimage_var ~length:Field.size_in_bits x
          and y     = Field.Checked.choose_preimage_var ~length:Field.size_in_bits y
          in
          List.hd_exn y :: x
      end
    end

    let params = Array.init (5 * Impl.Field.size_in_bits) ~f:(fun _ ->
      Group.scale Group.generator (Scalar.random ()))

    module Pedersen = Snarky.Pedersen.Make(Impl)(Curve)(struct let params = params end)
    ...
    let hash_bits_checked bits =
      let open Impl.Let_syntax in
      Pedersen.hash ~init:(0, Curve.Checked.identity) bits
      >>| Pedersen.digest
    ...
    module Output_hash = struct
      ...
      let hash message pt =
        hash_bits (Group.to_bits message @ Group.to_bits pt)
      ...
      module Checked = struct
        let hash_for_proof m g1 g2 g3 =
          let%bind bs =
            ...
            >>| List.concat
          in
          hash_bits_checked bs
          >>= Pedersen.Digest.choose_preimage
    ...
    let%test_unit "completeness" =
      let gen =
        let open Quickcheck.Generator.Let_syntax in
        let%map private_key = Scalar.gen
        and message = Group.gen in
      ...
      Quickcheck.test gen ~trials:50 ~f:(fun (priv, public_key, message) ->
        ...
        then
          failwithf !"%{sexp:Vrf.Context.t}, %{sexp:Vrf.Evaluation.t}" ctx eval ()
      )
