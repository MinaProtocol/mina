...
let map (t : 'a t) ~(f : 'a -> 'b) : 'b t =
  { fold = fun ~init ~f:update ->
      t.fold ~init ~f:(fun acc x ->
        update acc (f x))
  }

let concat (t : 'a t t) : 'a t =
  { fold = fun ~init ~f ->
      t.fold ~init ~f:(fun acc inner ->
        inner.fold ~init:acc ~f)
  }

let concat_map (t : 'a t) ~(f : 'a -> 'b t) : 'b t =
  { fold = fun ~init ~f:update ->
      t.fold ~init ~f:(fun acc x ->
        (f x).fold ~init:acc ~f:update)
  }
  ...
  fun t ->
    let { curr; i ; acc } =
      t.fold ~init:{ curr = 0; acc = []; i = 0 } ~f:(fun { curr; acc; i } b ->
        let curr =
          if b then curr lor (1 lsl i) else curr
        in
        if i = 7
        then
          { i = 0
          ; acc = Char.of_int_exn curr :: acc
          ; curr = 0
          }
        else
          { i = i + 1
