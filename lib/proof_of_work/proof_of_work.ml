...
include Data_hash.Make_small(struct let bit_length = Target.bit_length end)
...
let meets_target_unchecked (pow : t) (target : Target.t) =
  Bigint.(compare (of_field (pow :> Field.t)) (of_field (target :> Field.t))) < 0

let meets_target_var (pow : var) (target : Target.Packed.var) =
  let open Let_syntax in
  let%map { less; _ } = Checked.compare ~bit_length (var_to_hash_packed pow) (target :> Cvar.t) in
  less
