...
module Make
    ...
    (Main_libsnark : Libsnark.S
     with type Field.t = Impl.Field.t
      ...
      and type R1CS_constraint_system.t = Impl.R1CS_constraint_system.t)
    (Other_curve : Libsnark.S)
    (Info : sig
       val input_size : int
     end)
= struct
  ...
  module Verification_key = struct
    ...
    let prefix = with_prefix Main_libsnark.prefix "r1cs_ppzksnark_verification_key_variable"
    ...
    let conv_input conv (all_bits : input) =
      ...
      List.iter all_bits ~f:(fun b ->
        Pb.Variable_array.emplace_back bits (conv (b :> Cvar.t)));
      bits
    ;;
    ...
    let create =
      ...
      in
      fun pb (conv : Cvar.t -> Pb.Variable.t) (bits : Pb.Variable_array.t) ->
    ...
    let to_field_list =
      let stub =
        foreign (with_prefix Main_libsnark.prefix "verification_key_other_to_field_vector")
          (Other_curve.Verification_key.typ @-> returning Field.Vector.typ)
      in
      fun k ->
        let v = stub k in
        let xs = List.init (Field.Vector.length v) ~f:(fun i -> Field.Vector.get v i) in
        Field.Vector.delete v;
        xs

    let to_bit_vector =
      let stub =
        foreign (with_prefix Main_libsnark.prefix "verification_key_other_to_bool_vector")
  ...
  module Proof = struct
    ...
    let prefix = with_prefix Main_libsnark.prefix "r1cs_ppzksnark_proof_variable"
    let func_name = with_prefix prefix
  ...
  module T = struct
    ...
    let prefix = with_prefix Main_libsnark.prefix "r1cs_ppzksnark_verifier_gadget"
    let func_name = with_prefix prefix
    ...
    let create =
      fun pb
        (conv : Cvar.t -> Pb.Variable.t)
        (conv_back : Pb.Variable.t -> Cvar.t)
        { vk; input; elt_size; proof } ->
        let input_pb = Pb.Variable_array.create () in
        List.iter ~f:(fun v -> Pb.Variable_array.emplace_back input_pb (conv v))
          (input :> Cvar.t list);
        let result_pb = Pb.allocate_variable pb in
        let result = conv_back result_pb in
        let t =
          create_stub pb vk input_pb elt_size proof result_pb
        in
        Caml.Gc.finalise delete t;
        { gadget=t; result }
    ;;
  ...
  module All_in_one = struct
    module T = struct
      ...
      let create =
        fun pb conv conv_back { vk_bits; input } ->
...
module Mnt4
    ...
    (Info : sig val input_size : int end)
  =
