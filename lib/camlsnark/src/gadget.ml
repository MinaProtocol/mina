...
module Make
    ...
    (M : sig
       ...
       val generate_witness : t -> input -> witness -> (unit, _) Impl.As_prover.t
       val create
...
= struct
  ...
  let create input get_witness =
    ...
    let conv cvar =
      ...
      in
      List.iter terms ~f:(fun (x, v) ->
        Linear_combination.add_term lc x v);
    ...
    let conv_back pb_var =
      ...
      (if i < num_input then failwithf "Cannot convert back %d (%d)" i num_input ());
    ...
    let%bind () =
      with_constraint_system (fun sys ->
        ...
        Protoboard.renumber_and_append_constraints pb sys
          lcs (next_aux - !num_input_vars - 1))
    in
    let%map () =
      let%bind () =
      ...
      with_state (ap (fun () -> Protoboard.auxiliary_input pb)) begin
        let rec go i =
          if i = num_aux_vars
          then return ()
          else begin
