...
module Gen_params
  ...
  let rec random_field_element () =
    ...
    in
    if Bigint.(n < Impl.Field.size)
    then Impl.Bigint.(to_field (of_bignum_bigint n))
  ...
  let sqrt x =
    ...
    if Impl.Bigint.(compare (of_field a) (of_field b)) = -1
    then (a, b)
    else (b, a)
  ...
  let rec random_point () =
    ...
    let y2 =
      ...
      let open Infix in
      x * (square x) + Snarky.Libsnark.Curves.Mnt6.G1.Coefficients.a * x + Snarky.Libsnark.Curves.Mnt6.G1.Coefficients.b
    in
    if Impl.Field.is_square y2
    then
      let (a, b) = sqrt y2 in
          ...
          ~f:(fun (g1, g2, g3, g4) ->
             E.pexp_tuple
               (List.map [g1; g2; g3; g4] ~f:(fun g ->
                  ...
                  E.pexp_tuple
                    [estring (Impl.Field.to_string x); estring (Impl.Field.to_string y)]))))
    in
    [%expr
      let conv (x, y) = Inner_curve.of_coords (Tick0.Field.of_string x, Tick0.Field.of_string y) in
      Array.map (fun (g1, g2, g3, g4) ->
        ( conv g1
        , conv g2
        , conv g3
        , conv g4))
...
let ith_bit s i =
  (Char.to_int s.[i / 8] lsr (i mod 8)) land 1 = 1

let update ({ digest; i; j } as state) =
  if j = digest_length_in_bits
  then
    let digest = (Digestif.SHA256.digest_string (seed ^ Int.to_string i) :> string) in
...
let params ~loc =
  let random_bool =
    ...
    fun () ->
      ...
      b
  in
  let module P = Gen_params(struct let random_bool = random_bool end) in
  P.expr ~loc
...
let main () =
  let fmt = Format.formatter_of_out_channel (Out_channel.create "pedersen_params.ml") in
