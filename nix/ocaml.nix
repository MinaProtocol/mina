...
let
  ...
  implicit-deps-overlay = self: super: {
    ...
    core = super.core.overrideAttrs
      (oa: { propagatedBuildInputs = [ pkgs.tzdata ]; });
    ...
    postgresql = super.postgresql.overrideAttrs (oa: {
      nativeBuildInputs = (oa.nativeBuildInputs or [ ])
        ++ [ pkgs.postgresql_16.dev pkgs.pkg-config ];
    ...
    conduit-async = super.conduit-async.overrideAttrs (oa: {
      ...
      postInstall = (oa.postInstall or "") + ''
        dp="$out/lib/ocaml/${super.ocaml.version}/site-lib/conduit-async/dune-package"
        if [ -f "$dp" ] && [ ! -s "$dp" ]; then echo '(lang dune 3.3)' > "$dp"; fi
      '';
    });

    mirage-crypto-rng-async = super.mirage-crypto-rng-async.overrideAttrs (oa: {
      postInstall = (oa.postInstall or "") + ''
        dp="$out/lib/ocaml/${super.ocaml.version}/site-lib/mirage-crypto-rng-async/dune-package"
        if [ -f "$dp" ] && [ ! -s "$dp" ]; then echo '(lang dune 3.3)' > "$dp"; fi
      '';
    });

    async_ssl = super.async_ssl.overrideAttrs (s: {
      NIX_CFLAGS_COMPILE = if pkgs.stdenv.isDarwin then
        "-Wno-implicit-function-declaration -Wno-incompatible-function-pointer-types"
      else
        s.NIX_CFLAGS_COMPILE or "";
      buildInputs = s.buildInputs ++ [ pkgs.openssl ];
      postInstall = (s.postInstall or "") + ''
        dp="$out/lib/ocaml/${super.ocaml.version}/site-lib/async_ssl/dune-package"
        if [ -f "$dp" ] && [ ! -s "$dp" ]; then echo '(lang dune 3.3)' > "$dp"; fi
      '';
    });

    caqti-async = super.caqti-async.overrideAttrs (oa: {
      postInstall = (oa.postInstall or "") + ''
        dp="$out/lib/ocaml/${super.ocaml.version}/site-lib/caqti-async/dune-package"
        if [ -f "$dp" ] && [ ! -s "$dp" ]; then echo '(lang dune 3.3)' > "$dp"; fi
      '';
    });

    core_bench = super.core_bench.overrideAttrs (oa: {
      postInstall = (oa.postInstall or "") + ''
        dp="$out/lib/ocaml/${super.ocaml.version}/site-lib/core_bench/dune-package"
        if [ -f "$dp" ] && [ ! -s "$dp" ]; then echo '(lang dune 3.3)' > "$dp"; fi
      '';
    });
    ...
    cohttp-async = super.cohttp-async.overrideAttrs (oa: {
      ...
      NIX_LDFLAGS = (oa.NIX_LDFLAGS or "")
        + " -L${core_unix_gettid_stub}/lib -lcore_unix_gettid_stub";
      postInstall = (oa.postInstall or "") + ''
        dp="$out/lib/ocaml/${super.ocaml.version}/site-lib/cohttp-async/dune-package"
        if [ -f "$dp" ] && [ ! -s "$dp" ]; then echo '(lang dune 3.3)' > "$dp"; fi
      '';
    });
  };
  ...
  # Create the stub library first for reuse
  core_unix_gettid_stub = pkgs.stdenv.mkDerivation {
    pname = "core_unix_gettid_stub";
    version = "1.0";
    src = pkgs.writeText "core_unix_gettid_stub.c" ''
    ...
    '';
    nativeBuildInputs = [ pkgs.gcc scope.ocaml scope.ocamlfind ];
    dontUnpack = true;
    buildPhase = ''
      # Find OCaml include directory
      OCAML_INC=$(ocamlc -where)
      echo "Using OCaml include directory: $OCAML_INC"
      # Compile the stub
      gcc -c -fPIC $src -o core_unix_gettid_stub.o -I"$OCAML_INC"
    ...
    installPhase = ''
      mkdir -p $out/lib
      cp libcore_unix_gettid_stub.a $out/lib/
    '';
  };
  ...
  commonOverrides = {
    ...
    NIX_LDFLAGS = "-L${core_unix_gettid_stub}/lib -lcore_unix_gettid_stub";
  };
  packageHasSrcApp =
