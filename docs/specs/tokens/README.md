...
A **token ID** is a field element in the Pasta Fp field that uniquely identifies a token type within the Mina protocol. Every account on the Mina ledger is associated with exactly one token ID.
...
- Has a maximum length of **up to 6 bytes**.
...
Where `caller_id` is computed according to the algorithm in [Section 5.2](#52-caller-id-computation).
...
The hierarchical call structure determines the **caller identity** of each account update: for a child account update, the relevant caller context comes from the token ID derived from the parent account update's `Account_id`, and may also be inherited further depending on the child's `may_use_token` setting (see [Section 5.2](#52-caller-id-computation)). This caller ID in turn determines which custom token accounts the child may access.
...
1. **Deploy** a zkApp to an owner account, typically `(owner_pk, MINA_token_id)`, with a verification key that implements the desired token policy. More generally, the owner can be any account `(owner_pk, owner_token_id)`.
2. **Initialize** the token owner's app state (e.g., setting supply, configuration flags, etc.).
3. The custom token ID is automatically derived from the owner `Account_id` as `derive_token_id(owner_pk, owner_token_id)`.
4. Optionally set the `token_symbol` on the owner account to provide a human-readable label.

If the owner account is on the default token, it may itself hold MINA, while the *custom token accounts* are separate accounts with the derived `token_id`. This derivation model also works when the owner account is itself on a non-default token.
